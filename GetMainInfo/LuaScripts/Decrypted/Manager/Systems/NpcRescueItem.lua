...
function RescueItens.Init()
	...
	InterfaceController.AfterMainProc(RescueItens.AfterInterface)
...
function RescueItens.RequestOpen()
...
function RescueItens.AfterInterface()
	...
	if RescueItens_Config.AlertWindow.Show then
		...
		if RescueItens.ButtonMouseOver(RescueItens_Config.ButtonID.ok, MousePosX(), MousePosY()) then
			RenderImage2(40031, addX+275, 120, 55, 21, 0.009000, 0.212000, 0.610000, 0.180000, 1, 1, 1.0)
		else
			RenderImage2(40031, addX+275, 120, 55, 21, 0.009000, 0.012000, 0.610000, 0.180000, 1, 1, 1.0)
		end
		...
		if RescueItens_Config.AlertWindow.Type == 0 then
			RenderText3(addX+300, 95, string.format(RescueItens_Config.Texts[GetLanguage()][4]), 200, 8)
			SetTextColor(255, 255, 255, 255)
			RenderText3(addX+300, 105, string.format(RescueItens_Config.Texts[GetLanguage()][5]), 200, 8)
		end
		if RescueItens_Config.AlertWindow.Type == 1 then
			RenderText3(addX+300, 95, string.format(RescueItens_Config.Texts[GetLanguage()][6]), 200, 8)
			SetTextColor(255, 255, 255, 255)
			RenderText3(addX+300, 105, string.format(RescueItens_Config.Texts[GetLanguage()][7]), 200, 8)
			RenderText3(addX+304, 126, string.format(RescueItens_Config.Texts[GetLanguage()][8]), 200, 8)
		end
...
function RescueItens.Interface()
	...
	RescueItens.CreateButton(RescueItens_Config.ButtonID.fechar,485, 395, 24, 24)
...
--#endregion
	...
	--#region BASE JANELA
		EnableAlphaTest()
		RenderImageAuto(33001, posX-base, 0, 191, 429)
	...
	--#region BUTTON RETIRAR
		...
		Draw.Button.Large(31323, buttonRetirarPosX, buttonRetirarPosY, buttonRetirarWidth, buttonRetirarHeight, state)
	...
	--#region BOX
		...
		--#region TEXTO
			...
			RenderText3(buttonRetirarPosX+35, boxPosY+6, RescueItens_Config.Texts[GetLanguage()][3], 150, 8)
		--#endregion
	...
	--#endregion
		...
		local btnState = 0
		if RescueItens.ButtonMouseOver(RescueItens_Config.ButtonID.fechar, MousePosX(), MousePosY()) then
			if CheckRepeatKey(Keys.LButton) == 1 then
				btnState = 2
			else
				btnState = 1
			end
			DrawTooltip(485, 378, "Fechar")
		end
		DrawButton.Close(btnState, 485, 395, 24, 24)
	...
	--region SELECT ITEM
		...
		--#region SELECT ITEM NAME
			...
			for s = RescueItens_Config.ListaConfig.Offset, RescueItens_Config.ListaConfig.MaxValue do
				...
				RenderImageAuto(40065, fixposX-RescueItens_Config.WindowOffset, line+2, 5, 5)
...
function RescueItens.ScrollMouse(Direction)
	...
	if RescueItens.CheckOpen() then
		if RescueItens_Config.AlertWindow.Show then return 1 end
