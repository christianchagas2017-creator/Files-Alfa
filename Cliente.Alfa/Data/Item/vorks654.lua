...
RANKING_PACKET_OPEN = 'RANKING_OPEN'

local tableCustomButton = {}
...
function Ranking.CreateButtonCustom(ButtonID, x, y, w, h, tableButtons)
    if tableButtons[ButtonID] == nil then
		table.insert(tableButtons, ButtonID)
		tableButtons[ButtonID] = {
			x = x,
			y = y,
			w = w,
			h = h,
        ...
        tableButtons[ButtonID].x = x
		tableButtons[ButtonID].y = y
		tableButtons[ButtonID].w = w
		tableButtons[ButtonID].h = h
	end
end

function Ranking.ButtonMouseOverCustom(ButtonID, x, y, tableButtons)
	if tableButtons[ButtonID] == nil then return false end
	if x >= tableButtons[ButtonID].x and x <= (tableButtons[ButtonID].x+tableButtons[ButtonID].w) then
		if y >= tableButtons[ButtonID].y and y <= (tableButtons[ButtonID].y+tableButtons[ButtonID].h) then
			return true
		end
	end
	return false
...
function Ranking.Interface()
    ...
    Ranking.CreateButtonCustom(Ranking_Config.ButtonID.open, 442, 449, 26, 26, tableCustomButton)
    if Ranking.ButtonMouseOverCustom(Ranking_Config.ButtonID.open, MousePosX(), MousePosY(), tableCustomButton) then
        SetFontType(1)
        SetTextBg(0, 0, 0, 255)
        SetTextColor(255, 255, 255, 255)
        RenderText2(450, 420, 'Ranking', 50, ALIGN_CENTER)
    end
    ...
    for i in ipairs(Ranking_Config.Players) do
        ...
        SetTextColor(255,255,255,255)
        RenderText3(Ranking_Config.WindowPosition.X+55, line, string.format("%d�",Ranking_Config.Players[i].Position), 150, 8)
