...
local function WindowGetWidth()
	return 640
end

local function WindowGetWidthAdd()
	return 0
end
...
function RescueItens.Init()
	--InterfaceController.AfterMainProc(RescueItens.Interface)
...
function RescueItens.AfterInterface()
	...
	--#region CHECKAR OUTRAS INTERFACES
		...
		if	CheckWindowOpen(UIExpandInventory)	== 1	then	RescueItens_Config.WindowOffset = 190 else RescueItens_Config.WindowOffset = 0	end
		if	CheckWindowOpen(UIExpandWarehouse)	== 1	then	RescueItens.Close()	end
		if	CheckWindowOpen(UIMuHelper)			== 1	then	RescueItens.Close()	end
	--#endregion
		...
		if RescueItens_Config.Description then
			if MousePosX() >= posX+18-item.X and MousePosX() <= posX+18-item.X+item.W then
				if MousePosY() >= item.Y and MousePosY() <= item.Y+item.H then
					SetBlend()
					ShowDescriptionComplete(MousePosX(), MousePosY()+10, item.ItemIndex, item.Level, item.Skill, item.Luck, item.Opt, item.Excelente, item.Ancient, item.Harmony, 255, 255, 255, 255, 255,255)
					EndDrawBar()
				end
			end
		end
	...
	if RescueItens_Config.AlertWindow.Show then
		EnableAlphaTest()
		RenderImage2(41007, addX+205, 86, 225, 4, 0, 0, 0.83203125, 0.625, 1, 1, 1.0)
		RenderImage2(41010, addX+205, 119, 225, 30, 0.0, 0.0, 0.8322874124, 0.6042874124, 1, 1, 1.0)
		RenderImage2(41010, addX+205, 90, 225, 30, 0.0, 0.0, 0.8322874124, 0.6042874124, 1, 1, 1.0)
		RenderImage2(41007, addX+205, 148, 225, 4, 0, 0, 0.83203125, 0.625, 1, 1, 1.0)

		RescueItens.CreateButton(RescueItens_Config.ButtonID.ok, addX+285, 125, 60, 18)
		if RescueItens.ButtonMouseOver(RescueItens_Config.ButtonID.ok, MousePosX(), MousePosY()) then
			RenderImage2(41011, addX+285, 125, 60, 18, 0.00688282, 0.313344848, 0.538486151, 0.333344848, 1, 1, 1.0)
		else
			RenderImage2(41011, addX+285, 125, 60, 18, 0.00688282, 0.002127840, 0.538486151, 0.323344848, 1, 1, 1.0)
		end
		...
		if RescueItens_Config.AlertWindow.Type == 0 then
			RenderText3(addX+320, 95, string.format(RescueItens_Config.Texts[GetLanguage()][4]), 200, 8)
			SetTextColor(255, 255, 255, 255)
			RenderText3(addX+320, 105, string.format(RescueItens_Config.Texts[GetLanguage()][5]), 200, 8)
		end
		if RescueItens_Config.AlertWindow.Type == 1 then
			RenderText3(addX+320, 95, string.format(RescueItens_Config.Texts[GetLanguage()][6]), 200, 8)
			SetTextColor(255, 255, 255, 255)
			RenderText3(addX+320, 105, string.format(RescueItens_Config.Texts[GetLanguage()][7]), 200, 8)
		end
...
function RescueItens.Interface()
	...
	--#region BASE JANELA
		...
		RenderImage(590039, posX-base-RescueItens_Config.WindowOffset, 0, 190, 429)
	...
	--region SELECT ITEM
		...
		--#region SELECT ITEM NAME
			...
			for s = RescueItens_Config.ListaConfig.Offset, RescueItens_Config.ListaConfig.MaxValue do
				...
				RenderImage2(590001, fixposX-RescueItens_Config.WindowOffset, line+2, 5, 5, 0, 0, 0.591827401425252121, 0.591827401425252121, 1, 1, 1.0)
