...
function LimparInventario.Render()
    ...
    if CheckWindowOpen(UIInventory) == 1
    then
             if MousePosX() >= 573 and MousePosX() <= 588 + 30
             and MousePosY() >= 399 and MousePosY() <= 404 + 30
             then
            ...
            SetTextBg(0, 0, 0, 180)
            RenderText3(580, 381, LIMPARINVENTARIO_MESSAGES[GetLanguage()][1], 67, 8)
	    end
    end 
...
function LimparInventario.RenderText(x, y)
	...
	RenderText3(x + 100, y + 5+ posY, LIMPARINVENTARIO_MESSAGES[GetLanguage()][7], 200, 8)
...
if	CheckWindowOpen(UIInventory)	    == 1	then
    if (MousePosX() >= 573 and MousePosX() <= 586 + 30)
        and (MousePosY() >= 401 and MousePosY() <= 399 + 30)
    then
    ...
    --Close button
    if MousePosX() >=  320 and MousePosY() >= 280 and MousePosX() <= 377 and MousePosY() <=300
    then
        if (CheckClickClient() == 1)
        then
...
function LimparInventario.ClickEvent()
	...
	if MousePosX() >=  260 and MousePosY() >= 280 and MousePosX() <= 313 and MousePosY() <=300 and LimparInventarioVisible == 1 and CheckWindowOpen(UIInventory) == 1
    then
	        local packetName = "clearinv"
            CreatePacket (packetName, 0x76)
...
function LimparInventario.LoadImages()
	LimparInventarioImage1 = LoadImageByDir("Interface///Trash.tga")
end
