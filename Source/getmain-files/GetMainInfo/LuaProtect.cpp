...
bool CFileLuaProtect::DecryptFile(char* path, char* n_path) // OK
{
	if (this->LoadFile(path) == 0)
	{
		return 0;
	...
	{
		this->m_buff[sizeof(gFileProtectLuaHeader) + n] -= gFileProtectLuaXorTable[this->m_PrivateCode[n % strlen(this->m_PrivateCode)] % sizeof(gFileProtectLuaXorTable)];
...
void CFileLuaProtect::FindFile(char * path, char * n_path)
{
...
void CFileLuaProtect::FindFile2(char* path, char* n_path)
{
	char wildcard_path[256];
