...
GLfloat FogColor[4] = { 30 / 256.f, 20 / 256.f, 10 / 256.f, 1.0f };
...
#define wglSwapIntervalEXT ((PFNWGLSWAPINTERVALPROC)wglGetProcAddress("wglSwapIntervalEXT"))
...
void gluPerspective2(float Fov, float Aspect, float ZNear, float ZFar)
{
	...
	//PerspectiveX = (float)ScreenCenterX/tanf(Fov*0.5f*Q_PI/180.f)*Aspect;
	//PerspectiveY = (float)ScreenCenterY/tanf(Fov*0.5f*Q_PI/180.f);
}
...
void CreateScreenVector(int sx, int sy, vec3_t Target, bool bFixView)
{
	...
	if (m_Resolution > 3) {
		sy = sy * WindowHeight / gWideScreen.g_WideWindowHeight;
	}
	else
	{
...
void Projection(vec3_t Position, int* sx, int* sy)
{
	...
	*sx = (gWideScreen.g_WideWindowWidth * (*sx)) / WindowWidth;

	if (m_Resolution > 3) {
		*sy = *sy * gWideScreen.g_WideWindowHeight / (int)WindowHeight;
	}
	else {
...
{
	if (m_Resolution > 2) {
		return y * (float)WindowHeight / gWideScreen.g_WideWindowHeight;
	}
...
{
	if (m_Resolution > 3) {
		x = x * WindowWidth / gWideScreen.g_WideWindowWidth;
		y = y * WindowHeight / gWideScreen.g_WideWindowHeight;
		Width = Width * WindowWidth / gWideScreen.g_WideWindowWidth;
		Height = Height * WindowHeight / gWideScreen.g_WideWindowHeight;
	}
	else {
		x = x * WindowWidth / 640;
...
void RenderSprite(int Texture, vec3_t Position, float Width, float Height, vec3_t Light, float Rotation, float u, float v, float uWidth, float vHeight)
{
	...
	GLfloat vertices[12] = {
		p[0][0], p[0][1], p[0][2], // Bottom-left
		p[1][0], p[1][1], p[1][2], // Bottom-right
		p[2][0], p[2][1], p[2][2], // Top-right
		p[3][0], p[3][1], p[3][2]  // Top-left
	};

	// Define texture coordinates
	GLfloat texCoords[8] = {
		u, v + vHeight,             // Bottom-left
		u + uWidth, v + vHeight,    // Bottom-right
		u + uWidth, v,              // Top-right
		u, v                        // Top-left
	};
	...
	for (int i = 0; i < 4; i++)
	{
		colors[i * 4 + 0] = Light[0]; // R
		colors[i * 4 + 1] = Light[1]; // G
		colors[i * 4 + 2] = Light[2]; // B
		colors[i * 4 + 3] = (Bitmaps[Texture].Components == 3)
			? 1.f
			: (Texture == BITMAP_BLOOD + 1 || Texture == BITMAP_FONT_HIT) ? 1.f : Light[0]; // Alpha
	}
...
void RenderColor(float x, float y, float Width, float Height, float Alpha, int Flag)
{
	...
	// Adjust y for top-down rendering
	y = WindowHeight - y;
	...
	{
		if (Flag == 0)
			glColor4f(1.f, 1.f, 1.f, Alpha); // White with alpha
		else if (Flag == 1)
			glColor4f(0.f, 0.f, 0.f, Alpha); // Black with alpha
	}
	...
	// Reset the color to default if alpha was applied
	if (Alpha > 0.f)
	{
		glColor4f(1.f, 1.f, 1.f, 1.f);
	}
...
void RenderColorRadius(float x, float y, float width, float height, float alpha, int flag, float radius)
{
	...
	// Clamp the radius to ensure it fits within the rectangle
	float maxRadius = fmin(width, height) / 2.0f;
	radius = fmax(0.0f, fmin(radius, maxRadius));
	...
	// Number of segments for rounding (higher = smoother curve)
	int segments = 32;

	// Enable client states
	glEnableClientState(GL_VERTEX_ARRAY);

	// Create a dynamic array for vertices
	std::vector<GLfloat> vertices;

	// Generate vertices for the rounded shape
	for (int i = 0; i <= segments; ++i)
	{
		// Calculate the angle for the current vertex
		float angle = 2.0f * Q_PI * (float)i / (float)segments;

		// Determine how far along the edge we are
		float edgeX = cos(angle);
		float edgeY = sin(angle);

		// Scale the distance based on whether we are within the square or transitioning
		float cornerX = (width / 2.0f) - radius;
		float cornerY = (height / 2.0f) - radius;

		// Compute the vertex position based on the radius and square interpolation
		float vx = x + width / 2.0f + (radius * edgeX) + (cornerX * (edgeX > 0 ? 1 : -1));
		float vy = y - height / 2.0f + (radius * edgeY) + (cornerY * (edgeY > 0 ? 1 : -1));
...
void RenderShapeWithStroke(float x, float y, float width, float height, float alpha, int flag, float radius, float strokeWidth, float strokeR, float strokeG, float strokeB, float strokeA)
{
	// First, draw the filled shape (no changes from the previous function)
	RenderColorRadius(x, y, width, height, alpha, flag, radius);

	// Now, draw the stroke (outline)
	DisableTexture();
	...
	// Create a dynamic array for stroke vertices
	std::vector<GLfloat> strokeVertices;

	// Generate vertices for the stroke
	for (int i = 0; i <= segments; ++i)
	{
		// Calculate the angle for the current vertex
		float angle = 2.0f * Q_PI * (float)i / (float)segments;
		...
		// Scale the distance based on the radius and square interpolation
		float cornerX = (width / 2.0f) - radius;
		float cornerY = (height / 2.0f) - radius;

		// Compute the vertex position based on the radius
		float vx = x + width / 2.0f + (radius * edgeX) + (cornerX * (edgeX > 0 ? 1 : -1));
		float vy = y - height / 2.0f + (radius * edgeY) + (cornerY * (edgeY > 0 ? 1 : -1));

		strokeVertices.push_back(vx);
		strokeVertices.push_back(vy);
	}

	// Set the vertex pointer for the stroke
	glVertexPointer(2, GL_FLOAT, 0, strokeVertices.data());

	// Set the line width for the stroke
	glLineWidth(strokeWidth);

	// Draw the stroke using a line loop
	glDrawArrays(GL_LINE_LOOP, 0, strokeVertices.size() / 2);
...
void RenderColorBitmap(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, unsigned int color)
{
	...
	// Extract color components from the packed `color` argument
	GLfloat red = static_cast<GLubyte>((color & 0xff)) / 255.0f;
	GLfloat green = static_cast<GLubyte>((color >> 8) & 0xff) / 255.0f;
	GLfloat blue = static_cast<GLubyte>((color >> 16) & 0xff) / 255.0f;
	GLfloat alpha = static_cast<GLubyte>((color >> 24) & 0xff) / 255.0f;
	...
	// Specify pointers to vertex attributes
	glVertexPointer(2, GL_FLOAT, 0, vertices);
	glTexCoordPointer(2, GL_FLOAT, 0, texCoords);
	glColorPointer(4, GL_FLOAT, 0, colors);
...
void RenderBitmap(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, bool Scale, bool StartScale, float Alpha)
{
	...
	// Define vertex colors (with alpha blending)
	GLfloat colors[16]; // 4 vertices * RGBA
	for (int i = 0; i < 4; ++i)
	{
		colors[i * 4 + 0] = 1.f; // R
		colors[i * 4 + 1] = 1.f; // G
		colors[i * 4 + 2] = 1.f; // B
		colors[i * 4 + 3] = Alpha > 0.f ? Alpha : 1.f; // A
	}
...
/*
void RenderBitmapScale(int Texture,float x,float y,float Width,float Height,float u,float v,float uWidth,float vHeight,bool Scale,bool StartScale,float Alpha)
{
	if(StartScale)
	{
		x = ConvertX(x);
	...
	if(Scale)
	{
		Width = ConvertX(Width);
		Height = ConvertY(Height);
		uWidth = ConvertX(uWidth);
		vHeight = ConvertY(vHeight);
	}
...
void RenderBitmapScale(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, bool Scale, bool StartScale, float Alpha)
{
	...
	// Reset global color to default if alpha was applied
	if (Alpha > 0.f)
	{
		glColor4f(1.f, 1.f, 1.f, 1.f);
	}
...
void RenderBitmapRotate(int Texture, float x, float y, float Width, float Height, float Rotate, float u, float v, float uWidth, float vHeight)
{
	...
	// Define the original rectangle vertices centered at (0, 0)
	Vector(-Width * 0.5f, Height * 0.5f, 0.f, p[0]); // Top-left
	Vector(-Width * 0.5f, -Height * 0.5f, 0.f, p[1]); // Bottom-left
	Vector(Width * 0.5f, -Height * 0.5f, 0.f, p[2]); // Bottom-right
	Vector(Width * 0.5f, Height * 0.5f, 0.f, p[3]); // Top-right
	...
	// Define transformed vertex positions
	GLfloat vertices[8] = {
		p2[0][0] + x, p2[0][1] + y, // Top-left
		p2[1][0] + x, p2[1][1] + y, // Bottom-left
		p2[2][0] + x, p2[2][1] + y, // Bottom-right
		p2[3][0] + x, p2[3][1] + y  // Top-right
	};
...
void RenderBitRotate2(int Texture, float x, float y, float Width, float Height, float Rotate)
{
	x = ConvertX(x);
	y = ConvertY(y);
	Width = ConvertX2(Width);
	Height = ConvertY(Height);
...
void RenderPointRotate(int Texture,float ix,float iy,float iWidth,float iHeight,float x,float y,float Width,float Height,float Rotate,float Rotate_Loc,float uWidth,float vHeight,int Num)
{
	...
	if(Num > -1)
	{
		...
		dx = dx * (float)(gWideScreen.g_WideWindowWidth/WindowWidth);
