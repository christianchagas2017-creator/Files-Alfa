...
namespace SEASON3B
{
	...
	class CNewUIMultipleTextInputMsgBox : public CNewUIMessageBoxBase
	{
		enum
		{
			INPUTBOX_TOP_BLANK = 10,
	...
	public:
		CNewUIMultipleTextInputMsgBox();
		virtual ~CNewUIMultipleTextInputMsgBox();

		bool Create(DWORD dwMsgBoxType, DWORD dwInputType, int iInputBoxWidth = 100, int iInputBoxHeight = 14, int iLimitText = 256, int InputBoxCount = 1);
		void Release();

		static CALLBACK_RESULT LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam);

		void AddMsg(int boxID, const type_string& strMsg, DWORD dwColor = CLRDW_WHITE, BYTE byFontType = MSGBOX_FONT_NORMAL);

		bool Update();
		bool Render();

		DWORD GetMsgBoxType();
		int GetInputBoxCount();
		void GetInputBoxText(int boxId, unicode::t_char* strText);
		void SetInputBoxOption(int boxId, int iOption);
		void SetInputBoxPosition(int boxId, int x, int y);
		void SetInputBoxSize(int boxId, int width, int height);
	...
	private:
		...
		DWORD m_dwMsgBoxType;
		DWORD m_dwInputType;
		int m_pInputBoxCount;

		CUITextInputBox* m_pInputBox[5];
		type_vector_msgdata m_MsgTextList;
	...
	class CChangeWareMsgBoxLayout : public TMsgBoxLayout<CNewUITextInputMsgBox>
	{
	public:
		bool SetLayout();
		static CALLBACK_RESULT ReturnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam);
	...
	class CPersonalShopItemValueMsgBoxLayout : public TMsgBoxLayout<CNewUIMultipleTextInputMsgBox>
	{
	public:
		bool SetLayout();
		static CALLBACK_RESULT ReturnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam);
