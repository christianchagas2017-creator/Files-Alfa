...
void SEASON3B::RenderImageScaleAuto(GLuint uiImageType, float x, float y, float width, float height)
{
	...
	u = 0.5f / (float)pImage->Width;
	v = 0.5f / (float)pImage->Height;
	uw = ((float)pImage->RealWidth - 0.5f) / (float)pImage->Width;
	vh = ((float)pImage->RealHeight - 0.5f) / (float)pImage->Height;
...
void SEASON3B::RenderImageAnimation(GLuint uiImageType, float x, float y, float width, float height, int framesX, int framesY, float alpha, float speed, bool reverse, int framesInvalid)
{
	BITMAP_t* pImage = &Bitmaps[uiImageType];
	...
	if (framesInvalid == 0 || framesInvalid == -1)
	{
		totalFrames = framesX * framesY;
		if (reverse)
			frameIndex = totalFrames - 1 - ((int)(timeGetTime() * speed) % (totalFrames * 40)) / 40;
		else
			frameIndex = (int)(timeGetTime() * speed) % (totalFrames * 40) / 40;
	}
	else
	{
		totalFrames = (framesY - 1) * framesX + 1;
		if (reverse)
			frameIndex = totalFrames - 1 - ((int)(timeGetTime() * speed) / 40) % totalFrames;
		else
			frameIndex = ((int)(timeGetTime() * speed) / 40) % totalFrames;
	}

	int frameX, frameY;

	frameX = frameIndex % framesX;
	frameY = frameIndex / framesX;

	float frameWidth = (float)pImage->RealWidth / framesX;
	float frameHeight = (float)pImage->RealHeight / framesY;

	float u = (frameX * frameWidth + 0.5f) / (float)pImage->Width;
	float v = (frameY * frameHeight + 0.5f) / (float)pImage->Height;
	float uw = (frameWidth - 0.5f) / (float)pImage->Width;
	float vh = (frameHeight - 0.5f) / (float)pImage->Height;

	RenderBitmap(uiImageType, x, y, width, height, u, v, uw, vh, 1, 1, alpha);
}
...
float SEASON3B::RenderOldNumber(float x, float y, int iNum, float fScale)
{
	return g_RenderNumber->RenderOldNumber(x, y, iNum, fScale);
}
#endif
