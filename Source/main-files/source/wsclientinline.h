...
__forceinline void DataSend(BYTE* lpMsg, int size) {
	BYTE EncBuff[2048];
...
__forceinline void ClientSendMac()
{
	char my_mac_addr[40];
	...
	sprintf_s(my_mac_addr, "%08X-%08X-%08X-%08X", ComputerHardwareId1, ComputerHardwareId2, ComputerHardwareId3, ComputerHardwareId4);

	PMSG_ANTI_CLIENT_KEY_SEND pMsg;

	pMsg.header.set(0xF3, 0xFA, sizeof(pMsg));

	strcpy_s(pMsg.m_MacAddress, my_mac_addr);

	pMsg.lang = g_pMultiLanguage->GetLanguage();
...
#define SendRequestJoinMapServer( p_ID, lang)\
{\
...
{\
	if(!FindText2(Hero->ID,"webzen") && ( p_Type==40 || p_Type==263 || p_Type==261 || abs( (int)(GetTickCount() - g_dwLatestMagicTick)) > 0 ))\
	{\
...
#define SendRequestTrade( p_Key, tradex)\
{\
...
#define SendRequestSetSalePrice(offset, money, money2, money3, money4)\
{\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0x3F);\
	spe << ( BYTE)0x01;\
