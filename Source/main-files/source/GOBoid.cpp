...
void MoveBugs()
{
	...
	float randomValues[10];
	for (int i = 0; i < 10; i++) {
		randomValues[i] = (float)(rand() % 1000) / 1000.0f;
	}
	int randIndex = 0;
...
std::map<std::pair<int, int>, float> terrainHeightCache;
float getCachedTerrainHeight(float x, float y) {
	auto key = std::make_pair((int)(x / TERRAIN_SCALE), (int)(y / TERRAIN_SCALE));
	if (terrainHeightCache.find(key) == terrainHeightCache.end()) {
		terrainHeightCache[key] = RequestTerrainHeight(x, y);
	}
	return terrainHeightCache[key];
}

void MoveButterFly(OBJECT* o) {
	static float scaledRandomFactor = 0.3f * FPS_ANIMATION_FACTOR; // Atualizar por quadro
	static float randomValues[10];
	...
	if (randomValues[randIndex] < 0.03125f) { // Equivalente a rand_fps_check(32)
		o->Angle[2] = (float)(rand() % 360);
	...
	float height = getCachedTerrainHeight(o->Position[0], o->Position[1]);
	...
	o->Position[2] += (float)(rand() % 15 - 7) * scaledRandomFactor;
}
...
void MoveBoidGroup(OBJECT* o, int index)
{
	...
	float scaledVelocity = o->Velocity * FPS_ANIMATION_FACTOR;
	o->Position[0] += scaledVelocity * (float)sinf(o->Angle[2]);
	o->Position[1] -= scaledVelocity * (float)cosf(o->Angle[2]);
