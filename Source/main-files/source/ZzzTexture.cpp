...
#ifdef FOR_WORK
			PopUpErrorCheckMsgBox("ErrorCheckBox", szErrorMsg);
#else // FOR_WORK
			PopUpErrorCheckMsgBox("ErrorCheckBox", szErrorMsg, true);
#endif // FOR_WORK
			return false;
		}
		return true;
...
bool LoadBitmapCustom(const char* szFileName, GLuint uiTextureIndex, GLuint uiFilter, GLuint uiWrapMode, bool bCheck, bool bFullPath)
{
	char szFullPath[256] = { 0, };
	if (bFullPath == true)
	{
	...
	if (bCheck)
	{
		if (false == Bitmaps.LoadImageCustom(uiTextureIndex, szFullPath, uiFilter, uiWrapMode))
		{
			char szErrorMsg[256] = { 0, };
			sprintf(szErrorMsg, "%d %s %s", uiTextureIndex, szFullPath, szFileName);
			PopUpErrorCheckMsgBox("LoadBitmapCustom Failed", szErrorMsg);

			return false;
		}
		return true;
	}
	return Bitmaps.LoadImageCustom(uiTextureIndex, szFullPath, uiFilter, uiWrapMode);
}
...
void PopUpErrorCheckMsgBox(const char* szTitleMsg, const char* szErrorMsg, bool bForceDestroy)
{
	char szMsg[1024] = {0, };
	...
	if(bForceDestroy)
	{
		MessageBox(g_hWnd, szErrorMsg, szTitleMsg, MB_OK|MB_ICONERROR);
	}
	else
	{
		int iResult = MessageBox(g_hWnd, szMsg, szTitleMsg, MB_YESNO|MB_ICONERROR);
		if(IDYES == iResult)
		{
			return;
		}
