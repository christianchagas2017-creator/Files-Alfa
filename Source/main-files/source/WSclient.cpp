	...
	for (int i = 0; i < ReceiveBuffer->count; i++)
	{
		...
		*/
		...
		if (c)
		{
			...
			gCharacterList.SetCharacterPosition(c->Key);
#endif
...
void ReceiveCreateCharacter(BYTE* ReceiveBuffer)
{
	...
	if (Data->Result == 1)
	{
		...
		*/
		...
		auto c = CreateHero(Data->Index, CharacterView.Class, CharacterView.Skin, fPos[0], fPos[1], fAngle);
...
void ReceiveMagicList(BYTE* ReceiveBuffer)
{
	...
	else
	{
		...
		auto elementFirst = &CharacterMachine->Equipment[NEW_EQUIPMENT_HELPER_FLY];
...
BOOL ReceiveInventory(BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		else if ((itemindex >= NEW_EQUIPMENT_HELPER_FLY && itemindex <= NEW_EQUIPMENT_EARRING_LEFT) || itemindex == NEW_EQUIPMENT_ARTIFACT)
		{
			gVisualInventory.AddItemElemental(Data2->Index, Data2->Item);
		}
		else if (itemindex >= NEW_EQUIPMENT_WEAPON_RIGHT && itemindex < NEW_EQUIPMENT_ARTIFACT)
		{
...
void GCItemCounterRecv(PMSG_ITEM_COUNTER_RECV* lpMsg)
{
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		gCounterItem.InsertCounterItem(lpInfo->ItemIndex, lpInfo->Slot, lpMsg->Type, lpInfo->kills, lpInfo->Top);
	}
}
...
void ReceiveChat(BYTE* ReceiveBuffer)
{
	...
	else
	{
		...
		else if (Data->ChatText[0] == '#')
		{
			...
			CHARACTER* pCustomColor = nullptr;
			...
			for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
			{
				...
				if (g_bCustomColor) //CUSTOM COLOR CHAT LEFT
				{
					for (auto it = gCustomColorChat.m_CustomColorChatLeft.begin(); it != gCustomColorChat.m_CustomColorChatLeft.end(); ++it)
					{
				...
				if (o->Live && o->Kind == KIND_PLAYER && (g_isCharacterBuff((&c->Object), eBuff_GMEffect) || (c->CtlCode == CTLCODE_20OPERATOR) || (c->CtlCode == CTLCODE_08OPERATOR)) && strcmp(c->ID, ID) == 0)
				{
					pFindGm = c;
					break;
				}
			...
			if (pCustomColor)
			{
				g_pChatListBox->AddText(ID, Text, SEASON3B::TYPE_CHAT_MESSAGE_CUSTOM);
			}
			else if (pFindGm)
			{
...
void ReceiveChatKey(BYTE* ReceiveBuffer)
{
	...
	if (Hero->GuildStatus == G_MASTER && !strcmp(CharactersClient[Index].ID, "Lawrence"))
	{
...
void ReceiveNotice(BYTE* ReceiveBuffer)
{
	...
	else if (Data->Result >= 25 && Data->Result <= 100)
	{
		...
		if (msgInfo != 0) {
			...
			{
				CreateNotice((char*)Data->Notice, Data->Result);
			}
		}
	}
...
void ReceiveChangePlayer(BYTE* ReceiveBuffer)
{
	...
	if (Data->ElementSlot == 236)
	{
		...
		else
		{
			...
			{
				gElementPetFirst.CreateHelper(Type, gHelperSystem.GetHelperModel(Type + MODEL_ITEM), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(Type + MODEL_ITEM));
			}
		}
	}
	else if (Data->ElementSlot == 237)
	{
		...
		else
		{
			if (gHelperSystem.CheckIsHelper(Type + MODEL_ITEM))
			{
				...
				gElementPetSecond.CreateHelper(Type, gHelperSystem.GetHelperModel(Type + MODEL_ITEM), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(Type + MODEL_ITEM));
			}
		}
	}
	...
	case 0:
		...
		else
		{
			...
			gSummonEffect.RemoveSummonEffectsAll(c);
		}
		break;
	case 1:
		if (Type == 0x1FFF)
	...
	case 8:
		...
		else if (gHelperSystem.CheckIsHelper(Type + MODEL_ITEM))
		{
			...
			gHelperManager.CreateHelper(Type, gHelperSystem.GetHelperModel(c->Helper.Type), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(c->Helper.Type));
...
void ReceiveCreatePlayerViewport(BYTE* ReceiveBuffer, int Size)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		if (FindText(Temp, "webzen") == false)
		{
			...
			{
				RegisterBuff(static_cast<eBuffState>(Data2->s_BuffEffectState[j]), o, 0, c);
...
void ReceiveAction(BYTE* ReceiveBuffer, int Size)
{
	...
	//FENRIR
	//if(c->Helper.Type == MODEL_HELPER+37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1) return;
...
BOOL ReceiveMagic(BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case AT_SKILL_STRONG_PIER:
		...
		//DRAGON
		if ((gHelperSystem.CheckHelperType(sc->Helper.Type, 32) == 1) && !sc->SafeZone)
		{
			SetAction(so, PLAYER_ATTACK_RIDE_STRIKE);
		}
		else
	...
	case AT_SKILL_SPEAR:	// Ã¢Âî¸£±â
		if (sc->Helper.Type == MODEL_HELPER + 37 || sc->Helper.Type == gHelperSystem.CheckHelperType(sc->Helper.Type, 8) == 1)
	...
	case AT_SKILL_BRAND_OF_SKILL:
		...
		else if ((sc->Helper.Type == MODEL_HELPER + 37 && !sc->SafeZone) || (gHelperSystem.CheckHelperType(sc->Helper.Type, 8) == 1 && !sc->SafeZone))
		{
	...
	case AT_SKILL_LIFE_UP + 4:
	case AT_SKILL_VITALITY:
		if (!g_isCharacterBuff(to, eBuff_HpRecovery)) //eBuff_Life //BUFF BK 
		{
			int color = 1;

			int customColor = gBuffColorBK.IsBuffColorBK((DWORD)tc);

			if (customColor >= 100) color = customColor;

			DeleteEffect(BITMAP_LIGHT, to, color);
			CreateEffect(BITMAP_LIGHT, to->Position, to->Angle, to->Light, color, to);
		}

		g_CharacterRegisterBuff(to, eBuff_HpRecovery); //eBuff_Life


		if (sc->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(sc->Helper.Type, 8) == 1)
			SetAction(so, PLAYER_FENRIR_SKILL_VITALITY);
		else if (gHelperSystem.CheckHelperType(sc->Helper.Type, 32) == 1)
			SetAction(so, PLAYER_FENRIR_SKILL_VITALITY);
		else
			SetAction(so, PLAYER_SKILL_VITALITY);
		sc->AttackTime = 1;
	...
	if (so->Type == MODEL_PLAYER)
	{
		if (sc != Hero)
		{
			...
			case AT_SKILL_FURY_STRIKE:
				//SKILL BK RAGEBLOW
				if (sc->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(sc->Helper.Type, 8) == 1)
					SetAction(so, PLAYER_FENRIR_SKILL_FURY_STRIKE);
				else if (gHelperSystem.CheckHelperType(sc->Helper.Type, 32) == 1)
					SetAction(so, PLAYER_FENRIR_SKILL_FURY_STRIKE);
				else
...
BOOL ReceiveEquipmentItem(BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (Data->SubCode != 255)
	{
		...
		if (iSourceIndex >= MAX_MY_INVENTORY_INDEX)
		{
			...
			int price3 = 0;
			int price4 = 0;
			if (GetPersonalItemPrice(iSourceIndex, price, price2, price3, price4, g_IsPurchaseShop))
			{
				RemovePersonalItemPrice(iSourceIndex, g_IsPurchaseShop);
				if (Data->Index >= MAX_MY_INVENTORY_INDEX)
				{
					AddPersonalItemPrice(Data->Index, price, price2, price3, price4, g_IsPurchaseShop);
				}
			}
		}
		...
		if (Data->SubCode == 0)
		{
			...
			if ((itemindex >= NEW_EQUIPMENT_HELPER_FLY && itemindex <= NEW_EQUIPMENT_EARRING_LEFT) || itemindex == NEW_EQUIPMENT_ARTIFACT)
			{
				gVisualInventory.AddItemElemental(itemindex, Data->Item);
			}

			if (itemindex >= NEW_EQUIPMENT_WEAPON_RIGHT && itemindex < NEW_EQUIPMENT_ARTIFACT)
			{
			...
			else if (itemindex >= MAX_EQUIPMENT_INDEX && itemindex < MAX_MY_INVENTORY_INDEX)
			{
				...
				g_pTrade->ProcessItemAutoMoveSuccess();
				itemindex = itemindex - MAX_EQUIPMENT_INDEX;
		...
		{
			g_pTradeX->ProcessToReceiveTradeXItemsSource(Data->Index, Data->Item);
		}
		else if (43 == Data->SubCode)
		{
			g_pTradeX->ProcessToRemoveTradeXItemsSource(Data->Index, Data->Item);
		}
	...
	else
	{
		...
		g_pTrade->ProcessItemAutoMoveFailure();
		g_pTradeX->ProcessItemAutoMoveFailure();
	}
...
void GCNewHealthBarRecv(PMSG_NEW_HEALTH_BAR_RECV* lpMsg) // OK
{
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		gHealthBar.InsertNewHealthBar(lpInfo->index, lpInfo->type, lpInfo->rate);
	}
}
...
void ReceiveMana(BYTE* ReceiveBuffer)
{
	...
	{
	case 0xff:
		CharacterAttribute->Mana = Data->Life;
		CharacterAttribute->SkillMana = Data->Shield;
		break;
	case 0xfe:
		if (gCharacterManager.IsMasterLevel(Hero->Class) == true)
		{
			...
			//	Master_Level_Data.wMaxMana			= Data->ManaMax;
			//	Master_Level_Data.wMaxShield		= Data->ShieldMax;
			Master_Level_Data.wMaxMana = Data->Life;
			Master_Level_Data.wMaxBP = Data->Shield;
		}
		else
		{
			CharacterAttribute->ManaMax = Data->Life;
			CharacterAttribute->SkillManaMax = Data->Shield;
		}
		break;
	default:
		CharacterAttribute->Mana = Data->Life;
...
void ReceiveStorageCost(BYTE* ReceiveBuffer)
{
	...
	unicode::_sprintf(szTemp, GlobalText[3199], Data->value, Data->coinname);
...
void ReceiveDuelScore(BYTE* ReceiveBuffer)
{
	...
	if (g_DuelMgr.IsDuelPlayer(MAKEWORD(Data->bIndexL1, Data->bIndexH1), DUEL_HERO))
	{
		...
		g_DuelMgr.SetGuild(DUEL_HERO, Data->guild1);
		g_DuelMgr.SetGuild(DUEL_ENEMY, Data->guild2);
	}
	else if (g_DuelMgr.IsDuelPlayer(MAKEWORD(Data->bIndexL2, Data->bIndexH2), DUEL_HERO))
	{
		...
		g_DuelMgr.SetGuild(DUEL_HERO, Data->guild2);
		g_DuelMgr.SetGuild(DUEL_ENEMY, Data->guild1);
	}
}
...
void ReceivePersonalShopItemList(BYTE* ReceiveBuffer)
{
	...
	if (Header->byResult == 0x01)
	{
		...
		{
			if (pShopItem->iItemPrice > 0 || pShopItem->iItemPrice2 > 0 || pShopItem->iItemPrice3 > 0 || pShopItem->iItemPrice4 > 0)
			{
				...
				AddPersonalItemPrice(pShopItem->byPos, pShopItem->iItemPrice, pShopItem->iItemPrice2, pShopItem->iItemPrice3, pShopItem->iItemPrice4, PSHOPWNDTYPE_PURCHASE);
			}
			else
			{
...
void ReceivePurchaseItem(BYTE* ReceiveBuffer)
{
	...
	else
	{
		...
		case 0x0B:
		{
			g_pChatListBox->AddText("", GlobalText[3194], SEASON3B::TYPE_ERROR_MESSAGE);

		}
		break;
		case 0x0C:
		{
			g_pChatListBox->AddText("", GlobalText[3195], SEASON3B::TYPE_ERROR_MESSAGE);

		}
		break;
		case 0x0D:
		{
			g_pChatListBox->AddText("", GlobalText[3196], SEASON3B::TYPE_ERROR_MESSAGE);

		}
		break;
		case 0x0E:
		{
			g_pChatListBox->AddText("", GlobalText[3197], SEASON3B::TYPE_ERROR_MESSAGE);

		}
		break;
		case 0x0F:
		{
			g_pChatListBox->AddText("", GlobalText[3198], SEASON3B::TYPE_ERROR_MESSAGE);
...
void ReceiveBuffState(BYTE* ReceiveBuffer)
{
	...
	{
		RegisterBuff(bufftype, (&Hero->Object), Data->wEffectTime, Hero);
...
void ReceivePreviewPort(BYTE* ReceiveBuffer)
{
	...
	for (int i = 0; i < pData->Value; i++)
	{
		...
		case 1:
		{
			...
			{
				RegisterBuff(static_cast<eBuffState>(pData2->s_BuffEffectState[j]), o, 0, c);
			}
...
BOOL TranslateProtocol(int HeadCode, BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case 0xF5: //CUSTOM PACKETS
	{
		...
		g_ConsoleDebug->Write(MCD_RECEIVE, "Recv [0xF5][0x%02x]", subcode);

		switch (subcode)
		{
#if CLIENT_VERSION == 1002
		case 0x00: //receive AccountLevel
			LPVIP_RECV Data = (LPVIP_RECV)ReceiveBuffer;
			g_ConsoleDebug->Write(MCD_NORMAL, "AcoountLevelRecv: %d", (int)Data->AccountLevel);
			g_pBuffWindow->ReceiveAccountLevel((int)Data->AccountLevel);
			break;
#endif
		}
		break;
	...
	{
		//AddDebugText(ReceiveBuffer,Size);
		auto lpMsg = (PMSG_TRADE_RESPONSE_RECV*)ReceiveBuffer;
		if (lpMsg->TradeX) {
			g_pTradeX->GCTradeResponseRecv((PMSG_TRADE_RESPONSE_RECV*)ReceiveBuffer);
		}
		else {
	...
	{
		//AddDebugText(ReceiveBuffer,Size);
		auto lpMsg = (PMSG_TRADE_OK_BUTTON_RECV*)ReceiveBuffer;
		if (lpMsg->tradeX) {
			g_pTradeX->TradeXOkButtonRecv((PMSG_TRADE_OK_BUTTON_RECV*)ReceiveBuffer);
		}
		else {
	...
	case 0x2E:
		//recebe buff party
		gPartyBuffs.RecvPartyBuffs(ReceiveBuffer);
		break;
#endif
	case 0x46:
	...
	case 0xFB:
		...
		{
		case 0x16:
			gHunting.ReceiveHunting((PMSG_CHARACTER_HUNTING_INFO_RECV*)ReceiveBuffer);
			break;
		case 0x2C:
			GCItemCounterRecv((PMSG_ITEM_COUNTER_RECV*)ReceiveBuffer);
			break;
		case 0x10:
			g_pTradeX->TradeXStateRecv((PMSG_TRADEX_STATE_RECV*)ReceiveBuffer);
			break;
		case 0x11:
			g_pTradeX->TradeXListRecv((PMSG_TRADEX_ITEM_LIST_RECV*)ReceiveBuffer);
			break;
		case 0x13:
			g_pTradeX->TradeXDelItemTarget((PMSG_TRADEX_ITEM_DEL_RECV*)ReceiveBuffer);
			break;
		}
		break;
	case 0x90:
	...
	case 0xB1:
	{
		...
		case 0x01:
			...
			break;
		case 0x15:
			g_pTradeX->TradeXItemAddRecv((PMSG_TRADEX_ITEM_ADD_RECV*)ReceiveBuffer);
			break;
		}
	}
	break;
...
void InsertBuffPhysicalEffect(eBuffState buff, OBJECT* o, CHARACTER* c = NULL)
{
	switch (buff)
	{
	...
	break;
	case eBuff_PhysDefense://BUFF SM COLOR
	{
		if (o->Type == MODEL_PLAYER)
		{
			int color = 0;
			...
			DeleteJoint(MODEL_SPEARSKILL, o, color);
			for (int j = 0; j < 5; ++j)
			{
				CreateJoint(MODEL_SPEARSKILL, o->Position, o->Position, o->Angle, color, o, 50.f);
			}
	...
	break;
	case eBuff_HpRecovery: //BUFF BK
		//case eBuff_Life:
	{
		int color = 1;

		int customColor = gBuffColorBK.IsBuffColorBK((DWORD)c);

		if (customColor >= 100) color = customColor;

		DeleteEffect(BITMAP_LIGHT, o, color);
		CreateEffect(BITMAP_LIGHT, o->Position, o->Angle, o->Light, color, o);
	}
	break;
	case eDeBuff_Harden:
	{
...
void ClearBuffPhysicalEffect(eBuffState buff, OBJECT* o)
{
		...
		//case eBuff_Life:
	{
		...
		for (auto& it : gBuffColorBK.m_BuffColorBKInfo) {
			DeleteEffect(BITMAP_LIGHT, o, it.first);
		}
		for (auto& it : gBuffColorBK.m_BuffColorBK_2Info) {
			DeleteEffect(BITMAP_LIGHT, o, it.first);
		}
		for (auto& it : gBuffColorBK.m_BuffColorBK_3Info) {
			DeleteEffect(BITMAP_LIGHT, o, it.first);
		}
		for (auto& it : gBuffColorBK.m_BuffColorBK_4Info) {
			DeleteEffect(BITMAP_LIGHT, o, it.first);
		}
		for (auto& it : gBuffColorBK.m_BuffColorBK_5Info) {
			DeleteEffect(BITMAP_LIGHT, o, it.first);
		}
	}
	break;
	...
	case eBuff_PhysDefense: //BUFF SM COLOR
	{
		if (o->Type == MODEL_PLAYER)
		{
			...
			for (auto& it : gBuffColorSM.m_BuffColorSMInfo) {
				DeleteJoint(MODEL_SPEARSKILL, o, it.first);
			}
		}
	}
	break;
...
void RegisterBuff(eBuffState buff, OBJECT* o, const int bufftime, CHARACTER* c)
{
	...
	InsertBuffPhysicalEffect(buff, o, c);
