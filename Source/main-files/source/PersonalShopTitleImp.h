...
class CPersonalItemPriceTable 
{
	...
	std::map<int, SHOP_PRICE_INFO>	m_mapTable;
...
public:
	...
	void AddItemPrice(int index, int price, int price2, int price3, int price4)
	{
		std::map<int, SHOP_PRICE_INFO>::iterator mi = m_mapTable.find(index);
		if(mi != m_mapTable.end())
		{
			m_mapTable.erase(mi);	
		}	
		SHOP_PRICE_INFO info;
		info.price1 = price;
		info.price2 = price2;
		info.price3 = price3;
		info.price4 = price4;

		m_mapTable.insert(std::make_pair(index, info));
	}
	...
	bool GetItemPrice(int index, int& price, int& price2, int& price3, int& price4)
	{
		std::map<int, SHOP_PRICE_INFO>::iterator mi = m_mapTable.find(index);
		if(mi != m_mapTable.end()) 
		{
			price = mi->second.price1;
			price2 = mi->second.price2;
			price3 = mi->second.price3;
			price4 = mi->second.price4;
			return true;
		}
		return false;
	}
...
inline void AddPersonalItemPrice(int index, int price, int price2, int price3, int price4, int type)
{
	CPersonalItemPriceTable* pPersonalItemTable = CPersonalItemPriceTable::GetObjPtr(type);
	if(pPersonalItemTable) 
	{
		pPersonalItemTable->AddItemPrice(index, price, price2, price3, price4);
	}
}
...
inline bool GetPersonalItemPrice(int index, int& price, int& price2, int& price3, int& price4, int type)
{
	CPersonalItemPriceTable* pPersonalItemTable = CPersonalItemPriceTable::GetObjPtr(type);
	if(!pPersonalItemTable) 
		return false;

	return pPersonalItemTable->GetItemPrice(index, price, price2, price3, price4);
}
