...
void SEASON3B::CNewUIKanturu2ndEnterNpc::LoadImages()
{
	...
	LoadBitmap("Interface\\messagebox_top.tga", IMAGE_KANTURU2ND_TOP, GL_LINEAR);
	LoadBitmap("Interface\\messagebox_middle.tga", IMAGE_KANTURU2ND_MIDDLE, GL_LINEAR);
	LoadBitmap("Interface\\messagebox_bottom.tga", IMAGE_KANTURU2ND_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\newui_msgbox_back.jpg", IMAGE_KANTURU2ND_BACK, GL_LINEAR);
	LoadBitmap("Interface\\btn_small.jpg", IMAGE_KANTURU2ND_BTN, GL_LINEAR);
...
bool SEASON3B::CNewUIKanturu2ndEnterNpc::BtnProcess()
{
	...
	if(m_BtnEnter.UpdateMouseEvent() == true)
	{
		if(m_pNpcObject)
		{
			...
			if(pItemHelper->Type == ITEM_HELPER+2 || gHelperSystem.CheckHelperType(pItemHelper->Type, 1) == 1) //UNIRIA CUSTOM
			{
			...
			if(	!((pItemWing->Type >= ITEM_WING+0 && pItemWing->Type <= ITEM_WING+6)
				|| (pItemWing->Type >= ITEM_WING+36 && pItemWing->Type <= ITEM_WING+43 || gCustomWing.CheckCustomWingByItem(pItemWing->Type)) //WING CUSTOM
				...
				//|| gHelperSystem.CheckIsHelper(pItemHelper->Type)
				|| gHelperSystem.CheckHelperType(pItemHelper->Type, 2) == 1 //DINO CUSTOM
				|| gHelperSystem.CheckHelperType(pItemHelper->Type, 4) == 1 //FENRIR CUSTOM
