...
void SEASON3B::CNewUICommandWindow::InitButtons( )
{
	...
	for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
	{
		m_BtnCommand[i].SetButtonSize(2);
		m_BtnCommand[i].ChangeButtonImgState(true, IMAGE_COMMAND_BTN, true);
		m_BtnCommand[i].SetFont(g_hFontBold);
		m_BtnCommand[i].ChangeTextColor(RGBA(245, 232, 202, 255));
		m_BtnCommand[i].ChangeButtonInfo(m_Pos.x + (COMMAND_WINDOW_WIDTH / 2 - 120 / 2), (m_Pos.y + 80) + ((i - 1) * 29), 120, 27);
	}
#endif
...
#elif CLIENT_VERSION >= 1001
	g_pRenderText->RenderText(m_Pos.x + 60, m_Pos.y + 14, GlobalText[938], 72, 0, RT3_SORT_CENTER);
#endif
...
#elif CLIENT_VERSION >= 1001
			RenderImageScale(BITMAP_COMMAND_WINDOW_BEGIN, x, y, Width, Height, 0.f, 0.f, 295.f, 54.f);
#endif
...
#elif CLIENT_VERSION >= 1001
	//RenderBitmap(IMAGE_COMMAND_BASE_WINDOW_BACK_VOID, m_Pos.x, m_Pos.y, COMMAND_WINDOW_WIDTH, COMMAND_WINDOW_HEIGHT, 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
	RenderImageScaleAuto(IMAGE_COMMAND_BASE_WINDOW_BACK_VOID, m_Pos.x, m_Pos.y, COMMAND_WINDOW_WIDTH, COMMAND_WINDOW_HEIGHT);
#endif
}
...
void SEASON3B::CNewUICommandWindow::SetBtnState(int iBtnType, bool bStateDown)
{
	...
	if (bStateDown)
	{
		m_BtnCommand[iBtnType].ForceButtonDown(true);
		m_BtnCommand[iBtnType].ChangeTextColor(RGBA(245, 232, 202, 200));
	}
	else
	{
		m_BtnCommand[iBtnType].ForceButtonDown(false);
		m_BtnCommand[iBtnType].ChangeTextColor(RGBA(245, 232, 202, 255));
	}
#endif
...
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_COMMAND_BASE_WINDOW_BTN_EXIT, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_msgbox_back.jpg", IMAGE_COMMAND_BASE_WINDOW_BACK, GL_LINEAR);
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_COMMAND_BASE_WINDOW_BACK_VOID, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back01.tga", IMAGE_COMMAND_BASE_WINDOW_TOP, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-L.tga", IMAGE_COMMAND_BASE_WINDOW_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_COMMAND_BASE_WINDOW_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back03.tga", IMAGE_COMMAND_BASE_WINDOW_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\btn_large.jpg", IMAGE_COMMAND_BTN, GL_LINEAR);
	LoadBitmap("Interface\\command_cursor_back.jpg", IMAGE_COMMAND_SELECTID_BG, GL_LINEAR);
...
void SEASON3B::CNewUICommandWindow::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_COMMAND_BASE_WINDOW_BACK_VOID);
#endif
}
