...
void BMD::RenderMesh(int meshIndex, int renderFlags, float alpha, int blendMeshIndex, float blendMeshAlpha, float blendMeshTextureCoordU, float blendMeshTextureCoordV, int explicitTextureIndex)
{
	...
	if (textureIndex == BITMAP_HIDE)
		return;
	else if (textureIndex == BITMAP_SKIN)
	{
		if (HideSkin) return;
...
void BMD::RenderBodyShadow(const int blendMesh, const int hiddenMesh, const int startMeshNumber, const int endMeshNumber, void* pClothes, const int clothesCount)
{
	...
	glColor4f(0.0f, 0.0f, 0.0f, 0.6f); // 50% opacity for shadows
...
bool BMD::OpenCustom(char* DirName, char* ModelFileName, bool bReAlloc)
{
	if (true == m_bCompletedAlloc)
	{
		if (true == bReAlloc)
	...
	const BYTE gFileProtectXorTable[16] = { 0xDF, 0xCA, 0x81, 0xCD, 0x05, 0xBD, 0xAD, 0x7A, 0xF8, 0xE1, 0xBC, 0x8B, 0x4F, 0x33, 0xDB, 0xD9 };

	for (int n = 0; n < DataBytes; n++)
	{
		Data[n] ^= gFileProtectXorTable[n % sizeof(gFileProtectXorTable)];
	}
...
bool BMD::SaveCustom(char* DirName, char* ModelFileName)
{
	char ModelName[64];
	strcpy(ModelName, DirName);
	strcat(ModelName, ModelFileName);
