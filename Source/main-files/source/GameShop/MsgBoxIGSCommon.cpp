...
void CMsgBoxIGSCommon::SetButtonInfo()
{
	m_BtnOk.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, GetPos().x + (IMAGE_IGS_FRAME_WIDTH / 2) - (IMAGE_IGS_BTN_WIDTH / 2), (GetPos().y + m_iMsgBoxHeight+10) - (IMAGE_IGS_BTN_HEIGHT + IGS_BTN_POS_Y), IMAGE_IGS_BTN_WIDTH, IMAGE_IGS_BTN_HEIGHT, CNewUIMessageBoxButton::MSGBOX_BTN_CUSTOM, true);
	m_BtnOk.MoveTextPos(0, -1);
...
void CMsgBoxIGSCommon::RenderTexts()
{
	...
	// Title
	g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_TITLE_POS_Y+2, m_szTitle, IMAGE_IGS_FRAME_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->SetFont(g_hFont);
	g_pRenderText->SetTextColor(255, 255, 255, 255);

	// Text
	int iY = IGS_TEXT_ITEM_INFO_POS_Y+5;
	if (m_iNumTextLine <= IGS_NUM_TEXT_LIMIT_RENDER_MIDDLE_LINE)
	{
		iY = iY + ((IGS_NUM_TEXT_LIMIT_RENDER_MIDDLE_LINE - m_iNumTextLine) * 5);
	}
