...
void CNewUIInGameShop::RenderFrame()
{
	...
	RenderImageScale(IMAGE_IGS_BACK, m_Pos.x, m_Pos.y, 475, IMAGE_IGS_BACK_HEIGHT, 0.f, 0.f, 1000.f, 903.f);
	RenderImageScale(IMAGE_IGS_CATEGORY_DECO_DOWN, m_Pos.x+475, m_Pos.y, 164, IMAGE_IGS_BACK_HEIGHT, 0.f, 0.f, 345.f, 903.f);
	RenderImageScale(IMAGE_IGS_ITEMBOX_LOGO, (m_Pos.x + 98) + (122 * 2), m_Pos.y + 9, 113, 20, 0.f, 0.f, 330.f, 53.f);
	RenderImageScale(IMAGE_IGS_ITEMBOX_LOGO, m_Pos.x + TEXT_IGS_CASH_POS_X - 10, m_Pos.y + 272.5f, 80, 20, 0.f, 0.f, 330.f, 53.f);

	glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
	RenderColor(m_Pos.x + TEXT_IGS_CASH_POS_X-10, m_Pos.y + TEXT_IGS_CHAR_NAME_POS_Y-3, 140, 15);
	glColor4f(0.0f, 0.0f, 0.0f, 0.4f);
	RenderColor(m_Pos.x + TEXT_IGS_CASH_POS_X - 10, m_Pos.y + TEXT_IGS_CHAR_NAME_POS_Y-3+15, 140, 60);

	glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
	RenderColor(m_Pos.x + TEXT_IGS_CASH_POS_X - 10, m_Pos.y + 128, 140, 15);
	glColor4f(0.0f, 0.0f, 0.0f, 0.4f);
	RenderColor(m_Pos.x + TEXT_IGS_CASH_POS_X - 10, m_Pos.y + 128+15, 140, 125);
	EndRenderColor();
#endif
}
...
void CNewUIInGameShop::RenderTexts()
{
	...
	for (int i = 0; i < g_InGameShopSystem->GetSizePackageAsDisplayPackage(); i++)
	{
		RenderImageScale(IMAGE_IGS_CATEGORY_DECO_MIDDLE, (m_Pos.x + 98) + (122 * column), (m_Pos.y + 34) + (120 * row), 115, 92, 0.f, 0.f, 665.f, 457.f);
		CShopPackage* pPackage = g_InGameShopSystem->GetDisplayPackage(i);
		// Package
		g_pRenderText->SetTextColor(255, 255, 255, 255);
		g_pRenderText->RenderText((m_Pos.x + 98) + (122 * column), (m_Pos.y + 34) + (120 * row)+5, pPackage->PackageProductName, 115, 0, RT3_SORT_CENTER);
		// Package
		ConvertGold(pPackage->Price, szValue);
		sprintf(szText, "%s %s", szValue, pPackage->PricUnitName);
		g_pRenderText->SetTextColor(255, 238, 161, 255);
		g_pRenderText->RenderText((m_Pos.x + 98) + (122 * column), (m_Pos.y + 34) + (120 * row) + 81, szText, 115, 0, RT3_SORT_CENTER);
		if (column < 2) {
			column++;
		}
		else {
	...
	sprintf(szText, GlobalText[2883], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_CASH_POS_Y-5, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	sprintf(szText, GlobalText[3145], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_MILEAGE_POS_Y-5, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);
	sprintf(szText, GlobalText[2884], "");
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_POINT_POS_Y-5, szText, TEXT_IGS_CASH_WIDTH, 0, RT3_SORT_LEFT);


	g_pRenderText->SetTextColor(255, 255, 255, 255);
	ConvertGold(g_InGameShopSystem->GetCashCreditCard(), szValue);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_CASH_POS_Y-5, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);
	ConvertGold(g_InGameShopSystem->GetCashPrepaid(), szValue);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_MILEAGE_POS_Y-5, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);
	ConvertGold(g_InGameShopSystem->GetTotalMileage(), szValue, 1);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X + 50, m_Pos.y + TEXT_IGS_POINT_POS_Y-5, szValue, TEXT_IGS_CASH_WIDTH - 56, 0, RT3_SORT_RIGHT);

	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->SetTextColor(247, 128, 0, 255);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_STORAGE_NAME_POS_X, m_Pos.y + TEXT_IGS_STORAGE_NAME_POS_Y+1, GlobalText[2951], TEXT_IGS_STORAGE_NAME_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_STORAGE_TIME_POS_X, m_Pos.y + TEXT_IGS_STORAGE_NAME_POS_Y+1, GlobalText[2952], TEXT_IGS_STORAGE_TIME_WIDTH, 0, RT3_SORT_CENTER);
...
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + TEXT_IGS_PAGE_POS_Y, szText, 475, 0, RT3_SORT_CENTER);	
#elif CLIENT_VERSION >= 1001
	g_pRenderText->RenderText((m_Pos.x + 98) + (122 * 2), m_Pos.y + 15, szText, 115, 0, RT3_SORT_CENTER);
#endif

	// Storage Page Info

	sprintf(szText, "%d / %d", m_iStorageCurrentPage, m_iStorageTotalPage);
	g_pRenderText->RenderText(m_Pos.x + TEXT_IGS_CASH_POS_X - 10, m_Pos.y + 279, szText, 77, 0, RT3_SORT_CENTER);
	g_pRenderText->SetTextColor(255, 255, 255, 255);
#endif
}
...
void CNewUIInGameShop::RenderDisplayItems()
{
	...
	gluPerspective2(1.9f, (float)(WindowWidth) / (float)(WindowHeight), RENDER_ITEMVIEW_NEAR, RENDER_ITEMVIEW_FAR);
	...
	for (int i = 0; i < g_InGameShopSystem->GetSizePackageAsDisplayPackage(); i++)
	{
		int iPosX = gWideScreen.GetCenterPosition(IGS_ITEMRENDER_POS_X_STANDAD) + (IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X * (i % IGS_NUM_ITEMS_WIDTH));
		int iPosY = m_Pos.y+IGS_ITEMRENDER_POS_Y_STANDAD + (IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_Y * (i / IGS_NUM_ITEMS_HEIGHT));
...
void CNewUIInGameShop::SetBtnInfo()
{
...
#elif CLIENT_VERSION >= 1001
	m_ListBoxTabButton.CreateRadioGroup(IGS_TOTAL_LISTBOX, BITMAP_UNKNOWN);
	m_ListBoxTabButton.ChangeRadioButtonInfo(true, m_Pos.x + IMAGE_IGS_TAB_BTN_POS_X, m_Pos.y + 105, 70, 16, 3);
	m_ListBoxTabButton.ChangeButtonState(SEASON3B::BUTTON_STATE_DOWN, 0);
	m_ListBoxTabButton.ChangeButtonState(IGS_SAFEKEEPING_LISTBOX, BITMAP_UNKNOWN, SEASON3B::BUTTON_STATE_UP, 0);
	m_ListBoxTabButton.ChangeButtonState(IGS_PRESENTBOX_LISTBOX, BITMAP_UNKNOWN, SEASON3B::BUTTON_STATE_UP, 0);
	m_ListBoxTabButton.ChangeButtonState(IGS_SAFEKEEPING_LISTBOX, IMAGE_IGS_LEFT_TAB, SEASON3B::BUTTON_STATE_DOWN, 0);
	m_ListBoxTabButton.ChangeButtonState(IGS_PRESENTBOX_LISTBOX, IMAGE_IGS_RIGHT_TAB, SEASON3B::BUTTON_STATE_DOWN, 0);
	m_CloseButton.ChangeButtonInfo(m_Pos.x + IMAGE_IGS_EXIT_BTN_POS_X+118, m_Pos.y + IMAGE_IGS_EXIT_BTN_POS_Y-23, 25, 24);
	m_ListBoxTabButton.SetButtonSize(5);
	m_CloseButton.SetButtonSize(0);
#endif
...
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
		m_ViewDetailButton[i].ChangeButtonInfo(gWideScreen.GetCenterPosition(IMAGE_IGS_VIEWDETAIL_BTN_POS_X) + ((i % IGS_NUM_ITEMS_WIDTH) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X), IMAGE_IGS_VIEWDETAIL_BTN_POS_Y + ((i / IGS_NUM_ITEMS_HEIGHT) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_Y), IMAGE_IGS_VIEWDETAIL_BTN_WIDTH, IMAGE_IGS_VIEWDETAIL_BTN_HEIGHT);
#elif CLIENT_VERSION >= 1001
		m_ViewDetailButton[i].ChangeButtonInfo(gWideScreen.GetCenterPosition(IMAGE_IGS_VIEWDETAIL_BTN_POS_X) + ((i % IGS_NUM_ITEMS_WIDTH) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X)-2, m_Pos.y+IMAGE_IGS_VIEWDETAIL_BTN_POS_Y + ((i / IGS_NUM_ITEMS_HEIGHT) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_Y)+2, IMAGE_IGS_VIEWDETAIL_BTN_WIDTH, IMAGE_IGS_VIEWDETAIL_BTN_HEIGHT);
		m_ViewDetailButton[i].SetButtonSize(1);
		m_ViewDetailButton[i].SetFont(g_hFontBold);
		m_ViewDetailButton[i].ChangeTextColor(RGBA(245, 232, 202, 255));
#endif
	}
...
#elif CLIENT_VERSION >= 1001
	m_CashRefreshButton.ChangeButtonInfo(m_Pos.x + IMAGE_IGS_REFRESH_BTN_POS_X+43, m_Pos.y + TEXT_IGS_CHAR_NAME_POS_Y - 1, 11, 11);
#endif
	...
	m_UseButton.ChangeButtonInfo(m_Pos.x + TEXT_IGS_CASH_POS_X + 77, m_Pos.y + 274, IMAGE_IGS_VIEWDETAIL_BTN_WIDTH, IMAGE_IGS_VIEWDETAIL_BTN_HEIGHT);
	m_UseButton.SetFont(g_hFontBold);
	m_UseButton.SetButtonSize(1);
#endif
...
#elif CLIENT_VERSION >= 1001
	m_PrevButton.ChangeButtonInfo((m_Pos.x + 98) + (122 * 2)+2, m_Pos.y + 11, 16, 16);
	m_NextButton.ChangeButtonInfo((m_Pos.x + 98) + (122 * 2) + 95, m_Pos.y + 11, 16, 16);
#endif

	m_StoragePrevButton.ChangeButtonImgState(true, IMAGE_IGS_PAGE_LEFT, true);
	m_StorageNextButton.ChangeButtonImgState(true, IMAGE_IGS_PAGE_RIGHT, true);
...
#elif CLIENT_VERSION >= 1001
	m_StoragePrevButton.ChangeButtonInfo(m_Pos.x + TEXT_IGS_CASH_POS_X - 10 + 2, m_Pos.y + 275, 16, 16);
	m_StorageNextButton.ChangeButtonInfo(m_Pos.x + TEXT_IGS_CASH_POS_X - 10 + 62, m_Pos.y + 275, 16, 16);
#endif
}
...
void CNewUIInGameShop::RenderBanner()
{
	...
	RenderImage(IMAGE_IGS_BANNER, gWideScreen.GetCenterPosition(IMAGE_IGS_BANNER_POS_X), IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH, IMAGE_IGS_BANNER_HEIGHT);
}
...
bool CNewUIInGameShop::UpdateBanner()
{
	...
	if ((SEASON3B::IsPress(VK_LBUTTON))
		&& (SEASON3B::CheckMouseIn(gWideScreen.GetCenterPosition(IMAGE_IGS_BANNER_POS_X), IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH, IMAGE_IGS_BANNER_HEIGHT)))
	{
...
#if CLIENT_VERSION >= 1001
	m_ZoneButton.ChangeRadioButtonInfo(true, m_Pos.x + IMAGE_IGS_ZONE_BTN_POS_X+5, m_Pos.y + IMAGE_IGS_ZONE_BTN_POS_Y+9, 65, 23, 10);
	m_ZoneButton.SetButtonSize(1);
#endif
}
...
void CNewUIInGameShop::InitCategoryBtn()
{
	...
	m_CategoryButton.ChangeRadioButtonInfo(false, m_Pos.x + IMAGE_IGS_CATEGORY_BTN_POS_X, m_Pos.y + IMAGE_IGS_CATEGORY_BTN_POS_Y+7, IMAGE_IGS_CATEGORY_BTN_WIDTH, IMAGE_IGS_CATEGORY_BTN_HEIGHT, IMAGE_IGS_CATEGORY_BTN_DISTANCE);
...
#if CLIENT_VERSION >= 1001
	m_CategoryButton.SetButtonSize(1);
#endif
}
...
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_IGS_EXIT_BTN, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\InGameShop\\Ingame_shopback_Left.jpg", IMAGE_IGS_BACK, GL_LINEAR);
	LoadBitmap("Interface\\btn_medium.jpg", IMAGE_IGS_CATEGORY_BTN, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\item_box.jpg", IMAGE_IGS_CATEGORY_DECO_MIDDLE, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\Ingame_shopback_Right.jpg", IMAGE_IGS_CATEGORY_DECO_DOWN, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\storage_left.jpg", IMAGE_IGS_LEFT_TAB, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\storage_right.jpg", IMAGE_IGS_RIGHT_TAB, GL_LINEAR);
	LoadBitmap("Interface\\btn_medium_decor.jpg", IMAGE_IGS_ZONE_BTN, GL_LINEAR);
	LoadBitmap("Interface\\btn_shop.jpg", IMAGE_IGS_ITEMGIFT_BTN, GL_LINEAR);
	LoadBitmap("Interface\\btn_balance.jpg", IMAGE_IGS_CASHGIFT_BTN, GL_LINEAR);
	LoadBitmap("Interface\\btn_reload.jpg", IMAGE_IGS_REFRESH_BTN, GL_LINEAR);
	LoadBitmap("Interface\\btn_medium.jpg", IMAGE_IGS_VIEWDETAIL_BTN, GL_LINEAR);
	LoadBitmap("Interface\\store_name.jpg", IMAGE_IGS_ITEMBOX_LOGO, GL_LINEAR);
	LoadBitmap("Interface\\btn_previous.jpg", IMAGE_IGS_PAGE_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\btn_next.jpg", IMAGE_IGS_PAGE_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\IGS_Storage_Page.tga", IMAGE_IGS_STORAGE_PAGE, GL_LINEAR);
	LoadBitmap("Interface\\btn_previous.jpg", IMAGE_IGS_STORAGE_PAGE_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\btn_next.jpg", IMAGE_IGS_STORAGE_PAGE_RIGHT, GL_LINEAR);
