...
void CMsgBoxIGSBuyPackageItem::RenderFrame()
{
	...
	RenderImageScale(IMAGE_IGS_FRAME, x + 15, y + 122, IMAGE_IGS_FRAME_WIDTH - 30, 110, 0.f, 0.f, 330.f, 212.f, 0.7f);
	RenderImageScale(BITMAP_INVENTORY+11, x + 15, y + 238, IMAGE_IGS_FRAME_WIDTH - 30, 27, 0.f, 0.f, 330.f, 53.f, 0.7f);
#endif
...
void CMsgBoxIGSBuyPackageItem::RenderTexts()
{
	...
	g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_TITLE_POS_Y+4, GlobalText[2890], IMAGE_IGS_FRAME_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->SetTextColor(247, 186, 0, 255);

	g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_NAME_POS_Y+26, m_szPackageName, IMAGE_IGS_FRAME_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->SetFont(g_hFont);

	g_pRenderText->SetTextColor(255, 238, 161, 255);

	g_pRenderText->RenderText(GetPos().x + IGS_TEXT_PRICE_POS_X-7, GetPos().y + IGS_TEXT_PRICE_POX_Y+17, m_szPrice, IGS_TEXT_PRICE_WIDTH, 0, RT3_SORT_RIGHT);
#endif
...
#elif CLIENT_VERSION >= 1001
	RenderItem3D(GetPos().x+IGS_3DITEM_POS_X, GetPos().y+IGS_3DITEM_POS_Y+20,IGS_3DITEM_WIDTH, IGS_3DITEM_HEIGHT, m_wItemCode, 0, 0, 0, true);
#endif
}
...
#elif CLIENT_VERSION >= 1001
	m_PackageInfo.SetPosition(GetPos().x+IGS_LISTBOX_POS_X+5, GetPos().y+IGS_LISTBOX_POS_Y+15);
	m_PackageInfo.SetLineColorRender(false);
#endif
}
