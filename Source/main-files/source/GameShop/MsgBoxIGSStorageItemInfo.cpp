...
#elif CLIENT_VERSION >= 1001
	m_BtnUse.SetInfo(IMAGE_IGS_BUTTON, GetPos().x + IGS_BTN_OK_POS_X, GetPos().y + IGS_BTN_POS_Y-20, IMAGE_IGS_BTN_WIDTH, IMAGE_IGS_BTN_HEIGHT);
	m_BtnUse.SetText(GlobalText[228]);
...
void CMsgBoxIGSStorageItemInfo::RenderFrame()
{
	...
	RenderImageScale(IMAGE_IGS_FRAME, x + ((IMAGE_IGS_FRAME_WIDTH/2)-(95/2)), y + 7, 95, 70, 0.f, 0.f, 330.f, 106.f, 0.7f);
	RenderImageScale(IMAGE_IGS_FRAME, x + 15, y + 85, IMAGE_IGS_FRAME_WIDTH - 30, 55, 0.f, 0.f, 330.f, 106.f, 0.7f);
#endif
...
void CMsgBoxIGSStorageItemInfo::RenderTexts()
{
	...
	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_ITEM_NAME_POS_Y-10, m_szName, IMAGE_IGS_FRAME_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->SetFont(g_hFont);
	g_pRenderText->SetTextColor(255, 255, 255, 255);

	g_pRenderText->RenderText(GetPos().x + IGS_TEXT_ITEM_INFO_POS_X, GetPos().y + IGS_TEXT_ITEM_INFO_NUM_POS_Y-17, m_szNum, IGS_TEXT_ITEM_INFO_WIDTH, 0, RT3_SORT_LEFT);
	g_pRenderText->RenderText(GetPos().x + IGS_TEXT_ITEM_INFO_POS_X, GetPos().y + IGS_TEXT_ITEM_INFO_PERIOD_POS_Y-17, m_szPeriod, IGS_TEXT_ITEM_INFO_WIDTH, 0, RT3_SORT_LEFT);
#endif
...
void CMsgBoxIGSStorageItemInfo::LoadImages()
{
	...
	LoadBitmap("Interface\\store_name_large.jpg", IMAGE_IGS_FRAME, GL_LINEAR);
#endif
}
