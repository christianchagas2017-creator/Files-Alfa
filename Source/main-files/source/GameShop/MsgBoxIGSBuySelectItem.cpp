...
bool CMsgBoxIGSBuySelectItem::Create(float fPriority)
{
	...
	CNewUIMessageBoxBase::Create((gWideScreen.GetAddPosition(IMAGE_IGS_WINDOW_WIDTH)/2)-(IMAGE_IGS_FRAME_WIDTH/2),(IMAGE_IGS_WINDOW_HEIGHT/2)-(IMAGE_IGS_FRAME_HEIGHT/2), IMAGE_IGS_FRAME_WIDTH, IMAGE_IGS_FRAME_HEIGHT, fPriority);
...
void CMsgBoxIGSBuySelectItem::RenderFrame()
{
	...
	RenderImageScale(IMAGE_IGS_MGSBOX_BACK, x + ((IMAGE_IGS_FRAME_WIDTH / 2) - (95 / 2)), y + 42, 95, 70, 0.f, 0.f, 330.f, 212.f, 0.7f);
	RenderImageScale(IMAGE_IGS_MGSBOX_BACK, x + 15, y + 185, IMAGE_IGS_FRAME_WIDTH - 30, 111, 0.f, 0.f, 330.f, 212.f, 0.7f);
	RenderImageScale(BITMAP_INVENTORY + 11, x + 15, y + 302, IMAGE_IGS_FRAME_WIDTH - 30, 27, 0.f, 0.f, 330.f, 53.f, 0.7f);
#endif
}
...
void CMsgBoxIGSBuySelectItem::RenderTexts()
{
	...
	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_TITLE_POS_Y-5, GlobalText[2890], IMAGE_IGS_FRAME_WIDTH, 0, RT3_SORT_CENTER);
	g_pRenderText->SetTextColor(255, 255, 0, 255);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_NAME_POS_Y+5, m_szPackageName, IMAGE_IGS_FRAME_WIDTH, 0, RT3_SORT_CENTER);
	g_pRenderText->SetFont(g_hFont);
	g_pRenderText->SetTextColor(255, 255, 255, 255);

	for (int i = 0; i < m_iDescriptionLine; i++)
	{
		g_pRenderText->RenderText(GetPos().x + IGS_TEXT_ATTR_POS_X+5, GetPos().y + IGS_TEXT_ATTR_POS_Y + (i * 10), m_szDescription[i], IGS_TEXT_ATTR_WIDTH, 0, RT3_SORT_LEFT);
	}
	
	g_pRenderText->SetTextColor(255, 238, 161, 255);

	g_pRenderText->RenderText(GetPos().x + IGS_TEXT_PRICE_POS_X, GetPos().y + IGS_TEXT_PRICE_POX_Y+13, m_szPrice, IGS_TEXT_PRICE_WIDTH, 0, RT3_SORT_RIGHT);
	g_pRenderText->SetTextColor(255, 255, 255, 255);
#endif
...
#elif CLIENT_VERSION >= 1001
	RenderItem3D(GetPos().x+IGS_3DITEM_POS_X, GetPos().y+IGS_3DITEM_POS_Y+10, IGS_3DITEM_WIDTH, IGS_3DITEM_HEIGHT, m_wItemCode, 0, 0, 0, true);
#endif
...
#elif CLIENT_VERSION >= 1001
	m_BtnBuy.SetInfo(IMAGE_IGS_BUTTON, GetPos().x+IGS_BTN_BUY_POS_X, GetPos().y+IGS_BTN_POS_Y+20, IMAGE_IGS_BTN_WIDTH, IMAGE_IGS_BTN_HEIGHT);
	m_BtnBuy.SetButtonSize(1);
	m_BtnBuy.SetText(GlobalText[2891]);	
	
	m_BtnPresent.SetInfo(IMAGE_IGS_BUTTON, GetPos().x+IGS_BTN_PRESENT_POS_X, GetPos().y+IGS_BTN_POS_Y+20,IMAGE_IGS_BTN_WIDTH, IMAGE_IGS_BTN_HEIGHT);
	m_BtnPresent.SetButtonSize(1);
	m_BtnPresent.SetText(GlobalText[2892]);	
	
	m_BtnCancel.SetInfo(IMAGE_IGS_BUTTON, GetPos().x+IGS_BTN_CANCEL_POS_X, GetPos().y+IGS_BTN_POS_Y+20, IMAGE_IGS_BTN_WIDTH, IMAGE_IGS_BTN_HEIGHT);
...
{
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	LoadBitmap("Interface\\InGameShop\\Ingame_pack_back01.tga", IMAGE_IGS_MGSBOX_BACK, GL_LINEAR);
	LoadBitmap("Interface\\InGameShop\\Ingame_Bt03.tga", IMAGE_IGS_BUTTON, GL_LINEAR);
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\store_name_big.jpg", IMAGE_IGS_MGSBOX_BACK, GL_LINEAR);
...
#elif CLIENT_VERSION >= 1001
	m_SelectBuyListBox.SetPosition(GetPos().x + IGS_LISTBOX_POS_X-2, GetPos().y + IGS_LISTBOX_POS_Y+3);
#endif
