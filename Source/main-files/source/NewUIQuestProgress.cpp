...
bool CNewUIQuestProgress::Create(CNewUIManager* pNewUIMng,int x, int y)
{
	...
	m_btnComplete.ChangeButtonInfo(x + (QP_WIDTH - 108) / 2, y + 352, 108, 29);
...
#elif CLIENT_VERSION >= 1001
	//RenderBitmap(IMAGE_QP_BACK_VOID, m_Pos.x, m_Pos.y, QP_WIDTH, QP_HEIGHT, 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
	RenderImageScaleAuto(IMAGE_QP_BACK_VOID, m_Pos.x, m_Pos.y, QP_WIDTH, QP_HEIGHT);
	RenderImageScaleAuto(IMAGE_QP_LINE, m_Pos.x + 7.1f, m_Pos.y + 185, 176.f, 10.f);
#endif
...
#elif CLIENT_VERSION >= 1001
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y+25,		g_QuestMng.GetSubject(m_dwCurQuestIndex), QP_WIDTH, 0, RT3_SORT_CENTER);
	g_pRenderText->SetTextColor(255, 255, 10, 255);
	g_pRenderText->RenderText(m_Pos.x + 13, m_Pos.y + 47, g_QuestMng.GetNPCName(),
#endif
...
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_QP_BTN_CLOSE, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_msgbox_back.jpg", IMAGE_QP_BACK, GL_LINEAR);
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_QP_BACK_VOID, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back04.tga", IMAGE_QP_TOP, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-L.tga", IMAGE_QP_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_QP_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back03.tga", IMAGE_QP_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\division_bar.tga", IMAGE_QP_LINE, GL_LINEAR);
	LoadBitmap("Interface\\btn_previous.jpg", IMAGE_QP_BTN_L, GL_LINEAR);
	LoadBitmap("Interface\\btn_next.jpg", IMAGE_QP_BTN_R, GL_LINEAR);
	LoadBitmap("Interface\\btn_large.jpg", IMAGE_QP_BTN_COMPLETE, GL_LINEAR);
#endif
}
...
void CNewUIQuestProgress::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_QP_BACK_VOID);
#endif
}
