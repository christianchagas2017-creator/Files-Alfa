...
void SetDarkSpirit(int ItemIndex, std::string modelNameDir, std::string modelName, std::string objectNameDir, std::string objectName, int Type)
{
	...
	strcpy_s(info.ModelNameDir, modelNameDir.c_str());

	strcpy_s(info.ObjectName, objectName.c_str());

	strcpy_s(info.ObjectNameDir, objectNameDir.c_str());
...
void CDarkSpirit::Init()
{
	...
	sprintf_s(name, "%s\\CharacterSystem\\DarkSpirit.lua", LUA_LOAD_PATH);

	this->m_Lua.DoFile(name);

	this->m_Lua.Generic_Call("StartLoadDarkSpirit", ">");
}


void CDarkSpirit::RestartLua()
{
	this->m_Lua.CloseLua();
