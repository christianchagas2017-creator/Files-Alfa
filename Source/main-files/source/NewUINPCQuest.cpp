							...
							CNewUI3DRenderMng* pNewUI3DRenderMng, int x, int y)
{
	...
	m_btnComplete.ChangeText(GlobalText[699]);
	m_btnComplete.ChangeButtonImgState(true, IMAGE_NPCQUEST_BTN_COMPLETE, true);
	m_btnComplete.SetButtonSize(2);
	m_btnComplete.ChangeButtonInfo(x + 41, y + 355, 108, 29);

	m_btnClose.ChangeButtonImgState(true, IMAGE_NPCQUEST_BTN_CLOSE);
	m_btnClose.ChangeButtonInfo(x + 23, y + 392, 25, 24);
	m_btnClose.ChangeToolTipText(GlobalText[1002], true);
#endif
...
#elif CLIENT_VERSION >= 1001
		RenderImageScaleAuto(IMAGE_NPCQUEST_LINE, m_Pos.x + 7.1f, m_Pos.y + 325, 176.f, 10.f);

		if (RenderItemMobText())
		{
			m_btnComplete.UnLock();
			m_btnComplete.ForceButtonDisabled(false);
			m_btnComplete.SetFont(g_hFontBold);
			m_btnComplete.ChangeTextColor(RGBA(245, 232, 202, 255));
		}
		else
		{
			m_btnComplete.Lock();
			m_btnComplete.ForceButtonDisabled(true);
			m_btnComplete.SetFont(g_hFontBold);
			m_btnComplete.ChangeTextColor(RGBA(245, 232, 202, 200));
		}
#endif
...
#elif CLIENT_VERSION >= 1001
		RenderBitmap(IMAGE_NPCQUEST_ZEN, m_Pos.x + 10, m_Pos.y + 361, 168.f, 18, 0.002, 0.0, 0.640, 0.818956, 1, 1, 1.0);
		
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->SetBgColor(0);

		g_pRenderText->SetTextColor(255, 220, 150, 255);
		g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 366, GlobalText[198]);

		unicode::t_char szTemp[128];
		g_pRenderText->SetTextColor(::getGoldColor(g_csQuest.GetNeedZen()));
		::ConvertGold(g_csQuest.GetNeedZen(), szTemp);
		g_pRenderText->RenderText(m_Pos.x + 170, m_Pos.y + 366, szTemp, 0, 0, RT3_WRITE_RIGHT_TO_LEFT);
#endif
	}
...
#elif CLIENT_VERSION >= 1001
	//RenderBitmap(IMAGE_NPCQUEST_BACK_VOID, m_Pos.x, m_Pos.y, NPCQUEST_WIDTH, NPCQUEST_HEIGHT, 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
	RenderImageScaleAuto(IMAGE_NPCQUEST_BACK_VOID, m_Pos.x, m_Pos.y, NPCQUEST_WIDTH, NPCQUEST_HEIGHT);
	RenderImageScaleAuto(IMAGE_NPCQUEST_LINE, m_Pos.x + 7.1f, m_Pos.y + 220, 176.f, 10.f);
#endif
...
#elif CLIENT_VERSION >= 1001
		g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 45,
			g_csQuest.getQuestTitle(), NPCQUEST_WIDTH, 0, RT3_SORT_CENTER);
#endif
...
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_NPCQUEST_BTN_CLOSE, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_msgbox_back.jpg", IMAGE_NPCQUEST_BACK, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back04.tga", IMAGE_NPCQUEST_TOP, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-L.tga", IMAGE_NPCQUEST_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_NPCQUEST_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back03.tga", IMAGE_NPCQUEST_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_NPCQUEST_BACK_VOID, GL_LINEAR);
	LoadBitmap("Interface\\division_bar.tga", IMAGE_NPCQUEST_LINE, GL_LINEAR);
	LoadBitmap("Interface\\item_money_2.jpg", IMAGE_NPCQUEST_ZEN, GL_LINEAR);
	LoadBitmap("Interface\\btn_large.jpg", IMAGE_NPCQUEST_BTN_COMPLETE, GL_LINEAR);
#endif
}
...
void CNewUINPCQuest::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_NPCQUEST_BACK_VOID);
#endif
