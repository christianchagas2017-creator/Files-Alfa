...
extern void RegisterBuff( eBuffState buff, OBJECT* o, const int bufftime = 0, CHARACTER* c = NULL);
...
bool g_bGMNOObservation = false; //UNCHARACTERNAME
bool g_bCustomTitle = true;		 //CUSTOM TITLE
bool g_bCustomColor = true;		 //CUSTOM COLOR CHAT
...
void RenderTipTextBorder(int sx, int sy, const char* Text)
{
	SIZE TextSize = { 0, 0 };
	...
	float width = (float)TextSize.cx / g_fScreenRate_x;

	float pos_x = sx - (width / 2);


	glColor4f(1.0, 1.0, 1.0, 1.0);
	SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 1, (float)pos_x, (float)sy - 3.5f, (float)TextSize.cx / g_fScreenRate_x, 8.0f); //Topo meio
	SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 6, (float)pos_x, (float)sy + 4.5f, (float)TextSize.cx / g_fScreenRate_x, 8.0f); //Baixo meio

	SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START, (float)pos_x - 8, (float)sy - 3.5f, 8.0f, 8.0f); //Topo esquerda
	SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 2, (float)pos_x + (float)TextSize.cx / g_fScreenRate_x, (float)sy - 3.5f, 8.0f, 8.0f); //Topo direita
	SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 5, (float)pos_x - 8, (float)sy + 4.5f, 8.0f, 8.0f);  //Baixo esquerda
	SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 7, (float)pos_x + (float)TextSize.cx / g_fScreenRate_x, (float)sy + 4.5f, 8.0f, 8.0f);  //Baixo direita
	//SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 8, (float)sx, (float)sy, (float)TextSize.cx / g_fScreenRate_x, (float)TextSize.cy / g_fScreenRate_y); //fundo
	...
	//g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->RenderText(pos_x, sy, Text);
	switch (BackupAlphaBlendType)
	{
	case 0:
		DisableAlphaBlend();
...
void RenderNotices()
{
	...
	for(int i=0;i<MAX_NOTICE;i++)
	{
		...
		else if (n->Color >= 25 && n->Color <= 100)
		{
			if (gMessageColor.CheckIsColor(n->Color, 0))
			{
				gMessageColor.SetMessageColor(n->Color);
			}
			else
			{
				g_pRenderText->SetBgColor(0, 0, 0, 128);
		...
		{
			g_pRenderText->RenderText(gWideScreen.g_WideWindowWidth / 2, gWideScreen.GetBottomPosition(300+i*13)-(g_pChatListBox->GetSize().cy+15), n->Text, 0, 0, RT3_WRITE_CENTER);
		}
		else
		{
			g_pRenderText->RenderText(gWideScreen.g_WideWindowWidth / 2, gWideScreen.GetBottomPosition(300+i*13), n->Text, 0, 0, RT3_WRITE_CENTER);
		}
	}
...
void SetBooleanPosition(CHAT *c)
{
	...
	if (g_bCustomTitle)
	{
		int titleIndex = gCustomTitleItem.IsCustomTitleItem((DWORD)c->Owner);

		if (titleIndex > -1)
		{
			auto ItemTitle = &gCustomTitleItem.m_CustomTitleItem[titleIndex];
			...
			if (ItemTitle != nullptr) {
				...
				if (ItemTitle->Align > -1)
				{
					if (ItemTitle->DynamicTitle) {
						char dynamicTitle[100];
						wsprintf(dynamicTitle, "%s", gCustomTitleItem.GetDinamycTitle((DWORD)c->Owner, titleIndex));
						bResult[5] = g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), dynamicTitle, lstrlen(c->Text[0]), &Size[5]);
					}
					else {
						bResult[5] = g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), ItemTitle->Title, lstrlen(c->Text[0]), &Size[5]);
					}
				}
				else
				{
					if (ItemTitle->DynamicTitle) {
						char dynamicTitle[100];
						wsprintf(dynamicTitle, "%s", gCustomTitleItem.GetDinamycTitle((DWORD)c->Owner, titleIndex));
						bResult[5] = g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), dynamicTitle, lstrlen(dynamicTitle), &Size[5]);
					}
					else {
						bResult[5] = g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), ItemTitle->Title, lstrlen(ItemTitle->Title), &Size[5]);
					}
				}
	...
	//CUSTOM COLOR CHAT TITLE
	if (g_bCustomColor)
	{
		for (std::map<std::string, CUSTOM_COLOR_TITLE>::iterator it = gCustomColorChat.m_CustomColorTitle.begin(); it != gCustomColorChat.m_CustomColorTitle.end(); it++)
		{
			auto titleInfo = &(*it).second;
			if (strcmp(c->ID, titleInfo->Player) == 0)
			{
				if (titleInfo->Bold == 1)
					g_pRenderText->SetFont(g_hFontBold);
				else
				...
				if (titleInfo->Align > -1)
					bResult[6] = g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), titleInfo->Name, lstrlen(c->Text[0]), &Size[6]);
				else
					bResult[6] = g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), titleInfo->Name, lstrlen(titleInfo->Name), &Size[6]);
	...
	if(c->LifeTime[1] > 0)
		c->Width = max(max(max(max(max(Size[0].cx, Size[1].cx), max(Size[2].cx,Size[3].cx)), Size[4].cx), Size[5].cx), Size[6].cx);

	else if(c->LifeTime[0] > 0)
		c->Width = max(max(max(max(Size[0].cx, Size[1].cx), max(Size[3].cx,Size[4].cx)), Size[5].cx), Size[6].cx);
	else
		c->Width = max(max(max(max(Size[0].cx, Size[3].cx), Size[4].cx), Size[5].cx), Size[6].cx);
	c->Height = FontHeight * (bResult[0] + bResult[1] + bResult[2] + bResult[3] + bResult[4] + bResult[5] + bResult[6]);
...
void RenderBoolean(int x, int y, CHAT* c) // GUILD VISIBLE
{
	...
	if (g_bCustomTitle)
	{
		...
		if (titleIndex > -1)
		{
			auto matchedTitle = &gCustomTitleItem.m_CustomTitleItem[titleIndex];

			if (matchedTitle != nullptr) {
				if (matchedTitle->Bold == 1)
					g_pRenderText->SetFont(g_hFontBold);
				else
					g_pRenderText->SetFont(g_hFont);

				g_pRenderText->SetTextColor(matchedTitle->ColorR, matchedTitle->ColorG, matchedTitle->ColorB, matchedTitle->ColorA);
				g_pRenderText->SetBgColor(matchedTitle->BackgroundR, matchedTitle->BackgroundG, matchedTitle->BackgroundB, matchedTitle->BackgroundA);
				if (matchedTitle->DynamicTitle) {
					char dynamicTitle[100];
					wsprintf(dynamicTitle, "%s", gCustomTitleItem.GetDinamycTitle((DWORD)c->Owner, titleIndex));
					g_pRenderText->RenderText(RenderPos.x, RenderPos.y, dynamicTitle, RenderBoxSize.cx, iLineHeight, matchedTitle->Align);
				}
				else {
					g_pRenderText->RenderText(RenderPos.x, RenderPos.y, matchedTitle->Title, RenderBoxSize.cx, iLineHeight, matchedTitle->Align);
				}
				g_pRenderText->SetFont(g_hFont);
				RenderPos.y += iLineHeight;
			}
	...
	if (g_bCustomColor)
	{
		for (std::map<std::string, CUSTOM_COLOR_TITLE>::iterator it = gCustomColorChat.m_CustomColorTitle.begin(); it != gCustomColorChat.m_CustomColorTitle.end(); it++)
		{
			...
			if (strcmp(c->ID, titleInfo->Player) == 0)
			{
				...
				g_pRenderText->SetTextColor(titleInfo->ColorR, titleInfo->ColorG, titleInfo->ColorB, titleInfo->ColorA);
				g_pRenderText->SetBgColor(titleInfo->BackgroundR, titleInfo->BackgroundG, titleInfo->BackgroundB, titleInfo->BackgroundA);
				g_pRenderText->RenderText(RenderPos.x, RenderPos.y, titleInfo->Name, RenderBoxSize.cx, iLineHeight, titleInfo->Align);
				g_pRenderText->SetFont(g_hFont);
				RenderPos.y += iLineHeight;
	...
	if (g_bCustomColor)
	{
		for (std::map<std::string, CUSTOM_COLOR_PLAYER>::iterator it = gCustomColorChat.m_CustomColorPlayer.begin(); it != gCustomColorChat.m_CustomColorPlayer.end(); it++)
		{
			auto chatInfo = &(*it).second;
			if (strcmp(c->ID, chatInfo->Player) == 0)
			{
				bCustomColorPlayer = true;
	...
	{
		//CUSTOM COLOR CHAT GUILD
		if (g_bCustomColor)
		{
			for (std::map<std::string, CUSTOM_COLOR_GUILD>::iterator it = gCustomColorChat.m_CustomColorGuild.begin(); it != gCustomColorChat.m_CustomColorGuild.end(); it++)
			{
				auto guildInfo = &(*it).second;
				if (strcmp(c->ID, guildInfo->Player) == 0)
				{
					if (guildInfo->Bold == 1)
						g_pRenderText->SetFont(g_hFontBold);
					else
					...
					if (guildInfo->BackgroundR > -1 && guildInfo->BackgroundG > -1 && guildInfo->BackgroundB > -1)
					{
						g_pRenderText->SetBgColor(guildInfo->BackgroundR, guildInfo->BackgroundG, guildInfo->BackgroundB, guildInfo->BackgroundA);
					}
					if (guildInfo->ColorR > -1 && guildInfo->ColorG > -1 && guildInfo->ColorB > -1)
					{
						g_pRenderText->SetTextColor(guildInfo->ColorR, guildInfo->ColorG, guildInfo->ColorB, guildInfo->ColorA);
					}
	...
	if ( c->x <= MouseX && MouseX < (int)(c->x+c->Width*640/WindowWidth) && 
		...
		InputEnable && Hero->SafeZone 
		&& strcmp(c->ID,Hero->ID)!=NULL
	...
	if (bCustomColorPlayer)
	{
		if (g_bCustomColor)
		{
			for (std::map<std::string, CUSTOM_COLOR_PLAYER>::iterator it = gCustomColorChat.m_CustomColorPlayer.begin(); it != gCustomColorChat.m_CustomColorPlayer.end(); it++)
			{
				auto nameInfo = &(*it).second;
				if (strcmp(c->ID, nameInfo->Player) == 0)
				{
					if (nameInfo->Bold == 1)
						g_pRenderText->SetFont(g_hFontBold);
					else
					...
					if (c->Color != 3)
						...
						if (nameInfo->ColorR > -1 && nameInfo->ColorG > -1 && nameInfo->ColorB > -1)
						{
							g_pRenderText->SetTextColor(nameInfo->ColorR, nameInfo->ColorG, nameInfo->ColorB, nameInfo->ColorA);
						}

					if (nameInfo->BackgroundR > -1 && nameInfo->BackgroundG > -1 && nameInfo->BackgroundB > -1)
					{
						g_pRenderText->SetBgColor(nameInfo->BackgroundR, nameInfo->BackgroundG, nameInfo->BackgroundB, nameInfo->BackgroundA);
					}

					if (nameInfo->Name == "")
						g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->ID, RenderBoxSize.cx, iLineHeight, nameInfo->Align);
					else
						g_pRenderText->RenderText(RenderPos.x, RenderPos.y, nameInfo->Name, RenderBoxSize.cx, iLineHeight, nameInfo->Align);
	...
	//CUSTOM COLOR CHAT
	CUSTOM_COLOR_CHAT* chatInfo = nullptr;

	if (g_bCustomColor)
	{
		for (std::map<std::string, CUSTOM_COLOR_CHAT>::iterator it = gCustomColorChat.m_CustomColorChat.begin(); it != gCustomColorChat.m_CustomColorChat.end(); it++)
		{
			if (strcmp(c->ID, (*it).second.Player) == 0)
			{
				chatInfo = &(*it).second;
				...
				if (chatInfo->ColorR > -1 && chatInfo->ColorG > -1 && chatInfo->ColorB > -1)
				{
					dwTextColor[0] = RGBA(chatInfo->ColorR, chatInfo->ColorG, chatInfo->ColorB, byAlpha[0]);
					dwTextColor[1] = RGBA(chatInfo->ColorR, chatInfo->ColorG, chatInfo->ColorB, byAlpha[1]);
				}
	...
	if (c->LifeTime[1] > 0)
	{
		...
		if (chatInfo && chatInfo->Align > 1)
			g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[1], RenderBoxSize.cx, iLineHeight, chatInfo->Align);
		else
	...
	if (c->LifeTime[0] > 0)
	{
		...
		if (chatInfo && chatInfo->Align > 1)
			g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[0], RenderBoxSize.cx, iLineHeight, chatInfo->Align);
		else
			g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[0], RenderBoxSize.cx, iLineHeight, RT3_SORT_LEFT);
	}
...
bool CheckAttack_Fenrir(CHARACTER* c)
{
	...
	else if(c->Object.Kind==KIND_PLAYER)
	{
		...
		if( c->GuildRelationShip == GR_RIVAL || c->GuildRelationShip == GR_RIVALUNION )		//夢謙�� ル衛
		{
			return true;
		}
...
bool CheckMacroLimit ( char* Text )
{
	...
	if( strcmp( string, GlobalText[249] )==0 || stricmp(string, "/battlesoccer") == 0 || stricmp(string, "/soccer") == 0)
    {
        return  true;
    }
...
bool CheckCommand(char *Text, bool bMacroText )
{
    ...
    if( bMacroText==false && LogOut == false )
    {
		...
		if (stricmp(Text, "/charactername") == 0)
		{
			if (IsGMCharacter())
			{
			...
			{
				for (std::map<std::string, CHARACTER_NAME>::iterator it = gCharactername.m_Charactername.begin(); it != gCharactername.m_Charactername.end(); it++)
				{
					auto titleInfo = &(*it).second;

					if (FindText(Hero->ID, titleInfo->Name))
					{
						g_bGMObservation = !g_bGMObservation;
						handled = true;
						break;
		...
		if (!handled && stricmp(Text, "/uncharactername") == 0)
		{
			if (IsGMCharacter())
			{
				g_bGMNOObservation = !g_bGMNOObservation;
				handled = true;
			}
			else
			{
		...
		if (!handled)
		{
			for (std::map<std::string, CHARACTER_NAME>::iterator it = gCharactername.m_Charactername.begin(); it != gCharactername.m_Charactername.end(); it++)
			{
				...
				if (stricmp(Text, titleInfo->Charactername) == 0)
				{
					if (IsGMCharacter() || FindText(Hero->ID, titleInfo->Name))
					{
						g_bGMObservation = !g_bGMObservation;
						return false;
					}
				...
				if (stricmp(Text, titleInfo->Uncharactername) == 0)
				{
					if (IsGMCharacter() || FindText(Hero->ID, titleInfo->Name))
					{
						g_bGMNOObservation = !g_bGMNOObservation;
		...
		//CUSTOM TITLE NAME COMMAND
		for (std::map<int, CUSTOM_TITLE_ITEM>::iterator it = gCustomTitleItem.m_CustomTitleItem.begin(); it != gCustomTitleItem.m_CustomTitleItem.end(); it++)
		{
			auto titleInfo = &(*it).second;

			if (stricmp(Text, titleInfo->Command) == 0)
			{
				g_bCustomTitle = !g_bCustomTitle;
				return false;
			}
		}

		//CUSTOM COLOR CHAT COMMAND
		for (std::map<std::string, CUSTOM_COLOR_COMMAND>::iterator it = gCustomColorChat.m_CustomColorCommand.begin(); it != gCustomColorChat.m_CustomColorCommand.end(); it++)
		{
			auto chatInfo = &(*it).second;
			if (stricmp(Text, chatInfo->Command) == 0)
			{
				if (IsGMCharacter() || FindText(Hero->ID, chatInfo->Player))
				{
					g_bCustomColor = !g_bCustomColor;
					return false;
				}
			}
		}

		if (stricmp(Text, "/chatcolor") == 0)
		{
			if (IsGMCharacter())
			{
				g_bCustomColor = !g_bCustomColor;
...
void CheckChatText(char *Text)
{
	...
	else if(FindText(Text,GlobalText[783]) || FindText(Text, "hustle") || FindText(Text, "/creu"))
	{
	...
	//CUSTOM SET ACTION
	int ItemIndex = gSetAction.IsSetAction((DWORD)c);

	if (ItemIndex > -1)
	{
		for (std::map<int, CUSTOM_SET_ACTION>::iterator it = gSetAction.m_SetAction.begin(); it != gSetAction.m_SetAction.end(); it++)
		{
			auto ItemInfo = &(*it).second;

			if (ItemInfo->ItemIndex == ItemIndex)
			{
				if (FindText(Text, ItemInfo->Command))
				{
					//SetActionClass(c, o, ItemInfo->Action, AT_TEST);
					//SendRequestAction(AT_TEST, ((BYTE)((o->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
					//o->m_iAnimation = 0;
  
					SetAction(o, ItemInfo->Action);
					SendRequestAction(ItemInfo->Action, ((BYTE)((o->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
					break;
				}
			}
		}
...
void AttackKnight(CHARACTER *c, int Skill, float Distance)
{
		...
		) )
    {
		...
		if (Skill == AT_SKILL_WHEEL && iTypeR == -1)
		{
			Success = false;
		}
...
//									SetPlayerMagic(c);
			...
			case AT_SKILL_THUNDER_STRIKE:
				...
				{
					//HORSE
					if ((c->Helper.Type == MODEL_HELPER + 4 || gHelperSystem.CheckHelperType(c->Helper.Type, 16) == 1 || gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1) || !c->SafeZone)
					{
						int TargetX = (int)(c->TargetPosition[0] / TERRAIN_SCALE);
						int TargetY = (int)(c->TargetPosition[1] / TERRAIN_SCALE);
						BYTE byValue = GetDestValue((c->PositionX), (c->PositionY), TargetX, TargetY);
            ...
            case AT_SKILL_VITALITY:
                SendRequestMagic(Skill,HeroKey);
				if (c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1)
					SetAction(o, PLAYER_FENRIR_SKILL_VITALITY);
            ...
            case AT_SKILL_FURY_STRIKE:
                {
                    ...
                    if( CheckTile( c, o, Distance ) )
                    {
						...
						//SKILL BK RAGEBLOW
						if (c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1)
							SetAction(o, PLAYER_FENRIR_SKILL_FURY_STRIKE);
						else if (gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1)
							SetAction(o, PLAYER_FENRIR_SKILL_FURY_STRIKE);
						else
	...
	if(Wall)
	{
		...
		if(Success)
		{
			...
			case AT_SKILL_FLASH:
				...
				return;
			case AT_SKILL_FLAME:
				SendRequestMagicContinue(Skill,(BYTE)(c->TargetPosition[0]/TERRAIN_SCALE),(BYTE)(c->TargetPosition[1]/TERRAIN_SCALE),(BYTE)(o->Angle[2]/260.f*156.f), 0, 0, 0xffff, 0);
				SetPlayerMagic(c);
				return;
				...
				if(( LoadingWorld)==0 && Hero->JumpTime==0)
				{
					...
					if(Success)
					{
                            ...
                            ( ( CharacterMachine->Equipment[EQUIPMENT_HELPER].Type>=ITEM_HELPER+2  && CharacterMachine->Equipment[EQUIPMENT_HELPER].Type<=ITEM_HELPER+3 ) ) )
						{
							...
							|| (gHelperSystem.CheckIsHelper(CharacterMachine->Equipment[EQUIPMENT_HELPER].Type))
...
void MoveHero()
{
	...
	if ( ( 
		...
		&& gMapManager.WorldActive!=WD_6STADIUM 
		&& gMapManager.InChaosCastle()==false ) 
		|| o->CurrentAction == PLAYER_ATTACK_SKILL_FURY_STRIKE || o->CurrentAction == PLAYER_FENRIR_SKILL_FURY_STRIKE
#ifdef PBG_ADD_NEWCHAR_MONK_SKILL
		|| o->CurrentAction == PLAYER_SKILL_GIANTSWING
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
		)
...
void SelectObjects()
{
	...
	if(!MouseOnWindow && false == g_pNewUISystem->CheckMouseUse() && SEASON3B::CheckMouseIn(0, 0, GetScreenWidth(), gWideScreen.GetBottomPosition(429)))
	{
		if(HIBYTE(GetAsyncKeyState(VK_MENU))==128)
		{
...
void RenderOutSides()
{
	if(gMapManager.WorldActive == WD_8TARKAN)
	{
		...
		RenderBitmapUV(BITMAP_CHROME+2,0.f,0.f,gWideScreen.g_WideWindowWidth,gWideScreen.g_WideWindowHeight,WindX,0.f,0.3f,0.3f);
		float WindX2 = (float)((int)WorldTime%100000)*0.001f; 
		RenderBitmapUV(BITMAP_CHROME+3,0.f,0.f, gWideScreen.g_WideWindowWidth, gWideScreen.g_WideWindowHeight,WindX2,0.f,3.f,2.f);
	}
	...
	else if (IsKarutanMap())
	{
		...
		RenderBitmapUV(BITMAP_CHROME+3, 0.f, 0.f, gWideScreen.g_WideWindowWidth, gWideScreen.g_WideWindowHeight, fWindX, 0.f, 3.f, 2.f);
	}
...
bool IsGMCharacter()
{
	if( (Hero->Object.Kind == KIND_PLAYER && Hero->Object.Type == MODEL_PLAYER && Hero->Object.SubType == MODEL_GM_CHARACTER )|| ( g_isCharacterBuff((&Hero->Object), eBuff_GMEffect)) 
...
bool IsIllegalMovementByUsingMsg(const char *szChatText)
{
	...
	//MOVE ICARUS
	//if ((pEquipedWingType == -1 && pEquipedHelperType != ITEM_HELPER+3 && pEquipedHelperType != ITEM_HELPER+37	&& pEquipedHelperType != ITEM_HELPER+4) || pEquipedHelperType == ITEM_HELPER+2)
	if (pEquipedWingType == -1 && pEquipedHelperType == -1)
	{
		bCantFly = true; //N鬃 PODE VOAR
	}
	if (pEquipedHelperType == ITEM_HELPER+2 || pEquipedHelperType == ITEM_HELPER+3)
	{
		bCantSwim = true; //N鬃 PODE NADAR
	}
	...
	if ((strlen(GlobalText[260]) > 0 && strstr(szChatTextUpperChars, GlobalText[260]) != NULL))
	{
		...
		while (li != m_listMoveInfoData.end())
		{
			...
			if (strstr(szChatText, ((*li)->_ReqInfo.szMainMapName)) != NULL || strstr(szChatTextUpperChars, cMapNameUpperChars) != NULL) break;
			li++;
		}
