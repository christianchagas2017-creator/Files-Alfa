...
CUISocketListBox::CUISocketListBox()
{
	...
	SetPosition(gWideScreen.GetAddPosition(275), 360);
	SetSize(160, 65);
}
...
BOOL CUINewGuildMemberListBox::RenderDataLine(int iLineNumber)
{
	...
	if (iCharacterLevel == 0)
	{
		SEASON3B::RenderImageScale(33014, iPos_x - 4, iPos_y - 1.5f, 7.f, 10.f, 0.f, 0.f, 127.f, 173.f);
	}
	else if (iCharacterLevel == 1)
	{
		SEASON3B::RenderImageScale(33015, iPos_x - 4, iPos_y - 1.5f, 7.f, 10.f, 0.f, 0.f, 127.f, 173.f);
	}

	else if (iCharacterLevel == 2)
	{
		SEASON3B::RenderImageScale(33016, iPos_x - 4, iPos_y - 1.5f, 7.f, 10.f, 0.f, 0.f, 127.f, 173.f);
	}

	char Text[MAX_TEXT_LENGTH + 1] = { 0 };
	sprintf(Text, "%s", m_TextListIter->m_szID);
	g_pRenderText->RenderText(iPos_x + 7, iPos_y - 1, Text);

	if (CheckMouseIn(iPos_x - 4, iPos_y - 2.f, 8.f, 11.f))
	{
		if (iCharacterLevel == 0)
		{
			sprintf(Text, "%s", GlobalText[1300]);
			SIZE TextSize = { 0, 0 };
			g_pRenderText->SetFont(g_hFont);
			g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text, lstrlen(Text), &TextSize);
			RenderTipText((iPos_x - 8 - ((float)TextSize.cx / g_fScreenRate_x + 4)), iPos_y, Text);
		}
		else if (iCharacterLevel == 1)
		{
			sprintf(Text, "%s", GlobalText[1301]);
			SIZE TextSize = { 0, 0 };
			g_pRenderText->SetFont(g_hFont);
			g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text, lstrlen(Text), &TextSize);
			RenderTipText((iPos_x - 8 - ((float)TextSize.cx / g_fScreenRate_x + 4)), iPos_y, Text);
		...
		{
			sprintf(Text, "%s", GlobalText[1302]);
			SIZE TextSize = { 0, 0 };
			g_pRenderText->SetFont(g_hFont);
			g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text, lstrlen(Text), &TextSize);
			RenderTipText((iPos_x - 8 - ((float)TextSize.cx / g_fScreenRate_x + 4)), iPos_y, Text);
	...
	if (m_TextListIter->m_Server != 255 && m_TextListIter->m_Number != 0)
	{
		...
		//g_pRenderText->RenderText(m_iPos_x + m_iWidth - 25, iPos_y, Text);
	}
#elif CLIENT_VERSION >= 1001
		g_pRenderText->SetTextColor(41, 196, 41, 255);
		g_pRenderText->RenderText(m_iPos_x + m_iWidth - 40, iPos_y - 1, "Online");
}
	else {
		g_pRenderText->SetBgColor(0);
		g_pRenderText->SetTextColor(196, 41, 41, 255);
		g_pRenderText->RenderText(m_iPos_x + m_iWidth - 40, iPos_y - 1, "Offline");
	}
#endif
...
CUIBCDeclareGuildListBox::CUIBCDeclareGuildListBox()
{
	...
	SetPosition(gWideScreen.GetAddPosition(465), 364);
	SetSize(160, 235);
}
...
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	SetPosition(gWideScreen.GetCenterPosition(490), 360);
#elif CLIENT_VERSION >= 1001
	SetPosition(gWideScreen.GetCenterPosition(490), 273);
#endif
}
