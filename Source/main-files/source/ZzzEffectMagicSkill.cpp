...
void RenderCircle(int Type, vec3_t ObjectPosition, float ScaleBottom, float ScaleTop, float Height, float Rotation, float LightTop, float TextureV)
{
	...
	const int TotalVertices = Num * VerticesPerQuad;

	// Buffers for vertex positions, texture coordinates, and colors
	GLfloat vertexBuffer[TotalVertices * 3];
	GLfloat texCoordBuffer[TotalVertices * 2];
	GLfloat colorBuffer[TotalVertices * 3];

	int index = 0; // Index for populating the buffers

	// Precompute light values
	vec3_t Light[4];
	Vector(1.f, 1.f, 1.f, Light[0]);
	Vector(1.f, 1.f, 1.f, Light[1]);
	Vector(LightTop, LightTop, LightTop, Light[2]);
	...
	for (int x = 0; x < Num; ++x)
	{
		...
		{
			// Vertex positions
			vertexBuffer[index * 3 + 0] = Position[i][0];
			vertexBuffer[index * 3 + 1] = Position[i][1];
			vertexBuffer[index * 3 + 2] = Position[i][2];

			// Texture coordinates
			texCoordBuffer[index * 2 + 0] = UV[i][0];
			texCoordBuffer[index * 2 + 1] = UV[i][1] + TextureV;

			// Colors
			colorBuffer[index * 3 + 0] = Light[i][0];
			colorBuffer[index * 3 + 1] = Light[i][1];
			colorBuffer[index * 3 + 2] = Light[i][2];
	...
	WORD SkillIndex, WORD Skill, WORD SKKey)
{
	...
	else
	{
		...
		default:
			...
			{
				vec3_t light;
				gCustomBow.GetEffectColor(Right - MODEL_ITEM, light);
				CreateEffect(gCustomBow.GetSkillNumber(Right - MODEL_ITEM), o->Position, o->Angle, light, gCustomBow.GetEffectLevel(Right - MODEL_ITEM), o, o->PKKey, SkillIndex, Skill);
				return;
			}
			break;
		}
		...
		default:
			...
			{
				vec3_t light;
				gCustomBow.GetEffectColor(Left - MODEL_ITEM, light);
				CreateEffect(gCustomBow.GetSkillNumber(Left - MODEL_ITEM), o->Position, o->Angle, light, gCustomBow.GetEffectLevel(Left - MODEL_ITEM), o, o->PKKey, SkillIndex, Skill);
				return;
			}
			break;
		}
