...
SEASON3B::CNewUISystem::CNewUISystem() : m_Lua()
{
	m_pNewUIMng = NULL;
	m_pNewUIHotKey = NULL;
...
bool SEASON3B::CNewUISystem::Create()
{
	...
	if(false == m_pNewChatLogWindow->Create(m_pNewUIMng, 0, gWideScreen.GetBottomPosition(480-50-47), 9))
#endif
		return false;

	m_pNewOptionWindow = new CNewUIOptionWindow;
	if(m_pNewOptionWindow->Create(m_pNewUIMng, (gWideScreen.g_WideWindowWidth/2)-(380/2), ((gWideScreen.g_WideWindowHeight-50)/2) - (280/2)) == false)
	{
		return false;
	}
...
void SEASON3B::CNewUISystem::RestartLua()
{
	this->m_Lua.CloseLua();
	...
	char luaInterfacePath[MAX_PATH] = { 0 };
	sprintf_s(luaInterfacePath, "%s\\Controller\\Interface.lua", LUA_LOAD_PATH);

	this->m_Lua.DoFile(luaInterfacePath);

	this->m_Lua.Generic_Call("AfterScriptLoad", ">");
}
...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
...
#elif  CLIENT_VERSION >= 1001
	if(false == m_pNewChatInputBox->Create(m_pNewUIMng, m_pNewChatLogWindow, (gWideScreen.g_WideWindowWidth/2)-(254/2), gWideScreen.GetBottomPosition(480 - 51 - 47)))
	{
		return false;
	}
#endif
	...
	if(false == m_pNewMyInventory->Create(m_pNewUIMng, m_pNewUI3DRenderMng, gWideScreen.g_WideWindowWidth -190, 0))
		return false;

	m_pNewNPCShop = new CNewUINPCShop;
	if(false == m_pNewNPCShop->Create(m_pNewUIMng, gWideScreen.GetAddPosition(CNewUINPCShop::NPCSHOP_POS_X), CNewUINPCShop::NPCSHOP_POS_Y))
		return false;

	m_pNewPetInfoWindow = new CNewUIPetInfoWindow;
	if(false == m_pNewPetInfoWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth-(190*2), 0))
		return false;
	
	m_pNewMixInventory = new CNewUIMixInventory;
	if(m_pNewMixInventory->Create(m_pNewUIMng, gWideScreen.GetAddPosition(260), 0) == false)
		return false;

	m_pChaosGenesis = new CChaosGenesis;
	if (m_pChaosGenesis->Create(m_pNewUIMng, gWideScreen.GetAddPosition(260), 0) == false)
		return false;


	m_pTradeX = new CTradeX;
	if (m_pTradeX->Create(m_pNewUIMng, gWideScreen.GetAddPosition(260), 0) == false)
		return false;

	m_pNewCastleWindow = new CNewUICastleWindow;
	if(m_pNewCastleWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewGuardWindow = new CNewUIGuardWindow;
	if(m_pNewGuardWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewGatemanWindow = new CNewUIGatemanWindow;
	if(m_pNewGatemanWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewGateSwitchWindow = new CNewUIGateSwitchWindow;
	if(m_pNewGateSwitchWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewStorageInventory = new CNewUIStorageInventory;
	if(m_pNewStorageInventory->Create(m_pNewUIMng, gWideScreen.GetAddPosition(260), 0) == false)
		return false;

	m_pNewGuildInfoWindow = new CNewUIGuildInfoWindow;
	if(m_pNewGuildInfoWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewGuildMakeWindow = new CNewUIGuildMakeWindow;
	if(m_pNewGuildMakeWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	CreatePersonalItemTable();

	m_pNewMyShopInventory = new CNewUIMyShopInventory;
	if(m_pNewMyShopInventory->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190*2, 0) == false)
		return false;
	
	m_pNewPurchaseShopInventory = new CNewUIPurchaseShopInventory;
	if(m_pNewPurchaseShopInventory->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190*2, 0) == false)
		return false;

	m_pNewCharacterInfoWindow = new CNewUICharacterInfoWindow;
	if(m_pNewCharacterInfoWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewMyQuestInfoWindow = new CNewUIMyQuestInfoWindow;
	if(m_pNewMyQuestInfoWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewPartyInfoWindow = new CNewUIPartyInfoWindow;
	if( m_pNewPartyInfoWindow->Create( m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0 ) == false )
		return false;

	m_pNewPartyListWindow = new CNewUIPartyListWindow;
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	if( m_pNewPartyListWindow->Create( m_pNewUIMng, gWideScreen.g_WideWindowWidth -79, 14 ) == false )
#elif CLIENT_VERSION >= 1001
	if( m_pNewPartyListWindow->Create( m_pNewUIMng, gWideScreen.g_WideWindowWidth -79, 2 ) == false )
#endif
		return false;

	m_pNewNPCQuest = new CNewUINPCQuest;
	if (m_pNewNPCQuest->Create(m_pNewUIMng, m_pNewUI3DRenderMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewEnterBloodCastle = new CNewUIEnterBloodCastle;
	if( m_pNewEnterBloodCastle->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;

	m_pNewEnterDevilSquare = new CNewUIEnterDevilSquare;
	if( m_pNewEnterDevilSquare->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;
	
	m_pNewBloodCastle = new CNewUIBloodCastle;
	if( m_pNewBloodCastle->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -127, 480-132) == false )
		return false;

	m_pNewTrade = new CNewUITrade;
	if (m_pNewTrade->Create(m_pNewUIMng, gWideScreen.GetAddPosition(260), 0) == false)
		return false;

	m_pNewKanturu2ndEnterNpc = new CNewUIKanturu2ndEnterNpc;
	if(m_pNewKanturu2ndEnterNpc->Create(m_pNewUIMng, (gWideScreen.g_WideWindowWidth /2) - (230/2), 20) == false)
	{
		return false;
	}

	m_pNewKanturuInfoWindow = new CNewUIKanturuInfoWindow;
	if(m_pNewKanturuInfoWindow->Create(m_pNewUIMng, gWideScreen.GetAddPosition(541), 351) == false)
	{
		return false;
	}

	m_pNewChaosCastleTime = new CNewUIChaosCastleTime;
	if( m_pNewChaosCastleTime->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -127, 480-132) == false )
		return false;

	m_pNewBattleSoccerScore = new CNewUIBattleSoccerScore;
	if (m_pNewBattleSoccerScore->Create(m_pNewUIMng, (gWideScreen.g_WideWindowWidth / 2) - 150, 0) == false)
		return false;
	
	m_pNewCommandWindow = new CNewUICommandWindow;
	if( m_pNewCommandWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;

	m_pNewCatapultWindow = new CNewUICatapultWindow;
	if(m_pNewCatapultWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
	{
		return false;
	}

	m_pNewWindowMenu = new CNewUIWindowMenu;
	if(m_pNewWindowMenu->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -112, 480-171) == false)
	{
		return false;
	}
...
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	if(m_pNewDuelWindow->Create(m_pNewUIMng, gWideScreen.GetAddPosition(509), 359) == false)
#elif CLIENT_VERSION >= 1001
	if(m_pNewDuelWindow->Create(m_pNewUIMng, (gWideScreen.g_WideWindowWidth/2)-150, 0) == false)
#endif
	{
		return false;
	}
	...
	if(m_pNewSiegeWarfare->Create(m_pNewUIMng, gWideScreen.GetAddPosition(486), 234) == false)
		return false;
	...
	if(m_pNewBuffWindow->Create(m_pNewUIMng, gWideScreen.GetAddPosition(220), 15) == false)
	{
		return false;
	}

	m_pNewCursedTempleEnterWindow = new CNewUICursedTempleEnter;
	if(m_pNewCursedTempleEnterWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth /2-230/2, 80) == false)
	{
		return false;
	}
	...
	if(m_pNewCursedTempleResultWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth /2-230/2, 120) == false)
	{
		return false;
	}
...
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	if(m_pNewMiniMap->Create(m_pNewUIMng, gWideScreen.GetCenterPosition(0), 0) == false)
#elif CLIENT_VERSION >= 1001
	if(m_pNewMiniMap->Create(m_pNewUIMng, gWideScreen.GetAddPosition(485), 280) == false)
#endif
		return false;

	m_pNewGoldBowman = new CNewUIGoldBowmanWindow;
	if( m_pNewGoldBowman->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;

	m_pNewGoldBowmanLena = new CNewUIGoldBowmanLena;
	if( m_pNewGoldBowmanLena->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;

	m_pNewLuckyCoinRegistration = new CNewUIRegistrationLuckyCoin;
	if( m_pNewLuckyCoinRegistration->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth - (190 * 2), 0) == false )
		return false;	

	m_pNewExchangeLuckyCoinWindow = new CNewUIExchangeLuckyCoin;
	if( m_pNewExchangeLuckyCoinWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -(190*2), 0) == false )
		return false;

	m_pNewDuelWatchWindow = new CNewUIDuelWatchWindow;
	if( m_pNewDuelWatchWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;
	...
	if(m_pNewDuelWatchUserListWindow->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -57, gWideScreen.GetBottomPosition(480-51)) == false)
		return false;

#ifdef PBG_ADD_INGAMESHOP_UI_MAINFRAME
	m_pNewInGameShop = new CNewUIInGameShop;
#if CLIENT_VERSION >= 401 && CLIENT_VERSION <= 502
	if(m_pNewInGameShop->Create(m_pNewUIMng, gWideScreen.GetCenterPosition(0), 0) == false)
#elif CLIENT_VERSION >= 1001
	if(m_pNewInGameShop->Create(m_pNewUIMng, gWideScreen.GetCenterPosition(0), 12) == false)
#endif
		return false;
#endif //PBG_ADD_INGAMESHOP_UI_MAINFRAME

	m_pNewDoppelGangerWindow = new CNewUIDoppelGangerWindow;
	if( m_pNewDoppelGangerWindow->Create(m_pNewUIMng, m_pNewUI3DRenderMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;

	m_pNewDoppelGangerFrame = new CNewUIDoppelGangerFrame;
	if( m_pNewDoppelGangerFrame->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -227, gWideScreen.GetBottomPosition(480-51-87)) == false )
		return false;

	m_pNewNPCDialogue = new CNewUINPCDialogue;
	if (m_pNewNPCDialogue->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewQuestProgress = new CNewUIQuestProgress;
	if (m_pNewQuestProgress->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;

	m_pNewQuestProgressByEtc = new CNewUIQuestProgressByEtc;
	if (m_pNewQuestProgressByEtc->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth -190, 0) == false)
		return false;
	...
	if( m_pNewEmpireGuardianTimer->Create(m_pNewUIMng, gWideScreen.GetAddPosition(507), 342) == false )
		return false;
	
#ifdef PBG_MOD_STAMINA_UI
	m_pNewUIStamina = new CNewUIStamina;
	if( m_pNewUIStamina->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth, 480) == false )
		return false;
#endif //PBG_MOD_STAMINA_UI

	m_pNewGensRanking = new CNewUIGensRanking;
	if(m_pNewGensRanking->Create(m_pNewUIMng, gWideScreen.g_WideWindowWidth, 480) == false)
		return false;

	m_pNewUnitedMarketPlaceWindow = new CNewUIUnitedMarketPlaceWindow;
	if( m_pNewUnitedMarketPlaceWindow->Create(m_pNewUIMng, m_pNewUI3DRenderMng, gWideScreen.g_WideWindowWidth -190, 0) == false )
		return false;

#ifdef LEM_ADD_LUCKYITEM
	m_pNewUILuckyItemWnd = new CNewUILuckyItemWnd;
	if(m_pNewUILuckyItemWnd->Create(m_pNewUIMng, gWideScreen.GetAddPosition(260), 0) == false)
		return false;
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if(m_pNewUIMng) 
	{
		if(dwKey == SEASON3B::INTERFACE_FRIEND)
		{
			...
			m_pNewFriendWindow->OpenMainWnd(gWideScreen.g_WideWindowWidth -250, 432-173);
		}
		else if(dwKey == SEASON3B::INTERFACE_INVENTORY)
		{
			...
			{
				g_pMyInventory->SetPos(gWideScreen.g_WideWindowWidth -190*2, 0);
			}
		...
		else if(dwKey == SEASON3B::INTERFACE_CHARACTER)
		{
			...
			{
				g_pMyQuestInfoWindow->SetPos(gWideScreen.g_WideWindowWidth -190*2, 0);
			}
		...
		else if(dwKey == SEASON3B::INTERFACE_MYQUEST)
		{
			...
			if(IsVisible(SEASON3B::INTERFACE_CHARACTER))
			{
				...
				g_pMyQuestInfoWindow->SetPos(gWideScreen.g_WideWindowWidth - 190, 0);
			}
			if(IsVisible(SEASON3B::INTERFACE_INVENTORY))
			{
				Hide(SEASON3B::INTERFACE_INVENTORY);
		...
		else if (dwKey == SEASON3B::INTERFACE_TRADEX)
		{
			HideAllGroupA();
			m_pNewUIMng->ShowInterface(SEASON3B::INTERFACE_INVENTORY);
			g_pMainFrame->SetBtnState(MAINFRAME_BTN_MYINVEN, true);
		...
		else if(dwKey == SEASON3B::INTERFACE_QUEST_PROGRESS_ETC)
		{
			...
			{
				Hide(SEASON3B::INTERFACE_MYQUEST);
				g_pQuestProgressByEtc->SetPos(gWideScreen.g_WideWindowWidth -190, 0);
			}
			if (IsVisible(SEASON3B::INTERFACE_CHARACTER))
				g_pQuestProgressByEtc->SetPos(gWideScreen.g_WideWindowWidth -190*2, 0);
		...
		else if(dwKey == SEASON3B::INTERFACE_GENSRANKING)
		{
			...
			g_pNewUIGensRanking->SetPos(gWideScreen.g_WideWindowWidth -190, 0);
		}
...
void SEASON3B::CNewUISystem::Hide(DWORD dwKey)
{
	if(m_pNewUIMng) 
	{
		...
		else if(dwKey == SEASON3B::INTERFACE_CHARACTER)
		{
			...
			if(IsVisible((SEASON3B::INTERFACE_INVENTORY)))
			{
				g_pMyInventory->SetPos(gWideScreen.g_WideWindowWidth -190, 0);
			}
			if(IsVisible((SEASON3B::INTERFACE_PET)))
			{
				Hide(SEASON3B::INTERFACE_PET);
		...
		else if(dwKey == SEASON3B::INTERFACE_INVENTORY)
		{
			...
			if (IsVisible(SEASON3B::INTERFACE_TRADEX))
			{
...
bool SEASON3B::CNewUISystem::Update()
{
	...
	if (gProtect->m_MainInfo.LuaReload) {
		if (SEASON3B::IsPress(VK_HOME) == true)
		{
	...
	this->m_Lua.Generic_Call("UpdateProc", ">");
	this->m_Lua.Generic_Call("ScrollMouse", "i>i", MouseWheel, &retWheel);
	g_pChaosGenesis->Scrolling(MouseWheel);
	g_pTradeX->MouseWheel(MouseWheel);

	if (retWheel != 0 && MouseWheel != 0) {
		MouseWheel = 0;
	}
	this->m_Lua.Generic_Call("UpdateMouse", ">");
	this->m_Lua.Generic_Call("UpdateKey", ">");
...
bool SEASON3B::CNewUISystem::Render()
{
	...
	this->m_Lua.Generic_Call("AfterMainProc", ">");
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
...
{
	if(IsVisible(SEASON3B::INTERFACE_MIXINVENTORY)
		|| IsVisible(SEASON3B::INTERFACE_KANTURU2ND_ENTERNPC) || IsVisible(SEASON3B::INTERFACE_CHAOSGENESIS) || LockPlayerWalk || BlockClick
#ifdef LEM_ADD_LUCKYITEM
...
CTradeX* SEASON3B::CNewUISystem::GetUI_TradeX() const
{
	return m_pTradeX;
}
