...
void CServerSelWin::Create()
{
	...
	CWin::SetSize(SERVER_GROUP_BTN_WIDTH + SSW_GAP_WIDTH + SERVER_BTN_WIDTH, SERVER_BTN_HEIGHT * SSW_SERVER_MAX + SSW_GAP_HEIGHT * 2 + SERVER_GROUP_BTN_HEIGHT + m_winDescription.GetHeight());
}
...
void CServerSelWin::SetPosition(int nXCoord, int nYCoord)
{
	...
	//m_aServerGroupBtn[icntServreGroup++].SetPosition( ((gWideScreen.g_WideWindowWidth/2)-(nServerGBtnWidth / 2))+100 , nYCoord + CWin::GetHeight() - nServerGBtnHeight - SSW_GAP_HEIGHT - nDescGgHeight);
	...
	for( i=0 ; i<m_icntServer ; i++ )
	{
		if (m_aServerBtn[i].IsClick())
		{
			...
			if (pServerInfo->m_iPercent < 100)
			{
				...
				int iCensorshipIndex = CGameCensorship::STATE_15;
