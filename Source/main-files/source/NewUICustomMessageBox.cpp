...
SEASON3B::CNewUIMultipleTextInputMsgBox::CNewUIMultipleTextInputMsgBox()
{
	m_pInputBox[0] = NULL;
	m_pInputBox[1] = NULL;
	m_pInputBox[2] = NULL;
...
bool SEASON3B::CNewUIMultipleTextInputMsgBox::Create(DWORD dwMsgBoxType, DWORD dwInputType, int iInputBoxWidth, int iInputBoxHeight, int iTextLimit, int InputBoxCount)
{
	...
	m_pInputBoxCount = InputBoxCount;

	AddCallbackFunc(SEASON3B::CNewUIMultipleTextInputMsgBox::LButtonUp, MSGBOX_EVENT_MOUSE_LBUTTON_UP);

	int x, y, width, height, addY;

	x = (gWideScreen.g_WideWindowWidth / 2) - (MSGBOX_WIDTH / 2);
	y = 100;
	width = MSGBOX_WIDTH;

	float newH = (80.f / 460.f) * width;

	height = newH + (newH * InputBoxCount) + newH;
	...
	float espacoTotalSemFundo = height - newH;
	float espacoOcupadoPelaInput = (iInputBoxHeight + 10) * InputBoxCount;
	float espacoDisponivelAposInput = espacoTotalSemFundo - espacoOcupadoPelaInput;
	float metadeDoEspacoDisponivel = espacoDisponivelAposInput / 2;

	float StartPosy = GetPos().y + metadeDoEspacoDisponivel;
	...
	for (int i = 0; i < InputBoxCount; i++) {
		...
		if (m_pInputBox[i])
		{
			m_pInputBox[i]->Init(g_hWnd, iInputBoxWidth, iInputBoxHeight, iTextLimit, false);
			...
			m_pInputBox[i]->SetState(UISTATE_NORMAL);
			m_pInputBox[i]->GiveFocus();

			addY += iInputBoxHeight+10;
		}
	}
...
void SEASON3B::CNewUIMultipleTextInputMsgBox::Release()
{
...
void SEASON3B::CNewUIMultipleTextInputMsgBox::SetButtonInfo()
{
	float x, y, width, height;

	float newH = (80.f / 460.f) * GetSize().cx;

	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
		x = GetPos().x + (GetSize().cx / 2) - (MSGBOX_BTN_WIDTH / 2);
		y = GetPos().y + GetSize().cy - newH;
...
CALLBACK_RESULT SEASON3B::CNewUIMultipleTextInputMsgBox::LButtonUp(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	CNewUIMultipleTextInputMsgBox* pMsgBox = dynamic_cast<CNewUIMultipleTextInputMsgBox*>(pOwner);
	if (pMsgBox)
	{
...
void SEASON3B::CNewUIMultipleTextInputMsgBox::AddMsg(int boxId, const type_string& strMsg, DWORD dwColor, BYTE byFontType)
{
	...
	if (iSize > 1)
	{
		...
		if (m_pInputBox[boxId])
		{
			m_pInputBox[boxId]->SetPosition(m_pInputBox[boxId]->GetPosition_x(), m_pInputBox[boxId]->GetPosition_y() + (iLine * MSGBOX_MIDDLE_HEIGHT));
		}
	}
}

void SEASON3B::CNewUIMultipleTextInputMsgBox::AddButtonBlank(int iAddLine)
{
	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
...
int SEASON3B::CNewUIMultipleTextInputMsgBox::SeparateText(const type_string& strMsg, DWORD dwColor, BYTE byFontType)
{
...
DWORD SEASON3B::CNewUIMultipleTextInputMsgBox::GetMsgBoxType()
{
	return m_dwMsgBoxType;
}

bool SEASON3B::CNewUIMultipleTextInputMsgBox::Update()
{
	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
	...
	for (int i = 0; i < 5; i++)
	{
		if (m_pInputBox[i])
		{
			...
			if (false == m_pInputBox[i]->HaveFocus() && g_MessageBox->GetRelatedWnd() != g_hWnd)
			{
				g_MessageBox->SetRelatedWnd(g_hWnd);
			}
		}
...
bool SEASON3B::CNewUIMultipleTextInputMsgBox::Render()
{
	float x, y, width;
	...
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x, y, width, newH, 0.001, 0.001, 0.89692983, 0.62088972, 1, 1, 1.0);
	y += newH;
	if (m_pInputBoxCount > 0) {
		for (int i = 0; i < m_pInputBoxCount; i++) {
			RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x, y, width, newH + 0.5, 0.001, 0.005, 0.89692983, 0.61988972, 1, 1, 1.0);
			y += newH;
		}
	}
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x, y, width, newH, 0.001, 0.005, 0.89692983, 0.62088972, 1, 1, 1.0);
...
void SEASON3B::CNewUIMultipleTextInputMsgBox::RenderTexts()
{
	extern float g_fScreenRate_x;
	...
	for (; vi != m_MsgTextList.end(); vi++)
	{
		if (inputCount == -1)
		{
			...
			if (m_pInputBox[inputCount])
			{
				x = m_pInputBox[inputCount]->GetPosition_x() - TextExtentWidth - 10;
				y = m_pInputBox[inputCount]->GetPosition_y()+ ((m_pInputBox[inputCount]->GetHeight()-TextExtentHeight)/2);
				g_pRenderText->RenderText((int)x, (int)y, (*vi)->strMsg.c_str());
...
void SEASON3B::CNewUIMultipleTextInputMsgBox::RenderButtons()
{
	switch (m_dwMsgBoxType)
	{
	case MSGBOX_COMMON_TYPE_OK:
...
int SEASON3B::CNewUIMultipleTextInputMsgBox::GetInputBoxCount()
{
	return m_pInputBoxCount;
}

void SEASON3B::CNewUIMultipleTextInputMsgBox::GetInputBoxText(int boxID, unicode::t_char* strText)
{
	if (m_pInputBox[boxID])
	{
		m_pInputBox[boxID]->GetText(strText);
	}
}

void SEASON3B::CNewUIMultipleTextInputMsgBox::SetInputBoxOption(int boxID, int iOption)
{
	if (m_pInputBox[boxID])
	{
		m_pInputBox[boxID]->SetOption(iOption);
	}
}

void SEASON3B::CNewUIMultipleTextInputMsgBox::SetInputBoxPosition(int boxID, int x, int y)
{
	if (m_pInputBox[boxID])
	{
		m_pInputBox[boxID]->SetPosition(x, y);
	}
}

void SEASON3B::CNewUIMultipleTextInputMsgBox::SetInputBoxSize(int boxID, int width, int height)
{
	if (m_pInputBox[boxID])
	{
		m_pInputBox[boxID]->SetSize(width, height);
	}
}
...
void SEASON3B::CNewUIKeyPadButton::Render()
{
	...
	{
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_VERY_SMALL, GetPosX(), GetPosY(), GetWidth(), GetHeight(), 0.008, 0.213, 0.83, 0.20, 1, 1, 1.0);
	}
	else if (GetEventState() == EVENT_BTN_DOWN)
	{
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_VERY_SMALL, GetPosX(), GetPosY(), GetWidth(), GetHeight(), 0.008, 0.429, 0.83, 0.20, 1, 1, 1.0);
	}
	else
	{
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_VERY_SMALL, GetPosX(), GetPosY(), GetWidth(), GetHeight(), 0.008, 0.002, 0.83, 0.20, 1, 1, 1.0);
	}
...
void SEASON3B::CNewUIDeleteKeyPadButton::Render()
{
	if (GetEventState() == EVENT_BTN_HOVER)
	{
		RenderBitmap(CNewUIMyInventory::IMAGE_INVENTORY_PREVIOUS, GetPosX(), GetPosY(), GetWidth(), GetHeight(), 0.008, 0.213, 0.83, 0.20, 1, 1, 1.0);
	}
	else if (GetEventState() == EVENT_BTN_DOWN)
	{
		RenderBitmap(CNewUIMyInventory::IMAGE_INVENTORY_PREVIOUS, GetPosX(), GetPosY(), GetWidth(), GetHeight(), 0.008, 0.429, 0.83, 0.20, 1, 1, 1.0);
	}
	else
	{
		RenderBitmap(CNewUIMyInventory::IMAGE_INVENTORY_PREVIOUS, GetPosX(), GetPosY(), GetWidth(), GetHeight(), 0.008, 0.002, 0.83, 0.20, 1, 1, 1.0);
	}
}
...
void SEASON3B::CNewUIKeyPadMsgBox::RenderButtons()
{
	for (int i = 0; i < MAX_KEYPADINPUT; ++i)
	{
		...
		glColor3f(0.91f, 0.8581f, 0.7371f);
		SEASON3B::RenderOldNumber(m_BtnKeyPad[i].GetPosX() + 16, m_BtnKeyPad[i].GetPosY() + 8, m_iKeyPadMapping[i], 1.2f);
		glColor3f(1.0f, 1.0f, 1.0f);
	}
...
void SEASON3B::CUseFruitCheckMsgBox::RenderFrame()
{
	...
	RenderImageScale(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x, y, width, height, 0.f, 0.f, 460.f, 80.f);

	x = GetPos().x; y += MSGBOX_TOP_HEIGHT; width = MSGBOX_WIDTH; height = MSGBOX_MIDDLE_HEIGHT;
	for (int i = 0; i < 1; ++i)
	{
		RenderImageScale(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x, y, width, height, 0.f, 0.f, 460.f, 80.f);
		y += height;
	}

	x = GetPos().x; width = MSGBOX_WIDTH; height = MSGBOX_BOTTOM_HEIGHT;
	RenderImageScale(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x, y, width, height, 0.f, 0.f, 460.f, 80.f);
}
...
void SEASON3B::CSystemMenuMsgBox::SetButtonInfo()
{
	...
	x = GetPos().x + msgboxhalfwidth - btnhalfwidth;
	y = GetPos().y + 10;
	m_BtnGameOver.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_DECOR, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnGameOver.SetButtonSize(3);
	m_BtnGameOver.SetText(GlobalText[381]);

	y += 35.f;
	m_BtnChooseServer.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_DECOR, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnChooseServer.SetButtonSize(3);
	m_BtnChooseServer.SetText(GlobalText[382]);

	y += 35.f;
	m_BtnChooseCharacter.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_DECOR, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnChooseCharacter.SetButtonSize(3);
	m_BtnChooseCharacter.SetText(GlobalText[383]);

	y += 35.f;
	m_BtnOption.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_DECOR, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnOption.SetButtonSize(3);
	m_BtnOption.SetText(GlobalText[385]);

	y += 35.f;
	m_BtnCancel.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_DECOR, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY);
	m_BtnCancel.SetButtonSize(3);
...
void SEASON3B::CBloodCastleResultMsgBox::RenderFrame()
{
	...
	x = GetPos().x; y = GetPos().y, width = MSGBOX_WIDTH; height = 40;
	RenderImageScaleAuto(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x, y, width, height);

	x = GetPos().x; y += 40; width = MSGBOX_WIDTH; height = 40;
	for (int i = 0; i < 3; ++i)
	{
		RenderImageScaleAuto(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x, y, width, height);
		y += height;
	}

	x = GetPos().x; width = MSGBOX_WIDTH; height = 40;
	RenderImageScaleAuto(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x, y, width, height);
}
...
bool SEASON3B::CDevilSquareRankMsgBox::Create(float fPriority)
{
	...
	x = (gWideScreen.g_WideWindowWidth / 2) - (460 / 2);
	y = 60;
	width = 460;
	height = 80 + (3 * 80) + 80;
...
void SEASON3B::CDevilSquareRankMsgBox::RenderFrame()
{
	...
	x = GetPos().x; y = GetPos().y, width = 460; height = 50;

	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x, y, 50, height, 0.0009765625f, 0.00390625f, 0.1560801630434783f, 0.62109375f, 1, 1, 0);
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x + 50 + 360, y, 50, height, 0.0009765625f + 0.1560801630434783f + 0.5853006114130434f, 0.00390625f, 0.1560801630434783f, 0.62109375f, 1, 1, 0);
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP, x + 50, y, 360, height, 0.0009765625f + 0.1560801630434783f, 0.00390625f, 0.5853006114130434f, 0.62109375f, 1, 1, 0);

	x = GetPos().x; y += 50; width = 460; height = 50;
	for (int i = 0; i < 6; ++i)
	{
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x, y, 50, height, 0.0009765625f, 0.00390625f, 0.1560801630434783f, 0.62109375f, 1, 1, 0);
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x + 50 + 360, y, 50, height, 0.0009765625f + 0.1560801630434783f + 0.5853006114130434f, 0.00390625f, 0.1560801630434783f, 0.62109375f, 1, 1, 0);
		RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_MIDDLE, x + 50, y, 360, height, 0.0009765625f + 0.1560801630434783f, 0.00390625f, 0.5853006114130434f, 0.62109375f, 1, 1, 0);
		y += height;
	}

	x = GetPos().x; width = 460; height = 50;

	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x, y, 50, height, 0.0009765625f, 0.00390625f, 0.1560801630434783f, 0.62109375f, 1, 1, 0);
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x + 50 + 360, y, 50, height, 0.0009765625f + 0.1560801630434783f + 0.5853006114130434f, 0.00390625f, 0.1560801630434783f, 0.62109375f, 1, 1, 0);
	RenderBitmap(CNewUIMessageBoxMng::IMAGE_MSGBOX_BOTTOM, x + 50, y, 360, height, 0.0009765625f + 0.1560801630434783f, 0.00390625f, 0.5853006114130434f, 0.62109375f, 1, 1, 0);



	x = GetPos().x + 13; y = GetPos().y + 52; width = 434; height = MSGBOX_SEPARATE_LINE_HEIGHT;
	RenderImageScaleAuto(CNewUIMessageBoxMng::IMAGE_MSGBOX_SEPARATE_LINE, x, y, width, height);
...
bool SEASON3B::CChaosMixMenuMsgBox::Create(float fPriority)
{
	...
	x = (gWideScreen.g_WideWindowWidth / 2) - (225 / 2);
	y = 50;
	width = MSGBOX_WIDTH;
	height = MSGBOX_TOP_HEIGHT + (MIDDLE_COUNT * MSGBOX_MIDDLE_HEIGHT) + MSGBOX_BOTTOM_HEIGHT;
...
void SEASON3B::CChaosMixMenuMsgBox::SetButtonInfo()
{
	...
	m_BtnGeneralMix.SetButtonSize(2);
	...
	//m_BtnMix380.SetButtonSize(2);
	//m_BtnMix380.SetText(GlobalText[2193]);

	width = MSGBOX_BTN_EMPTY_SMALL_WIDTH;
	btnhalfwidth = width / 2.f;
	x = GetPos().x + msgboxhalfwidth - btnhalfwidth;
	y = GetPos().y + 230; //GetPos().y + GetSize().cy - (MSGBOX_BTN_EMPTY_HEIGHT + MSGBOX_BTN_BOTTOM_BLANK) + 10;
	m_BtnCancel.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x, y, width, height, CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL);
	m_BtnCancel.SetButtonSize(1);
	m_BtnCancel.SetText(GlobalText[1002]);
}
...
void SEASON3B::CChaosMixMenuMsgBox::RenderFrame()
{
	...
	//RenderImage(CNewUIMessageBoxMng::IMAGE_MSGBOX_BACK, x, y, width, height, 0.f, 0.f, 460.f, 80.f);

	//x = GetPos().x; y = GetPos().y, width = MSGBOX_WIDTH; height = MSGBOX_TOP_HEIGHT;
	//RenderImageScale(CNewUIMessageBoxMng::IMAGE_MSGBOX_TOP_TITLEBAR, x, y, width, height, 0.f, 0.f, 460.f, 80.f);
	//
	//x = GetPos().x; y += MSGBOX_TOP_HEIGHT; width = MSGBOX_WIDTH; height = MSGBOX_MIDDLE_HEIGHT;
	//for (int i = 0; i < MIDDLE_COUNT; ++i)
	//{
...
void SEASON3B::CDuelMsgBox::RenderTexts()
{
	...
	g_pRenderText->RenderText(GetPos().x, GetPos().y + 15, strDuelID, MSGBOX_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->SetBgColor(0, 0, 0, 0);
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFont);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + 35, GlobalText[910], MSGBOX_WIDTH, 0, RT3_SORT_CENTER);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + 45, GlobalText[911], MSGBOX_WIDTH, 0, RT3_SORT_CENTER);
}
...
void SEASON3B::CDuelResultMsgBox::RenderTexts()
{
	...
	g_pRenderText->SetTextColor(255, 255, 0, 255);
	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + 15, GlobalText[2694], MSGBOX_WIDTH, 0, RT3_SORT_CENTER);

	g_pRenderText->SetBgColor(0, 0, 0, 0);
	g_pRenderText->SetTextColor(255, 255, 255, 255);
	g_pRenderText->SetFont(g_hFont);
	sprintf(strDuelID, GlobalText[2695], m_szWinnerID);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + 35, strDuelID, MSGBOX_WIDTH, 0, RT3_SORT_CENTER);
	sprintf(strDuelID, GlobalText[2696], m_szLoserID);
	g_pRenderText->RenderText(GetPos().x, GetPos().y + 45, strDuelID, MSGBOX_WIDTH, 0, RT3_SORT_CENTER);
}
...
bool SEASON3B::CChangeWareMsgBoxLayout::SetLayout()
{
	CNewUITextInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;
	...
	pMsgBox->AddCallbackFunc(CChangeWareMsgBoxLayout::ReturnDown, MSGBOX_EVENT_PRESSKEY_RETURN);
	pMsgBox->AddCallbackFunc(CChangeWareMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CChangeWareMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT SEASON3B::CChangeWareMsgBoxLayout::ReturnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CChangeWareMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	return ProcessOk(pOwner, xParam);
}

CALLBACK_RESULT SEASON3B::CChangeWareMsgBoxLayout::ProcessOk(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	CNewUITextInputMsgBox* pMsgBox = dynamic_cast<CNewUITextInputMsgBox*>(pOwner);
	unicode::t_char strText[MAX_TEXT_LENGTH] = { 0, };
	pMsgBox->GetInputBoxText(strText);
...
CALLBACK_RESULT SEASON3B::CChangeWareMsgBoxLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	PlayBuffer(SOUND_CLICK01);
	g_MessageBox->SendEvent(pOwner, MSGBOX_EVENT_DESTROY);
...
{
	CNewUIMultipleTextInputMsgBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
		return false;

	int inputboxcount = (gProtect->m_MainInfo.m_PersonalShop_Coins <= 4 ? gProtect->m_MainInfo.m_PersonalShop_Coins : 4);

	if (false == pMsgBox->Create(MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_NUMBER, INPUTBOX_WIDTH, INPUTBOX_HEIGHT, INPUTBOX_TEXTLIMIT, inputboxcount))
		return false;

	pMsgBox->AddMsg(-1, GlobalText[1129]);

	for (int i = 0; i < inputboxcount; i++) {
		pMsgBox->SetInputBoxOption(i, UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK);
		pMsgBox->AddMsg(i, GlobalText[3190+i]);
	}
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemValueMsgBoxLayout::ProcessOk(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	int inputvalue[4] = {0,0,0,0};

	bool haveValue = false;

	for (int i = 0; i < gProtect->m_MainInfo.m_PersonalShop_Coins; i++) {
		pMsgBox->GetInputBoxText(i, strText);
	...
	*/
	{
		...
		if (pPickedItem)
		{
			...
			if (pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl())
			{
				...
				SendRequestSetSalePrice(byIndex, inputvalue[0], inputvalue[1], inputvalue[2], inputvalue[3]);
			...
			AddPersonalItemPrice(MAX_MY_INVENTORY_INDEX + iTargetIndex, inputvalue[0], inputvalue[1], inputvalue[2], inputvalue[3], g_IsPurchaseShop);
		}
		else
		{
			...
			BYTE byIndex = iSourceIndex;
			SendRequestSetSalePrice(byIndex, inputvalue[0], inputvalue[1], inputvalue[2], inputvalue[3]);
			AddPersonalItemPrice(iSourceIndex, inputvalue[0], inputvalue[1], inputvalue[2], inputvalue[3], g_IsPurchaseShop);
		}
	}
...
void SEASON3B::CGuild_ToPerson_Position::SetButtonInfo()
{
	...
	y = GetPos().y + 30;
	m_BtnBlessing.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x, y, width, height);
	m_BtnBlessing.SetText(GlobalText[1311]);

	y += 27;
	m_BtnSoul.SetInfo(CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x, y, width, height);
	...
	m_BtnBlessing.SetButtonSize(2);
	m_BtnSoul.SetButtonSize(2);
	m_BtnOk.SetButtonSize(1);
	m_BtnCancel.SetButtonSize(1);
		
	m_BtnBlessing.SetPos(m_BtnBlessing.GetPosX(), m_BtnBlessing.GetPosY() - 3);
	m_BtnSoul.SetPos(m_BtnSoul.GetPosX(), m_BtnSoul.GetPosY() + 3);
	m_BtnOk.SetPos(m_BtnOk.GetPosX()-5, m_BtnOk.GetPosY()+10);
	m_BtnCancel.SetPos(m_BtnCancel.GetPosX()+5, m_BtnCancel.GetPosY()+10);
...
void SEASON3B::CGuild_ToPerson_Position::RenderTexts()
{
	...
	x = GetPos().x; y = (GetPos().y + (MSGBOX_TEXT_TOP_BLANK / 2)) + 85;
...
void SEASON3B::CGuild_ToPerson_Position::RenderButtons()
{
	...
	unicode::_sprintf(strText, GlobalText[1314], GuildList[DeleteIndex].Name);
	...
	if (COMGEM::m_cGemType == COMGEM::CELE)
	{
		...
		unicode::_sprintf(strText, GlobalText[1315], GlobalText[1301]);
		AddMsg(strText, RGBA(255, 128, 0, 255), MSGBOX_FONT_BOLD);
	}
	else
	{
	...
	{		
		...
		unicode::_sprintf(strText, GlobalText[1315], GlobalText[1302]);
		AddMsg(strText, RGBA(255, 128, 0, 255), MSGBOX_FONT_BOLD);
	}
	else
	{
