...
{
	// Defina um número de quadros de referência (pode ser ajustado conforme a necessidade)
	const int reference_frames = 10; // Exemplo: Checar a cada 10 quadros, ajuste conforme necessário
	...
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
		...
		// Usar rand_fps_check para determinar se a verificação deve ocorrer
		if (!rand_fps_check(reference_frames)) {
			continue; // Pula para o próximo ciclo se a verificação não for executada
		}
...
bool AddRangeAttack(vec3_t Position, float Range, short PKKey, int DamageKey[], int* piCount, int iMaxKey, int reference_frames)
{
	// Usar rand_fps_check para controlar a frequência das verificações
	if (!rand_fps_check(reference_frames)) {
		return true;  // Pula a execução se a chance não for atendida
	}
	...
	for (int icntEffect = 0; icntEffect < MAX_EFFECTS; icntEffect++)
	{
		...
		if (!o->Live)
		{
			...
			case MODEL_SUMMONER_WRISTRING_EFFECT:
				...
				Vector(0.f, 0.f, 0.f, o->Direction);
				break;
			case MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT: //EFEITO DE CABEÇA GIRANDO DE FOGO
				if (o->SubType == 0)
				{
					o->LifeTime = 100;
					o->Scale = 0.8f;
				...
				break;
			case MODEL_SUMMONER_EQUIP_HEAD_NEIL: //EFEITO DE CABEÇA GIRANDO NEGRO
				if (o->SubType == 0)
				{
					o->LifeTime = 100;
					o->Scale = 0.8f;
				...
				break;
			case MODEL_SUMMONER_EQUIP_HEAD_LAGUL: //EFEITO DE CABEÇA GIRANDO ROXA
			{
				o->LifeTime = 100;
				o->Scale = 0.8f;
				...
				{
					Vector(o->Light[0], o->Light[1], o->Light[2], o->Direction);
				}
			...
			case MODEL_SUMMONER_CASTING_EFFECT222: //CASTING EFFECT
			{
				o->LifeTime = 40;
				o->Scale = 1.0f;
				o->Alpha = 1.0f;
			...
			break;
			case MODEL_DEATH_SCYTHE: //DEATH SCYTHE
			{
				o->LifeTime = 35;
				o->Scale = 0.8f;
				o->Alpha = 1.0f;
			...
			case BITMAP_BOSS_LASER + 2:
				...
				{
				case BITMAP_BOSS_LASER:
					Vector(0.5f, 0.7f, 5.f, o->Light);
					Vector(0.f, -35.f, 0.f, p1);
					o->Scale = 16.f;
			...
			case BITMAP_MAGIC:
				...
				else if (o->SubType == 9 || o->SubType >= 10100) //CASTING EFFECT
				{
					o->LifeTime = 30.f;
					o->Scale = 2.4f;
					Vector(0, 0, 0, o->HeadAngle);
				}
				else if (o->SubType == 10 || o->SubType >= 100) //CASTING EFFECT
				{
					o->LifeTime = 33;
					o->Scale = 12.f;
					o->Alpha = 0.0f;
			...
			case BITMAP_MAGIC + 2:
				o->LifeTime = 20;
				if (o->SubType == 4 || o->SubType == 10 || o->SubType >= 100) //BUFF BK
				{
					o->LifeTime = 40;
					...
					if (o->SubType >= 100)
					{
						int customFloorSize = gBuffColorBK.GetBuffSize4(o->SubType);

						if (customFloorSize > -1)
							o->Scale = customFloorSize;
						else
							o->Scale = ((rand() % 50) + 50) / 100.f * 4.f;
					}
				}
			...
			case BITMAP_DAMAGE_01_MONO:
				...
				break;
			case BITMAP_FLARE:
				if (SubType == 1 || SubType == 2 || SubType >= 100) //BUFF BK
				{
					o->LifeTime = 30;
				}
				else if (SubType == 3)
			...
			break;
			case MODEL_FENRIR_THUNDER:  //EFEITO DE RAIOS
			case MODEL_FENRIR_THUNDER2: //EFEITO DE RAIOS 2
			{
				if (o->SubType == 0)
				{
					o->LifeTime = 100;
				...
				//NOVOS RAIOS
				else if (o->SubType == 4) //RAIO PEQUENO COLORIDO
				{
					o->LifeTime = 4;
					o->Scale = 0.1f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
				...
				else if (o->SubType == 5) //RAIO PEQUENO SPRITE PRETO
				{
					o->LifeTime = 4;
					o->Scale = 0.1f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
					...
					Vector(Luminosity * 1.f, Luminosity * 1.f, Luminosity * 1.f, vLight);

					CreateSprite(BITMAP_LIGHT, o->Position, 1.0f, vLight, o, 0, 1);
					CreateSprite(BITMAP_LIGHT, o->Position, 2.0f, vLight, o, 0, 1);
				}
				else if (o->SubType == 6) //RAIO PEQUENO PRETO COMPLETO SPRITE COLORIDO
				{
					o->LifeTime = 4;
					o->Scale = 0.3f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
					...
					vLight[1] = o->Light[1] - 0.3f;
					vLight[2] = o->Light[2] - 0.3f;
					CreateSprite(BITMAP_LIGHT, o->Position, 3.0f, vLight, o, 0, 1);
				}
				else if (o->SubType == 7) //RAIO PEQUENO PRETO COMPLETO
				{
					o->LifeTime = 4;
					o->Scale = 0.2f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
					o->m_iAnimation = 0;
				...
				else if (o->SubType == 8) //RAIOS GRANDES PRETO COMPLETO
				{
					o->LifeTime = 4;
					o->Scale = 0.4f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
					o->m_iAnimation = 0;
				...
				else if (o->SubType == 9) //RAIOS MUITO PEQUENO COLORIDO
				{
					o->LifeTime = 4;
					o->Scale = 0.05f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
					o->m_iAnimation = 0;
				...
				else if (o->SubType == 10) //RAIOS MUITO PEQUENO SPRITE COLORIDO
				{
					o->LifeTime = 4;
					o->Scale = 0.05f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
				...
				else if (o->SubType == 11) //RAIOS MUITO PEQUENO PRETO
				{
					o->LifeTime = 4;
					o->Scale = 0.1f + (float)(rand() % 100) * 0.002f;
					o->Alpha = 1.0f;
					...
					float Luminosity = 1.f;
					Vector(Luminosity * 1.f, Luminosity * 1.f, Luminosity * 1.f, vLight);
					CreateSprite(BITMAP_LIGHT, o->Position, 1.5f, vLight, o, 0, 1);
				}
				else if (o->SubType == 12) //RAIOS FENRIR - COLORIDOS 32002
				{
					o->LifeTime = 100;
					o->Scale = 0.3f + (float)(rand() % 100) * 0.002f;
					o->m_iAnimation = 0;
					...
					vLight[1] = o->Light[1] - 0.3f;
					vLight[2] = o->Light[2] - 0.3f;
					CreateSprite(BITMAP_LIGHT, o->Position, 2.0f, vLight, o, 0, 0);
				}
				else if (o->SubType == 13) //RAIOS FENRIR - COLORIDOS 32400 (32380 SEASON 6)
				{
					o->LifeTime = 100;
					o->Scale = 0.3f + (float)(rand() % 100) * 0.002f;
					o->m_iAnimation = 0;
					...
					vLight[1] = o->Light[1] - 0.3f;
					vLight[2] = o->Light[2] - 0.3f;
					CreateSprite(BITMAP_LIGHTMARKS, o->Position, 2.0f, vLight, o, 0, 0);
				}
				else if (o->SubType == 14 || o->SubType == 15) //RAIOS FENRIR - PRETO 32002
				{
					o->LifeTime = 100;
					o->Scale = 0.3f + (float)(rand() % 100) * 0.002f;
					o->m_iAnimation = 0;
				...
				else if (o->SubType == 16 || o->SubType == 17) //RAIOS FENRIR - PRETO 32400 (32380 SEASON 6)
				{
					o->LifeTime = 100;
					o->Scale = 0.3f + (float)(rand() % 100) * 0.002f;
					o->m_iAnimation = 0;
					...
					CreateSprite(BITMAP_LIGHTMARKS, o->Position, 2.0f, vLight, o, 0, 1);
				}
			...
			break;
			case MODEL_FENRIR_FOOT_THUNDER: //EFEITO DE RAIO NO CHÃO
				o->LifeTime = 200;
				o->m_iAnimation = 0;
				VectorCopy(Position, o->Position);
				VectorCopy(Light, o->Light);
				...
				o->Position[2] = 0.0f;
				break;
			case MODEL_TWINTAIL_EFFECT: //EFEITO PEGADAS NO CHÃO
			{
				if (o->SubType == 0)
				{
					o->LifeTime = 200;
			...
			case MODEL_FEATHER: //EFEITO DE PENAS
			{
				switch (o->SubType)
				{
				case 0:
				...
				case 7:
				{
					...
					if (o->SubType == 4)
						o->Scale = o->Scale + ((float)(rand() % 20 - 10) * (o->Scale * 0.02f));

					if (o->SubType == 5)
						o->Scale = o->Scale + ((float)(rand() % 20 - 10) * (o->Scale * 0.01f));

					if (o->SubType == 6)
						o->Scale = o->Scale - 0.2f;

					if (o->SubType == 7)
						o->Scale = o->Scale - 0.5f;
...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	case MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT: //EFEITO DE CABEÇA GIRANDO DE FOGO
		if (o->SubType == 0)
		{
			...
			else
			{
				...
				if (sinf(WorldTime * 0.0004f + o->Skill * 0.024f) < 0.3f)
					o->Kind = 1;
		...
		break;
	case MODEL_SUMMONER_EQUIP_HEAD_NEIL: //EFEITO DE CABEÇA GIRANDO NEGRA
		if (o->SubType == 0)
		{
			OBJECT* pObject = o->Owner;
			o->Position[0] = pObject->Position[0] + cosf(WorldTime * 0.003f + o->Skill * 0.024f) * 60.0f;
	...
	case MODEL_SUMMONER_EQUIP_HEAD_LAGUL: //EFEITO DE CABEÇA GIRANDO ROXA
	{
		...
		else if (o->SubType == 1) //COR STYLE
		{
			Vector(o->Alpha * 0.56f, o->Alpha * 0.45f, o->Alpha * 1.f, vLight);
			CreateParticle(BITMAP_CLUD64, o->Position, o->Angle, vLight, 10, 1.f, pObject);
		}
		else //COR PELO LUA
		{
			//gSummonEffect.SummonEffectColor(o->SubType, vLight);
			Vector(o->Alpha * vLight[0], o->Alpha * vLight[1], o->Alpha * vLight[2], vLight);
			CreateParticle(BITMAP_CLUD64, o->Position, o->Angle, vLight, 10, 1.f, pObject);
		}
	}
	case MODEL_SUMMONER_CASTING_EFFECT1:
	...
	break;
	case BITMAP_FIRE:
		CreateParticle(BITMAP_FIRE, o->Position, o->Angle, Light, 1, 1.f, o->Owner);
		break;
	...
	case BITMAP_MAGIC + 2:
		if (o->SubType >= 1
			&& o->SubType != 4 && o->SubType != 6 && o->SubType != 7 && o->SubType != 8 && o->SubType != 9 && o->SubType != 10 && o->SubType != 11 && o->SubType != 12 && o->SubType != 13
			&& o->SubType < 100) //BUFF BK
		{
			CreateHealing(o);
		}
		...
		else if (o->SubType == 4)
		{
			...
			for (int j = 0; j < 18; j++)
			{
				if (rand_fps_check(1))
				{
					...
					Position[0] += rand() % 64 - 32;
					Position[1] += rand() % 64 - 32;
					CreateParticle(BITMAP_FLAME, Position, o->Angle, Light, 7, 1.2f);
				}
			}
			CreateEffect(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 12);
		}
		...
		else if (o->SubType == 3 || o->SubType == 4)
		{
			...
			else
			{
				...
				if (!pModel->Bones[iBone].Dummy)
				{
					...
					{
						if (rand_fps_check(2)) // PAREI AQUI LEMBRAR WALLACE
						{
							vWorldPos[2] -= 20.f;
	...
	case MODEL_LIGHTNING_SHOCK:
	{
		...
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 6; ++i)
			{
				...
				CreateParticleFpsChecked(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPosition, pObject->Angle, vLight, 0, fScale);	// Àü±â
			}
	...
	case BITMAP_LIGHT:
		...
		else if (o->SubType == 1 || o->SubType >= 100) //BUFF BK
		{
			...
			if (o->SubType >= 100)
			{
				...
				int Black = gBuffColorBK.GetBuffBlack3(o->SubType);
				if (Black == 1)
				{
				...
				{
					gBuffColorBK.GetBuffColorBK3(o->SubType, Light); //BUFF BK COLOR EMCIMA DA CABEÇA
				}
			}
			else
			{
		...
		else if (o->SubType == 3) // COR DO BRILHO ANCIENTE
		{
			o->Velocity += FPS_ANIMATION_FACTOR;
	...
	case MODEL_MAGIC1:
		...
		break;
	case MODEL_SKILL_WHEEL1:
		CreateEffect(MODEL_SKILL_WHEEL2, o->Position, o->Angle, o->Light, 0 - o->LifeTime, o->Owner, o->PKKey, o->Skill, o->Kind);
		break;
	...
	case MODEL_SKILL_WHEEL2:
		...
		// Ajuste da direção do vetor com base na arma do jogador
		...
		// Aplicar rotação e somar a posição
		AngleMatrix(o->Angle, Matrix);
		VectorRotate(p, Matrix, Position);
		VectorAdd(Position, o->Owner->Position, o->Position);
		o->Angle[2] -= 25 * FPS_ANIMATION_FACTOR;

		// Ajuste da criação de partículas com base no FPS
		if (rand_fps_check(5))  // Menor probabilidade de criar partículas
		{
		...
		// Ajustar a altura com base no mapa ativo
		Height = 20.f;
		...
		else
		{
			for (int j = 0; j < 4; j++)
			{
				// Limitar a criação de partículas com base no FPS
				if (rand_fps_check(10))  // Menor probabilidade de execução
				{
					Vector((float)(rand() % 60 + 60 - 90), 0.f, (float)(rand() % 30 + 90), Angle);
					VectorAdd(Angle, o->Angle, Angle);
					VectorCopy(o->Position, Position);
					...
					// Criar partículas ou efeitos com menor frequência
					if (rand_fps_check(4)) // Menor chance de gerar partículas
	...
	case MODEL_DARK_SCREAM:
	{
		...
		CreateParticle(BITMAP_FLAME, o->Position, o->Angle, o->Light, 1, (o->Scale - 0.4f) * 3.5f);
		if (o->Type == MODEL_DARK_SCREAM)
		{
			CheckClientArrow(o);
		}
	...
	case BITMAP_FLARE:
		...
		else if (o->SubType == 2 || o->SubType >= 100) //BUFF BK
		{
			CreateParticle(BITMAP_FLARE_BLUE, o->Position, o->Angle, Light, 2, 1.0f);
		}
		else
			if (o->SubType == 3)
			{
	...
	case MODEL_FENRIR_THUNDER:  //EFEITO DE RAIOS
	case MODEL_FENRIR_THUNDER2: //EFEITO DE RAIOS 2
		if (o->SubType == 0 || o->SubType == 12 || o->SubType == 13 || o->SubType == 14 || o->SubType == 15 || o->SubType == 16 || o->SubType == 17 || o->SubType == 18 || o->SubType == 19)
		{
			if (o->Live)
			{
				if (o->m_iAnimation == 0)
	...
	break;
	case MODEL_FENRIR_FOOT_THUNDER: //EFEITO DE RAIOS NO CHÃO
		if (o->Live == true)
		{
	...
	case MODEL_PROTECTGUILD:
	{
		...
		//				o->Alpha = sin((o->LifeTime/130.0f)*Q_PI) - 0.1f;
		if (o->LifeTime > 120)
		{
	...
	case MODEL_FEATHER:
	{
		...
		case 4: //EFEITO DE PENAS COLORIDA NORMAL
		{
			o->Light[0] *= 0.97f;
			o->Light[1] *= 0.97f;
			o->Light[2] *= 0.97f;
		...
		case 5: //EFEITO DE PENAS COLORIDA
		{
			o->Light[0] *= pow(0.97f, FPS_ANIMATION_FACTOR);
			o->Light[1] *= pow(0.97f, FPS_ANIMATION_FACTOR);
			o->Light[2] *= pow(0.97f, FPS_ANIMATION_FACTOR);
		...
		case 6: //EFEITO DE PENAS COLORIDA PEQUENAS
		{
			o->Light[0] *= pow(0.97f, FPS_ANIMATION_FACTOR);
			o->Light[1] *= pow(0.97f, FPS_ANIMATION_FACTOR);
			o->Light[2] *= pow(0.97f, FPS_ANIMATION_FACTOR);
		...
		case 7: //EFEITO DE PENAS COLORIDA MUITO PEQUENAS
		{
			o->Light[0] *= pow(0.97f, FPS_ANIMATION_FACTOR);
			o->Light[1] *= pow(0.97f, FPS_ANIMATION_FACTOR);
			o->Light[2] *= pow(0.97f, FPS_ANIMATION_FACTOR);
		...
		if (o->Live)
		{
			...
			if (o->Visible)
			{
				...
				case BATTLE_CASTLE_WALL4:
					RenderObject(o);
					break;
				case MODEL_FENRIR_THUNDER:  //EFEITO RAIO DO FENRIR
				case MODEL_FENRIR_THUNDER2: //EFEITO RAIO DO FENRIR 2
					RenderObject(o);
					break;
				case MODEL_FALL_STONE_EFFECT:
					RenderObject(o);
					break;
				case MODEL_FENRIR_FOOT_THUNDER: //EFEITO RAIO NO CHÃO
				{
					if (o->SubType == 0)
					{
						EnableAlphaBlend();
					...
					else if (o->SubType == 1)
					{
						...
						RenderTerrainAlphaBitmap(BITMAP_FENRIR_FOOT_THUNDER1 + (o->m_iAnimation % 5), o->Position[0], o->Position[1], 0.6f, 0.6f, vLight);
						//DisableAlphaBlend();
					}
				...
				case MODEL_TWINTAIL_EFFECT: //EFEITO PEGADAS NO CHÃO
				{
					...
					if (o->SubType == 3)
					{
						...
						//EnableAlphaBlend();
						EnableAlphaBlendMinus();
						RenderTerrainAlphaBitmap(BITMAP_SPARK + 1, o->Position[0], o->Position[1], o->Scale, o->Scale, vLight, 0.f, o->Alpha);
						DisableAlphaBlend();
					}
				}
				break;
			...
			if (o->Visible)
			{
				...
				case BITMAP_MAGIC:
					...
					else if (o->SubType == 9 || o->SubType >= 10100) //CASTING EFFECT
					{
						...
						int Black = gPlasmaStormColor.GetPlasmaStormRuneBlack(o->SubType - 10000);
						...
						if (o->SubType >= 10100)
						{
							...
							{
								gPlasmaStormColor.GetPlasmaStormRune(o->SubType - 10000, vLight);
							}
						}
						else
						{
						...
						int Black2 = gPlasmaStormColor.GetPlasmaStormRuneBlack2(o->SubType - 10000);
						...
						{
							EnableAlphaBlend();
							gPlasmaStormColor.GetPlasmaStormRune2(o->SubType - 10000, vLight);
						}
					...
					else if (o->SubType == 10 || o->SubType >= 100) //CASTING EFFECT
					{
						...
						gPlasmaStormColor.GetPlasmaStormCastingShadowColor(o->SubType, vLight);

						if (gPlasmaStormColor.GetPlasmaStormCastingShadow(o->SubType) == 1)
						{
							EnableAlphaBlend();

							if (gPlasmaStormColor.GetPlasmaStormCastingShadowAlpha(o->SubType) > -1)
								Alpha = gPlasmaStormColor.GetPlasmaStormCastingShadowAlpha(o->SubType);

							Vector(vLight[0] * Alpha, vLight[1] * Alpha, vLight[2] * Alpha, vLight);
						}
						else if (gPlasmaStormColor.GetPlasmaStormCastingShadow(o->SubType) == 2 || o->SubType == 10)
						{
							EnableAlphaBlendMinus();
							if (gPlasmaStormColor.GetPlasmaStormCastingShadowAlpha(o->SubType) > -1)
								Alpha = gPlasmaStormColor.GetPlasmaStormCastingShadowAlpha(o->SubType);
							...
							//Vector(1.f * o->Alpha, 1.f * o->Alpha, 1.f * o->Alpha, vLight);
						}
						...
						for (int i = 0; i < 5; ++i)
						{
							...
							if (gPlasmaStormColor.GetPlasmaStormCastingShadow(o->SubType) == 1 || gPlasmaStormColor.GetPlasmaStormCastingShadow(o->SubType) == 2 || o->SubType == 10)
							{
				...
				case BITMAP_MAGIC + 1:
					...
					if (o->SubType != 5 && o->SubType != 6 && o->SubType != 8)
					{
						...
						case 13:
							...
							break;
						default:
							if (o->SubType >= 100) //BUFF BK COLOR NO CHÃO
							{
								int Black = gBuffColorBK.GetBuffBlack4(o->SubType);
								if (Black == 1)
								{
									EnableAlphaBlendMinus();
									Vector(1.f, 1.f, 1.f, Light);
									...
									gBuffColorBK.GetBuffColorBK4(o->SubType, Light);
							}
							break;
						}
						int customImage = gBuffColorBK.GetBuffImage4(o->SubType);

						if (customImage > -1) {
							RenderTerrainAlphaBitmap(customImage, o->Position[0], o->Position[1], Scale, Scale, Light, -o->Angle[2]);
						}
						else {
