...
void renderBitmap(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, bool Scale, bool StartScale, float Alpha)
{
	if (StartScale)
	{
		x = x * (float)WindowWidth / 480.f;
		y = y * (float)WindowHeight / 480.f;
	}
	if (Scale)
	{
		Width = Width * (float)WindowWidth / 480.f;
...
void CUIMng::CreateTitleSceneUI()
{
	...
	m_asprTitle[UIM_TS_BACK0].Create(400, 480, BITMAP_TITLE, 0, NULL, 0, 0,
		false, SPR_SIZING_DATUMS_LT, fScaleX, fScaleY);
	m_asprTitle[UIM_TS_BACK0].SetPosition(0, 0);

	m_asprTitle[UIM_TS_BACK1].Create(400, 480, BITMAP_TITLE+1, 0, NULL, 0, 0,
		false, SPR_SIZING_DATUMS_LT, fScaleX, fScaleY);
...
void CUIMng::RenderTitleSceneUI(HDC hDC, DWORD dwNow, DWORD dwTotal)
{
	...
	m_pgbLoding->Render();
	SEASON3B::RenderImageScaleAuto(BITMAP_TITLE, 0, 0, gWideScreen.g_WideWindowWidth, gWideScreen.g_WideWindowHeight);
#endif
...
void CUIMng::CreateLoginScene()
{
	...
	m_LoginMainWin.SetPosition(40, nBaseY - m_LoginMainWin.GetHeight() - 11);

	m_ServerSelWin.Create();
	m_WinList.AddHead(&m_ServerSelWin);
	m_ServerSelWin.SetPosition((225 + rInput.GetScreenWidth() - m_ServerSelWin.GetWidth()) / 2, ((rInput.GetScreenHeight() - m_ServerSelWin.GetHeight()) / 2) + 70);
