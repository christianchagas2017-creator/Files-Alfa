...
void CPersonalShopTitleImp::UpdatePosition()
{
	...
	if (m_Resolution > 3) {
		Width = gWideScreen.g_WideWindowWidth, Height = gWideScreen.g_WideWindowHeight;
	}
	else {
	...
	if(!CameraTopViewEnable)
		Height = gWideScreen.g_WideWindowHeight-48;
...
void CPersonalShopTitleImp::CalculateBooleanPos(IN CHARACTER* pPlayer, IN const SIZE& size, OUT POINT& pos)
{
	...
	//. pos : real position
	pos.x = (ptFloating.x*(int)WindowWidth/gWideScreen.g_WideWindowWidth)-size.cx/2;
	pos.y = (ptFloating.y*(int)WindowHeight/gWideScreen.g_WideWindowHeight)-(12*3*(int)WindowHeight/ gWideScreen.g_WideWindowHeight);
}
