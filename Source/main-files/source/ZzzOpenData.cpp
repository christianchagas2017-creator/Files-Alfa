...
void OpenPlayers()
{
	...
	//Models = new BMD [MAX_MODELS+1024];
	...
	Models[MODEL_PLAYER].Actions[PLAYER_STOP_MALE_NEW].PlaySpeed = 0.28f;
...
void OpenExtraItens()
{
	...
	for (std::map<int, CUSTOM_WING_INFO>::iterator it = gCustomWing.m_CustomWingInfo.begin(); it != gCustomWing.m_CustomWingInfo.end(); it++)
	{
		...
		if (lpInfo->Type == 0)
		{
			gLoadData.AccessModel(lpInfo->ItemIndex + MODEL_ITEM, lpInfo->Dir, lpInfo->ModelName);
			gLoadData.OpenTexture((lpInfo->ItemIndex + MODEL_ITEM), lpInfo->Dir);
		}
		else {
			gLoadData.AccessModelCustom(lpInfo->ItemIndex + MODEL_ITEM, lpInfo->Dir, lpInfo->ModelName);
			gLoadData.OpenTextureCustom((lpInfo->ItemIndex + MODEL_ITEM), lpInfo->Dir);
		}
	}
	*/
	...
	for (int n = 0; n < MAX_CUSTOM_JEWEL; n++)
	{
		...
		{
			if (gCustomJewel.m_CustomJewelInfo[n].Type == 0)
			{
				gLoadData.AccessModel(gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM, gCustomJewel.m_CustomJewelInfo[n].Dir, gCustomJewel.m_CustomJewelInfo[n].ModelName);
				gLoadData.OpenTexture((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), gCustomJewel.m_CustomJewelInfo[n].Dir);
			}
			else
			{
				gLoadData.AccessModelCustom(gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM, gCustomJewel.m_CustomJewelInfo[n].Dir, gCustomJewel.m_CustomJewelInfo[n].ModelName);
				gLoadData.OpenTextureCustom((gCustomJewel.m_CustomJewelInfo[n].ItemIndex + MODEL_ITEM), gCustomJewel.m_CustomJewelInfo[n].Dir);
			}
	...
	for (std::map<int, HELPER_INFO>::iterator it = gHelperSystem.m_HelperInfo.begin(); it != gHelperSystem.m_HelperInfo.end(); it++)
	{
		...
		if (lpInfo->FileType == 0)
		{
			gLoadData.AccessModel(lpInfo->ItemIndex, lpInfo->ModelDir, lpInfo->Model);
			gLoadData.OpenTexture(lpInfo->ItemIndex, lpInfo->ModelDir);
		}
		else
		{
			gLoadData.AccessModelCustom(lpInfo->ItemIndex, lpInfo->ModelDir, lpInfo->Model);
			gLoadData.OpenTextureCustom(lpInfo->ItemIndex, lpInfo->ModelDir);
		}
		...
		if (lpInfo->FileType == 0)
		{
			gLoadData.AccessModel(lpInfo->ModelID, lpInfo->ObjectModelDir, lpInfo->ObjectModel);
			gLoadData.OpenTexture(lpInfo->ModelID, lpInfo->ObjectModelDir);
		}
		else
		{
			gLoadData.AccessModelCustom(lpInfo->ModelID, lpInfo->ObjectModelDir, lpInfo->ObjectModel);
			gLoadData.OpenTextureCustom(lpInfo->ModelID, lpInfo->ObjectModelDir);
		}
	...
	int DarkSpirit = gItemManager.GET_ITEM_MODEL(16, 255);

	for (std::map<int, DARK_SPIRIT_INFO>::iterator it = gDarkSpirit.m_DarkSpiritInfo.begin(); it != gDarkSpirit.m_DarkSpiritInfo.end(); it++)
	{
		auto lpInfo = &(*it).second;

		if (lpInfo->FileType == 0)
		{
			gLoadData.AccessModel(lpInfo->ItemIndex + MODEL_ITEM, lpInfo->ModelNameDir, lpInfo->ModelName);
			gLoadData.OpenTexture(it->second.ItemIndex + MODEL_ITEM, lpInfo->ModelNameDir);
		}
		else
		{
			gLoadData.AccessModelCustom(lpInfo->ItemIndex + MODEL_ITEM, lpInfo->ModelNameDir, lpInfo->ModelName);
			gLoadData.OpenTextureCustom(it->second.ItemIndex + MODEL_ITEM, lpInfo->ModelNameDir);
		}
		
		it->second.ItemModel = DarkSpirit++;

		if (lpInfo->FileType == 0)
		{
			gLoadData.AccessModel(it->second.ItemModel, lpInfo->ObjectNameDir, lpInfo->ObjectName);
			gLoadData.OpenTexture(it->second.ItemModel, lpInfo->ObjectNameDir);
		}
		else
		{
			gLoadData.AccessModelCustom(it->second.ItemModel, lpInfo->ObjectNameDir, lpInfo->ObjectName);
			gLoadData.OpenTextureCustom(it->second.ItemModel, lpInfo->ObjectNameDir);
		}

		gDarkSpirit.m_DarkSpiritModels[it->second.ItemModel] = it->second.ItemIndex;
	}

	for (std::map<int, RING_SKIN_MODEL>::iterator it = gRingTransform.m_RingSkinModel.begin(); it != gRingTransform.m_RingSkinModel.end(); it++) //RING TRANSFORM
	{
		auto ItemInfo = &(*it).second;

		if (ItemInfo->Type == 0)
		{
			gLoadData.AccessModel(ItemInfo->Index, ItemInfo->Dir, ItemInfo->ModelName);
			gLoadData.OpenTexture(ItemInfo->Index, ItemInfo->Dir);
		}
		else
		{
			gLoadData.AccessModelCustom(ItemInfo->Index, ItemInfo->Dir, ItemInfo->ModelName);
			gLoadData.OpenTextureCustom(ItemInfo->Index, ItemInfo->Dir);
		}
	}

	int patentemodel = gItemManager.GET_ITEM_MODEL(18, 255);

	for (std::map<int, PATENT_MODEL_INFO>::iterator it = gPatente.m_PatentModelInfo.begin(); it != gPatente.m_PatentModelInfo.end(); it++)
	{
		auto lpInfo = &(*it).second;

		if (lpInfo->Type == 0)
		{
			gLoadData.AccessModel(patentemodel, lpInfo->Dir, lpInfo->ModelName);
			gLoadData.OpenTexture(patentemodel, lpInfo->Dir);
		}
		else {
			gLoadData.AccessModelCustom(patentemodel, lpInfo->Dir, lpInfo->ModelName);
			gLoadData.OpenTextureCustom(patentemodel, lpInfo->Dir);
		}

		lpInfo->ModelIndex = patentemodel++;
	}
}
...
void OpenMonsterModel(int Type)
{
	...
	gMonsters.LoadMonsterModel(Index, "Data\\Monster\\", "Monster", Type + 1);
    ...
    else
    {
		...
		gMonsters.LoadMonsterTexture(Index, "Monster\\");
    }
...
void OpenSkills()
{
	...
	gLoadData.AccessModel(MODEL_FENRIR_THUNDER, "Data\\Effect\\", "lightning_type01");  //EFEITO DE RAIO
	gLoadData.OpenTexture(MODEL_FENRIR_THUNDER, "Effect\\");

	gLoadData.AccessModel(MODEL_FENRIR_THUNDER2, "Data\\Effect\\", "lightning_type02"); //EFEITO DE RAIO2
	gLoadData.OpenTexture(MODEL_FENRIR_THUNDER2, "Effect\\");
...
//	LoadWaveFile(SOUND_PHOENIXFIRE		,"Data\\Sound\\ePhoenixFire.wav",1,Enable3DSound);
	...
	//DRAGON
	LoadWaveFile(SOUND_DRAGON_RUN_1,			"Data\\Custom\\Sound\\pDragonWalk-01.wav", 1);
	LoadWaveFile(SOUND_DRAGON_RUN_2,			"Data\\Custom\\Sound\\pDragonWalk-02.wav", 1);
	LoadWaveFile(SOUND_DRAGON_RUN_3,			"Data\\Custom\\Sound\\pDragonWalk-03.wav", 1);
	LoadWaveFile(SOUND_DRAGON_DEATH,			"Data\\Custom\\Sound\\pDragonDeath.wav", 1);
	LoadWaveFile(SOUND_DRAGON_GROWL,			"Data\\Custom\\Sound\\pDragonGrowl.wav", 1);
	LoadWaveFile(SOUND_DRAGON_ROAR,				"Data\\Custom\\Sound\\pDragonRoar.wav", 1);
...
#else
	options[25] = static_cast<BYTE>(255);
#endif
...
void OpenLogoSceneData()
{
	...
	//::LoadBitmap("Interface\\login_box.jpg", BITMAP_LOG_IN + 7);
	::LoadBitmap("Interface\\login_box.tga", BITMAP_LOG_IN + 7);
	::LoadBitmap("Interface\\store_name.tga", BITMAP_LOG_IN + 8);
	//::LoadBitmap("Interface\\store_name.jpg", BITMAP_LOG_IN + 8);
...
{
	gCharacterList.OpenCharacterSceneData();
...
void OpenBasicData(HDC hDC)
{
	...
	LoadBitmap("Effect\\FlareSC.jpg"     ,BITMAP_FLARE_SC,  GL_LINEAR,GL_CLAMP_TO_EDGE);
	...
	LoadBitmap("Effect\\NSkillSC.jpg"     	,BITMAP_FLARE_FORCE_SC,  GL_LINEAR,GL_CLAMP_TO_EDGE);
...
//	LoadBitmap("Monster\\gigantiscorn_R.jpg", BITMAP_GIGANTIS_EFFECT, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	::LoadBitmap("Interface\\btn_medium.tga", BITMAP_BUTTON + 2);
	...
	//std::string strFileName = "Local\\" + g_strSelectedML + "\\ImgsMapName\\battlezone.tga";
	...
	LoadWaveFile(SOUND_PURCHASE			,"Data\\Sound\\item_sound_purchase.wav",1); 
	LoadWaveFile(SOUND_SELL				,"Data\\Sound\\item_sound_sell.wav",1); 
