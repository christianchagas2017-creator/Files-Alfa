...
struct COUNTER_ITEM_INFO {
	...
	int Color;
	int Bold;
	std::vector<KILL_RANGE> KillRanges;
};

struct COUNTER_ITEMS {
	COUNTER_ITEMS() {
		ItemIndex = -1;
		Slot = -1;
		Top = 0;
...
public:
	...
	void InsertCounterItem(WORD ItemIndex, BYTE Slot, WORD Type, DWORD kills, BYTE Top);
	void getCount(int x, int y, int* count, int* top);
	const char* getKillRangeName(int ItemIndex, DWORD kills);
	int getKillRangeColorType(int ItemIndex, DWORD kills);

	DWORD GetColorByType(int colorType);
