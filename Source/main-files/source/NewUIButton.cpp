...
bool SEASON3B::CNewUIButton::Render( bool RendOption )
{
	...
	if( m_Name.size() != 0 )
	{
		...
		{
			RenderText( m_Name.c_str(), x+m_iMoveTextPosX+1, y+m_iMoveTextPosY+1, m_Size.x, 0, m_hTextFont, m_NameColor, m_NameBackColor, 8 );
		}
		else
		{
			RenderText( m_Name.c_str(), x+m_iMoveTextPosX, y+m_iMoveTextPosY, m_Size.x, 0, m_hTextFont, m_NameColor, m_NameBackColor, 8 );
		}
#else // KJH_ADD_INGAMESHOP_UI_SYSTEM
