...
void SEASON3B::CNewUINPCShop::RenderFrame()
{
	...
	//RenderBitmap(IMAGE_NPCSHOP_BACK_VOID, m_Pos.x, m_Pos.y, NPCSHOP_WIDTH, NPCSHOP_HEIGHT, 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
	RenderImageScaleAuto(IMAGE_NPCSHOP_BACK_VOID, m_Pos.x, m_Pos.y, NPCSHOP_WIDTH, NPCSHOP_HEIGHT);
#endif
...
#elif CLIENT_VERSION == 1001
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 14, GlobalText[230], NPCSHOP_WIDTH, 0, RT3_SORT_CENTER);
#elif CLIENT_VERSION == 1002
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 12, GlobalText[230], NPCSHOP_WIDTH, 0, RT3_SORT_CENTER);
	unicode::t_char strText[256];
	unicode::_sprintf(strText, GlobalText[1623], m_iTaxRate);
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 25, strText, NPCSHOP_WIDTH, 0, RT3_SORT_CENTER);
#endif
...
#elif CLIENT_VERSION >= 1001
		RenderBitmap(IMAGE_NPCSHOP_REPAIR_MONEY, m_Pos.x + 10, m_Pos.y + 358, 168.f, 18, 0.002, 0.0, 0.640, 0.818956, 1, 1, 1.0);
		g_pRenderText->SetBgColor(255, 255, 255, 0);
		g_pRenderText->SetTextColor(255, 220, 150, 255);
		unicode::t_char strText[256];
		ConvertGold(AllRepairGold, strText);
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->RenderText(m_Pos.x + 25, m_Pos.y + 363, GlobalText[239]);
		g_pRenderText->SetTextColor(getGoldColor(AllRepairGold));
		g_pRenderText->RenderText(m_Pos.x + 100, m_Pos.y + 363, strText);
#endif
...
void SEASON3B::CNewUINPCShop::LoadImages()
{
	...
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_NPCSHOP_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back03.tga", IMAGE_NPCSHOP_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\btn_repair.jpg", IMAGE_NPCSHOP_BTN_REPAIR, GL_LINEAR);
	LoadBitmap("Interface\\item_money_2.jpg", IMAGE_NPCSHOP_REPAIR_MONEY, GL_LINEAR);
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_NPCSHOP_BACK_VOID, GL_LINEAR);
...
void SEASON3B::CNewUINPCShop::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_NPCSHOP_BACK_VOID);
#endif
}
...
#elif CLIENT_VERSION >= 1001
	m_BtnRepair.ChangeButtonImgState(true, IMAGE_NPCSHOP_BTN_REPAIR, false);
	m_BtnRepair.ChangeButtonInfo(m_Pos.x + 60, m_Pos.y + 390, 25, 23);
	m_BtnRepair.ChangeToolTipText(GlobalText[233], true);

	m_BtnRepairAll.ChangeButtonImgState(true, IMAGE_NPCSHOP_BTN_REPAIR, false);
	m_BtnRepairAll.ChangeButtonInfo(m_Pos.x + 100, m_Pos.y + 390, 25, 24);
	m_BtnRepairAll.ChangeToolTipText(GlobalText[237], true);
#endif
