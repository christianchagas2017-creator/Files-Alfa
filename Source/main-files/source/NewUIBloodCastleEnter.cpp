...
bool CNewUIEnterBloodCastle::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	for(int i=0 ; i<MAX_ENTER_GRADE ; i++)
	{
		...
		m_BtnEnter[i].ChangeButtonInfo( m_BtnEnterStartPos.x, m_BtnEnterStartPos.y+iVal, 120, 27);
		m_BtnEnter[i].SetButtonSize(2);
		m_BtnEnter[i].SetFont(g_hFontBold);
		m_BtnEnter[i].ChangeTextColor(RGBA(245, 232, 202, 255));
	}
...
void CNewUIEnterBloodCastle::SetPos(int x, int y)
{
	...
	SetBtnPos( m_Pos.x + (ENTERBC_BASE_WINDOW_WIDTH / 2 - 120 / 2), m_Pos.y+125 );
...
bool CNewUIEnterBloodCastle::Render()
{
	...
	// Base Window
	RenderImageScaleAuto(IMAGE_ENTERBC_BASE_WINDOW_BACK_VOID, m_Pos.x, m_Pos.y, ENTERBC_BASE_WINDOW_WIDTH, ENTERBC_BASE_WINDOW_HEIGHT);
	
	g_pRenderText->SetFont( g_hFontBold );
	g_pRenderText->SetTextColor( 0xFFFFFFFF );
	g_pRenderText->SetBgColor( 0x00000000 );
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y+15, GlobalText[846], 190, 0, RT3_SORT_CENTER);
	...
	for ( int j = 0; j < tl; ++j)
	{
		g_pRenderText->RenderText(m_EnterUITextPos.x, m_EnterUITextPos.y+j*15,txtline[j], 190, 0, RT3_SORT_CENTER);
	}
...
void CNewUIEnterBloodCastle::OpenningProcess()
{
	...
	for( int i=0 ; i<MAX_ENTER_GRADE ; i++ )
	{
		m_BtnEnter[i].ChangeTextColor(RGBA(245, 232, 202, 155));
		m_BtnEnter[i].Lock( );
	}
	...
	m_BtnEnter[m_iNumActiveBtn].ChangeTextColor(RGBA(245, 232, 202, 255));
...
void CNewUIEnterBloodCastle::LoadImages()
{
	...
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_ENTERBC_BASE_WINDOW_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back03.tga", IMAGE_ENTERBC_BASE_WINDOW_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_ENTERBC_BASE_WINDOW_BTN_EXIT, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\btn_large.jpg", IMAGE_ENTERBC_BASE_WINDOW_BTN_ENTER, GL_LINEAR);		// Enter Button
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_ENTERBC_BASE_WINDOW_BACK_VOID, GL_LINEAR);
