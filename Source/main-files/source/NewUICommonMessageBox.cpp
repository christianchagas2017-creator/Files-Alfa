...
void SEASON3B::CNewUIMessageBoxButton::Render()
{
	switch (m_BtnSize) {
	case 0:
		if (m_EventState == BUTTON_STATE_UP && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.008, 0.002, 0.83, 0.20, 1, 1, 1.0);
		}
		else if ((m_EventState == EVENT_BTN_DOWN && m_bEnable) || m_BtnForceDown) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.008, 0.429, 0.83, 0.20, 1, 1, 1.0);
		}
		else if (m_EventState == EVENT_BTN_HOVER && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.008, 0.213, 0.83, 0.20, 1, 1, 1.0);
		}
		else if (!m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.008, 0.002, 0.83, 0.20, 1, 1, 0.5);
		}
		break;
	case 1:
		if (m_EventState == BUTTON_STATE_UP && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.002, 0.95, 0.20, 1, 1, 1.0);
		}
		else if ((m_EventState == EVENT_BTN_DOWN && m_bEnable) || m_BtnForceDown) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.429, 0.95, 0.20, 1, 1, 1.0);
		}
		else if (m_EventState == EVENT_BTN_HOVER && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.213, 0.95, 0.20, 1, 1, 1.0);
		}
		else if (!m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.002, 0.95, 0.20, 1, 1, 0.5);
		}
		break;
	case 2:
		if (m_EventState == BUTTON_STATE_UP && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.002, 0.775, 0.20, 1, 1, 1.0);
		}
		else if ((m_EventState == EVENT_BTN_DOWN && m_bEnable ) || m_BtnForceDown) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.429, 0.775, 0.2, 1, 1, 1.0);
		}
		else if (m_EventState == EVENT_BTN_HOVER && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.213, 0.775, 0.2, 1, 1, 1.0);
		}
		else if (!m_bEnable) {
			RenderBitmap(m_dwTexType,  m_x, m_y,  m_width, m_height, 0.002, 0.002, 0.775, 0.2, 1, 1, 0.5);
		}
		break;
	case 3:
		if (m_EventState == BUTTON_STATE_UP && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0.002, 0.001, 0.575, 0.21, 1, 1, 1.0);
		}
		else if ((m_EventState == EVENT_BTN_DOWN && m_bEnable) || m_BtnForceDown) {
			RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0.002, 0.419, 0.575, 0.21, 1, 1, 1.0);
		}
		else if (m_EventState == EVENT_BTN_HOVER && !m_BtnForceDown && m_bEnable) {
			RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0.002, 0.21, 0.575, 0.21, 1, 1, 1.0);
		}
		else if (!m_bEnable) {
			RenderBitmap(m_dwTexType, m_x, m_y, m_width, m_height, 0.002, 0.001, 0.575, 0.21, 1, 1, 0.3);
		}
		break;
	}
	...
	if (m_strText.size() > 0)
	{
		...
		if (m_bEnable == false)
		{
			g_pRenderText->SetTextColor(245, 232, 202, 200);
		}
		else
		{
...
//BUFF ELF ATTACK
bool SEASON3B::CBuffElfAttackCancelMsgBoxLayout::SetLayout()
{
	extern int g_iCancelSkillTarget;
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
	...
	unicode::t_char strText[MAX_GLOBAL_TEXT_STRING];
	unicode::_sprintf(strText, "%s %s", SkillAttribute[AT_SKILL_ATTACK].Name, GlobalText[2046]);
	g_iCancelSkillTarget = AT_SKILL_ATTACK;

	pMsgBox->AddMsg(strText, RGBA(255, 255, 0, 255), MSGBOX_FONT_BOLD);
	pMsgBox->AddCallbackFunc(CBuffElfAttackCancelMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CBuffElfAttackCancelMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
	return true;
}

CALLBACK_RESULT SEASON3B::CBuffElfAttackCancelMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	SendRequestCancelMagic(g_iCancelSkillTarget, HeroKey);
	g_iCancelSkillTarget = 0;
...
CALLBACK_RESULT SEASON3B::CBuffElfAttackCancelMsgBoxLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	g_iCancelSkillTarget = 0;
...
//BUFF ELF DEFENSE
bool SEASON3B::CBuffElfDefenseCancelMsgBoxLayout::SetLayout()
{
	extern int g_iCancelSkillTarget;
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
	...
	unicode::t_char strText[MAX_GLOBAL_TEXT_STRING];
	unicode::_sprintf(strText, "%s %s", SkillAttribute[AT_SKILL_DEFENSE].Name, GlobalText[2046]);
	g_iCancelSkillTarget = AT_SKILL_DEFENSE;

	pMsgBox->AddMsg(strText, RGBA(255, 255, 0, 255), MSGBOX_FONT_BOLD);
	pMsgBox->AddCallbackFunc(CBuffElfDefenseCancelMsgBoxLayout::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CBuffElfDefenseCancelMsgBoxLayout::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);
	return true;
}

CALLBACK_RESULT SEASON3B::CBuffElfDefenseCancelMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	SendRequestCancelMagic(g_iCancelSkillTarget, HeroKey);
	g_iCancelSkillTarget = 0;
...
CALLBACK_RESULT SEASON3B::CBuffElfDefenseCancelMsgBoxLayout::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	g_iCancelSkillTarget = 0;
...
//BUFF SM SOUL BARRIER / MANA SHIELD
bool SEASON3B::CBuffSMCancelMsgBoxLayOut::SetLayout()
{
	extern int g_iCancelSkillTarget;
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
	...
	unicode::t_char strText[MAX_GLOBAL_TEXT_STRING];
	unicode::_sprintf(strText, "%s %s", SkillAttribute[AT_SKILL_WIZARDDEFENSE].Name, GlobalText[2046]);
	g_iCancelSkillTarget = AT_SKILL_WIZARDDEFENSE;

	pMsgBox->AddMsg(strText, RGBA(255, 255, 0, 255), MSGBOX_FONT_BOLD);

	pMsgBox->AddCallbackFunc(CBuffSMCancelMsgBoxLayOut::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CBuffSMCancelMsgBoxLayOut::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT SEASON3B::CBuffSMCancelMsgBoxLayOut::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	SendRequestCancelMagic(g_iCancelSkillTarget, HeroKey);
	g_iCancelSkillTarget = 0;
...
CALLBACK_RESULT SEASON3B::CBuffSMCancelMsgBoxLayOut::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	g_iCancelSkillTarget = 0;
...
//BUFF BK VITALITY
bool SEASON3B::CBuffBKCancelMsgBoxLayOut::SetLayout()
{
	extern int g_iCancelSkillTarget;
	CNewUICommonMessageBox* pMsgBox = GetMsgBox();
	if (0 == pMsgBox)
	...
	unicode::t_char strText[MAX_GLOBAL_TEXT_STRING];
	unicode::_sprintf(strText, "%s %s", SkillAttribute[AT_SKILL_VITALITY].Name, GlobalText[2046]);
	g_iCancelSkillTarget = AT_SKILL_VITALITY;

	pMsgBox->AddMsg(strText, RGBA(255, 255, 0, 255), MSGBOX_FONT_BOLD);

	pMsgBox->AddCallbackFunc(CBuffBKCancelMsgBoxLayOut::OkBtnDown, MSGBOX_EVENT_USER_COMMON_OK);
	pMsgBox->AddCallbackFunc(CBuffBKCancelMsgBoxLayOut::CancelBtnDown, MSGBOX_EVENT_USER_COMMON_CANCEL);

	return true;
}

CALLBACK_RESULT SEASON3B::CBuffBKCancelMsgBoxLayOut::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	SendRequestCancelMagic(g_iCancelSkillTarget, HeroKey);
	g_iCancelSkillTarget = 0;
...
CALLBACK_RESULT SEASON3B::CBuffBKCancelMsgBoxLayOut::CancelBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	extern int g_iCancelSkillTarget;
	g_iCancelSkillTarget = 0;
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemValueCheckMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pPickedItem)
	{
		...
		if (pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl())
		{
			...
			SendRequestSetSalePrice(byIndex, iItemPrice, iItemPrice, iItemPrice, iItemPrice);
		...
		AddPersonalItemPrice(MAX_MY_INVENTORY_INDEX + iTargetIndex, pMsgBox->GetItemValue(), pMsgBox->GetItemValue(), pMsgBox->GetItemValue(), pMsgBox->GetItemValue(), g_IsPurchaseShop);
	}
	else
	{
		...
		BYTE byIndex = iSourceIndex;
		SendRequestSetSalePrice(byIndex, iItemPrice, iItemPrice, iItemPrice, iItemPrice);
		AddPersonalItemPrice(byIndex, iItemPrice, iItemPrice, iItemPrice, iItemPrice, g_IsPurchaseShop);
	}
