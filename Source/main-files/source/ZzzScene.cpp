...
bool CheckAbuseFilter(char* Text, bool bCheckSlash)
{
	...
	for (int i = 0; i < AbuseFilterNumber; i++)
	{
		...
		{
			return false; //CHAT FILTER = TRUE ATIVA FILTRO
		}
	}
	return false;
}
...
void WebzenScene(HDC hDC)
{
	...
	g_pNewUISystem->m_Lua.Generic_Call("FinalBoot", ">");
}
...
#ifdef SELECT_CHARACTER_S3
	BeginBitmap();
	Width = gWideScreen.g_WideWindowWidth / 2;
	Height = 320 + gWideScreen.g_WideWindowHeightAdd;
	RenderBitmap(BITMAP_LOG_IN + 9, (float)0, (float)25, (float)Width, (float)Height, 0.f, 0.f);
	RenderBitmap(BITMAP_LOG_IN + 10, (float)gWideScreen.g_WideWindowWidth / 2, (float)25, (float)Width, (float)Height, 0.f, 0.f);
	EndBitmap();
#endif
#endif //PJH_NEW_SERVER_SELECT_MAP
	Height = 480;
	Width = 640;// GetScreenWidth();

	glClearColor(0.f, 0.f, 0.f, 1.f);
	if (m_Resolution > 3) {
		BeginOpengl(0, 25, gWideScreen.g_WideWindowWidth, gWideScreen.g_WideWindowHeight - 50);
	}
	else
	{
...
bool NewRenderLogInScene(HDC hDC)
{
	...
	Height = 320 + gWideScreen.g_WideWindowHeightAdd;
	RenderBitmap(BITMAP_LOG_IN + 9, (float)0, (float)25, (float)Width, (float)Height, 0.f, 0.f);
	RenderBitmap(BITMAP_LOG_IN + 10, ((float)gWideScreen.g_WideWindowWidth / 2) - 1, (float)25, (float)Width + 1, (float)Height, 0.f, 0.f);
	EndBitmap();
#endif
#endif //PJH_NEW_SERVER_SELECT_MAP
	...
	if (m_Resolution > 3) {
		BeginOpengl(0, 0, gWideScreen.g_WideWindowWidth, gWideScreen.g_WideWindowHeight);
	}
	else
	{
		BeginOpengl(0, 25, 640, 430);
...
#ifdef SELECT_SERVER_S3
	float poss[3] = { 11679.0f, 14191.0f, 333.0f };
	CreateFrustrum((float)Width / (float)640, poss);
#else
	CreateFrustrum((float)Width / (float)640, pos);
#endif
	...
	if (CCameraMove::GetInstancePtr()->IsTourMode())
	{
		...
		RenderBitmap(BITMAP_LOG_IN + 17, gWideScreen.GetCenterPosition(320) - 432 * 0.4f * 0.5f, 25.0f, 432 * 0.4f, 384 * 0.4f, 0, 0, (432 - 0.5f) / pImage->Width, (384 - 0.5f) / pImage->Height);
#else //PBG_ADD_MUBLUE_LOGO
		RenderBitmap(BITMAP_LOG_IN + 17, gWideScreen.GetCenterPosition(320) - 128.0f * 0.8f, 25.0f, 256.0f * 0.8f, 128.0f * 0.8f);
		...
		RenderBitmap(BITMAP_LOG_IN + 16, gWideScreen.GetCenterPosition(320) - 432 * 0.4f * 0.5f, 25.0f, 432 * 0.4f, 384 * 0.4f, 0, 0, 432 / pImage->Width, 384 / pImage->Height);
#else //PBG_ADD_MUBLUE_LOGO
		RenderBitmap(BITMAP_LOG_IN + 16, gWideScreen.GetCenterPosition(320) - 128.0f * 0.8f, 25.0f, 256.0f * 0.8f, 128.0f * 0.8f);
#endif //PBG_ADD_MUBLUE_LOGO
	}
	...
	g_pRenderText->RenderText(0, gWideScreen.g_WideWindowHeight - 10, Text, gWideScreen.g_WideWindowWidth, 0, 3);

	/*
	strcpy(Text,GlobalText[455]);

	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text,lstrlen(Text),&Size);
	g_pRenderText->RenderText(gWideScreen.GetCenterPosition(335), gWideScreen.g_WideWindowHeight -Size.cy*640/WindowWidth-1,Text);

	sprintf(Text,GlobalText[456],m_ExeVersion);

	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), Text,lstrlen(Text),&Size);
	g_pRenderText->RenderText(0, gWideScreen.g_WideWindowHeight -Size.cy*640/WindowWidth-1,Text);
	*/
...
bool MoveMainCamera()
{
	...
	{
		if (gMapManager.WorldActive == WD_74NEW_CHARACTER_SCENE || gMapManager.WorldActive == WD_73NEW_LOGIN_SCENE || gMapManager.WorldActive == 94) // Fix for opening scene and character selection
		{
			CameraZoom = 0;
			AngleY3D = 0;
		}
	...
	else
	{
		...
		else
		{
			...
			case 0:
				...
				{
					CameraViewFar = 2000.f; // talvez 102000 3d camera
				}
				break;
...
void MoveMainScene()
{
	if (!InitMainScene)
	{
		...
		SendRequestJoinMapServer(CharactersClient[SelectedHero].ID, g_pMultiLanguage->GetLanguage());
	...
	//if((!CameraTopViewEnable || m_CameraOnOff == 1)  && LoadingWorld < 30 ) //3d camera
	if (LoadingWorld < 30) //3d camera
	{
		if (MouseY >= (int)(gWideScreen.g_WideWindowHeight - 50))
			MouseOnWindow = true;
...
bool RenderMainScene()
{
	...
	FogEnable = true; //3d camera, original = false
	...
	Height = gWideScreen.g_WideWindowHeight;

	if (m_Resolution > 3) {
		Width = gWideScreen.g_WideWindowWidth;
	}
	else
	{
		Width = 640;// GetScreenWidth();
	...
	if (!CameraTopViewEnable)// || m_CameraOnOff == 1) //3d camera
		RenderItems();
...
void SetTargetFps(float targetFps)
{
	...
	nextTime.QuadPart += freq.QuadPart / 62;
...
void MainScene(HDC hDC)
{
	...
	/*
		...
		g_pRenderText->RenderText(10, gWideScreen.GetBottomPosition(440), szDebugText);
		g_pRenderText->RenderText(10, gWideScreen.GetBottomPosition(450), szMousePos);
		g_pRenderText->SetFont(g_hFont);
		EndBitmap();
	#endif
	*/
