...
bool Monsters::CheckCustomMonster(int MonsterID)
{
	for (std::map<int, CUSTOM_MONSTER_INFO>::iterator it = this->m_CustomMonsterInfo.begin(); it != this->m_CustomMonsterInfo.end(); ++it)
	{
		...
		{
			if (MonsterID = (it->second.MonsterID))
			{
				return true;
			}
		}
...
{
	gMonsters.LoadMonsterModel(MonsterID + MODEL_MONSTER01, (char*)Dir.c_str(), (char*)ModelName.c_str(), -1);
	
	gMonsters.LoadMonsterTexture(MonsterID + MODEL_MONSTER01, (char*)Dir.c_str());
	
	int Index = MODEL_MONSTER01 + MonsterID;
...
void Monsters::LoadMonsterModel(int ModelID, char* pathName, char* FileName, int Class)
{
	int returnValue = 0;

	gMonsters.m_Lua.Generic_Call("LoadModelMonster", "i>i", ModelID, &returnValue);
	...
	gLoadData.AccessModel(ModelID, pathName, FileName, Class);
}
...
void Monsters::LoadMonsterTexture(int ModelID, char* a2)
{
	...
	gMonsters.m_Lua.Generic_Call("LoadTextureMonster", "i>i", ModelID, &returnValue);
...
bool Monsters::MonsterType(int MonsterIndex, int Type)
{
	for (std::map<int, CUSTOM_MONSTER_INFO>::iterator it = this->m_CustomMonsterInfo.begin(); it != this->m_CustomMonsterInfo.end(); ++it)
	{
		...
		if (MonsterIndex == (it->second.MonsterID) && Type == it->second.Type)
		{
			return true;
		}
	}
...
void Monsters::Init()
{
	...
	sprintf_s(name, "%s\\Monster\\CustomMonster.lua", LUA_LOAD_PATH);
...
void Monsters::RestartLua()
{
	gMonsters.m_CustomMonsterInfo.clear();
