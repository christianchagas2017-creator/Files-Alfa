...
{
	m_BtnExit.ChangeButtonInfo(gWideScreen.g_WideWindowWidth - 27, 3, 30, 25 );
}
...
bool SEASON3B::CNewUIMiniMap::Render()
{
	...
	float Rot = 45.f;
	EnableAlphaTest();
	RenderColor(0, 0, gWideScreen.g_WideWindowWidth, gWideScreen.g_WideWindowHeight, 0.90f, 1);
	DisableAlphaBlend();
	EnableAlphaTest();
	...
	float Ch_wid = 12;
	RenderImage(IMAGE_MINIMAP_INTERFACE+3, gWideScreen.GetCenterPosition(325), gWideScreen.GetBottomPosition(230), Ch_wid,Ch_wid ,0.f,0.f,17.5f/32.f,17.5f/32.f);


	for(i = 0; i < 25; i++)
	{
		RenderImage(IMAGE_MINIMAP_INTERFACE+2, gWideScreen.GetCenterPosition(i) * Ui_wid, 0, Ui_wid,Ui_Hig ,0.f,1.f,uvxy,-uvxy_Line);
		RenderImage(IMAGE_MINIMAP_INTERFACE+2, gWideScreen.GetCenterPosition(i) * Ui_wid, gWideScreen.GetBottomPosition(430) - Ui_Hig, Ui_wid,Ui_Hig ,0.f,0.f,uvxy,uvxy_Line);
	}
	for(i = 0; i < 20; i++)
	{
		RenderBitmapRotate(IMAGE_MINIMAP_INTERFACE+2, (Ui_Hig/2.f), i * (Ui_wid - 3.f),Ui_wid,Ui_Hig,-90.f,0.f,0.f,uvxy,uvxy_Line);
		RenderBitmapRotate(IMAGE_MINIMAP_INTERFACE+2, gWideScreen.GetAddPosition(640) - (Ui_Hig/2.f), i * (Ui_wid - 3.f),Ui_wid,Ui_Hig,90.f,0.f,0.f,uvxy,uvxy_Line);
	}

	RenderImage(IMAGE_MINIMAP_INTERFACE+1, 0, 0, Ui_wid,Ui_wid ,0.f,0.f,uvxy,uvxy);
	RenderImage(IMAGE_MINIMAP_INTERFACE+1, gWideScreen.g_WideWindowWidth - Ui_wid, 0, Ui_wid,Ui_wid ,uvxy,0.f,-uvxy,uvxy);
	RenderImage(IMAGE_MINIMAP_INTERFACE+1, 0, 480 - Ui_wid, Ui_wid,Ui_wid ,0.f,uvxy,uvxy,-uvxy);
	RenderImage(IMAGE_MINIMAP_INTERFACE+1, gWideScreen.g_WideWindowWidth - Ui_wid, gWideScreen.g_WideWindowHeight - Ui_wid, Ui_wid,Ui_wid ,uvxy,uvxy,-uvxy,-uvxy);
...
void SEASON3B::CNewUIMiniMap::LoadImages(const char* Filename)
{
	...
	sprintf(Fname,"Data\\Local\\%s\\Minimap\\Minimap_%s.bmd", g_strSelectedML.c_str(), Filename);
...
bool SEASON3B::CNewUIMiniMap::UpdateMouseEvent()
{
	...
	if(CheckMouseIn(0, 0, gWideScreen.GetCenterPosition(640), 430))
	{
		return false;
	}
