...
void CMapManager::Load() // OK
{
	...
	case WD_55LOGINSCENE:
		...
		gLoadData.AccessModel(162, "Data\\Logo\\", "Logo", 3);

		gLoadData.OpenTexture(162, "Logo\\", GL_REPEAT, GL_LINEAR, true);
...
void CMapManager::LoadWorld(int Map)
{
    ...
    if ( CreateWaterTerrain( this->WorldActive )==false )
    {
		...
		sprintf(FileName, (Map == 0 || Map == 3 || Map == 63) ? "%s\\leaf01.tga" : "%s\\leaf01.jpg", WorldName);
	    LoadBitmap(FileName,BITMAP_LEAF1          ,GL_NEAREST,GL_CLAMP_TO_EDGE,false);
		sprintf(FileName, (Map == 0 || Map == 3 || Map == 63) ? "%s\\leaf01.tga" : "%s\\leaf01.jpg", WorldName);
		LoadBitmap(FileName, BITMAP_LEAF3, GL_NEAREST, GL_CLAMP_TO_EDGE, false);
