...
namespace SEASON3B
{
	...
	public:
		...
		bool Create(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, DWORD MsgColor)
		{
		...
		int GetColor() const { return m_MsgColor; }
	};
	...
	public:
		enum IMAGE_LIST
		{
			...
			IMAGE_NEW_SCROLL_BACK = BITMAP_NEW_SCROLL_BACK,
			IMAGE_NEW_SCROLL_MIDDLE,
			IMAGE_NEW_DRAG_BAR,
#endif
		};
	...
	private:
		...
		type_vector_msgs	m_VecChatMsgsCustom;
		...
		SIZE	m_WndSize;
		int		m_nShowingSystemLines;
		int		m_nShowingLines;
		int		m_nShowingLinesBackup;

		MESSAGE_TYPE		m_CurrentRenderMsgType;
		bool				m_bShowChatLog;
		int		m_iCurrentRenderEndSystemLine;
	...
	public:
		...
		void AddText(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, DWORD MsgColor = 0, MESSAGE_TYPE ErrMsgType = TYPE_ALL_MESSAGE);
		...
		int GetCurrentRenderEndSystemLine() const;
		...
		size_t GetNumberOfShowingLinesSystem() const;
	...
	protected:
		...
		type_vector_msgs* GetMsgsSystem(MESSAGE_TYPE MsgType);
		void ProcessAddText(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, DWORD MsgColor = 0, MESSAGE_TYPE ErrMsgType = TYPE_ALL_MESSAGE);
