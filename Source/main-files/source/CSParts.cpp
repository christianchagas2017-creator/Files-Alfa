...
{
    if (FindText( c->ID, "webzen" ) || g_isCharacterBuff( (&c->Object), eBuff_GMEffect ) || ((c->CtlCode == CTLCODE_20OPERATOR) || (c->CtlCode == CTLCODE_08OPERATOR))) //SIGN GM
    {
    ...
    //    {
    //        //c->m_pParts = (CSIPartsMDL*)new CSParts(10767, 20, true, 70.f, -5.f, 0.f, 0.f, 0.f, 45.f);
    //        c->m_pParts = (CSIPartsMDL*)new CSParts2D(70025, 2, 20, 70.f, 0.f, 0.f);
    //    }
    //    else if (c->MonsterIndex == 56)
    //    {
    //        c->m_pParts = (CSIPartsMDL*)new CSParts2D(70026, 2, 20, 120.f, 0.f, 0.f);
    //    }
    //    else if (c->MonsterIndex == 14 || c->MonsterIndex == 20215)
    //    {
    //        //c->m_pParts = (CSIPartsMDL*)new CSParts(MODEL_SWORD + 20, 20, true, 70.f, -5.f, 0.f, 0.f, 0.f, 45.f);
    //    }
    //}

    if (c->m_pParts == NULL)
    {
        auto lpInfo = gPatente.GetPlayer(c->Key);

        if (lpInfo != nullptr && lpInfo->Patente > 0)
        {
            c->EtcPart = lpInfo->Patente + 19;

            if (lpInfo->Type == 0) {
                auto patente = gPatente.GetPatentModelInfo(lpInfo->Patente);
                if (patente != NULL)
                {
                    c->m_pParts = (CSIPartsMDL*)new CSParts(patente->ModelIndex, patente->Bone, patente->BillBoard, patente->Position[0], patente->Position[1], patente->Position[2], patente->Angle[0], patente->Angle[1], patente->Angle[2]);
                }
            }
            else if (lpInfo->Type == 1) {
                auto patente = gPatente.GetPatentImageInfo(lpInfo->Patente);
                if (patente != NULL)
                {
                    c->m_pParts = (CSIPartsMDL*)new CSParts2D(patente->ImageIndex, patente->Transparent, patente->Bone, patente->Position[0], patente->Position[1], patente->Position[2]);
                }
            }
            else {
                auto patente = gPatente.GetPatentImageAnimationInfo(lpInfo->Patente);
                if (patente != NULL)
                {
                    c->m_pParts = (CSIPartsMDL*)new CSParts2D(patente->ImageIndex, patente->Transparent, patente->Bone, patente->Position[0], patente->Position[1], patente->Position[2]);
                }
...
void CSParts2D::IRender ( CHARACTER* c )
{
    ...
    CreateSprite ( m_pObj.Type, m_pObj.Position, 1.f, m_pObj.Light, NULL, 0, bSubType, true);
}
