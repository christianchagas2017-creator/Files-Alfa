...
void MovePosition(vec3_t Position, vec3_t Angle, vec3_t Speed)
{
	...
	VectorScale(Velocity, FPS_ANIMATION_FACTOR * 0.8f, Velocity);
...
void SetAction(OBJECT* o, int Action, bool bBlending)
{
	...
	if (o->Type == MODEL_PLAYER && Action == PLAYER_STOP_MALE) {
		Action = PLAYER_STOP_MALE_NEW;
	}
...
void MoveHead(CHARACTER* c)
{
	...
	if (o->CurrentAction != MONSTER01_DIE)
	{
		if (o->CurrentAction == MONSTER01_STOP1)
		{
			...
			{
				o->HeadTargetAngle[0] = (float)(rand_fps_check(128) * 128 - 64);
				o->HeadTargetAngle[1] = (float)(rand_fps_check(48) * 48 - 16);
				for (int i = 0; i < 2; i++)
					if (o->HeadTargetAngle[i] < 0) o->HeadTargetAngle[i] += 360.f;
			}
		}
...
void Damage(vec3_t soPosition, CHARACTER* tc, float AttackRange, int AttackPoint, bool Hit)
{
	...
	if (AttackPoint > 0)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			if (rand_fps_check(2))  // Reduz a quantidade de partículas em FPS baixos
			{
			...
			vec3_t Angle;
			Vector(
				-(rand_fps_check(2) ? 60.0f : 30.0f),  // 50% de chance para 30 ou 60
				0.f,
				rand_fps_check(4) ? 90.0f : 270.0f,    // 25% de chance para 90, senão 270
				Angle
			);
...
bool MovePath(CHARACTER* c, bool Turn)
{
	...
	if (p->CurrentPath < p->PathNum)
	{
		...
		if (!Success && Turn)
		{
			...
			{
				o->Angle[2] = TurnAngle2(o->Angle[2], Angle, TargetAngle * 0.3f); // Reduz o fator de virada para 0.3f
			}
		}
	}
...
bool PathFinding2(int sx, int sy, int tx, int ty, PATH_t* a, float fDistance, int iDefaultWall) //MANEIRA EM QUE OS MONSTROS ACHAM CAMINHOS PARA ANDAR

{
	...
	bool Success = path->FindPath(sx, sy, tx, ty, true, Wall, Value, fDistance * 1.5f); // Aumenta a distância de parada
	if (!Success)
	{
...
bool rand_fps_check(int reference_frames) {
	const float chance = (1.0f / reference_frames) * FPS_ANIMATION_FACTOR * 0.7f; // Reduz a frequência
	return (distrib(gen) <= chance);
}

void CalcFPS()
{

	FPS = 60.0;  // Força 60 FPS
	FPS_ANIMATION_FACTOR = 1.0f;  // Remove a variação temporal
	FPS_AVG = 60.0;
	...
	FPS_ANIMATION_FACTOR = minf(static_cast<float>(REFERENCE_FPS / FPS), 2.5f);
