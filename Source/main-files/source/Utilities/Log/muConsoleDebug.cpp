...
bool CmuConsoleDebug::CheckCommand(const std::string& strCommand)
{
	...
	if (strCommand.compare(0, 4, "$fps") == 0) // começa com "$fps"
	{
	...
	if (strCommand == "$vsync off")
	{
		DisableVSync();
		return true;
	}

	if (strCommand.compare(0, 7, "$winmsg") == 0) // começa com "$winmsg"
	{
	...
	else if (strCommand.compare("$editobject off") == NULL)
	{
		EditFlag = EDIT_NONE;
		return true;
	}
	else if (strCommand.compare("$editobject on") == NULL)
	{
		EditFlag = EDIT_OBJECT;
		return true;
	}
	else if (strCommand.compare("$editmonster off") == NULL)
	{
		EditFlag = EDIT_NONE;
		return true;
	}
	else if (strCommand.compare("$editmonster on") == NULL)
	{
		EditFlag = EDIT_MONSTER;
