...
WORD SkillIndex,WORD SkillSerialNum,int iChaIndex, const float* vPriorColor, short int sTargetindex)
{
	for(int i=0;i<MAX_JOINTS;i++)
	{
		...
		if(!o->Live )
		{
			...
			else if ( MODEL_SPEARSKILL == Type && (o->SubType == 2 || o->SubType >= 10000)) //DEATH STAB
			{
				VectorCopy(TargetPosition,o->TargetPosition);
				o->Target = Target;
			}
			...
			case BITMAP_JOINT_SPIRIT2:
                ...
                switch( o->SubType )
                {
					case 0: // EVIL SPIRIT
						...
						o->Velocity = 160 * FPS_ANIMATION_FACTOR;
						o->LifeTime = 60;
						o->Scale = Scale;
						o->MaxTails = 3 / FPS_ANIMATION_FACTOR;
						break;
					case 1:
						o->Velocity = 70.f;
						o->LifeTime = 49;
					...
					case 25:
						...
						break;
					default:
						if (o->SubType >= 100 ) //BUFF BK COLOR EXPLOS홒
						{ 
							int Black = gBuffColorBK.GetBuffBlack2(o->SubType);
							if (Black == 1)
							{
								vec3_t vLight;
								o->RenderType = RENDER_TYPE_ALPHA_BLEND_MINUS;
							...
							else
							{
								o->RenderType = RENDER_TYPE_ALPHA_BLEND;
								o->RenderFace = RENDER_FACE_TWO;
								gBuffColorBK.GetBuffColorBK2(o->SubType, o->Light);
							}
			...
			case BITMAP_JOINT_SPARK:
				...
				case BITMAP_SMOKE:
					...
					break;
				case MODEL_FENRIR_SKILL_THUNDER: //FENRIR PLASMA STORM
					o->Scale    = Scale;
					...
					if(o->SubType == 0) //FENRIR BLACK
					{
						o->TexType = BITMAP_JOINT_THUNDER;
						Vector ( 0.7f, 1.0f, 0.7f, o->Light );
					}
					else if(o->SubType == 1) //FENRIR RED
					{
						o->TexType = BITMAP_JOINT_THUNDER;
						Vector ( 1.0f, 0.6f, 0.6f, o->Light );
					}
					else if(o->SubType == 2) //FENRIR BLUE
					{
						o->TexType = BITMAP_JOINT_THUNDER;
						Vector ( 0.7f, 0.7f, 1.0f, o->Light );
					}
					else if(o->SubType == 3) //FENRIR GOLD
					{
						o->TexType = BITMAP_JOINT_THUNDER;
						Vector ( 0.9f, 0.9f, 0.3f, o->Light );
					}
					else if(o->SubType == 4) //FENRIR BLACK
					{
						o->TexType = BITMAP_FLASH;
						Vector ( 0.1f, 0.8f, 0.1f, o->Light );
					}
					else if(o->SubType == 5) //FENRIR RED
					{
						o->TexType = BITMAP_FLASH;
						Vector ( 1.0f, 0.3f, 0.2f, o->Light );
					}
					else if(o->SubType == 6) //FENRIR BLUE
					{
						o->TexType = BITMAP_FLASH;
						Vector ( 0.2f, 0.3f, 1.0f, o->Light );
					}
					else if(o->SubType == 7) //FENRIR GOLD
					{
						o->TexType = BITMAP_FLASH;
						Vector ( 0.8f, 0.8f, 0.1f, o->Light );
					}
					else if (o->SubType >= 100) //FENRIR PLASMA STORM COLOR 1
					{
						//1 PLASMA
						o->TexType = BITMAP_JOINT_THUNDER;
						int Black = gPlasmaStormColor.GetPlasmaStormBlack(o->SubType);
						...
						{
							gPlasmaStormColor.GetPlasmaStormColor(o->SubType, o->Light);
						}
						...
						if (o->SubType >= 10100)
						{
							...
							int Black = gPlasmaStormColor.GetPlasmaStormBlack2(o->SubType - 10000);
							...
							{
								gPlasmaStormColor.GetPlasmaStormColor2(o->SubType - 10000, o->Light);
							}
						}
					}
					break;
			...
			case MODEL_SPEARSKILL:
				...
				switch ( o->SubType)
				{
					case 0: //BUFF SM COLOR
						//Vector(1.f,1.f,1.f,o->Light);
						//o->TexType  = BITMAP_FLARE_BLUE;

						Vector(0.7f,1.f,1.f,o->Light);
						o->TexType  = BITMAP_FLARE_SC;
					...
					case 17:
						...
						VectorCopy(o->Target->Owner->Position, o->StartPosition);
						break;
					default: //BUFF SM COLOR
						if (o->SubType >= 100 && o->SubType < 10000)
						{
							...
							int Black = gBuffColorSM.GetBuffBlack(o->SubType);
							...
							if (Black > -1)
							{
								...
								gBuffColorSM.GetBuffColorSM(o->SubType, vLight);
						
							int customImage = gBuffColorSM.GetBuffImage(o->SubType);
						
							if (customImage > -1)
								o->TexType = customImage;
						}
						else if (o->SubType >= 10000) //DEATH STAB COLOR
						{
							gDeathStabColor.GetDeathStabColor(o->SubType, o->Light);
							o->TexType = BITMAP_FLARE_FORCE;
							VectorCopy(TargetPosition, o->TargetPosition);
							o->LifeTime = 20;
							o->MaxTails = 5;
							...
							{
								Vector(1.f, 1.f, 1.f, o->Light);
								o->TexType = BITMAP_FLARE_FORCE_SC;
								o->RenderType = RENDER_TYPE_ALPHA_BLEND_MINUS;
							}

							int customImage = gDeathStabColor.GetDeathStabImage(o->SubType);
            ...
            case BITMAP_FLARE_BLUE:
                ...
                else if(o->SubType==2 || o->SubType==24 || o->SubType == 50 || o->SubType == 51 || o->SubType >= 100) //BUFF BK
                {
					if(o->SubType == 24)
						...
						if (o->SubType >= 100) //BUFF BK COLOR AURA
						{
							o->TexType = BITMAP_FLARE_SC;

							int customImage = gBuffColorBK.GetBuffImage5(o->SubType);

							if (customImage > -1)
								o->TexType = customImage;

							int Black = gBuffColorBK.GetBuffBlack5(o->SubType);
							if (Black == 1)
							{
								o->RenderType = RENDER_TYPE_ALPHA_BLEND_MINUS;
								Vector(1.f,1.f,1.f,o->Light);
								...
								gBuffColorBK.GetBuffColorBK5(o->SubType, o->Light);
						}
						else
						{
							Vector(1.f,1.f,1.f,o->Light);
...
//					Vector(1.f,1.f,1.f,o->Light);
            ...
            case BITMAP_FLARE_FORCE:
                ...
                if ( o->SubType>=1 && o->SubType<=4 || (o->SubType >= 11 && o->SubType <= 13) || o->SubType >= 100)
                {
                    o->TexType = BITMAP_JOINT_THUNDER;
                    o->Velocity = -3.f;
                    ...
                    o->Weapon = rand()%3+2;
                    if ( o->SubType==2 || o->SubType==4 || (o->SubType >= 11 && o->SubType <= 13) || o->SubType >= 100)
                    {
						switch(o->SubType) 
						{
						case 11:
						...
						case 13:
							Vector ( 0.7f, 0.7f, 1.0f, o->Light );
							break;
						default: //FENRIR PLASMA STORM COLOR
							if (o->SubType >= 100)
							{
								gPlasmaStormColor.GetPlasmaStormColor(o->SubType, o->Light);
								...
								if (o->SubType >= 10100)
								{
									gPlasmaStormColor.GetPlasmaStormColor(o->SubType - 10000, o->Light);
								}
...
void MoveJoint( JOINT *o, int iIndex)
{
	...
	case BITMAP_JOINT_SPIRIT2:
        ...
        else if ( o->SubType==2 || o->SubType==6 || o->SubType==7 || o->SubType==21 || o->SubType == 22 || o->SubType == 23 || o->SubType >= 100) //BUFF BK
        {
            vec3_t Angle;
			...
			switch ( o->SubType )
			{
				...
				default:
					if (o->SubType >= 100) //BUFF BK COLOR EXPLOS홒
					{
						if (gBuffColorBK.GetBuffBlack(o->SubType) == 1)
						{
							vec3_t vLight;
							Vector(1.f, 1.f, 1.f, o->Light);
							...
							if (gBuffColorBK.GetBuffSize(o->SubType) > -1)
								Size = gBuffColorBK.GetBuffSize(o->SubType);

							CreateSprite(BITMAP_LIGHT, o->Position, Size, o->Light, o->Target, (float)(rand() % 360), 1);
						}
						else
						{
							gBuffColorBK.GetBuffColorBK(o->SubType, Light);
						}
					}
					break;
			}
            ...
            else if ( o->LifeTime>18 && o->LifeTime<20 && ( o->SubType==2 || o->SubType == 21 || o->SubType >= 100) ) //BUFF BK
            {
				...
				if (o->SubType == 2 || o->SubType >= 100)
				{
					int Color = o->SubType;
					float size = 40.f;
					float customSize = gBuffColorBK.GetBuffSize5(Color);

					if (Color >= 100)
					{
						if (customSize > -1)
							size = gBuffColorBK.GetBuffSize5(Color);
					}

					CreateJoint(BITMAP_FLARE, Position, Position, Angle, Color, NULL, size);
				}
            }
	...
	case BITMAP_JOINT_FIRE:
		...
		break;
	case MODEL_SPEARSKILL:	//BUFF SM COLOR
		CHARACTER* c;
		if(o->m_iChaIndex != -1)
		{
		...
		if (o->SubType == 2 || o->SubType >= 10000) //DEATH STAB
		{
        ...
        else
		{
			...
			if (o->SubType == 0 || o->SubType==4 || o->SubType == 9|| o->SubType == 10 || o->SubType == 11|| o->SubType == 14|| o->SubType == 16 || o->SubType >= 100 ) //BUFF SM COLOR
			{
				for(int j=o->NumTails-1;j>=0;j--)
				{
					for(int k=0;k<4;k++)
			...
			if (o->SubType == 14) //SUMMON EFFECT WRIST
			{
				int ItemIndex = gSummonEffect.IsSummonEffect_Wrist((DWORD)c);
				int Bone = gSummonEffect.SummonEffectBone_Wrist(ItemIndex);
				...
				//if (Bone > -1)
				//	pModel->TransformPosition(o->Target->BoneTransform[Bone], vRelative, o->TargetPosition, false);
				//else
	...
	case BITMAP_JOINT_THUNDER+1:
		...
		else if ( o->SubType == 0 || o->SubType == 2 || o->SubType == 3 || o->SubType == 4 || o->SubType == 5 || o->SubType == 6 || o->SubType == 8 || o->SubType == 9 
			|| o->SubType == 10)
        {
            if ( o->LifeTime<=10 )
            {
