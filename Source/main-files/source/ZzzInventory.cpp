...
int RenderTextList(int sx, int sy, int TextNum, int Tab, int iSort = RT3_SORT_CENTER)
{
	...
	for (int i = 0; i < TextNum; i++)
	{
		...
		case TEXT_COLOR_GRAY:
			...
			break;
		case TEXT_COLOR_RED:
			g_pRenderText->SetTextColor(255, 52, 25, 255);
			break;
		case TEXT_COLOR_YELLOW:
			g_pRenderText->SetTextColor(255, 205, 25, 255);
			break;
		case TEXT_COLOR_GREEN:
			g_pRenderText->SetTextColor(25, 255, 129, 255);
			break;
		case TEXT_COLOR_PURPLE:
			g_pRenderText->SetTextColor(255, 25, 255, 255);
			break;
		case TEXT_COLOR_REDPURPLE:
			g_pRenderText->SetTextColor(204, 126, 204, 255);
			break;
		case TEXT_COLOR_VIOLET:
			g_pRenderText->SetTextColor(179, 102, 255, 255);
			break;
		case TEXT_COLOR_ORANGE:
			g_pRenderText->SetTextColor(230, 108, 9, 255);
			break;
		}

		gDescriptions.SetNewColorBackGroundDescription(TextListColor[i]);
		if (TextBold[i])
		{
			g_pRenderText->SetFont(g_hFontBold);
		}
...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG, bool offsetBoxWidth) //SetDrawBarItem
{
	SIZE TextSize = { 0, 0 };
	...
	for (int i = 0; i < TextNum; ++i)
	{
		...
		case 7:
			...
			break;
		default:
			if (gCreateFont.m_newFont.count(TextBold[i]) != 0) {
				g_pRenderText->SetFont(gCreateFont.m_newFont[TextBold[i]]);
			}
			else {
				g_pRenderText->SetFont(g_hFont);
			}
	...
	{
		if (gDrawBarItem.m_Item.ItemID != -1)
		{
			auto ItemInfo = &ItemAttribute[gDrawBarItem.m_Item.ItemID];
			const int WidthValue = 18;
			const int HeightValue = 30;
			float barWidth = WidthValue * ItemInfo->Width;
			float barHeight = HeightValue * ItemInfo->Height;

			int StartX = iPos_x + (fWidth / 2) - (barWidth / 2);
			int StartY = fsy - barHeight - 10;

			glColor3f(1.0, 1.0, 1.0);

			if (gDrawBarItem.m_ItemBorderColor.ItemID != -1) {
				glColor4f(gDrawBarItem.m_ItemBorderColor.R, gDrawBarItem.m_ItemBorderColor.G, gDrawBarItem.m_ItemBorderColor.B, gDrawBarItem.m_ItemBorderColor.A);
			}
			else {
				glColor4f(1.0, 1.0, 1.0, 1.0);
			}

			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 8, iPos_x, fsy - barHeight, fWidth, fHeight + barHeight); //fundo
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 1, iPos_x, fsy - 8.0 - barHeight, fWidth, 8.0f); //Topo meio
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 6, iPos_x, fsy + fHeight, fWidth, 8.0f); //Baixo meio
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 3, iPos_x - 8.0, fsy - barHeight, 8.0f, fHeight + barHeight); //Lateral esquerda
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 4, iPos_x + fWidth, fsy - barHeight, 8.0f, fHeight + barHeight); //Lateral direita
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START, iPos_x - 8.0, fsy - 8.0 - barHeight, 8.0f, 8.0f); //Topo esquerda
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 2, iPos_x + fWidth, fsy - 8.0 - barHeight, 8.0f, 8.0f); //Topo direita
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 5, iPos_x - 8.0, fsy + fHeight, 8.0f, 8.0f);  //Baixo esquerda
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 7, iPos_x + fWidth, fsy + fHeight, 8.0f, 8.0f);  //Baixo direita

			DisableAlphaBlend();
			glEnable(GL_TEXTURE_2D);

			gDrawBarItem.m_Item.PosX = (float)StartX;
			gDrawBarItem.m_Item.PosY = (float)StartY;
			gDrawBarItem.m_Item.Width = barWidth;
			gDrawBarItem.m_Item.Height = barHeight;

			gDrawBarItem.DrawItem();
		}
		else
		{
			glColor3f(1.0, 1.0, 1.0);
			...
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 8, iPos_x, fsy, fWidth, fHeight); //fundo
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 1, iPos_x, fsy - 8.0, fWidth, 8.0f); //Topo meio
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 6, iPos_x, fsy + fHeight, fWidth, 8.0f); //Baixo meio
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 3, iPos_x - 8.0, fsy, 8.0f, fHeight); //Lateral esquerda
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 4, iPos_x + fWidth, fsy, 8.0f, fHeight); //Lateral direita
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START, iPos_x - 8.0, fsy - 8.0, 8.0f, 8.0f); //Topo esquerda
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 2, iPos_x + fWidth, fsy - 8.0, 8.0f, 8.0f); //Topo direita
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 5, iPos_x - 8.0, fsy + fHeight, 8.0f, 8.0f);  //Baixo esquerda
			SEASON3B::RenderImage(BITMAP_NEW_TOOLTIP_START + 7, iPos_x + fWidth, fsy + fHeight, 8.0f, 8.0f);  //Baixo direita
...
bool IsCanUseItem()
{
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_STORAGE) || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_TRADE) || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_TRADEX))
	{
		return false;
	}
	else
...
WORD calcMaxDurability(const ITEM* ip, ITEM_ATTRIBUTE* p, int Level)
{
	...
	if ((ip->ExtOption % 0x04) == EXT_A_SET_OPTION || (ip->ExtOption % 0x04) == EXT_B_SET_OPTION)
	{
		...
		&& (ip->Type<ITEM_WING + 42 || ip->Type>ITEM_WING + 43 || !gCustomWing.CheckCustomWingByItem(ip->Type))
...
void RenderItemInfo(int sx, int sy, ITEM* ip, bool Sell, int Inventype, bool bItemTextListBoxUse, bool offsetBoxWidth)
{
	if (ip->Type == -1)
		return;
	...
	{
		{
			...
			if (GetPersonalItemPrice(indexInv, price, price2, price3, price4, g_IsPurchaseShop))
			{

				sprintf(TextList[TextNum], GlobalText[3188]);
				...
				if (price > 0) {
					...
					sprintf(TextList[TextNum], GlobalText[3189], GlobalText[3190], Text);
					TextListColor[TextNum] = TEXT_COLOR_ORANGE;
					TextBold[TextNum] = true;
					TextNum++;
				}

				if (price2 > 0 && gProtect->m_MainInfo.m_PersonalShop_Coins >= 2) {
					ConvertGold(price2, Text);
					sprintf(TextList[TextNum], GlobalText[3189], GlobalText[3191], Text);
					TextListColor[TextNum] = TEXT_COLOR_ORANGE;
					TextBold[TextNum] = true;
					TextNum++;
				}

				if (price3 > 0 && gProtect->m_MainInfo.m_PersonalShop_Coins >= 3) {
					ConvertGold(price3, Text);
					sprintf(TextList[TextNum], GlobalText[3189], GlobalText[3192], Text);
					TextListColor[TextNum] = TEXT_COLOR_ORANGE;
					TextBold[TextNum] = true;
					TextNum++;
				}

				if (price4 > 0 && gProtect->m_MainInfo.m_PersonalShop_Coins >= 4) {
					ConvertGold(price4, Text);
					sprintf(TextList[TextNum], GlobalText[3189], GlobalText[3193], Text);
					TextListColor[TextNum] = TEXT_COLOR_ORANGE;
					TextBold[TextNum] = true;
					TextNum++;
				}
	...
	if (gCounterItem.CheckIsItem(ip->Type)) {
		...
		const char* rankName = gCounterItem.getKillRangeName(ip->Type, count);

		int colorType = gCounterItem.getKillRangeColorType(ip->Type, count);
		DWORD rankColor = gCounterItem.GetColorByType(colorType);

		sprintf(TextList[TextNum], "Contador de Mortes: %d", count);
		TextListColor[TextNum] = itemInfo->Color;
		TextBold[TextNum] = itemInfo->Bold;
		TextNum++;


		if (rankName[0] != '\0') { // Só adiciona se rankName não for vazio
			sprintf(TextList[TextNum], "Patente: %s", rankName);
			TextListColor[TextNum] = colorType;
			TextBold[TextNum] = itemInfo->Bold;
			TextNum++;
		}
		...
		else if (ip->Type != ITEM_ETC + 5 && ip->Type != ITEM_ETC + 14 && ip->Type != ITEM_ETC + 15)
		{
			...
			else
			{
				if (DamageMin + minindex >= DamageMax + maxindex)
					...
					sprintf(TextList[TextNum], "%s: %d ~ %d", GlobalText[40 + p->TwoHand], DamageMin + minindex, DamageMax + maxindex); //TOOLTIP DANO PATENTE
			}
		}
		else
		{
	...
	{
		if ((p->m_byItemSlot >= 240 && p->m_byItemSlot <= 248) || (ip->Jewel_Of_Harmony_Option > 0 || ip->Jewel_Of_Harmony_OptionLevel > 0))
		{
	...
	{
		if (bItemTextListBoxUse)
			RenderTipTextList(sx, sy, TextNum, 0, RT3_SORT_CENTER, STRP_BOTTOMCENTER, 1, offsetBoxWidth);
		else
			RenderTipTextList(sx, sy, TextNum, 0, 3, 0, 1, offsetBoxWidth);
	}
}
...
void RenderItemName(int i, OBJECT* o, int ItemLevel, int ItemOption, int ItemExtOption, bool Sort)
{
	...
	bool bFirstOK = true;
	int itemType = o->Type - MODEL_ITEM;

	if (o->Type == MODEL_POTION + 15)
	{
		glColor3f(1.f, 0.8f, 0.1f);
		sprintf(Name, "%s %d", ItemAttribute[itemType].Name, ItemLevel);
	}
		...
		)
	{
		g_pRenderText->SetFont(g_hFontBold);
		glColor3f(1.f, 0.8f, 0.1f);
		sprintf(Name, "%s", ItemAttribute[itemType].Name);
	}
	...
	else if (o->Type == MODEL_HELPER + 46)
	{
		...
		sprintf(Name, ItemAttribute[itemType].Name);
	}
	...
	else if (o->Type == MODEL_POTION + 17 || o->Type == MODEL_POTION + 18 || o->Type == MODEL_POTION + 19)
	{
		...
		{
			sprintf(Name, "%s +%d", ItemAttribute[itemType].Name, ((ItemLevel >> 3) & 15));
		}
	}
	...
	else if (o->Type == MODEL_HELPER + 15)
	{
		...
		switch (Level)
		{
		case 0:sprintf(Name, "%s %s", GlobalText[168], ItemAttribute[itemType].Name); break;
		case 1:sprintf(Name, "%s %s", GlobalText[169], ItemAttribute[itemType].Name); break;
		case 2:sprintf(Name, "%s %s", GlobalText[167], ItemAttribute[itemType].Name); break;
		case 3:sprintf(Name, "%s %s", GlobalText[166], ItemAttribute[itemType].Name); break;
		case 4:sprintf(Name, "%s %s", GlobalText[1900], ItemAttribute[itemType].Name); break;
		}
	}
	else if (o->Type == MODEL_HELPER + 31)
	{
		...
		switch (Level)
		{
		case 0:sprintf(Name, "%s of %s", ItemAttribute[itemType].Name, GlobalText[1187]); break;
		case 1:sprintf(Name, "%s of %s", ItemAttribute[itemType].Name, GlobalText[1214]); break;
		}
	}
	...
	if (bFirstOK == false)
	{
			...
			|| (o->Type >= MODEL_WING + 100 && o->Type <= MODEL_WING + 129))
		{
			...
			strcpy(Name, ItemAttribute[itemType].Name);
		}
		...
		else
		{
			...
			char TextName[64];
			if (g_csItemOption.GetSetItemName(TextName, itemType, ItemExtOption))
			{
				glColor3f(1.f, 1.f, 1.f);
				g_pRenderText->SetFont(g_hFontBold);
				g_pRenderText->SetTextColor(0, 255, 0, 255);
				...
				strcat(TextName, ItemAttribute[itemType].Name);
			}
			else
			{
				strcpy(TextName, ItemAttribute[itemType].Name);
			}
	...
	if (!Sort)
	{
		RenderTipTextBorder(MouseX, MouseY - 15, Name);
		//g_pRenderText->RenderText(MouseX,MouseY-15,Name,0, 0, RT3_WRITE_CENTER);
	}
	else
	{
		RenderTipTextBorder(o->ScreenX, o->ScreenY - 15, Name);
...
int GetScreenWidth()
{
	...
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INVENTORY)
		&& (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHARACTER)
			...
			|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_CHAOSGENESIS)
			|| g_pNewUISystem->CustomLuaWindow
#ifdef LEM_ADD_LUCKYITEM
			|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_LUCKYITEMWND)
#endif // LEM_ADD_LUCKYITEM
			))
	{
		iWidth = gWideScreen.g_WideWindowWidth - (190 * 2);
	}
	...
	{
		iWidth = gWideScreen.g_WideWindowWidth - 190;
	}
	else
	{
		iWidth = gWideScreen.g_WideWindowWidth;
	}
...
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
					Type == ITEM_WING + 40 ||
					gCustomWing.CheckCustomWingIsCape(Type) > 0)
				{
...
bool IsRepairBan(ITEM* pItem)
{
	...
	//ITEM VISUAL BAN REPAIR HARMONY
	if (pItem->Jewel_Of_Harmony_Option > 0 || pItem->Jewel_Of_Harmony_OptionLevel > 0) {
		return true;
	}

	//ITEM VISUAL BAN REPAIR SLOT
	auto att = &ItemAttribute[pItem->Type];
	if (att != nullptr) {

		if (att->m_byItemSlot >= 240 && att->m_byItemSlot <= 248) {
			return true;
		}
	}
...
void RenderObjectScreen(int Type, int ItemLevel, int Option1, int ExtOption, vec3_t Target, int Select, bool PickUp)
{
	...
	float HeightRate = (float)gWideScreen.g_WideWindowHeight / 480.f;

	VectorSubtract(Target, MousePosition, Direction);
	if (PickUp)
		VectorMA(MousePosition, (0.07f * HeightRate), Direction, Position);
	else
		VectorMA(MousePosition, (0.1f * HeightRate), Direction, Position);
	...
	else if (Type == MODEL_HELPER + 71 || Type == MODEL_HELPER + 72 || Type == MODEL_HELPER + 73 || Type == MODEL_HELPER + 74 || Type == MODEL_HELPER + 75)
	{
		...
		if (Select != 1)
		{
			ObjectSelect.Angle[1] = WorldTime * 0.1f;
		}
	}
	...
	//ROATAÇÃO DOS ITENS GIRANDO VELOCIDADE
	if (1 == Select)
	{
		ObjectSelect.Angle[1] = WorldTime * 0.15f;
	}
	...
	else
	{
		...
		else if (gCustomItemSize.CheckCustomItemSize(Type - MODEL_ITEM))
		{
			static float Itemsize = 0.0;
...
void RenderItem3D(float sx, float sy, float Width, float Height, int Type, int Level, int Option1, int ExtOption, bool PickUp)
{
		...
		&& SEASON3B::CheckMouseIn(sx, sy, Width, Height))
	{
		...
		{
			//ROTAÇÃO DOS ITENS COM MOUSE EMCIMA
			// Caso a interface não esteja com itens girando, atribuí-la a condição a baixo
...
void RenderEqiupmentBox()
{
	...
	//RENDERIZAR EQUIPAMENTOS IMAGENS
	EnableAlphaTest();
...
bool IsExistUndecidedPrice()
{
	...
	for (int i = 0; i < MAX_PERSONALSHOP_INVEN; ++i)
	{
		...
		if (pItem)
		{
			...
			if (GetPersonalItemPrice(iIndex, iPrice, iPrice2, iPrice3, iPrice4, g_IsPurchaseShop) == false)
			{
				return true;
			}

			if (iPrice <= 0 && iPrice2 <= 0 && iPrice3 <= 0 && iPrice4 <= 0)
			{
				return true;
			}
		}
