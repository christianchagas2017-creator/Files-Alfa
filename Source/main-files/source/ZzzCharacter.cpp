...
void RegisterBuff(eBuffState buff, OBJECT* o, const int bufftime = 0, CHARACTER* c = NULL);
...
extern vec3_t cameraPosition; // Declaração global

bool IsObjectInFrustum(vec3_t position) {
	// Obter a posição e direção da câmera
	vec3_t cameraDir; // Direção da câmera (ex.: vetor de visão)
	// Suponha que cameraDir seja obtido de uma variável global ou calculado
	// Exemplo: cameraDir = direção do Hero ou matriz de visão
	cameraDir[0] = 0.0f; // Placeholder, substitua pela direção real
	cameraDir[1] = 0.0f;
	cameraDir[2] = 1.0f; // Ex.: olhando para frente no eixo Z

	// Calcular vetor do monstro para a câmera
	vec3_t toMonster;

	VectorSubtract(position, cameraPosition, toMonster);

	// Verificar distância máxima (ex.: 500 unidades)
	float distance = VectorLength(toMonster);
	const float MAX_VISIBLE_DISTANCE = 500.0f; // Ajuste conforme necessário
	if (distance > MAX_VISIBLE_DISTANCE) {
		return false;
	}

	// Verificar se está dentro do campo de visão (FOV)
	float dot = DotProduct(toMonster, cameraDir);
	float fov = 60.0f; // Campo de visão em graus
	float cosFov = cosf(fov * 0.5f * 3.14159f / 180.0f);
	if (dot < distance * cosFov) {
		return false; // Fora do FOV
	}

	return true; // Dentro do frustum
}

vec3_t cameraPosition; // Posição da câmera (x, y, z)

// No início do loop principal ou função de renderização
void UpdateCameraPosition() {
	// Exemplo: usar a posição do Hero como câmera
	if (Hero != NULL) {
		VectorCopy(Hero->Object.Position, cameraPosition);
	}
	else {
		// Fallback: definir uma posição padrão ou usar uma variável global
...
void FallingMonster(CHARACTER* c, OBJECT* o) {
	if (!IsObjectInFrustum(o->Position)) {
		return; // Pula monstros fora do frustum
	}

	if (rand_fps_check(2048)) {
		float AngleY;
		o->Gravity += 2.5f * FPS_ANIMATION_FACTOR;
...
BOOL PlayMonsterSound(OBJECT* pObject) {
	if (rand_fps_check(16)) { // Reduz a frequência de sons
...
void SetPlayerStop(CHARACTER* c)
{
	...
	auto pEquippedItem = &CharacterMachine->Equipment[NEW_EQUIPMENT_HELPER_FLY];

	if (c->Object.Type == MODEL_PLAYER)
	{
		//FENRIR
		if ((c->Helper.Type == MODEL_HELPER + 37 && !c->SafeZone) || (gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1) && !c->SafeZone)
		{
		...
		else if ((c->Weapon[1].Type == MODEL_HELPER + 5 || gDarkSpirit.checkIsDarkSpirit(c->RavenCustom - MODEL_ITEM)) && c->SafeZone && !gMapManager.InChaosCastle())	// AQUI ELE FICA DURO NO BRAÇO
		{
			...
			//SetAction ( &c->Object, PLAYER_STOP_MALE); //DARK RAVEN OMBRO
		}
		...
		//DRAGON
		else if ((gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1) && !c->SafeZone)
		{
			if (c->Weapon[0].Type == -1 && c->Weapon[1].Type == -1)
				SetAction(&c->Object, PLAYER_DRAGON_STOP);
			else
				SetAction(&c->Object, PLAYER_DRAGON_STOP);
		}
		else
				...
				else
				{
					...
					if ((c->Weapon[0].Type == -1 && c->Weapon[1].Type == -1) || (c->SafeZone && (gMapManager.InBloodCastle() == false)))
					{
						if (gCharacterManager.GetBaseClass(c->Class) == CLASS_ELF)
							...
							SetAction(&c->Object, PLAYER_STOP_MALE_NEW);
					}
					else
					{
						//  Ä® ÀåÂø.
	...
	if (rand_fps_check(32)) { // Reduz frequência de sons
		if (o->Type != MODEL_PLAYER || (o->SubType >= MODEL_SKELETON1 && o->SubType <= MODEL_SKELETON3)) {
			if (Models[o->Type].Sounds[0] != -1) {
				int offset = (o->SubType == 9) ? 5 : 0;
				PlayBuffer(SOUND_MONSTER + offset + Models[o->Type].Sounds[rand() % 2], o);
			}
		}
		//SOM DO FENRIR RUSNANDO
		else if (c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1)
		{
			if (!c->SafeZone)
			{
				if (rand_fps_check(3))
		...
		//SOM DO DRAGON RUSNANDO
		else if (gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1)
		{
			if (!c->SafeZone)
			{
				if (rand_fps_check(3))
				{
					if (rand_fps_check(2))
						PlayBuffer(SOUND_DRAGON_GROWL, o);
					else
						PlayBuffer(SOUND_DRAGON_ROAR, o);
				}
			}
		}
	}
...
void SetPlayerWalk(CHARACTER* c)
{
	...
	if (c->MonsterIndex >= 534 && c->MonsterIndex <= 539)
	{
		...
		if (!c->SafeZone && c->Run < 128 + rand_fps_check(512))
		{
			//CORRER SEM BOTA
			if (IsGMCharacter() || gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK || gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD
			...
			{
				c->Run++;
				//c->Run + FPS_ANIMATION_FACTOR + 150;
			}
		}
	OBJECT* o = &c->Object;
	if (c->Object.Type == MODEL_PLAYER)
	{
		...
		else if ((c->Weapon[1].Type == MODEL_HELPER + 5 || gDarkSpirit.checkIsDarkSpirit(c->RavenCustom - MODEL_ITEM)) && c->SafeZone)	// AQUI ELE ANDA E FICA DURINHO
		{
			...
			//SetAction(&c->Object, PLAYER_WALK_MALE); //DARK RAVEN OMBRO
		}
...
//				    SetAction(&c->Object,PLAYER_FLY_RIDE_WEAPON);
		...
		//DRAGON
		else if (gHelperSystem.CheckHelperType(c->Helper.Type, 32) && !c->SafeZone)
		{
			SetAction(&c->Object, PLAYER_DRAGON_RUN);
		}
		else
		{
			int Index = TERRAIN_INDEX_REPEAT((int)(c->Object.Position[0] / TERRAIN_SCALE), (int)(c->Object.Position[1] / TERRAIN_SCALE));
	...
	//SOM DOS PETS ANDANDO E CORRENDO
	//HORSE
	else if ((c->Helper.Type == MODEL_HELPER + 4 || gHelperSystem.CheckHelperType(c->Helper.Type, 16) == 1) && !c->SafeZone)
	{
	...
	//FENRIR
	else if ((c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1) && !c->SafeZone && (c->Object.CurrentAction >= PLAYER_FENRIR_RUN && c->Object.CurrentAction <= PLAYER_FENRIR_RUN_ONE_LEFT_ELF))
	{
		PlayBuffer(SOUND_FENRIR_RUN_1 + rand() % 3, o);
	}
	else if ((c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1) && !c->SafeZone && (c->Object.CurrentAction >= PLAYER_FENRIR_WALK && c->Object.CurrentAction <= PLAYER_FENRIR_WALK_ONE_LEFT))
	{
	...
	{
		PlayBuffer(SOUND_DRAGON_RUN_1 + rand() % 3, o);
	}
...
void SetAttackSpeed()
{
	float AttackSpeed1 = (CharacterAttribute->AttackSpeed) * 0.004f;	// ±â»ç, ŔĎąÝ°ř°Ý
	if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_KNIGHT)
	{
		AttackSpeed1 = (CharacterAttribute->AttackSpeed) * 0.006f;
	}
	...
	if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_WIZARD)
	{
		MagicSpeed2 = (CharacterAttribute->MagicSpeed) * 0.00060f;	// ąý»ç
	}
	else if (gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_SUMMONER)
	{
	...
	Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_SKILL_SWORD2].PlaySpeed = 0.30f + AttackSpeed1; // Âî¸Ł±â
	Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_SKILL_SWORD3].PlaySpeed = 0.27f + AttackSpeed1; // żĂ·ÁÄˇ±â
	Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_SKILL_SWORD4].PlaySpeed = 0.30f + AttackSpeed1; // µą·ÁÄˇ±â
	...
	Models[MODEL_PLAYER].Actions[PLAYER_ATTACK_SKILL_SPEAR].PlaySpeed = 0.30f + AttackSpeed1;	// Ă˘Âî¸Ł±â

	// Ĺ» °Í Ĺş »óĹÂ
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_RIDER].PlaySpeed = 0.3f + AttackSpeed1;	// ˝şĹĎ, ˝şĹĎÁ¦°Ĺ, Ĺő¸í, Ĺő¸íÇŘÁ¦, ¸¶łŞÁő°ˇ, ş¸Á¶¸¶ąý ŔĎ˝ĂÁ¦°Ĺ, ·ąŔĚµĺ˝¸	
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_RIDER_FLY].PlaySpeed = 0.3f + AttackSpeed1;	// ˝şĹĎ, ˝şĹĎÁ¦°Ĺ, Ĺő¸í, Ĺő¸íÇŘÁ¦, ¸¶łŞÁő°ˇ, ş¸Á¶¸¶ąý ŔĎ˝ĂÁ¦°Ĺ, ·ąŔĚµĺ˝¸
	...
	//±âĹ¸¸¶ąý
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_TELEPORT].PlaySpeed = 0.30f + MagicSpeed2; // Ľř°ŁŔĚµż
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_FLASH].PlaySpeed = 0.40f + MagicSpeed2; // ľĆÄíľĆÇĂ·ˇ˝¬
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_INFERNO].PlaySpeed = 0.60f + MagicSpeed2; // ŔÎĆä¸Łłë
	...
	//^ Ćć¸± °řĽÓ °ü·Ă(Áßżä : °ř°Ý ĽÓµµżÍ °ü·Ă)
	...
	Models[MODEL_PLAYER].Actions[PLAYER_FENRIR_SKILL_FURY_STRIKE].PlaySpeed = 0.38f;
	Models[MODEL_PLAYER].Actions[PLAYER_SKILL_VITALITY].PlaySpeed = 0.34f;
	Models[MODEL_PLAYER].Actions[PLAYER_FENRIR_SKILL_VITALITY].PlaySpeed = 0.34f;
...
void SetPlayerShock(CHARACTER* c, int Hit)
{
	...
	if (gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1) return;

	OBJECT* o = &c->Object;

	if (o->CurrentAction == PLAYER_ATTACK_SKILL_FURY_STRIKE || o->CurrentAction == PLAYER_FENRIR_SKILL_FURY_STRIKE || o->CurrentAction == PLAYER_SKILL_VITALITY || o->CurrentAction == PLAYER_FENRIR_SKILL_VITALITY) return;
	...
	if (c->Object.AnimationFrame == 0.f)
	{
		...
		else
		{
			...
			else
			{
				...
				//SOM DO FENRIR MORRENDO
				if (c->Helper.Type == MODEL_HELPER + 37 || gHelperSystem.CheckHelperType(c->Helper.Type, 8) == 1)
				{
					PlayBuffer(SOUND_FENRIR_DEATH, o);
				}
				//SOM DO HORSE MORRENDO
				else if (c->Helper.Type == MODEL_HELPER + 4 || gHelperSystem.CheckHelperType(c->Helper.Type, 16) == 1)
				{
					PlayBuffer(SOUND_DARKLORD_DEAD, o);
				}
				//SOM DO DRAGON MORRENDO
				else if (gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1)
				{
					PlayBuffer(SOUND_DRAGON_DEATH, o);
				}
			}
		}
...
bool CharacterAnimation(CHARACTER* c, OBJECT* o)
{
	...
	if (b->NumActions > 0)
	{
		...
		if (c->Change && o->CurrentAction >= MONSTER01_ATTACK1 && o->CurrentAction <= MONSTER01_ATTACK2)
			PlaySpeed *= 1.5f;
		if ((o->CurrentAction == PLAYER_SKILL_VITALITY || o->CurrentAction == PLAYER_FENRIR_SKILL_VITALITY) && o->AnimationFrame > 6.f)
		{
...
bool AttackStage(CHARACTER* c, OBJECT* o)
{
	...
	case AT_SKILL_ONETOONE: //DEATH STAB
	{
		BMD* b = &Models[o->Type];
		...
		if (c->AttackTime <= 2 && c->AttackTime <= 8)
		{
			for (int j = 0; j < 3; ++j)
			{
				...
				CurPos[2] += 190.0f;
				vec3_t TempPos;
				GetNearRandomPos(CurPos, 1200, TempPos);
				float fDistance = 3540.0f;
				...
				int customColor = gDeathStabColor.IsDeathStabColor((DWORD)c); //DEATH STAB EFEITO

				if (customColor >= 10000)
				{
					float customSize = gDeathStabColor.GetDeathStabSize(customColor);

					if (customSize > -1)
						Size = gDeathStabColor.GetDeathStabSize(customColor);

					Color = customColor;
				}

				CreateJoint(MODEL_SPEARSKILL, TempPos, TempPos, o->Angle, Color, o, Size + 30);
	...
	case AT_SKILL_VITALITY:
		if ((o->CurrentAction == PLAYER_SKILL_VITALITY || o->CurrentAction == PLAYER_FENRIR_SKILL_VITALITY) && c->AttackTime > 9 && o->Type == MODEL_PLAYER)
		{
			c->AttackTime = 15;
		}
		break;
...
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
	default:
		if ((o->CurrentAction == PLAYER_ATTACK_SKILL_FURY_STRIKE || o->CurrentAction == PLAYER_FENRIR_SKILL_FURY_STRIKE) && o->AnimationFrame >= 1.f && o->Type == MODEL_PLAYER)
		{
			c->AttackTime = 15;
		}
...
void  PushingCharacter(CHARACTER* c, OBJECT* o)
{
	...
	if (c->JumpTime > 0)
	{
		...
		else
		{
			...
			c->JumpTime = 0;// FPS_ANIMATION_FACTOR;
			if (c->JumpTime > 15)
			{
				if (o->Type == MODEL_MONSTER01 + 52)
					SetPlayerStop(c);
...
void PlayerNpcStopAnimationSetting(CHARACTER* c, OBJECT* o)
{
	...
	if (o->CurrentAction != PLAYER_STOP_MALE_NEW)
	{
		SetAction(&c->Object, PLAYER_STOP_MALE_NEW);
	}
	else
...
void PlayerStopAnimationSetting(CHARACTER* c, OBJECT* o)
{
	...
	if (o->CurrentAction < PLAYER_WALK_MALE ||
		...
		//NOVAS ANIMAÇÕES
		|| (o->CurrentAction >= PLAYER_FENRIR_SKILL_VITALITY && o->CurrentAction <= PLAYER_DRAGON_RUN - 1))
...
void CreateWeaponBlur(CHARACTER* c, OBJECT* o, BMD* b)
{
		...
		)
	{
		...
		if (o->Type == MODEL_PLAYER)
		{
			...
			else if ((o->CurrentAction == PLAYER_SKILL_BLOW_OF_DESTRUCTION || o->CurrentAction == PLAYER_FENRIR_SKILL_FURY_STRIKE) && o->AnimationFrame >= 2.f && o->AnimationFrame <= 8.f)
			{
				BlurType = 1;
				BlurMapping = 2;
			}
...
void MoveCharacter(CHARACTER* c, OBJECT* o)
{
	...
	if (c->Dead > 0) // AQUI ALTERA A ANIMAÇÃO DE MORTE GERAL
	{
		c->Dead++;
		if (c->Dead >= 15)
		{
	...
	if (c->AttackTime >= g_iLimitAttackTime)
	{
		...
		case AT_SKILL_VITALITY:
		{
			...
			for (int i = 0; i < iCount; ++i)
			{
				...
				int customColor = gBuffColorBK.IsBuffColorBK((DWORD)c); //BUFF BK COLOR SET ITEMM
				...
				float Size = gBuffColorBK.GetBuffSize(customColor);

				if (Size > -1)
					Size = gBuffColorBK.GetBuffSize(customColor);
				else
					Size = 60.f;

				CreateJoint(BITMAP_JOINT_SPIRIT, Position, Position, Angle, Color1, o, Size, 0, 0);

				if ((i % 20) == 0)
				{
					CreateEffect(BITMAP_MAGIC + 1, o->Position, Angle, o->Light, Color2, o);
				}
			}
		}
		...
		case AT_SKILL_WHEEL:
			o->Weapon = c->Weapon[0].Type - MODEL_SWORD;
			o->WeaponLevel = (BYTE)c->Weapon[0].Level;
			CreateEffect(MODEL_SKILL_WHEEL1, o->Position, o->Angle, o->Light, 5, o, o->PKKey, FindHotKey((c->Skill)));
		...
		case AT_SKILL_DARK_SCREAM:
		{
			...
			//DRAGON
			if ((gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1) && !c->SafeZone)
			{
				SetAction(o, PLAYER_ATTACK_RIDE_STRIKE);
			}
			else
		...
		case AT_SKILL_EVIL:
			...
			PlayBuffer(SOUND_EVIL);
			break;
		case AT_SKILL_PLASMA_STORM_FENRIR: //FENRIR PLASMA STORM
		{
			...
			if (c->m_iFenrirSkillTarget != -1 && c->m_iFenrirSkillTarget < MAX_CHARACTERS_CLIENT)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					...
					//FENRIR PLASMA STORM
					int Color1 = 0 + GetFenrirType(c);
					int Color2 = 3 + GetFenrirType(c);

					int customColor = gPlasmaStormColor.IsPlasmaStormColor((DWORD)c);

					if (customColor >= 100)
					{
						Color1 = customColor;
						Color2 = customColor + 10000;
					}

					CreateJoint(MODEL_FENRIR_SKILL_THUNDER, Position, p_to->Position, vAngle, Color1, p_to, 100.f);
					CreateJoint(MODEL_FENRIR_SKILL_THUNDER, Position, p_to->Position, vAngle, Color2, p_to, 80.f);
				}
				...
				for (int i = 0; i < iMonsterNum; i++)
				{
					for (int j = 0; j < 2; j++)
					{
						...
						CreateJoint(MODEL_FENRIR_SKILL_THUNDER, Position, p_o[i]->Position, vAngle, Color1, p_o[i], 100.f);
						CreateJoint(MODEL_FENRIR_SKILL_THUNDER, Position, p_o[i]->Position, vAngle, Color2, p_o[i], 80.f);
					}
				}
				...
				for (int k = 0; k < 6; k++)
				{
					...
					CreateJoint(BITMAP_FLARE_FORCE, Position, Position, vAngle, Color, NULL, 60.f);
				}

				int customColor = gPlasmaStormColor.IsPlasmaStormColor((DWORD)c); //CASTING EFFECT

				if (customColor >= 100)
				{
					vec3_t vLight;

					int Color1 = customColor;
					int Color2 = customColor + 10000;
					int SubType = 0;

					vec_t* target;
					vec_t* target2 = nullptr;

					if (gPlasmaStormColor.GetPlasmaStormCastingEnabled(customColor))
					{
						if (gPlasmaStormColor.GetPlasmaStormCastingTarget(customColor) == 0)
						{
							target = o->Position;
						}
						else if (gPlasmaStormColor.GetPlasmaStormCastingTarget(customColor) == 1)
						{
							target = p_to->Position;
						}
						else
						{
							target = o->Position;
							target2 = p_to->Position;
						}

						int Black = gPlasmaStormColor.GetPlasmaStormCastingBlack(customColor);

						if (Black == 1)
						{
							SubType = 1;
							EnableAlphaBlendMinus();
							Vector(10.f, 10.f, 10.f, vLight);
						}
						else
						{
							gPlasmaStormColor.GetPlasmaStormCasting(customColor, vLight);
						}

						if (!target2)
						{
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT1, target, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT11, target, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT111, target, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT2, target, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT22, target, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT222, target, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT4, target, o->Angle, vLight, SubType);
						}
						else
						{
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT1, target, o->Angle, vLight, SubType);
							...
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT1, target2, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT11, target2, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT111, target2, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT2, target2, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT22, target2, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT222, target2, o->Angle, vLight, SubType);
							CreateEffect(MODEL_SUMMONER_CASTING_EFFECT4, target2, o->Angle, vLight, SubType);
						}
					}

					if (gPlasmaStormColor.GetPlasmaStormCastingShadow(customColor))
					{
						if (gPlasmaStormColor.GetPlasmaStormRuneTarget(customColor) == 0)
						{
							target = o->Position;
						}
						else if (gPlasmaStormColor.GetPlasmaStormRuneTarget(customColor) == 1)
						{
							target = p_to->Position;
						}
						else
						...
						int Black = gPlasmaStormColor.GetPlasmaStormCastingShadow(customColor);
						...
						{
							gPlasmaStormColor.GetPlasmaStormCastingShadowColor(customColor, vLight);
						}

						if (!target2)
						{
							CreateEffect(BITMAP_MAGIC, target, vAngle, vLight, Color1);
						}
						else
						{
							CreateEffect(BITMAP_MAGIC, target, vAngle, vLight, Color1);
							CreateEffect(BITMAP_MAGIC, target2, vAngle, vLight, Color1);
						}
					}

					if (gPlasmaStormColor.GetPlasmaStormRuneEnabled(customColor))
					{
						if (gPlasmaStormColor.GetPlasmaStormRuneTarget(customColor) == 0)
						{
							target = o->Position;
						...
						int Black = gPlasmaStormColor.GetPlasmaStormRuneBlack(customColor);
						...
						{
							gPlasmaStormColor.GetPlasmaStormRune(customColor, vLight);
						}

						if (!target2)
						{
							CreateEffect(BITMAP_MAGIC, target, vAngle, vLight, Color2);
						}
						else
						{
							CreateEffect(BITMAP_MAGIC, target, vAngle, vLight, Color2);
							CreateEffect(BITMAP_MAGIC, target2, vAngle, vLight, Color2);
						}
					}
			...
			//vec3_t vDirection;
			//
			////Vector(o->Position[0], o->Position[1] - 80.0f, o->Position[2] + 200.0f, vPosition);
			//Vector(o->Position[0], o->Position[1], o->Position[2] + 200.0f, vPosition);
			//Vector(o->Angle[0]+30.0f, o->Angle[1], o->Angle[2], vAngle);
			//Vector(o->Direction[0], o->Direction[1], o->Direction[2]+30.f, vDirection);
			//
			//CreateEffect(MODEL_DEATH_SCYTHE, vPosition, vAngle, vLight, 1);  //DEATH SCYTHE
		...
		else
		{
			...
			case AT_SKILL_WIZARDDEFENSE: //BUFF SM COLOR
				...
				else
				{
					...
					int customColor = gBuffColorSM.IsBuffColorSM((DWORD)c);
					float customSize = gBuffColorSM.GetBuffSize(customColor);
					...
					if (customColor >= 100)
					{
						...
						if (customSize > -1)
							size = gBuffColorSM.GetBuffSize(Color);
					}

					if (customColor == o->SubType)
					{
						DeleteJoint(MODEL_SPEARSKILL, to, Color);
						for (int j = 0; j < 5; ++j)
						{
							CreateJoint(MODEL_SPEARSKILL, to->Position, to->Position, to->Angle, Color, to, size);
						}
					}
			...
			case AT_SKILL_FLAME:
				...
				Position[1] = to->Position[1];
				Position[2] = RequestTerrainHeight(Position[0], Position[1]);
				CreateEffect(BITMAP_FLAME, Position, o->Angle, o->Light, 1, o, o->PKKey, FindHotKey(AT_SKILL_FLAME));
				PlayBuffer(SOUND_FLAME);
				break;
...
void PlayWalkSound()
{
	...
	//SOM
	if (o->CurrentAction == PLAYER_FLY || o->CurrentAction == PLAYER_FLY_CROSSBOW || o->CurrentAction == PLAYER_FENRIR_RUN || o->CurrentAction == PLAYER_RUN_RIDE_HORSE)
	{
	}
	else
	{
...
float CharacterMoveSpeed(CHARACTER* c)
{
	...
	if (o->Type == MODEL_PLAYER && o->Kind == KIND_PLAYER)
	{
		...
		//DRAGON
		else if ((gHelperSystem.CheckHelperType(c->Helper.Type, 32) == 1) && !c->SafeZone && !isholyitem)
		{
			c->Run = 40;
			Speed = 17;
		}
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	if (Link)
	{
		...
		else if (Type >= MODEL_WING + 40)
		{
			...
			(Type >= MODEL_BOW + 18 && Type < MODEL_BOW + 20) || gCustomBow.CheckIsCrossBow(Type - MODEL_ITEM) == 1)
		{
	...
	case MODEL_SHIELD + 20: //SHIELD STYLE (FEITO PELO LUA)
	{
		//Vector(0.f, 0.f, 0.f, Position);
	...
	if (gMapManager.WorldActive != WD_10HEAVEN && gMapManager.InHellas() == FALSE && !g_Direction.m_CKanturu.IsMayaScene())
	{
		...
		case MODEL_WING + 134:      // Small Wings of Satan
			...
			// Staff e Shield (já existentes)
		case MODEL_STAFF + 9:       // STAFF
		case MODEL_STAFF + 5:       // STAFF
		case MODEL_SHIELD + 14:     // SHIELD
		case MODEL_SHIELD + 16:     // SHIELD

			// Novos itens para sombra (adicionados agora)
		case MODEL_HELM:            // Helm (capacete)
		case MODEL_HELM + 1:        // Helm +1
		case MODEL_HELM + 2:        // Helm +2
			// ... Adicione todos os modelos de Helm necessários

		case MODEL_ARMOR:           // Armor (armadura)
		case MODEL_ARMOR + 1:       // Armor +1
		case MODEL_ARMOR + 2:       // Armor +2
			// ... Adicione todos os modelos de Armor necessários

		case MODEL_PANTS:           // Pants (calças)
		case MODEL_PANTS + 1:       // Pants +1
		case MODEL_PANTS + 2:       // Pants +2
			// ... Adicione todos os modelos de Pants necessários

		case MODEL_GLOVES:         // Gloves (luvas)
		case MODEL_GLOVES + 1:      // Gloves +1
		case MODEL_GLOVES + 2:      // Gloves +2
			// ... Adicione todos os modelos de Gloves necessários

		case MODEL_BOOTS:           // Boots (botas)
		case MODEL_BOOTS + 1:       // Boots +1
		case MODEL_BOOTS + 2:       // Boots +2
	...
	gCustomEffects.SetInfo((DWORD)(&c->Object), b, Type, Level, c->PK);
...
void RenderCharacter(CHARACTER* c, OBJECT* o, int Select)
{
	...
	gMonsterEffect.CreateEffectMonster(o, b, c->MonsterIndex);//MONSTER
	...
	if ((gCharacterManager.GetCharacterClass(c->Class) >= CLASS_WIZARD || gCharacterManager.GetBaseClass(c->Class) <= CLASS_SUMMONER || gCustomWing.CheckCustomWingIsCape(c->Wing.Type) > 0
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
	...
	else if (o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && (o->SubType >= MODEL_NEW_TRANSFORM && o->SubType <= MAX_MODELS)) //RING TRANSFORM
	{
		RenderPartObject(&c->Object, o->SubType, NULL, c->Light, o->Alpha, 0, 0, 0, false, false, Translate, Select);
	}
	...
	else if (o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && o->SubType == MODEL_GM_CHARACTER) //RUNE EFFECT GM
	{
		RenderPartObject(&c->Object, o->SubType, NULL, c->Light, o->Alpha, 0, 0, 0, false, false, Translate, Select);
...
#endif// GUILD_WAR_EVENT
		{
			for (int i = MAX_BODYPART - 1; i >= 0; i--)
			{
				...
				if (p->Type != -1)
				{
					...
					if (o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && g_pOption->m_Equipments && i > 0 && strcmp(c->ID, Hero->ID) != 0)
					{
					...
					{
						b->Skin = gCharacterManager.GetBaseClass(c->Class) * 2 + IsThirdClass(c->Class);
					}
					else
#endif //PBG_ADD_NEWCHAR_MONK
...
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
								if (!g_pOption->m_Equipments || strcmp(c->ID, Hero->ID) == 0 || o->Kind == KIND_NPC || o->Kind == KIND_MONSTER)
								{
									RenderPartObject(&c->Object, Type, p, c->Light, o->Alpha, p->Level << 3, p->Option1, p->ExtOption, false, false, Translate, Select);
								}
	...
	gRuneColorEffect.IsRuneEffect((DWORD)c, (DWORD)o); //RUNE EFFECT
	...
	if (bCloak)
	{
		if (!c->Object.m_pCloth)
		{
			...
			else if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK) //MG CAPA
			{
				CPhysicsCloth* pCloth = new CPhysicsCloth[1];
			...
			else
			{
				...
				int CapeFront = gRingTransform.CRingTransformCapeFront(c->MonsterIndex); //RING TRANSFORM CAPE
				int CapeBack = gRingTransform.CRingTransformCapeBack(c->MonsterIndex);

				if (CapeFront > 0 || CapeBack > 0)
				{
					pCloth[0].Create(o, 19, 0.0f, 10.0f, 0.0f, 10, 10, /*45.0f*/75.0f, 120.0f, CapeFront, CapeBack, PCT_CURVED | PCT_SHORT_SHOULDER | PCT_HEAVY);
				}

				//ARMOR GOD
				/*
				if (c->BodyPart[BODYPART_ARMOR].Type == MODEL_ARMOR + 107)
				{
					pCloth[0].Create(o, 19, 0.0f, 15.0f, 0.0f, 10, 10, 120.0f, 120.0f, BITMAP_ROBE + 8, BITMAP_ROBE + 8, PCT_CURVED | PCT_SHORT_SHOULDER | PCT_HEAVY | PCT_MASK_ALPHA);
				}
				*/
		...
		{
			if ((c->BodyPart[BODYPART_ARMOR].Type != MODEL_BODY_ARMOR + CLASS_DARK_LORD && c->BodyPart[BODYPART_ARMOR].Type != MODEL_BODY_ARMOR + 14) && o->m_byNumCloth == 4)
			{
				if (o && o->m_pCloth)
				{
					CPhysicsCloth* pCloth = (CPhysicsCloth*)o->m_pCloth;
	...
	float Luminosity = (float)(rand() % 30 + 70) * 0.01f;
	if (c->PK >= PVP_MURDERER2) //LUZ DO PK
	{
		Vector(1.f, 0.1f, 0.1f, c->Light);
	}
	else if (c->PK == PVP_HERO2)  //LUZ DO HERO
	{
		Vector(0.1f, 0.1f, 1.f, c->Light);
	}
	else
	{
		VectorAdd(Light, o->Light, c->Light);
	...
	//CUSTOM MODEL ACTION
	int ItemIndex = gSetModelAction.IsSetModelAction((DWORD)c);

	if (ItemIndex != -1)
	{
		PART_t* parts[] = {
			&c->Weapon[0], &c->Weapon[1], &c->Wing, &c->Helper,
			&c->BodyPart[BODYPART_HELM], &c->BodyPart[BODYPART_ARMOR], &c->BodyPart[BODYPART_PANTS], &c->BodyPart[BODYPART_GLOVES], &c->BodyPart[BODYPART_BOOTS]
		};

		for (auto& part : parts)
		{
			for (std::map<int, CUSTOM_MODEL_ACTION>::iterator it = gSetModelAction.m_SetModelAction.begin(); it != gSetModelAction.m_SetModelAction.end(); it++)
			{
				auto ItemInfo = &(*it).second;

				if (part->Type == ItemIndex)
				{
					if (c->SafeZone)
					{
						part->CurrentAction = ItemInfo->ActionSafeZone;
						part->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_STOP_MALE_NEW].PlaySpeed;
					}
					else
					{
						if (c->Run)
							part->CurrentAction = ItemInfo->ActionRun;
						else
							part->CurrentAction = ItemInfo->ActionStop;
	...
	if (!Bind)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			if (i == 0)
			{
				if ((o->CurrentAction == PLAYER_ATTACK_SKILL_FURY_STRIKE || o->CurrentAction == PLAYER_FENRIR_SKILL_FURY_STRIKE) && o->AnimationFrame <= 4.f)
				{
					continue;
				}
			...
			{
				if (w->Type >= MODEL_SWORD && w->Type < MODEL_SHIELD + MAX_ITEM_INDEX)
				{
					...
					else if (w->Type == MODEL_MACE + 2)
					{
						...
						{
							w->CurrentAction = 1;
							w->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_STOP_MALE_NEW].PlaySpeed;
						}
					}
					else if (w->Type == MODEL_MACE + 5)
					{
						w->CurrentAction = 0;
						w->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_STOP_MALE_NEW].PlaySpeed * 2.f;
					}
					else if (w->Type == MODEL_STAFF + 6)
					{
						w->CurrentAction = 0;
						w->PlaySpeed = Models[MODEL_PLAYER].Actions[PLAYER_STOP_MALE_NEW].PlaySpeed * 15.f;
					}
	...
	case MODEL_PLAYER:
		...
		if (gCharacterManager.GetCharacterClass(c->Class) == CLASS_SOULMASTER) //Fix brilho no peito Soul Master
		{
			if (!g_isCharacterBuff(o, eBuff_Cloaking))
			{
		...
		if (gMapManager.InChaosCastle() == false)
		{
			...
			gCCustomSetEffect.CreateEffectSetPlayer(c, ObjectStruct, c->BodyPart[BODYPART_HELM].Type, c->BodyPart[BODYPART_HELM].Level, c->PK);
			gCCustomSetEffect.CreateEffectSetPlayer(c, ObjectStruct, c->BodyPart[BODYPART_ARMOR].Type, c->BodyPart[BODYPART_ARMOR].Level, c->PK);
			gCCustomSetEffect.CreateEffectSetPlayer(c, ObjectStruct, c->BodyPart[BODYPART_GLOVES].Type, c->BodyPart[BODYPART_GLOVES].Level, c->PK);
			gCCustomSetEffect.CreateEffectSetPlayer(c, ObjectStruct, c->BodyPart[BODYPART_PANTS].Type, c->BodyPart[BODYPART_PANTS].Level, c->PK);
			gCCustomSetEffect.CreateEffectSetPlayer(c, ObjectStruct, c->BodyPart[BODYPART_BOOTS].Type, c->BodyPart[BODYPART_BOOTS].Level, c->PK);
	...
	{
		int ItemIndex = gSummonEffect.IsSummonEffect((DWORD)c);
		...
		if (ItemIndex > -1)
		{
			for (std::map<int, CUSTOM_SUMMON_EFFECT>::iterator it = gSummonEffect.m_SummonEffectInfo.begin(); it != gSummonEffect.m_SummonEffectInfo.end(); it++)
			{
				...
				{
					gSummonEffect.MoveSummonEffect(c, ItemIndex);
				}
			}
		}

		int ItemIndexWrist = gSummonEffect.IsSummonEffect_Wrist((DWORD)c);
		...
		if (ItemIndexWrist > -1)
		{
			for (std::map<int, CUSTOM_SUMMON_EFFECT_WRIST>::iterator it = gSummonEffect.m_SummonEffectWristInfo.begin(); it != gSummonEffect.m_SummonEffectWristInfo.end(); it++)
			{
				...
				if (ItemInfo->ItemIndex == ItemIndexWrist)
				{
					gSummonEffect.MoveSummonEffect_Wrist(c, ItemIndexWrist);
				}
				else
				{
					//gSummonEffect.RemoveSummonEffect_Wrist(c);
				}
			}
		}
	}
...
void CreateCharacterPointer(CHARACTER* c, int Type, unsigned char PositionX, unsigned char PositionY, float Rotation)
{
	...
	{
		o->PriorAction = PLAYER_STOP_MALE_NEW;
		o->CurrentAction = PLAYER_STOP_MALE_NEW;
	}
	else
	{
		o->PriorAction = 0;
...
void SetCharacterClass(CHARACTER* c)
{
	...
	if (c == Hero)
	{
		...
		{
			if (CharacterMachine->Equipment[EQUIPMENT_HELM].Type == -1)
			{
				c->BodyPart[BODYPART_HELM].Type = MODEL_BODY_HELM + gCharacterManager.GetSkinModelIndex(c->Class);
			}
			else {
		...
		{
			if (CharacterMachine->Equipment[EQUIPMENT_ARMOR].Type == -1)
			{
				c->BodyPart[BODYPART_ARMOR].Type = MODEL_BODY_ARMOR + gCharacterManager.GetSkinModelIndex(c->Class);
			}
			else {
		...
		{
			if (CharacterMachine->Equipment[EQUIPMENT_PANTS].Type == -1)
			{
				c->BodyPart[BODYPART_PANTS].Type = MODEL_BODY_PANTS + gCharacterManager.GetSkinModelIndex(c->Class);
			}
			else {
		...
		{
			if (CharacterMachine->Equipment[EQUIPMENT_GLOVES].Type == -1)
			{
				c->BodyPart[BODYPART_GLOVES].Type = MODEL_BODY_GLOVES + gCharacterManager.GetSkinModelIndex(c->Class);
			}
			else {
		...
		{
			if (CharacterMachine->Equipment[EQUIPMENT_BOOTS].Type == -1)
			{
				c->BodyPart[BODYPART_BOOTS].Type = MODEL_BODY_BOOTS + gCharacterManager.GetSkinModelIndex(c->Class);
			}
			else {
		...
		if (CharacterMachine->Equipment[NEW_EQUIPMENT_HELPER].Type != -1)
		{
			...
			else
			{
				...
				c->Helper.Type = CharacterMachine->Equipment[NEW_EQUIPMENT_HELPER].Type + MODEL_ITEM;
				c->Helper.Level = (CharacterMachine->Equipment[NEW_EQUIPMENT_HELPER].Level >> 3) & 15;
				SetPlayerStop(c);
			}
		}
	}
...
void ChangeCharacterExt(int Key, BYTE* Equipment, CHARACTER* pCharacter, OBJECT* pHelper)
{
	...
	//fazer antilag de mudar itens aqui
		...
		if (lpCharList != 0)
		{
			...
			if (lpCharList->PetIndex > 0)
			{
				...
				if (gHelperSystem.CheckIsHelper(MODEL_HELPER + lpCharList->PetIndex))
				{
					...
					gHelperManager.CreateHelper(gItemManager.GET_ITEM(13, lpCharList->PetIndex), gHelperSystem.GetHelperModel(c->Helper.Type), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(c->Helper.Type));
				}
		...
		if (lpList != 0)
		{
			...
			if (lpList->PetIndex > 0)
			{
				...
				if (gHelperSystem.CheckIsHelper(MODEL_HELPER + lpList->PetIndex))
				{
					...
					gHelperManager.CreateHelper(gItemManager.GET_ITEM(13, lpList->PetIndex), gHelperSystem.GetHelperModel(c->Helper.Type), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(c->Helper.Type));
				}
			}
			...
			if (lpList->Element[0] > 0)
			{
				...
				{
					gElementPetFirst.CreateHelper(lpList->Element[0], gHelperSystem.GetHelperModel(lpList->Element[0] + MODEL_ITEM), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(lpList->Element[0] + MODEL_ITEM));
				}
			}
			if (lpList->Element[1] > 0)
			{
				...
				{
					gElementPetSecond.CreateHelper(lpList->Element[1], gHelperSystem.GetHelperModel(lpList->Element[1] + MODEL_ITEM), c->Object.Position, c, 0, gHelperSystem.GetHelperSubType(lpList->Element[1] + MODEL_ITEM));
				}
			}
		}
	}
...
void Setting_Monster(CHARACTER* c, int Type, int PositionX, int PositionY)
{
	...
	if (c != NULL)
	{
		...
		//CUSTOM MONSTER ITEM
		for (auto it = gCustomMonsterItem.m_CustomMonsterItem.begin(); it != gCustomMonsterItem.m_CustomMonsterItem.end(); ++it)
		{
			auto ItemInfo = &it->second;

			if (ItemInfo->Index == Type)
			{
				int Kind = ItemInfo->Kind;
				switch (Kind)
				{
				case 0:  Kind = KIND_PLAYER;	break;
				case 1:  Kind = KIND_MONSTER;	break;
				case 2:  Kind = KIND_NPC;		break;
				case 3:  Kind = KIND_TRAP;		break;
				case 4:  Kind = KIND_OPERATE;	break;
				case 5:  Kind = KIND_EDIT;		break;
				case 6:  Kind = KIND_PET;		break;
				case 7:  Kind = KIND_TMP;		break;
				default: Kind = KIND_PLAYER;	break;
				}
				o->Kind = Kind;
			}
		}
...
CHARACTER* CreateMonster(int Type, int PositionX, int PositionY, int Key)
{
	...
	case 9:
		...
		else
		{
			...
			c->Weapon[1].Type = MODEL_STAFF + 3;
			c->Wing.Type = MODEL_WING + 201;
			c->Level = 1;
			c->Object.Scale = 1.1f;
		}
		break;
	...
	for (auto it = gCustomMonsterItem.m_CustomMonsterItem.begin(); it != gCustomMonsterItem.m_CustomMonsterItem.end(); ++it)
	{
		...
		if (ItemInfo->Index == Type)
		{
			...
			//NOME
			strcpy(c->ID, ItemInfo->Name);

			//CLASSE
			int Class = ItemInfo->Class;
			switch (Class)
			{
			case 0:  Class = CLASS_WIZARD;				break;
			case 1:  Class = CLASS_KNIGHT;				break;
			case 2:  Class = CLASS_ELF;					break;
			case 3:  Class = CLASS_DARK;				break;
			case 4:  Class = CLASS_DARK_LORD;			break;
			case 5:  Class = CLASS_SUMMONER;			break;
			case 6:  Class = CLASS_SOULMASTER;			break;
			case 7:  Class = CLASS_BLADEKNIGHT;			break;
			case 8:  Class = CLASS_MUSEELF;				break;
			case 9:  Class = CLASS_BLOODYSUMMONER;		break;
			case 10: Class = CLASS_GRANDMASTER;			break;
			case 11: Class = CLASS_BLADEMASTER;			break;
			case 12: Class = CLASS_HIGHELF;				break;
			case 13: Class = CLASS_DUELMASTER;			break;
			case 14: Class = CLASS_LORDEMPEROR;			break;
			case 15: Class = CLASS_DIMENSIONMASTER;		break;
			default: Class = -1;						break;
			}

			//SIZE
			float Scale = ItemInfo->Size;
			float Edge = ItemInfo->Edge;
			c->Object.Scale = (Scale == -1) ? 0.9f : Scale;
			c->Object.m_fEdgeScale = (Edge == -1) ? 0.9f : Edge;

			//ANIMATION / ACTION
			int Action = ItemInfo->Action;
			c->Object.CurrentAction = (Action == -1) ? 0 : Action;

			//LEVEL
			int ItemLevel = ItemInfo->Level;
			c->Weapon[0].Level = ItemLevel;
			c->Weapon[1].Level = ItemLevel;
			c->Wing.Level = ItemLevel;
			c->Helper.Level = ItemLevel;

			//ITENS
			c->Weapon[0].Type = ItemInfo->WeaponR;
			c->Weapon[1].Type = ItemInfo->WeaponL;
			c->Wing.Type = ItemInfo->Wings;
			c->Helper.Type = ItemInfo->Helper;

			//c->Object.Angle[2] = 0.f;
			//CreateObject(MODEL_BC_BOX, c->Object.Position, c->Object.Angle);
			//c->Object.EnableShadow = false;
			c->Object.m_bRenderShadow = ItemInfo->Shadow;

			//CORPO
			int bodyParts[5] = { ItemInfo->Helm, ItemInfo->Armor, ItemInfo->Pants, ItemInfo->Gloves, ItemInfo->Boots };

			int modelBodyParts[5] = { MODEL_BODY_HELM, MODEL_BODY_ARMOR, MODEL_BODY_PANTS, MODEL_BODY_GLOVES, MODEL_BODY_BOOTS };

			for (int i = 0; i < 5; ++i) {
				if (Class > -1) {
					if (bodyParts[i] == -1) {
						c->BodyPart[i].Type = modelBodyParts[i] + Class;
						c->BodyPart[i].Level = 0;
					}
					else {
						c->BodyPart[i].Type = bodyParts[i];
						c->BodyPart[i].Level = ItemLevel;
					}
				}
				else {
					c->BodyPart[i].Type = bodyParts[i];
	...
	//RING TRANSFORM
	if (Type >= MODEL_NEW_TRANSFORM && Type <= MAX_MODELS) {
		c = CreateCharacter(Key, MODEL_PLAYER, PositionX, PositionY);
		strcpy(c->ID, "VoxMu");
		c->Object.SubType = Type;
		c->Object.Scale = gRingTransform.CRingTransformSize(Type);
	}
...
bool RenderCharacterBackItem(CHARACTER* c, OBJECT* o, bool bTranslate)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		{
			PART_t* w = &c->Wing;
			if (g_pOption->m_Wings)
			{
				DeleteCloth(c);
			}

			if (w->Type != -1 && !g_pOption->m_Wings)
			{
				w->LinkBone = 47;
				if (o->CurrentAction == PLAYER_FLY || o->CurrentAction == PLAYER_FLY_CROSSBOW)
				{
					if (w->Type >= MODEL_WING + 0)
					{
						w->PlaySpeed = 0.4f; // WING VOANDO
					}
					else
					{
						w->PlaySpeed = 0.0f;
				...
				{
					w->PlaySpeed = 0.09f; // WING NA CIDADE
				}

				if (w->Type == MODEL_WING + 40 || gCustomWing.CheckCustomWingIsCape(w->Type) > 0)
				{
					w->LinkBone = 19;
