...
void CNewUITrade::RenderBackImage()
{
	...
	//RenderBitmap(IMAGE_TRADE_BACK_VOID, m_Pos.x, m_Pos.y, TRADE_WIDTH, TRADE_HEIGHT, 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
...
int CNewUITrade::FindEmptySlot(ITEM* pItemObj)
{
	if (pItemObj == NULL)
		return -1;
	...
	if (m_pMyInvenCtrl)
		return m_pMyInvenCtrl->FindEmptySlot(pItemAttr->Width, pItemAttr->Height);

	return -1;
}

void CNewUITrade::MoveItemFromInventory()
{
	if (g_pPickedItem)
...
void CNewUITrade::ProcessItemAutoMove()
{
	if (g_pPickedItem)
		if (g_pPickedItem->GetItem())
			return;
	...
	if (pItemObj)
	{
		...
		if (-1 != nDstIndex)
		{
			...
			int nSrcIndex
				= pItemObj->y * m_pMyInvenCtrl->GetNumberOfColumn()
				+ pItemObj->x;
...
void CNewUITrade::SetItemAutoMove(bool bItemAutoMove)
{
...
void CNewUITrade::ProcessItemAutoMoveSuccess()
{
	if (!IsVisible())
		return;
	...
	if (IsItemAutoMove())
	{
		ITEM* pItemObj = m_pMyInvenCtrl->FindItemAtPt(m_nBackupMouseX, m_nBackupMouseY);
...
void CNewUITrade::ProcessItemAutoMoveFailure()
{
	if (!IsVisible())
		return;
