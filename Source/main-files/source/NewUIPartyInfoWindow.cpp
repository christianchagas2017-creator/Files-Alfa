...
#elif CLIENT_VERSION >= 1001
		if (iVal > 0) iVal -= 2;
		int iPosY = m_Pos.y + 45 + iVal;
		m_BtnPartyExit[i].ChangeButtonInfo( m_Pos.x+155, iPosY+2, 11, 11 );
#endif
	}
}
...
#elif CLIENT_VERSION >= 1001
	//RenderBitmap(IMAGE_PARTY_BACK_VOID, m_Pos.x, m_Pos.y, float(PARTY_INFO_WINDOW_WIDTH), float(PARTY_INFO_WINDOW_HEIGHT), 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
	RenderImageScaleAuto(IMAGE_PARTY_BACK_VOID, m_Pos.x, m_Pos.y, float(PARTY_INFO_WINDOW_WIDTH), float(PARTY_INFO_WINDOW_HEIGHT));
#endif
...
#elif CLIENT_VERSION >= 1001
	g_pRenderText->RenderText(m_Pos.x+60, m_Pos.y+14, GlobalText[190], 72, 0, RT3_SORT_CENTER);	
#endif
...
void CNewUIPartyInfoWindow::RenderGroupBox( int iPosX, int iPosY, int iWidth, int iHeight, int iTitleWidth/* =60 */, int iTitleHeight/* =20  */ )
{
	...
	RenderColor(iPosX, iPosY, iWidth, iTitleHeight);
	glColor4f(0.0f, 0.0f, 0.0f, 0.4f);
	RenderColor(iPosX, iPosY + iTitleHeight, iWidth, iHeight);
	EndRenderColor();
	//RenderImageScale(IMAGE_PARTY_FRAME, iPosX, iPosY, iWidth, iHeight+iTitleHeight, 0.f, 0.f, 330.f, 53.f);
...
void CNewUIPartyInfoWindow::RenderMemberStatue( int iIndex, PARTY_t* pMember, bool bExitBtnRender /*= false*/)
{
	...
	if (iVal > 0) iVal -= 2;
	int iPosY = m_Pos.y+45+iVal;
	RenderGroupBox( iPosX+10, iPosY, 150, 45, 150, 15 );
	g_pRenderText->SetFont(g_hFontMediumBold);
	g_pRenderText->SetTextColor(247, 128, 0, 255);
	if (iIndex == 0)
	{
		RenderImage(IMAGE_PARTY_FLAG, m_Pos.x+25, iPosY+1.5f, 10, 12);
	}

	g_pRenderText->RenderText(iPosX+10, iPosY + 2, pMember->Name, 150, 0, RT3_SORT_CENTER);
#endif
	...
	g_pRenderText->RenderText(iPosX + 13, iPosY + 17, gMapManager.GetMapName(pMember->Map), 150, 0, RT3_SORT_LEFT);

	unicode::_sprintf(szText, "Coord X: %d", pMember->x);
	g_pRenderText->RenderText(iPosX + 13, iPosY + 25, szText, 150, 0, RT3_SORT_LEFT);
	unicode::_sprintf(szText, "Coord Y: %d", pMember->y);
	g_pRenderText->RenderText(iPosX + 13, iPosY + 33, szText, 150, 0, RT3_SORT_LEFT);

	int iHP = (pMember->currHP * 144) / pMember->maxHP;
	RenderImageScale(IMAGE_PARTY_HPBAR_BACK, iPosX + 13, iPosY + 44, 144, 6, 0.f, 0.f, 333.f, 14.f);
	RenderImageScale(IMAGE_PARTY_HPBAR, iPosX + 13, iPosY + 44, iHP, 6, 0.f, 0.f, 333.f, 14.f);
	//

	g_pRenderText->SetFont(g_hFontSmall);
	g_pRenderText->SetTextColor(RGBA(237, 214, 161, 255));
	unicode::_sprintf(szText, "%d %s %d", pMember->currHP, GlobalText[2374], pMember->maxHP);
	g_pRenderText->RenderText(iPosX + 157, iPosY + 51, szText, 150, 0, RT3_WRITE_RIGHT_TO_LEFT);
#endif
...
void CNewUIPartyInfoWindow::LoadImages()
{
...
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_PARTY_BASE_WINDOW_BTN_EXIT, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_msgbox_back.jpg", IMAGE_PARTY_BASE_WINDOW_BACK, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back01.tga", IMAGE_PARTY_BASE_WINDOW_TOP, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-L.tga", IMAGE_PARTY_BASE_WINDOW_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_PARTY_BASE_WINDOW_RIGHT, GL_LINEAR);
	...
	LoadBitmap("Interface\\party_info_life_back.jpg", IMAGE_PARTY_HPBAR_BACK, GL_LINEAR);
	LoadBitmap("Interface\\party_info_life_top.jpg", IMAGE_PARTY_HPBAR, GL_LINEAR);
	LoadBitmap("Interface\\newui_party_flag.tga", IMAGE_PARTY_FLAG, GL_LINEAR);
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_PARTY_EXIT, GL_LINEAR);
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_PARTY_BACK_VOID, GL_LINEAR);
	LoadBitmap("Interface\\frame_box.jpg", IMAGE_PARTY_FRAME, GL_LINEAR);
#endif
...
void CNewUIPartyInfoWindow::UnloadImages()
{
	...
	DeleteBitmap(IMAGE_PARTY_BACK_VOID);
	DeleteBitmap(IMAGE_PARTY_FRAME);
#endif
}
