...
void SEASON3B::CNewUIRenderNumber::LoadImage()
{
	...
	LoadBitmap("Interface\\old_number.tga", IMAGE_NUMBER2, GL_LINEAR);
...
#if CLIENT_VERSION >= 1001
float SEASON3B::CNewUIRenderNumber::RenderOldNumber(float x, float y, int iNum, float fScale)
{
	EnableAlphaTest();
	...
	char strText[32];
	_itoa(iNum, strText, 10);
	int iLength = (int)strlen(strText);

	x -= (double)iLength * width / 2.0;

	for (int i = 0; i < iLength; ++i)
	{
		float fU = (float)(strText[i] - 48) * 16.f / 256.f;
		RenderBitmap(IMAGE_NUMBER2, x, y, width, height, fU, 0, 0.0625, 0.5, 1, 1, 0.0);
		x += width * 0.8f;
	}
