...
void ItemConvert(ITEM *ip,BYTE Attribute1,BYTE Attribute2, BYTE Attribute3 )
{
	...
	else if ((ip->Type>=ITEM_WING+36 && ip->Type<=ITEM_WING+40) || ip->Type == ITEM_WING+43 || gCustomWing.CheckCustomWingByItem(ip->Type)
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
			|| (ip->Type == ITEM_WING+50)
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
		)
	...
	//part
	if(ip->Type>=ITEM_BOW && ip->Type<ITEM_BOW+8 || ip->Type==ITEM_BOW+17 || gCustomBow.CheckIsBow(ip->Type) == 1)
...
int ItemValue(ITEM *ip,int goldType)
{
	...
	else if(gCustomJewel.CheckCustomJewelByItem(ip->Type))
	{
...
void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	...
	*/
	...
	float magicspeed = 8.f;

	switch (CharacterClass) {
	case CLASS_WIZARD:
		attackspeed = gProtect->m_MainInfo.m_AttackSpeedDW / 1000;
		magicspeed = gProtect->m_MainInfo.m_MagicSpeedDW / 1000;
		break;
	case CLASS_KNIGHT:
		attackspeed = gProtect->m_MainInfo.m_AttackSpeedDK / 1000;
		magicspeed = gProtect->m_MainInfo.m_MagicSpeedDK / 1000;
		break;
	case CLASS_ELF:
		attackspeed = gProtect->m_MainInfo.m_AttackSpeedELF / 1000;
		magicspeed = gProtect->m_MainInfo.m_MagicSpeedELF / 1000;
		break;
	case CLASS_DARK:
		attackspeed = gProtect->m_MainInfo.m_AttackSpeedMG / 1000;
		magicspeed = gProtect->m_MainInfo.m_MagicSpeedMG / 1000;
		break;
	case CLASS_DARK_LORD:
		attackspeed = gProtect->m_MainInfo.m_AttackSpeedDL / 1000;
		magicspeed = gProtect->m_MainInfo.m_MagicSpeedDL / 1000;
		break;
	case CLASS_SUMMONER:
		attackspeed = gProtect->m_MainInfo.m_AttackSpeedSUM / 1000;
		magicspeed = gProtect->m_MainInfo.m_MagicSpeedSUM / 1000;
		break;
	}
	...
	Character.AttackSpeed = Character.AttackSpeed / attackspeed; // -> 16.8f (Fix 380lvl item)
	Character.MagicSpeed = Character.MagicSpeed / magicspeed; // -> 16.8f (Fix 380lvl item)
