...
void SEASON3B::CNewUIInventoryCtrl::LoadImages()
{
	...
	LoadBitmap("Interface\\item_box_selection.jpg", IMAGE_ITEM_SQUARE_OCUPADO, GL_LINEAR, GL_CLAMP);
	LoadBitmap("Interface\\item_box_normal.jpg", IMAGE_ITEM_SQUARE_VAZIO, GL_LINEAR, GL_CLAMP);
#endif
...
#if CLIENT_VERSION >= 1001
	DeleteBitmap(IMAGE_ITEM_SQUARE_OCUPADO);
	DeleteBitmap(IMAGE_ITEM_SQUARE_VAZIO);
#endif
}
...
#if CLIENT_VERSION >= 1001
			EnableAlphaTest();
			//RenderBitmap(IMAGE_ITEM_SQUARE_VAZIO, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 20, 20, 0.0, 0.00, 0.678, 0.686, 1, 1, 0.0);
			RenderImageScale(IMAGE_ITEM_SQUARE_VAZIO, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 19.9, 19.9, 0.f, 0.f, 43.77f,43.77f);
#endif
...
#if CLIENT_VERSION >= 1001
					EnableAlphaTest();
					//RenderBitmap(IMAGE_ITEM_SQUARE_OCUPADO, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 20, 20, 0.0, 0.00, 0.678, 0.686, 1, 1, 0.0);
					RenderImageScale(IMAGE_ITEM_SQUARE_OCUPADO, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 19.9, 19.9, 0.f, 0.f, 44.f, 44.f);
#endif
...
void SEASON3B::CNewUIInventoryCtrl::BackupPickedItem()
{
	if (ms_pPickedItem && EquipmentItem == false)
	{
		...
		else if (pItemObj->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
		{
			...
			if (pItemObj->lineal_pos >= NEW_EQUIPMENT_HELPER_FLY && pItemObj->lineal_pos <= 239)
			{
				gVisualInventory.EffectPet(pItemObj->lineal_pos, pEquipmentItemSlot);
			}
			if (pItemObj->lineal_pos >= NEW_EQUIPMENT_WEAPON_RIGHT && pItemObj->lineal_pos <= NEW_EQUIPMENT_RING_RIGHT)
			{
				g_pMyInventory->CreateEquippingEffect(pEquipmentItemSlot);
				if (pItemObj->lineal_pos == NEW_EQUIPMENT_HELPER)
...
bool SEASON3B::CNewUIInventoryCtrl::IsOverlayItem(ITEM* pSourceItem, ITEM* pTargetItem)
{
	...
	if(iSrcType == iTarType)
	{
		...
		else if (iSrcType == iTarType && gJewelStack.CheckIsStack(iSrcType) && (iSrcDurability+iTarDurability < 255)) {
			return true;
		}
	}
