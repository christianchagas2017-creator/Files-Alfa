...
static BOOL LuaLoadWing(lua_State * Lua)
{
	...
	strcpy_s(info.Dir, lua_tostring(Lua, 23));
	
	strcpy_s(info.ModelName, lua_tostring(Lua, 24));

	info.Type = (int)lua_tointeger(Lua, 25);
...
void CCustomWing::Init() // OK
{
	...
	sprintf_s(name, "%s\\Configs\\CustomWings.lua", LUA_LOAD_PATH);
...
void CCustomWing::LoadWing(int ItemIndex)
{
	for (std::map<int, CUSTOM_WING_INFO>::iterator it = this->m_CustomWingInfo.begin(); it != this->m_CustomWingInfo.end(); it++)
	{
		auto lpInfo = &(*it).second;

		if (lpInfo->ItemIndex == (ItemIndex-MODEL_ITEM)) {
			if (lpInfo->Type == 0)
			{
