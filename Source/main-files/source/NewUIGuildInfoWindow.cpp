...
bool SEASON3B::CNewUIGuildInfoWindow::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_MoveWindow.Create(x, y, GUILDINFO_WIDTH, GUILDINFO_HEIGHT);
	SetPos(x, y);
	...
	m_Button = new CCustomButtons[BUTTON_END];
#endif
...
#elif CLIENT_VERSION >= 1001
		m_Button[i].ChangeButtonInfo(0, 0, 85, 25);
		m_Button[i].SetButtonSize(1);
#endif
...
#elif CLIENT_VERSION >= 1001
	m_Button[BUTTON_GET_OUT].SetPos(m_Pos.x + 52.5f, m_Pos.y + 325);
	m_Button[BUTTON_GET_POSITION].SetPos(m_Pos.x + 52.5f, m_Pos.y + 355);
	m_Button[BUTTON_FREE_POSITION].SetPos(m_Pos.x + 52.5f, m_Pos.y + 385);

	m_Button[BUTTON_GET_POSITION].ChangeTextColor(RGBA(245, 232, 202, 255));
	m_Button[BUTTON_FREE_POSITION].ChangeTextColor(RGBA(245, 232, 202, 255));
	m_Button[BUTTON_GET_OUT].ChangeTextColor(RGBA(245, 232, 202, 255));

	m_Button[BUTTON_GET_POSITION].SetFont(g_hFontBold);
	m_Button[BUTTON_FREE_POSITION].SetFont(g_hFontBold);
	m_Button[BUTTON_GET_OUT].SetFont(g_hFontBold);

	m_Button[BUTTON_UNION_CREATE].SetPos(m_Pos.x + 30, m_Pos.y + 180);
	m_Button[BUTTON_UNION_CREATE].SetSize(130, 25);
	m_Button[BUTTON_UNION_OUT].SetPos(m_Pos.x + 30, m_Pos.y + 180);
	m_Button[BUTTON_UNION_OUT].SetSize(130, 25);

	m_Button[BUTTON_GUILD_OUT].ChangeTextColor(RGBA(245, 232, 202, 255));
	m_Button[BUTTON_GUILD_OUT].SetFont(g_hFontBold);
	m_Button[BUTTON_GUILD_OUT].SetPos(m_Pos.x + 52.5f, m_Pos.y + 350);
	m_Button[BUTTON_GUILD_OUT].SetSize(85, 25);
#endif
...
bool SEASON3B::CNewUIGuildInfoWindow::Check_Btn()
{
	...
	else if(m_nCurrentTab == 2)
	{
		...
		if(Hero->GuildStatus == G_MASTER && !strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName))
		{
		...
		if( Hero->GuildStatus == G_MASTER)
		{
			if(m_Button[BUTTON_UNION_OUT].UpdateMouseEvent())
			{
				if( !strcmp( GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName ) )
				{
...
bool SEASON3B::CNewUIGuildInfoWindow::Check_Mouse(int mx,int my)
{
...
#elif CLIENT_VERSION >= 1001
			if(mx > (m_Pos.x+12+Tab_Pos) && mx < (m_Pos.x+12+Tab_Pos+56) && my > m_Pos.y+27 && my < (m_Pos.y+49))
#endif
			{
				if(m_nCurrentTab != i)
				{
	...
	{
		if((MouseX > m_Pos.x+156 && MouseX < (m_Pos.x+171) && MouseY > m_Pos.y+ 82 + m_Loc && MouseY < (m_Pos.y+132 + m_Loc)) && m_EventState == EVENT_NONE)
		{
			m_EventState = EVENT_SCROLL_BTN_DOWN;
			if(m_BackUp == 0)
				m_BackUp	= (82 + (MouseY - 82));
			return false;
		}
	}
	return true;
...
#elif CLIENT_VERSION >= 1001
	//RenderBitmap(IMAGE_GUILDINFO_BACK_VOID, m_Pos.x, m_Pos.y, GUILDINFO_WIDTH, GUILDINFO_HEIGHT, 0.0, 0.0, 0.83, 0.935, 1, 1, 0.0);
	RenderImageScaleAuto(IMAGE_GUILDINFO_BACK_VOID, m_Pos.x, m_Pos.y, GUILDINFO_WIDTH, GUILDINFO_HEIGHT);
...
#elif CLIENT_VERSION >= 1001
	RenderImageScale(IMAGE_GUILDINFO_TAB_LIST, m_Pos.x + 14, m_Pos.y + 27, 160.f, 22.f, 0.f, 0.f, 330.f, 44.f);
	switch (m_nCurrentTab) {
	case 0:
		RenderImageScale(IMAGE_GUILDINFO_TAB_POINT, m_Pos.x + 15.5f, m_Pos.y + 32, 50.5f, 15.f, 0.f, 0.f, 104.f, 30.f);
		break;
	case 1:
		RenderImageScale(IMAGE_GUILDINFO_TAB_POINT, m_Pos.x + 68.7f, m_Pos.y + 32, 50.5f, 15.f, 0.f, 0.f, 104.f, 30.f);
		break;
	case 2:
		RenderImageScale(IMAGE_GUILDINFO_TAB_POINT, m_Pos.x + 122.3f, m_Pos.y + 32, 50.5f, 15.f, 0.f, 0.f, 104.f, 30.f);
		break;
	}
#endif
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Text()
{
	...
	//g_pRenderText->SetTextColor(245, 232, 202, 255);
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 14, Text, 190, 0, RT3_SORT_CENTER);
	g_pRenderText->SetFont(g_hFont);
	g_pRenderText->SetTextColor(backuptextcolor);
#endif
...
#elif CLIENT_VERSION >= 1001
	RenderText( Text, m_Pos.x+13+(0*55), m_Pos.y+35, 55, 0, 0xFFFFFFFF, 0x00000000, RT3_SORT_CENTER );
#endif
	glColor4f( 0.6f, 0.6f, 0.6f, 1.f );
...
#elif CLIENT_VERSION >= 1001
	RenderText( Text, m_Pos.x+13+(1*55), m_Pos.y+35, 55, 0, 0xFFFFFFFF, 0x00000000, RT3_SORT_CENTER );
#endif
	glColor4f( 0.6f, 0.6f, 0.6f, 1.f );
...
#elif CLIENT_VERSION >= 1001
	RenderText( Text, m_Pos.x+13+(2*55), m_Pos.y+35, 55, 0, 0xFFFFFFFF, 0x00000000, RT3_SORT_CENTER );
#endif
	glColor4f( 0.6f, 0.6f, 0.6f, 1.f );
	...
	if( m_nCurrentTab == 0 ) 
	{
...
#elif CLIENT_VERSION >= 1001
		int Nm_Loc = m_Pos.y + 57;
		glColor4f(1.0, 1.0, 1.0, 1.0);
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->SetBgColor(0, 0, 0, 0);
		g_pRenderText->SetTextColor(200, 255, 100, 255);
		sprintf(Text, "%s", GuildMark[Hero->GuildMarkIndex].GuildName);
		g_pRenderText->RenderText(m_Pos.x, Nm_Loc, Text, 190, 0, RT3_SORT_CENTER);
		Nm_Loc += 103;
		...
		sprintf(Text, GlobalText[1323]);
		g_pRenderText->RenderText(m_Pos.x + 14.5f, m_Pos.y + 238.f, Text, 159, 0, RT3_SORT_CENTER);

		g_pRenderText->SetFont(g_hFont);
		g_pRenderText->SetTextColor(255, 255, 255, 255);
		g_pRenderText->SetBgColor(0, 0, 0, 0);
		m_GuildNotice.SetSize(160, 80);
		m_GuildNotice.SetPosition(m_Pos.x + 15, m_Pos.y + 250 + m_GuildNotice.GetHeight());
		m_GuildNotice.Render();
#endif
	...
	else if( m_nCurrentTab == 1 )
	{
		...
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->SetTextColor(247, 128, 0, 255);		
		g_pRenderText->RenderText(m_Pos.x + 36, m_Pos.y + 65, GlobalText[1389], 40, 0, RT3_SORT_LEFT);
		g_pRenderText->RenderText(m_Pos.x + 120, m_Pos.y + 65, GlobalText[1448], 40, 0, RT3_SORT_LEFT);
		//g_pRenderText->RenderText(m_Pos.x + 150, m_Pos.y + 65, GlobalText[1022], 50, 0, RT3_WRITE_RIGHT_TO_LEFT);
		g_pRenderText->SetFont(g_hFont);
		g_pRenderText->SetTextColor(backuptextcolor);

		m_GuildMember.SetSize(140, 420);
		m_GuildMember.SetPosition(m_Pos.x + 20, m_Pos.y + 80 + m_GuildMember.GetHeight());
		m_GuildMember.Render();
	...
	if(m_nCurrentTab == 2)
	{
...
#elif CLIENT_VERSION >= 1001
		m_Button[BUTTON_UNION_CREATE].SetFont(g_hFontBold);
		if (m_Button[BUTTON_UNION_CREATE].IsLock()) {
			m_Button[BUTTON_UNION_CREATE].ChangeTextColor(RGBA(245, 232, 202, 175));
		}
		else {
			m_Button[BUTTON_UNION_CREATE].ChangeTextColor(RGBA(245, 232, 202, 255));
		}
		m_Button[BUTTON_UNION_OUT].SetFont(g_hFontBold);
		m_Button[BUTTON_UNION_OUT].ChangeTextColor(RGBA(245, 232, 202, 255));
#endif
		...
		if( GuildMark[Hero->GuildMarkIndex].UnionName[0] != NULL )
		{
			...
			g_pRenderText->SetFont(g_hFontBold);
			g_pRenderText->SetTextColor(247, 128, 0, 255);
			g_pRenderText->RenderText(m_Pos.x + 35, m_Pos.y + 65, (char*)GlobalText[182]);
			g_pRenderText->RenderText(m_Pos.x + 165, m_Pos.y + 65, (char*)GlobalText[1330], 0, 0, RT3_WRITE_RIGHT_TO_LEFT);
			g_pRenderText->SetFont(g_hFont);
			g_pRenderText->SetTextColor(backuptextcolor);
#endif
		}
...
#elif CLIENT_VERSION >= 1001
	RenderImageScale(IMAGE_GUILDINFO_FRAME_BOX_BIG, m_Pos.x + 70, m_Pos.y + 75, 50.f, 50.f, 0.f, 0.f, 330.f, 212.f);
	CreateGuildMark(Hero->GuildMarkIndex);
	RenderBitmap(BITMAP_GUILD, m_Pos.x + 76.f, m_Pos.y + 80.5f, 39, 39);

	RenderImageScale(IMAGE_GUILDINFO_FRAME_BOX_BIG, m_Pos.x + 10, m_Pos.y + 145.f, 168.f, 70.f, 0.f, 0.f, 330.f, 212.f);

	glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
	RenderColor(m_Pos.x + 14.5f, m_Pos.y + 235.f, 159.f, 15);
	glColor4f(0.0f, 0.0f, 0.0f, 0.4f);
	RenderColor(m_Pos.x + 14.5f, m_Pos.y + 250.f, 159.f, 83);
...
void SEASON3B::CNewUIGuildInfoWindow::RenderScrollBar()
{
	...
	RenderImage(IMAGE_GUILDINFO_SCROLL_TOP, m_Pos.x+160, 82, 7, 3);
	
	for(int i=0; i<22; i++)
		RenderImage(IMAGE_GUILDINFO_SCROLL_MIDDLE, m_Pos.x+160, 81+(float)(i*10+4), 7, 15);
	
	RenderImage(IMAGE_GUILDINFO_SCROLL_BOTTOM, m_Pos.x+160, 309, 7, 3);
	...
	if (m_EventState == EVENT_SCROLL_BTN_DOWN) 
		...
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	RenderImage(IMAGE_GUILDINFO_SCROLLBAR_ON, m_Pos.x+156, 82 + m_Loc, 15, 30);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
#elif CLIENT_VERSION >= 1001
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);

	RenderImageScale(IMAGE_GUILDINFO_NEW_SCROLL_BACK, m_Pos.x + 155, 81, 15, 233, 0.f, 0.f, 30.f, 375.f);
	...
	if (m_EventState == EVENT_SCROLL_BTN_DOWN)
		...
		glColor4f(0.7f, 0.7f, 0.7f, 1.0f);
	RenderImageScale(IMAGE_GUILDINFO_NEW_SCROLL_MIDDLE, m_Pos.x + 156.f, 82 + m_Loc, 13, 30, 0.f, 0.f, 30.f, 60.f);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
#endif
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Guild_Info()
{
	...
	else
	{
		...
		//RenderImageScale(IMAGE_GUILDINFO_FRAME_BOX_LARGE, m_Pos.x + 13, m_Pos.y + 58.f, 162.f, 105.f, 0.f, 0.f, 330.f, 106.f);

		glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
		RenderColor(m_Pos.x + 14.5f, m_Pos.y + 61.5f, 159.f, 15);
		glColor4f(0.0f, 0.0f, 0.0f, 0.4f);
		RenderColor(m_Pos.x + 14.5f, m_Pos.y + 76.5f, 159.f, 83);
		
		EndRenderColor();

		m_UnionListBox.SetSize(160, 80);
		m_UnionListBox.SetPosition(m_Pos.x + 20, m_Pos.y + 160);
		m_UnionListBox.Render();
#endif
		...
		if (Hero->GuildStatus == G_MASTER && !strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName))
		{
			if (UNIONGUILD_TEXT* pText = m_UnionListBox.GetSelectedText())
			{
				if (strcmp(pText->szName, GuildMark[Hero->GuildMarkIndex].GuildName))
				{
					m_Button[BUTTON_UNION_CREATE].UnLock();
#if CLIENT_VERSION >= 1001
					m_Button[BUTTON_UNION_CREATE].ForceButtonDisabled(false);
#endif
				}
				else {
					m_Button[BUTTON_UNION_CREATE].Lock();
#if CLIENT_VERSION >= 1001
					m_Button[BUTTON_UNION_CREATE].ForceButtonDisabled(true);
#endif
				}
			}

			m_Button[BUTTON_UNION_CREATE].Render(); // remove outra guild da aliança
		}
		
		if (Hero->GuildStatus == G_MASTER && strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName))
		{
			m_Button[BUTTON_UNION_OUT].UnLock();
			m_Button[BUTTON_UNION_OUT].Render(); // remove minha guild da aliança
		}
		else {
			m_Button[BUTTON_UNION_OUT].Lock();
		}
	}
}
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Guild_Enum()
{
...
#elif CLIENT_VERSION >= 1001
	//RenderImageScale(IMAGE_GUILDINFO_FRAME_BOX_BIG_2, m_Pos.x + 13, m_Pos.y + 58.f, 162.f, 242.f, 0.f, 0.f, 330.f, 318.f);

	glColor4f(0.0f, 0.0f, 0.0f, 0.8f);
	RenderColor(m_Pos.x + 14.5f, m_Pos.y + 61.5f, 159.f, 15);
	glColor4f(0.0f, 0.0f, 0.0f, 0.4f);
	RenderColor(m_Pos.x + 14.5f, m_Pos.y + 76.5f, 159.f, 242.f);
	EndRenderColor();
#endif
	...
	if(gGuildManager.IsGuildMaster())
	{
		if (GUILDLIST_TEXT* pText = m_GuildMember.GetSelectedText())
		{
			...
			int iCharacterLevel;
			if (pText->m_GuildStatus == 128)
				iCharacterLevel = 0;
			else if (pText->m_GuildStatus == 64)
				iCharacterLevel = 1;
			else if (pText->m_GuildStatus == 32)
				iCharacterLevel = 2;
			else
			iCharacterLevel = 3;
		
			if (iCharacterLevel == 3)
			{
				m_Button[BUTTON_FREE_POSITION].Lock();
				m_Button[BUTTON_GET_OUT].UnLock();
				m_Button[BUTTON_GET_POSITION].UnLock();
#if CLIENT_VERSION >= 1001
				m_Button[BUTTON_GET_OUT].ForceButtonDisabled(false);
				m_Button[BUTTON_GET_POSITION].ForceButtonDisabled(false);
#endif
				m_Button[BUTTON_GET_POSITION].ChangeTextColor(RGBA(245, 232, 202, 255));
				m_Button[BUTTON_GET_OUT].ChangeTextColor(RGBA(245, 232, 202, 255));
			...
			else if(iCharacterLevel > 0) {
				m_Button[BUTTON_FREE_POSITION].UnLock();
				m_Button[BUTTON_GET_OUT].UnLock();
				m_Button[BUTTON_GET_POSITION].UnLock();
#if CLIENT_VERSION >= 1001
				m_Button[BUTTON_GET_OUT].ForceButtonDisabled(false);
				...
				m_Button[BUTTON_GET_OUT].Lock();
				m_Button[BUTTON_GET_POSITION].Lock();
				m_Button[BUTTON_FREE_POSITION].Lock();
#if CLIENT_VERSION >= 1001
				m_Button[BUTTON_GET_OUT].ForceButtonDisabled(true);
				m_Button[BUTTON_GET_POSITION].ForceButtonDisabled(true);
#endif
				m_Button[BUTTON_GET_POSITION].ChangeTextColor(RGBA(245, 232, 202, 150));
				m_Button[BUTTON_GET_OUT].ChangeTextColor(RGBA(245, 232, 202, 150));
...
void SEASON3B::CNewUIGuildInfoWindow::LoadImages()
{
...
#elif CLIENT_VERSION >= 1001
	LoadBitmap("Interface\\btn_exit.jpg", IMAGE_GUILDINFO_EXIT_BTN, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_msgbox_back.jpg", IMAGE_GUILDINFO_BACK, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back01.tga", IMAGE_GUILDINFO_TOP, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-L.tga", IMAGE_GUILDINFO_LEFT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back02-R.tga", IMAGE_GUILDINFO_RIGHT, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_back03.tga", IMAGE_GUILDINFO_BOTTOM, GL_LINEAR);
	LoadBitmap("Interface\\guild_tab.tga", IMAGE_GUILDINFO_TAB_LIST, GL_LINEAR);
	LoadBitmap("Interface\\guild_tab_on.jpg", IMAGE_GUILDINFO_TAB_POINT, GL_LINEAR);
	LoadBitmap("Interface\\newui_guild_tab03.tga", IMAGE_GUILDINFO_TAB_HEAD, GL_LINEAR);
	...
	LoadBitmap("Interface\\newui_item_table03(L).tga", IMAGE_GUILDINFO_LEFT_PIXEL, GL_LINEAR);
	LoadBitmap("Interface\\newui_item_table03(R).tga", IMAGE_GUILDINFO_RIGHT_PIXEL, GL_LINEAR);
	LoadBitmap("Interface\\btn_medium.jpg", IMAGE_GUILDINFO_BUTTON, GL_LINEAR);
	...
	LoadBitmap("Interface\\default_window_back.jpg", IMAGE_GUILDINFO_BACK_VOID, GL_LINEAR);
	LoadBitmap("Interface\\store_name.jpg", IMAGE_GUILDINFO_FRAME_BOX, GL_LINEAR);
	LoadBitmap("Interface\\store_name_large.jpg", IMAGE_GUILDINFO_FRAME_BOX_LARGE, GL_LINEAR);
	LoadBitmap("Interface\\store_name_big.jpg", IMAGE_GUILDINFO_FRAME_BOX_BIG, GL_LINEAR);
	LoadBitmap("Interface\\store_name_big_2.jpg", IMAGE_GUILDINFO_FRAME_BOX_BIG_2, GL_LINEAR);
	LoadBitmap("Interface\\guild_role_gm.tga", IMAGE_GUILDINFO_ROLE_GM, GL_LINEAR);
	LoadBitmap("Interface\\guild_role_am.tga", IMAGE_GUILDINFO_ROLE_AM, GL_LINEAR);
	LoadBitmap("Interface\\guild_role_bm.tga", IMAGE_GUILDINFO_ROLE_BM, GL_LINEAR);
	LoadBitmap("Interface\\scroll_back.jpg", IMAGE_GUILDINFO_NEW_SCROLL_BACK, GL_LINEAR);
	LoadBitmap("Interface\\scroll_middle.jpg", IMAGE_GUILDINFO_NEW_SCROLL_MIDDLE, GL_LINEAR);
...
#if CLIENT_VERSION >= 1001
	DeleteBitmap(IMAGE_GUILDINFO_BACK_VOID);
	DeleteBitmap(IMAGE_GUILDINFO_FRAME_BOX);
	DeleteBitmap(IMAGE_GUILDINFO_FRAME_BOX_LARGE);
	DeleteBitmap(IMAGE_GUILDINFO_FRAME_BOX_BIG);
	DeleteBitmap(IMAGE_GUILDINFO_FRAME_BOX_BIG_2);
	DeleteBitmap(IMAGE_GUILDINFO_ROLE_GM);
	DeleteBitmap(IMAGE_GUILDINFO_ROLE_AM);
	DeleteBitmap(IMAGE_GUILDINFO_ROLE_BM);
	DeleteBitmap(IMAGE_GUILDINFO_NEW_SCROLL_BACK);
	DeleteBitmap(IMAGE_GUILDINFO_NEW_SCROLL_MIDDLE);
...
#elif CLIENT_VERSION >= 1001
	CutText3( szText, szTemp[0], 145, 5, MAX_TEXT_LENGTH + 1);
#endif
...
void SEASON3B::CNewUIGuildInfoWindow::ResetMouseRButton()
{
	MouseRButton = false;
