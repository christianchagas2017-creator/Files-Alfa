...
void SEASON3B::CNewUIMoveCommandWindow::SettingCanMoveMap()
{
	...
	//	if( i < m_iRenderEndTextIndex-1 )	continue;
		...
		if( iLevel >= iReqLevel && (int)iZen >= iReqZen && (int)Hero->PK<PVP_MURDERER1)
		{
			...
			{
				if(( pEquipedHelper->Type == ITEM_HELPER+37 || pEquipedHelper->Type == ITEM_HELPER+3 || pEquipedHelper->Type == ITEM_HELPER+4 || pEquipedWing->Type == ITEM_HELPER+30 
					|| gHelperSystem.CheckIsHelper(pEquipedHelper->Type) //HELPER ICARUS
					|| (pEquipedWing->Type >= ITEM_WING+36 && pEquipedWing->Type <= ITEM_WING+43) || (pEquipedWing->Type >= ITEM_WING && pEquipedWing->Type <= ITEM_WING+6) 
					|| (pEquipedWing->Type <= ITEM_WING+130 && pEquipedWing->Type <= ITEM_WING+134) || gCustomWing.CheckCustomWingByItem(pEquipedWing->Type)
#ifdef PBG_ADD_NEWCHAR_MONK_ITEM
					|| (pEquipedWing->Type >= ITEM_WING+49 && pEquipedWing->Type <= ITEM_WING+50)
					|| (pEquipedWing->Type == ITEM_WING+135)
#endif //PBG_ADD_NEWCHAR_MONK_ITEM
					) && !( pEquipedHelper->Type == ITEM_HELPER+2 ) && ( g_ChangeRingMgr->CheckBanMoveIcarusMap(pEquipedRightRing->Type, pEquipedLeftRing->Type) == false))
				{
					(*li)->_bCanMove = true;
				}
				else
