...
//            if ( Range<=350.f && rand()%(int)(100-(Range-350.f)/10) )
			if((Hero->Helper.Type == MODEL_HELPER+37 && AniType == PLAYER_HIGH_SHOCK ) || (gHelperSystem.CheckHelperType(Hero->Helper.Type, 8) == 1 && AniType == PLAYER_HIGH_SHOCK))
			{
...
//                    if ( Range<=350.f && rand()%(int)(100-(Range-350.f)/10) )
    ...
    void RenderAurora2(int Type, int RenderType, float x, float y, float sx, float sy, vec3_t Light, float Speed, float Alpha)
    {
        if (RenderType == RENDER_DARK)
        {
            EnableAlphaBlendMinus();
        ...
        {
            EnableAlphaBlend();
            Vector(Alpha * Light[0], Alpha * Light[1], Alpha * Light[2], Light);
        }

        RenderTerrainAlphaBitmap(Type, x, y, sx, sy, Light, Speed);
