...
void cPartyBuffs::RecvPartyBuffs(BYTE* ReceiveBuffer)
{
	auto lpMsg = (PMSG_PARTY_EFFECT_LIST_SEND*)ReceiveBuffer;
	...
	{
		PMSG_PARTY_EFFECT_LIST* lpInfo = (PMSG_PARTY_EFFECT_LIST*)((ReceiveBuffer) + sizeof(PMSG_PARTY_EFFECT_LIST_SEND) + (sizeof(PMSG_PARTY_EFFECT_LIST) * n));
...
void cPartyBuffs::DrawPartyBuffs(float StartX, float StartY, char* MemberName)
{
	...
	if (it != this->m_PartyBuffsData.end())
	{
		...
		for (int n = 0; n < it->second.count; n++)
		{
			...
			if (it->second.Buffs[n].effect < 81) // eBuff_Berserker
			{
				...
				RenderBitmap((it->second.Buffs[n].effect - 1) / 80 + 31719, X, StartY, Width, Height, SourceX, SourceY, SourceWidth, SourceHeight, 1, 1, 0);
			}
			else {

				int EffectValue = (it->second.Buffs[n].effect - 81) % 81;
				float SourceX = (double)(EffectValue % 10) * Width / RenderW;
				float SourceY = (double)(EffectValue / 10) * Height / RenderW;
				float SourceWidth = Width / RenderW;
				float SourceHeight = Height / RenderW;

				RenderBitmap((it->second.Buffs[n].effect - 81) / 81 + 31720, X, StartY, Width, Height, SourceX, SourceY, SourceWidth, SourceHeight, 1, 1, 0);
			}
		}
