...
GLuint CGlobalBitmap::LoadImageCustom(const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	BITMAP_t* pBitmap = FindTexture(filename);
	if (pBitmap)
	{
	...
	{
		GLuint uiNewTextureIndex = GenerateTextureIndex();
		if (true == LoadImageCustom(uiNewTextureIndex, filename, uiFilter, uiWrapMode))
		{
			m_listNonamedIndex.push_back(uiNewTextureIndex);
...
bool CGlobalBitmap::LoadImageCustom(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	unsigned int UICLAMP = GL_CLAMP_TO_EDGE;
	unsigned int UIREPEAT = GL_REPEAT;
	...
	if (0 == _stricmp(ext.c_str(), "jpg"))
		return OpenJpegCustom(uiBitmapIndex, filename, uiFilter, uiWrapMode);
	else if (0 == _stricmp(ext.c_str(), "tga"))
		return OpenTgaCustom(uiBitmapIndex, filename, uiFilter, uiWrapMode);
...
bool CGlobalBitmap::OpenJpegCustom(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	std::string filename_ozj;
	ExchangeExt(filename, OZJ_EXT, filename_ozj);
	...
	fseek(infile, 28, SEEK_SET);	//. Skip Dump
...
bool CGlobalBitmap::OpenTgaCustom(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	std::string filename_ozt;
	ExchangeExt(filename, OZT_EXT, filename_ozt);
