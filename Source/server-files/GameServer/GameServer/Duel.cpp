...
bool CDuel::AddDuelArena(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	for (int n : indices) 
	{
		...
		if(lpInfo->State == DA_STATE_EMPTY)
		{
			...
			if(gServerInfo.m_DuelArenaAnnounceSwitch != 0){ gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(174),lpObj->Name,lpTarget->Name);}
#else
...
void CDuel::CheckDuelArenaGladiator(DUEL_ARENA_INFO* lpInfo) // OK
{
	...
	for(int n=0;n < MAX_DUEL_ARENA_GLADIATOR;n++)
	{
		...
		if(lpInfo->State != DA_STATE_CLEAN)
		{
			...
			{
#ifdef FAIRPLAY
				gObjMoveGate(lpInfo->Gladiator[n],2);
#else
...
void CDuel::ClearDuelArenaSpectator(DUEL_ARENA_INFO* lpInfo) // OK
{
	...
	for(int n=0;n < MAX_DUEL_ARENA_SPECTATOR;n++)
	{
		if(OBJECT_RANGE(lpInfo->Spectator[n]) != 0)
		{
			...
			gEffectManager.DelEffect(&gObj[lpInfo->Spectator[n]],EFFECT_DUEL_ARENA_WATCH);
#ifdef FAIRPLAY
			gObjMoveGate(lpInfo->Spectator[n],2);
#else
...
void CDuel::CheckDuelScore(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->DuelScore >= gServerInfo.m_DuelKills) // mortes duelo
	{
...
#ifdef GLOBAL_DASHBOARD
			if(gServerInfo.m_DuelArenaAnnounceSwitch != 0){ gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(175),lpObj->Name,lpTarget->Name);}
#else
...
{
	gMap[lpObj->Map].GetMapRandomPos(&lpObj->X,&lpObj->Y,15);
	...
	gMap[lpTarget->Map].GetMapRandomPos(&lpTarget->X,&lpTarget->Y,15);
...
void CDuel::GCDuelScoreSend(int aIndex,int bIndex) // OK
{
	...
	memcpy(pMsg.guild1, gObj[aIndex].GuildName, sizeof(pMsg.guild1));
	memcpy(pMsg.guild2, gObj[bIndex].GuildName, sizeof(pMsg.guild2));
...
void CDuel::GCDuelInterfaceSend(DUEL_ARENA_INFO* lpInfo) // OK
{
	...
	DataSend(lpInfo->Gladiator[0], (BYTE*)&pMsg, pMsg.header.size);
	DataSend(lpInfo->Gladiator[1], (BYTE*)&pMsg, pMsg.header.size);
...
void CDuel::GCDuelGladiatorAddSend(int aIndex,DUEL_ARENA_INFO* lpInfo) // OK
{
	...
	if(OBJECT_RANGE(lpInfo->Gladiator[0]) == 0)
	{
		...
		memset(pMsg.guild1,0,sizeof(pMsg.guild1));
	}
	else
	{
		memcpy(pMsg.name1,gObj[lpInfo->Gladiator[0]].Name,sizeof(pMsg.name1));
		memcpy(pMsg.guild1, gObj[lpInfo->Gladiator[0]].GuildName, sizeof(pMsg.guild1));
	}

	if(OBJECT_RANGE(lpInfo->Gladiator[1]) == 0)
	{
		memset(pMsg.name2,0,sizeof(pMsg.name2));
		memset(pMsg.guild2, 0, sizeof(pMsg.guild2));
	}
	else
	{
		memcpy(pMsg.name2,gObj[lpInfo->Gladiator[1]].Name,sizeof(pMsg.name2));
		memcpy(pMsg.guild2, gObj[lpInfo->Gladiator[1]].GuildName, sizeof(pMsg.guild2));
	}
...
void CDuel::StartDuelBet(LPOBJ lpObj, LPOBJ lpTarget)
{
	if(this->AddDuelArena(lpObj,lpTarget) == 0)
	{
