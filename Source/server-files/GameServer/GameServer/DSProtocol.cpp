		...
		case 0x05:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x01:
					...
					break;
				case 0x41:
					gTradeX.TradeXGetItemSaveRecv((SDHP_TRADEX_GET_ITEM_SAVE_RECV*)lpMsg);
					break;
				#ifdef CHAOS_MACHINE_GENESIS
				case 0x60:
					gChaosBoxGenesis.ChaosBoxGenesisGetItemSaveRecv((SDHP_CHAOS_BOX_GENESIS_GET_ITEM_SAVE_RECV*)lpMsg);
					break;
				#endif
			}
			break;
		...
		case 0x18:
			switch(((lpMsg[0]==0xC1)?lpMsg[3]:lpMsg[4]))
			{
				...
				case 0x06:
					...
					break;
				case 0x40:
					gDeathSystem.InventoryItemCountRecv((DEATH_SYSTEM_INVENTORY_RECV*)lpMsg);
					break;
				case 0x41:
					gDeathSystem.WarehouseItemCountRecv((DEATH_SYSTEM_WAREHOUSE_RECV*)lpMsg);
					break;
				case 0x43:
					gDeathSystem.ChaosBoxGenesisItemCountRecv((DEATH_SYSTEM_WAREHOUSE_RECV*)lpMsg);
					break;
				case 0x44:
					gDeathSystem.TradeXItemCountRecv((DEATH_SYSTEM_WAREHOUSE_RECV*)lpMsg);
					break;
				case 0x45:
					gDeathSystem.DGReceiveTopSerial((DEATH_SYSTEM_TOP_KILL_RECV*)lpMsg);
					break;
			}
			break;
		case 0x19:
...
void DGCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg) // OK
{
	...
	pMsgNews.header.set(0xF3, 0x70, 0);
	...
	pMsg.MaxCharacter = lpMsg->MaxCharacter;
	...
	for (int n = 0; n < lpMsg->count; n++)
	{
		...
		for(int i=0;i < 9;i++)
		{
			if(i == 0 || i == 1)
			{
				if (lpInfo->Inventory[0 + (i * maxCharacter)] == 0xFF && (lpInfo->Inventory[2 + (i * maxCharacter)] & 0x80) == 0x80 && (lpInfo->Inventory[4 + (i * maxCharacter)] & 0xF0) == 0xF0)
				{
					TempInventory[i] = 0xFFFF;
				}
				else
				{
					TempInventory[i] = (lpInfo->Inventory[0 + (i * maxCharacter)] + ((lpInfo->Inventory[2 + (i * maxCharacter)] & 0x80) * 2) + ((lpInfo->Inventory[4 + (i * maxCharacter)] & 0xF0) * 32));
				}
			}
			else
			{
				...
				{
					TempInventory[i] = (lpInfo->Inventory[0 + (i * maxCharacter)] + ((lpInfo->Inventory[2 + (i * maxCharacter)] & 0x80) * 2) + ((lpInfo->Inventory[4 + (i * maxCharacter)] & 0xF0) * 32)) % MAX_ITEM_TYPE;
				}
			}
		}
		...
		for(int i=0;i < 7;i++)
		{
			...
			{
				level |= ((((lpInfo->Inventory[1+(i* maxCharacter)]/8) & 0x0F)-1)/2) << (i*3);

				info.CharSet[10] |= ((lpInfo->Inventory[2+(i*maxCharacter)] & 0x3F)?2:0) << table[i];
				info.CharSet[11] |= ((lpInfo->Inventory[3+(i*maxCharacter)] & 0x03)?2:0) << table[i];
			}
		}
		...
		else if (gCustomPets.CheckCustomPet(GET_ITEM(13, TempInventory[8])) != 0)
		{
	...
	if (gServerInfo.m_MGCreateType == 0 && CharacterCreationLevel >= (WORD)gServerInfo.m_MGCreateLevel[gObj[lpMsg->index].AccountLevel])
	{
		pMsg.ClassCode |= 4;
	}

	if (gServerInfo.m_DLCreateType == 0 && CharacterCreationLevel >= (WORD)gServerInfo.m_DLCreateLevel[gObj[lpMsg->index].AccountLevel])
	{
		pMsg.ClassCode |= 2;
	}

	if (gServerInfo.m_SUCreateType == 0 && CharacterCreationLevel >= (WORD)gServerInfo.m_SUCreateLevel[gObj[lpMsg->index].AccountLevel])
	{
		pMsg.ClassCode |= 1;
	}

	if (gServerInfo.m_RFCreateType == 0 && CharacterCreationLevel >= (WORD)gServerInfo.m_RFCreateLevel[gObj[lpMsg->index].AccountLevel])
	{
		pMsg.ClassCode |= 8;
	}
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	pMsg.Shield = lpObj->Shield;
	pMsg.MaxShield = (lpObj->MaxShield+lpObj->AddShield);
	pMsg.BP = lpObj->BP;
...
#ifdef CHAOS_MACHINE_GENESIS
	gChaosBoxGenesis.ChaosBoxGenesisGetItemSaveSend(lpObj);
#endif

	gTradeX.TradeXGetItemSaveSend(lpObj);
	...
	gDeathSystem.GDInventoryGetInformation(lpObj->Index);

	gLuaGameServer.EnterCharacter(lpObj->Index);

	//SEND VipClient

	PMSG_ACCOUNT_VIP_SEND vipMsg;

	vipMsg.header.setE(0xF5, 0x00, sizeof(vipMsg));

	vipMsg.accountlevel = lpObj->AccountLevel;

	DataSend(lpObj->Index, (BYTE*)&vipMsg, vipMsg.header.size);

	gWarehouse.GDWarehouseInfoSend(lpObj->Index);
...
#ifdef GLOBAL_DASHBOARD
	gNotice.GCNoticeSendToDash(lpMsg->type, "%s",lpMsg->message);
#else
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	gTradeX.GDTradeXItemSaveSend(aIndex, 0);
...
#ifdef CHAOS_MACHINE_GENESIS
	gChaosBoxGenesis.ChaosboxGenesisSaveItemSend(lpObj, 1);
#endif
...
void GS_DGAnsMapSvrMsgMultiCast(LPBYTE lpRecv)
{
	...
	{
#ifdef GLOBAL_DASHBOARD
		gNotice.GCNoticeSendToDash(0, "%s",szNotice);
#else
