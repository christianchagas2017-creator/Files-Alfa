...
void CMemoryAllocatorInfo::Alloc() // OK
{
	...
	if(OBJECT_USER_RANGE(this->m_Index) != 0)
	{
		...
		this->m_TradeX = new CItem[TRADEX_SIZE];
		this->m_TradeXMap = new BYTE[TRADEX_SIZE];
#ifdef CHAOS_MACHINE_GENESIS
		this->m_ChaosBoxGenesis = new CItem[CHAOS_BOX_GENESIS_SIZE];
#endif
		#if(GAMESERVER_UPDATE>=802)
