...
{
	this->m_CounterItemTopSerial = 1;
	this->m_CounterItemTopKills = 0;
}
...
void CServerInfo::ReadCustomInfo() // OK
{
...
#ifdef FAIRPLAY
	gHappyHourZen.Load(gPath.GetFullPath("Custom\\HappyHourZen.txt"));
#endif
	...
	gCustomPets.Load(gPath.GetFullPath("Custom\\CustomPets.txt")); //CustomPets

	gDeathSystem.Load(gPath.GetFullPath("Custom\\CounterItem.txt"));

	gPatenteInfo.Load(gPath.GetFullPath("Custom\\PatenteInfo.txt"));

	gImmortalPet.Load(gPath.GetFullPath("Custom\\ImmortalPet.txt"));

	gCustomPick.LoadConfig("GameServerInfo", ".\\Data\\GameServerInfo - Custom.dat");
...
void CServerInfo::ReadItemInfo() // OK
{
	...
	gElementSlots.SetElementSlot(0, 1, 1);	// Slot Pet 1

	gElementSlots.SetElementSlot(1, 1, 1);	// Slot Pet 2

	gElementSlots.SetElementSlot(2, 1, 0);	// Slot Brinco 1

	gElementSlots.SetElementSlot(3, 1, 0);	// Slot Brinco 2

	gElementSlots.SetElementSlot(4, 1, 0);	// Slot Brinco 2

	gElementSlots.SetVisualInventorySwitch(1);
...
void CServerInfo::ReadMapInfo() // OK
{
	...
	LogAdd(LOG_BLUE, "[ServerInfo] Terrain loaded successfully");
}
...
void CServerInfo::ReadStartupInfo(char* section,char* path) // OK
{
	...
	GetPrivateProfileString(section,"MySQLServer","127.0.0.1",this->m_MySQLServer,sizeof(this->m_MySQLServer),path);
	GetPrivateProfileString(section,"MySQLDatabase","",this->m_MySQLDatabase,sizeof(this->m_MySQLDatabase),path);
	GetPrivateProfileString(section,"MySQLUser","",this->m_MySQLUser,sizeof(this->m_MySQLUser),path);
	GetPrivateProfileString(section,"MySQLPass","",this->m_MySQLPass,sizeof(this->m_MySQLPass),path);
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_UserLoggoutTimer = GetPrivateProfileInt(section, "UserLoggoutTimer", 5, path);

	this->m_PlayerKillStaff = GetPrivateProfileInt(section, "PlayerKillStaff", 0, path);

	this->m_StaffKillPlayer = GetPrivateProfileInt(section, "StaffKillPlayer", 0, path);

	this->m_StaffKillStaff = GetPrivateProfileInt(section, "StaffKillStaff", 1, path);

	this->m_ComboSwitch = GetPrivateProfileInt(section, "ComboSwitch", 1, path);
	...
	this->m_TradeXSwitch = GetPrivateProfileInt(section, "TradeXSwitch", 0, path);
	...
	this->m_DuelKills = GetPrivateProfileInt(section,"DuelKills",10,path);
...
void CServerInfo::ReadSkillInfo(char* section,char* path) // OK
{
	...
	this->m_BlockTwisterSlash = GetPrivateProfileInt(section, "BlockTwisterSlash", 0, path);

	this->m_TwisterSlashConstA = GetPrivateProfileInt(section, "TwisterSlashConstA", 150, path);

	this->m_TwoWeaponsDamageMinConstA = GetPrivateProfileInt(section, "TwoWeaponsDamageMinConstA", 100, path);

	this->m_TwoWeaponsDamageMaxConstA = GetPrivateProfileInt(section, "TwoWeaponsDamageMaxConstA", 100, path);

	this->m_DisablePushSkills = GetPrivateProfileInt(section, "DisablePushSkills", 0, path);
