...
public:
	...
	void luaSetPatent(int value) const
	{
		if (OBJECT_RANGE(this->aIndex))
		{
			...
			gObj[this->aIndex].m_PatentType = gPatenteInfo.GetPatentType(value);
		}
	};
...
void RegisterUserClass(lua_State* lua) {
	...
	luaUser.fun("getMonitored", &UserClass::luaGetMonitored);
	luaUser.fun("setMonitored", &UserClass::luaSetMonitored);
	luaUser.fun("getMonitoring", &UserClass::luaGetMonitoring);
	luaUser.fun("setMonitoring", &UserClass::luaSetMonitoring);
