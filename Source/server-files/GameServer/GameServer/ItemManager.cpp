...
int CItemManager::GetItemRepairMoney(CItem* lpItem, int type) // OK
{
	...
	if (lpItem->m_Index == GET_ITEM(13, 4) || lpItem->m_Index == GET_ITEM(13, 5) || gCustomDarkSpirit.checkIsDarkSpirit(lpItem->m_Index)) // Dark Horse,Dark Reaven
	{
...
bool CItemManager::CheckItemMoveToInventory(LPOBJ lpObj, CItem* lpItem, int slot) // OK
{
	...
	if (lpItem->m_Slot < 240 || (lpItem->m_JewelOfHarmonyOption == 0 && lpItem->IsJewelOfHarmonyItem() == 0)) {
	...
	if (lpItem->m_Slot >= 0 && lpItem->m_Slot <= 8 && slot >= 240 && slot <= 249)
	{
		if (lpItem->m_JewelOfHarmonyOption > 0 || lpItem->IsJewelOfHarmonyItem() != 0) {
			int tslot = lpItem->m_Slot + 240;

			if (tslot == 240 && slot == 241) {
				return 1;
			}
	...
	if (lpItem->m_Slot >= 0 && lpItem->m_Slot <= 8 && slot >= 0 && slot <= 8)
	{
		if (lpItem->m_JewelOfHarmonyOption > 0 || lpItem->IsJewelOfHarmonyItem() != 0) {
			return 0;
		}
	...
	if (lpItem->m_Slot != slot && slot != 0 && slot != 1 && slot != 10 && slot != 11 && slot != 240 && slot != 241)
	{
		return 0;
	}
	...
	if (lpItem->m_Slot != slot && ((slot == 240 || slot == 241) && lpItem->m_Slot != 240 && lpItem->m_Slot != 241))
	{
		return 0;
	}
	...
	if (slot == 240 && lpObj->Inventory[241].IsItem() != 0)
	{
		if (lpItem->m_TwoHand != 0 || lpObj->Inventory[241].m_TwoHand != 0)
		{
			return 0;
		}
	}

	if (slot == 241 && lpObj->Inventory[240].IsItem() != 0)
	{
		if (lpItem->m_TwoHand != 0 || lpObj->Inventory[240].m_TwoHand != 0)
		{
			return 0;
		}
	}
	...
	if (CC_MAP_RANGE(lpObj->Map) != 0
		...
		|| IT_MAP_RANGE(lpObj->Map) != 0
#endif
		)
	{
		if (lpItem->m_Index == GET_ITEM(13, 10) || (lpItem->m_Index >= GET_ITEM(13, 39) && lpItem->m_Index <= GET_ITEM(13, 42))
#if (GAMESERVER_UPDATE>=401)
			|| lpItem->m_Index == GET_ITEM(13, 68) || lpItem->m_Index == GET_ITEM(13, 76) || lpItem->m_Index == GET_ITEM(13, 122)
#endif
			) // Rings
		{
			return 0;
	...
	if (lpItem->m_Index == GET_ITEM(13, 10) || (lpItem->m_Index == GET_ITEM(13, 20) && lpItem->m_Level == 0) || (lpItem->m_Index >= GET_ITEM(13, 38) && lpItem->m_Index <= GET_ITEM(13, 42))
#if (GAMESERVER_UPDATE>=401)
		|| lpItem->m_Index == GET_ITEM(13, 68) || lpItem->m_Index == GET_ITEM(13, 76) || lpItem->m_Index == GET_ITEM(13, 107) || lpItem->m_Index == GET_ITEM(13, 122)
#endif
		) // Rings
	{
...
#ifdef FAIRPLAY
	int excCount = 0;
	excCount += ((lpItem->m_NewOption & 1) != 0) ? 1 : 0;
	excCount += ((lpItem->m_NewOption & 2) != 0) ? 1 : 0;
	excCount += ((lpItem->m_NewOption & 4) != 0) ? 1 : 0;
	excCount += ((lpItem->m_NewOption & 8) != 0) ? 1 : 0;
	excCount += ((lpItem->m_NewOption & 16) != 0) ? 1 : 0;
	excCount += ((lpItem->m_NewOption & 32) != 0) ? 1 : 0;
...
BYTE CItemManager::InventoryAddItem(int aIndex, CItem item, int slot, int sourceSlot, bool firstLogin) // OK
{
	...
	else
	{
		...
		if (!firstLogin) {
			if (gLuaGameServer.PlayerCanEquipItem(lpObj->Index, slot, sourceSlot) == 1) {
				return 0xFF;
			}
		}
		// pribe usar item temporario expirado
		if (item.m_IsPeriodicItem != 0) {
			if (item.m_IsExpiredItem == 1) {
				return 0xFF;
			}
		}

		if (!gElementSlots.CheckCanEquip(slot)) {
			return 0xFF;
		}

		if (slot >= 240 && slot <= 250) {
			if (gElementSlots.VisualInventorySwitch == 0) {
				return 0xFF;
			}
		}
		...
		if ((slot >= 0 && slot <= 11) || (slot >= 236 && slot <= 239) || slot == 250) {
...
BYTE CItemManager::TradeXAddItem(int aIndex, CItem item, int slot)
{
	if (TRADEX_RANGE(slot) == 0)
	{
		return 0xFF;
	}
	...
	if (lpObj->TradeX[slot].IsItem() != 0 || item.IsItem() == 0)
	{
		return 0xFF;
	}
	...
	lpObj->TradeX[slot] = item;

	return slot;
}

BYTE CItemManager::TradeXEmptySlot(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
...
void CItemManager::TradeXDelItem(int aIndex, int slot)
{
	gObj[aIndex].TradeX[slot].Clear();
}

BYTE CItemManager::ChaosBoxGenesisAddItem(int aIndex, CItem item, int slot)
{
#ifdef CHAOS_MACHINE_GENESIS
	if (CHAOS_BOX_GENESIS_RANGE(slot) == 0)
	{
		return 0xFF;
	}
	...
	if (lpObj->ChaosBoxGenesis[slot].IsItem() != 0 || item.IsItem() == 0)
	{
		return 0xFF;
	}
	...
	lpObj->ChaosBoxGenesis[slot] = item;
#endif

	return slot;
}

BYTE CItemManager::ChaosBoxGenesisGetEmptySlot(int aIndex)
{
#ifdef CHAOS_MACHINE_GENESIS
	for (int n = 0; n < CHAOS_BOX_GENESIS_SIZE; n++) {
		if (gObj[aIndex].ChaosBoxGenesis[n].IsItem() == 0) {
			return n;
		}
	}
...
void CItemManager::ChaosBoxGenesisDelItem(int aIndex, int slot)
{
#ifdef CHAOS_MACHINE_GENESIS
	gObj[aIndex].ChaosBoxGenesis[slot].Clear();
#endif
}

BYTE CItemManager::MoveItemToInventoryFromTradeX(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
#ifdef TRADE_X
	if (TRADEX_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (gObjInventorySearchSerialNumber(lpObj, lpObj->TradeX[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if (gObjWarehouseSearchSerialNumber(lpObj, lpObj->TradeX[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if (this->CheckItemMoveToInventory(lpObj, &lpObj->TradeX[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (this->InventoryAddItem(lpObj->Index, lpObj->TradeX[SourceSlot], TargetSlot, SourceSlot) == 0xFF)
	{
		return 0xFF;
	}

	this->TradeXDelItem(lpObj->Index, SourceSlot);

	if (OBJECT_RANGE(lpObj->TargetNumber) != 0)
	{
		lpObj->TradeXOk = 0;
		lpObj->TradeXTimeItem = GetTickCount();
		gTrade.GCTradeOkButtonSend(lpObj->Index, 0, 1);
		gObj[lpObj->TargetNumber].TradeXOk = 0;
		gObj[lpObj->TargetNumber].TradeXTimeItem = GetTickCount();
		gTrade.GCTradeOkButtonSend(lpObj->TargetNumber, 2, 1);
		gTradeX.TradeXItemDelSend(lpObj->TargetNumber, SourceSlot);
	}

	gTradeX.GDTradeXItemSaveSend(lpObj->Index, 1);

	gTradeX.TradeXCheckIsEmpty(lpObj);
...
BYTE CItemManager::MoveItemToTradeXFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE& TargetSlot, BYTE TargetFlag)
{
#ifdef TRADE_X
	...
	if (OBJECT_USER_RANGE(lpObj->TargetNumber) == 0) {
		return 0xFF;
	}

	TargetSlot = this->TradeXEmptySlot(lpObj->Index);

	if (TRADEX_RANGE(TargetSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (this->TradeXAddItem(lpObj->Index, lpObj->Inventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
	...
	this->ItemByteConvert(ItemInfo, lpObj->TradeX[TargetSlot]);

	gTradeX.TradeXItemAddSend(lpObj->TargetNumber, TargetSlot, ItemInfo);
...
BYTE CItemManager::MoveItemToChaosBoxGenesisFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE& TargetSlot, BYTE TargetFlag)
{
#ifdef CHAOS_MACHINE_GENESIS
	if (INVENTORY_FULL_RANGE(SourceSlot) == 0)
	{
	...
	TargetSlot = this->ChaosBoxGenesisGetEmptySlot(lpObj->Index);

	if (CHAOS_BOX_GENESIS_RANGE(TargetSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (this->ChaosBoxGenesisAddItem(lpObj->Index, lpObj->Inventory[SourceSlot], TargetSlot) == 0xFF)
	{
		return 0xFF;
	}
	...
	this->ItemByteConvert(ItemInfo, lpObj->ChaosBoxGenesis[TargetSlot]);

	this->UpdateInventoryViewport(lpObj->Index, SourceSlot);

	gChaosBoxGenesis.ChaosboxGenesisSaveItemSend(lpObj, 0);
#endif
	return 40;
}

BYTE CItemManager::MoveItemToInventoryFromChaosBoxGenesis(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag)
{
#ifdef CHAOS_MACHINE_GENESIS
	if (CHAOS_BOX_GENESIS_RANGE(SourceSlot) == 0)
	{
		return 0xFF;
	}
	...
	if (gObjInventorySearchSerialNumber(lpObj, lpObj->ChaosBoxGenesis[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if (gObjWarehouseSearchSerialNumber(lpObj, lpObj->ChaosBoxGenesis[SourceSlot].m_Serial) == 0)
	{
		return 0xFF;
	}

	if (this->CheckItemMoveToInventory(lpObj, &lpObj->ChaosBoxGenesis[SourceSlot], TargetSlot) == 0)
	{
		return 0xFF;
	}

	if (this->InventoryAddItem(lpObj->Index, lpObj->ChaosBoxGenesis[SourceSlot], TargetSlot, SourceSlot) == 0xFF)
	{
		return 0xFF;
	}

	this->ChaosBoxGenesisDelItem(lpObj->Index, SourceSlot);

	//need close chaos box
	//gTradeX.TradeXCheckIsEmpty(lpObj);
		...
		if (lpObj->Inventory[n].IsItem() != 0)
		{
			if (lpObj->Inventory[n].m_Index == index && (level == -1 || lpObj->Inventory[n].m_Level == level))
			{
				...
				else
				{
					...
					{
						if ((int)lpObj->Inventory[n].m_Durability == 0) {
							lpObj->Inventory[n].m_Durability = 1;
						}
...
int CItemManager::RepairItem(LPOBJ lpObj, CItem* lpItem, int slot, int type) // OK
{
	...
	if (lpItem->m_Index == GET_ITEM(13, 5) || gCustomDarkSpirit.checkIsDarkSpirit(lpItem->m_Index))
	{
...
BYTE CItemManager::MoveItemToInventoryFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	if (this->InventoryAddItem(lpObj->Index, lpObj->Inventory[SourceSlot], TargetSlot, SourceSlot) == 0xFF)
	{
...
BYTE CItemManager::MoveItemToInventoryFromTrade(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	if (this->InventoryAddItem(lpObj->Index, lpObj->Trade[SourceSlot], TargetSlot, SourceSlot) == 0xFF)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToInventoryFromWarehouse(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
...
#ifdef FAIRPLAY
	if (lpObj->WarehouseNumber == lpObj->WarehouseCount) {
	...
	if (this->InventoryAddItem(lpObj->Index, lpObj->Warehouse[SourceSlot], TargetSlot, SourceSlot) == 0xFF)
	{
		return 0xFF;
	}
...
BYTE CItemManager::MoveItemToInventoryFromChaosBox(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	if (this->InventoryAddItem(lpObj->Index, lpObj->ChaosBox[SourceSlot], TargetSlot, SourceSlot) == 0xFF)
	{
		return 0xFF;
	}
...
#ifdef FAIRPLAY
		lpObj->LastMoneyObtained = lpItem->m_BuyMoney;
		lpObj->MoneyObtainedTotal += lpItem->m_BuyMoney;
#endif
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg, int aIndex) // OK
{
	...
	auto playerDropResult = gLuaGameServer.PlayerDropItem(aIndex, lpMsg->x, lpMsg->y, lpMsg->slot);

	if (playerDropResult > 0)
	{
		if (playerDropResult == 1) {
			pMsg.result = 1;
		}
		else {
			pMsg.result = 0;
...
void CItemManager::CGItemMoveRecv(PMSG_ITEM_MOVE_RECV* lpMsg, int aIndex) // OK
{
...
#ifdef CHAOS_MACHINE_GENESIS
	if (lpMsg->SourceFlag == 40 || lpMsg->TargetFlag == 40) // ChaosBox Genesis
	{
		if (lpObj->Interface.use == INTERFACE_NONE || lpObj->Interface.type != INTERFACE_CHAOSGENESIS || lpObj->ChaosBoxGenesisUsing != 1 || lpObj->ChaosBoxGenesisSelectedComb == 0)
		{
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			return;
		}
	...
	else if (lpMsg->SourceFlag == 40 && lpMsg->TargetFlag == 0) // Chaos Box Genesis -> Inventory
	{
		...
		{
			pMsg.slot = lpMsg->SourceSlot;
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[lpMsg->TargetSlot]);
		}
	}
#endif
#ifdef TRADE_X
	else if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 41) // Inventory -> TradeX
	{
		if ((pMsg.result = this->MoveItemToTradeXFromInventory(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			pMsg.slot = lpMsg->TargetSlot;
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->TradeX[lpMsg->TargetSlot]);
			}
	}
	else if (lpMsg->SourceFlag == 41 && lpMsg->TargetFlag == 0) // TradeX -> Inventory
	{
		if ((pMsg.result = this->MoveItemToInventoryFromTradeX(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot, lpMsg->TargetFlag)) != 0xFF)
		{
			pMsg.slot = lpMsg->SourceSlot;
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[lpMsg->TargetSlot]);
		}
	...
	if (pMsg.result != 0xFF)
	{
		...
		gItemDuration.SendInformationBySlot(lpObj);
#endif
		...
		{
			gLuaGameServer.PlayerMoveItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot, 4);

			gDeathSystem.SendInformation(lpObj, 2);
			gDeathSystem.SendTradeTarget(lpObj);
		}
		...
		{
			gLuaGameServer.PlayerMoveItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot, 3); //Bau > Bau
			gDeathSystem.SendInformation(lpObj, 1);
		}
		
		else if (lpMsg->SourceFlag == 0 && lpMsg->TargetFlag == 41)
		{
			gLuaGameServer.PlayerMoveItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot, 7); //Inventory > TradeX

			gDeathSystem.SendInformation(lpObj, 6);
			gDeathSystem.SendInformation(lpObj, 0);

			gDeathSystem.SendTradeTargetX(lpObj);
		}
		else if (lpMsg->SourceFlag == 41 && lpMsg->TargetFlag == 0)
		{
			gLuaGameServer.PlayerMoveItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot, 8); //TradeX > Inventory
		...
		{
			gLuaGameServer.PlayerMoveItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot, 9); //Inventory > Chaos box genesis
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (gLuaGameServer.PlayerUseItem(aIndex, lpMsg->SourceSlot, lpMsg->TargetSlot) != 0)
	{
		return;
	}
	else if (gCustomJewelStack.CheckJewelSplit(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot) != 0) {
	...
	else if ((lpItem->m_Index >= GET_ITEM(12, 7) && lpItem->m_Index <= GET_ITEM(12, 24)) || lpItem->m_Index == GET_ITEM(12, 35) || (lpItem->m_Index >= GET_ITEM(12, 44) && lpItem->m_Index <= GET_ITEM(12, 48)) || lpItem->m_Index >= GET_ITEM(15, 0))
	{
...
void CItemManager::GCItemChangeSend(int aIndex, BYTE slot) // OK
{
	...
	if ((slot >= 0 && slot <= 8) || (slot >= 240 && slot <= 250))
	{
		if (lpObj->Inventory[slot].IsItem() == 0)
		{
			int targetSlot = 0;
			...
			{
				this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[targetSlot]);

				if (targetSlot >= 240)
				{
					pMsg.ItemInfo[1] = (slot - 240) * 16;
				}
				else
				{
					pMsg.ItemInfo[1] = targetSlot * 16;
				}

				pMsg.ItemInfo[1] |= ((lpObj->Inventory[targetSlot].m_Level - 1) / 2) & 0x0F;
			}
			else
			{
		...
		else
		{
			if (slot >= 0 && slot <= 8)
			{
				int targetSlot = slot + 240;

				if (lpObj->Inventory[targetSlot].IsItem() != 0)
				{
					this->ItemByteConvert(pMsg.ItemInfo, lpObj->Inventory[targetSlot]);

					pMsg.ItemInfo[1] = (targetSlot - 240) * 16;
					pMsg.ItemInfo[1] |= ((lpObj->Inventory[targetSlot].m_Level - 1) / 2) & 0x0F;
				}
				else
				{
					...
					pMsg.ItemInfo[1] = (slot > 12 ? 12 : slot) * 16;
					pMsg.ItemInfo[1] |= ((lpObj->Inventory[slot].m_Level - 1) / 2) & 0x0F;
				}
			}
			else
