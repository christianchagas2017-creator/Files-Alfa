...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	if(WSAStartup(MAKEWORD(2,2),&wsa) == 0)
	{
		...
		else
		{
			...
			SetTimer(hWnd, WM_TIMER_LICENSE, 60000 * 20, 0);
			...
			gQueueTimer.CreateTimer(QUEUE_TIMER_UPDATE_PARTY_BUFF,100,&QueueTimerCallback);
		}
	}
	else
	{
...
BOOL InitInstance(HINSTANCE hInstance,int nCmdShow) // OK
{
	...
	hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, 0, 600, 600, 0, 0, hInstance, 0);
		...
		case WM_TIMER:
			switch(wParam)
			{
				...
				case WM_TIMER_10000:
					...
					break;
				case WM_TIMER_LICENSE:
#if CLIENT_VERSION == 2
					gProtect.ReAuth(AUTH_SERVER_TYPE_GAME_SERVER);
#endif
					break;
			}
			break;
