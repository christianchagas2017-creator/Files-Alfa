		...
		if(lpObj->Type == OBJECT_USER)
		{
			...
			switch(lpObj->KillerType)
			{
				...
				case 3:
					...
					break;
				case 5:
					gLuaGameServer.RespawnUser(lpObj->Index);
					break;
			}
...
bool CObjectManager::CharacterGameClose(int aIndex) // OK
{
	...
	gLuaGameServer.PlayerLogout(aIndex, lpObj->Name, lpObj->Account);
...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	lpObj->CloseCount = gServerInfo.m_UserLoggoutTimer + 1;
...
bool CObjectManager::CharacterGetRespawnLocation(LPOBJ lpObj) // OK
{
	...
	{
#ifdef FAIRPLAY
		result = gGate.GetGate(2,&gate,&map,&x,&y,&dir,&level);
#else
		result = gGate.GetGate(17,&gate,&map,&x,&y,&dir,&level);
#endif
	}
...
#ifdef FAIRPLAY
		experience = (experience * gHappyHourZen.GetExpRate()) / 100;
#endif
	}
	else
	{
...
#ifdef FAIRPLAY
		//lpTarget->LastExpObtained = TotalExperience;
#endif
...
#ifdef FAIRPLAY
			lpObj->totalExpGained += AddExperience;
			lpObj->expReceiveCount++;
			lpObj->lastExpGainTick = GetTickCount();
			lpObj->lastExpGained = AddExperience;
#endif
...
bool CObjectManager::CharacterLevelUpPointAdd(LPOBJ lpObj,int type,int amount) // OK
{
	...
	if (gLuaGameServer.LevelUpPointAdd(lpObj->Index, type))
	{
		return 0;
	}
...
void CObjectManager::CharacterMakePreviewCharSet(int aIndex) // OK
{
	...
	for (int n = 240; n < 248; n++) 
	{
		if (lpObj->Inventory[n].IsItem() != 0) 
		{
			if (n == 240 || n == 241) 
			{
				TempInventory[n - 240] = lpObj->Inventory[n].m_Index;
			}
			else 
			{
				TempInventory[n - 240] = lpObj->Inventory[n].m_Index % MAX_ITEM_TYPE;
			}
		}
	}
	...
	for (int n = 0; n < 7; n++)
	{
		...
		{
			if (lpObj->Inventory[240 + n].IsItem() != 0) 
			{
				level |= ((lpObj->Inventory[240 + n].m_Level - 1) / 2) << (n * 3);

				lpObj->CharSet[10] |= ((lpObj->Inventory[240 + n].m_NewOption & 0x3F) ? 2 : 0) << table[n];
				lpObj->CharSet[11] |= ((lpObj->Inventory[240 + n].m_SetOption & 0x03) ? 2 : 0) << table[n];
			}
			else 
			{
	...
	for(int n=2;n < 10;n++)
	{
		if(lpObj->Inventory[n].IsItem() != 0)
		{
			...
			if(result != 0)
			{
				...
				if (gLuaGameServer.PlayerRingEquiped(lpObj->Index) != 1)
				{
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	{
		if (gCustomDarkSpirit.checkIsDarkSpirit(lpObj->Inventory[236].m_Index))
		{
			gDarkSpirit[lpObj->Index].Set(lpObj->Index, &lpObj->Inventory[236]);
		}
	}
	else
	{
	...
	if (lpObj->Type == OBJECT_USER) {
		gLuaGameServer.CharacterSet(lpObj->Index);
	}
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	*/
	...
	for(int n=0;n < INVENTORY_SIZE;n++)
	{
		...
		gItemManager.InventoryAddItem(aIndex,item, n, 0, 1);
	}
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			int PlayerKillMonster = gObjMonsterGetTopHitDamageUser(lpTarget);

			if (OBJECT_RANGE(PlayerKillMonster) != 0)
			{
				gLuaGameServer.MonsterDie(PlayerKillMonster, lpTarget->Index);
			}
		...
		if(lpTarget->Type == OBJECT_USER)
		{
			...
			gLuaGameServer.PlayerDie(SummonIndex, lpTarget->Index);
		}
