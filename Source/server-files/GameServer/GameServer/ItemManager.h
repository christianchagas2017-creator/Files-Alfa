...
#define INVENTORY_ELEMENT_SLOT_RANGE(x) ((x) <= 234 ? 0 : ((x) >= INVENTORY_SIZE ? 0 : 1))
...
#else
#define INVENTORY_WEAR_RANGE(x) (((x)<0)?0:((x)>=INVENTORY_WEAR_SIZE)?((INVENTORY_ELEMENT_SLOT_RANGE(x)==1)?1:0):1)
#define INVENTORY_FULL_RANGE(x) (((x)<0)?0:((x)>=INVENTORY_EXT4_SIZE)?((INVENTORY_ELEMENT_SLOT_RANGE(x)==1)?1:0):1)
#endif
...
#define TRADEX_RANGE(x) (((x)<0)?0:((x)>=TRADEX_SIZE)?0:1)
...
#define CHAOS_BOX_GENESIS_RANGE(x) (((x)<0)?0:((x)>=CHAOS_BOX_GENESIS_SIZE)?0:1)
...
public:
	...
	BYTE InventoryAddItem(int aIndex, CItem item, int slot, int sourceSlot, bool firstLogin = false);
	...
	BYTE ChaosBoxGenesisAddItem(int aIndex, CItem item, int slot);
	BYTE ChaosBoxGenesisGetEmptySlot(int aIndex);
	void ChaosBoxGenesisDelItem(int aIndex, int slot);
	...
	BYTE MoveItemToInventoryFromChaosBoxGenesis(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToChaosBoxGenesisFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE& TargetSlot, BYTE TargetFlag);
...
#ifdef TRADE_X
	BYTE TradeXAddItem(int aIndex, CItem item, int slot);
	BYTE TradeXEmptySlot(int aIndex);
	void TradeXDelItem(int aIndex, int slot);
	BYTE MoveItemToInventoryFromTradeX(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag);
	BYTE MoveItemToTradeXFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE& TargetSlot, BYTE TargetFlag);
#endif
private:
