...
bool CItemBag::DropItem(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	for(int n=0;n < this->m_ItemDropCount;n++)
	{
		...
		{
			if (gCustomPick.IsPickItem(lpObj, item.m_Index) && gItemManager.CheckItemInventorySpace(lpObj, item.m_Index)) {
				GDCreateItemSend(lpObj->Index, (BYTE)235, (BYTE)0, (BYTE)0, item.m_Index, (BYTE)item.m_Level, 0, item.m_Option1, item.m_Option2, item.m_Option3, lpObj->Index, item.m_NewOption, item.m_SetOption, item.m_JewelOfHarmonyOption, item.m_ItemOptionEx, item.m_SocketOption, item.m_SocketOptionBonus, 0);
			}
			else
			{
