...
bool CShopManager::GCShopItemListSendByMonsterClass(int MonsterClass, int aIndex, int Map, int X, int Y) // OK
{

	for (std::map<int, SHOP_MANAGER_INFO>::iterator it = this->m_ShopManagerInfo.begin(); it != this->m_ShopManagerInfo.end(); it++)
	{
		if (it->second.MonsterClass == MonsterClass && (it->second.Map == -1 || it->second.Map == Map) && (it->second.X == -1 || it->second.X == X) && (it->second.Y == -1 || it->second.Y == Y))
		{
			return it->second.Shop.GCShopItemListSend(aIndex);
		}
	}
