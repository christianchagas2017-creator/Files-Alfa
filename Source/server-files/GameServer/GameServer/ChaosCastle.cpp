	...
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				...
				gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(16),(lpLevel->MinutesLeft+1));
#else
...
void CChaosCastle::ProcState_STAND(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) Not enough users",(lpLevel->Level+1));
...
{
	gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) SetState BLANK",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_EMPTY(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) SetState EMPTY",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_STAND(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) SetState STAND",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_START(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) SetState START",(lpLevel->Level+1));
}
...
void CChaosCastle::SetState_CLEAN(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) SetState CLEAN",(lpLevel->Level+1));
}
...
void CChaosCastle::CheckSync(CHAOS_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Chaos Castle] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),(lpLevel->RemainTime/60));
}
