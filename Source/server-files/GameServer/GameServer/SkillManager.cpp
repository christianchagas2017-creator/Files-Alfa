...
bool CSkillManager::CheckSkillRange(int index,int x,int y,int tx,int ty, int addRange) // OK
{
	SKILL_INFO SkillInfo;
	...
	int newRange = SkillInfo.Range + addRange;

	if(sqrt(pow(((float)x-(float)tx),2)+pow(((float)y-(float)ty),2)) <= newRange)
	{
		return 1;
	}
	else
...
bool CSkillManager::BasicSkillAttack(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if (lpObj->Inventory[8].m_Index == GET_ITEM(13,4) || lpObj->Inventory[8].m_Index == GET_ITEM(13, 37)) {
		addRange = 2;
	}
...
void CSkillManager::ApplyPlasmaStormEffect(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,int damage) // OK
{
	if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER)
	{
		if (gLuaGameServer.PlayerAttackFenrirSkill(lpObj->Index, lpTarget->Index) == 1) {
			return;
		}
