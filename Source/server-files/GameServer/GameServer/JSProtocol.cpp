...
void JoinServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0x30:
			...
			break;
		case 0x2F:
			GJServerMessageInfoRecv((SDHP_SERVER_MESSAGE_INFO_RECV*)lpMsg);
			break;
	}
...
void GJServerMessageInfoSend(char* msg, int type)
{
	...
	strcpy_s(pMsg.Msg, msg);

	gJoinServerConnection.DataSend((BYTE*)&pMsg, pMsg.header.size);
}

void GJServerMessageInfoRecv(SDHP_SERVER_MESSAGE_INFO_RECV* lpMsg)
{
	int type = lpMsg->Type;

	if (type == 2)
	{
		ChatYellowMessageAll(lpMsg->Msg);
	}
	else if (type == 3)
	{
		ChatBlueMessageAll(lpMsg->Msg);
	}
	else if (type == 4)
	{
		ChatGreenMessageAll(lpMsg->Msg);
	}
	else
	{
		gNotice.GCNoticeSendToAll(type, 0, 0, 0, 0, 0, lpMsg->Msg);
	}
}