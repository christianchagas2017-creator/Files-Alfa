...
bool CKanturuBattleUserMng::AddUserData(int aIndex) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] Add User Fail - Disconnect User [%s][%s]",gObj[aIndex].Account,gObj[aIndex].Name);
		return 0;
	}

	if(this->IsOverMaxUser() != 0)
	{
		gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] Add User Fail - Over Max User [%s][%s]",gObj[aIndex].Account,gObj[aIndex].Name);
		return 0;
	}
...
bool CKanturuBattleUserMng::DelUserData(int aIndex) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] Delete User Fail - Invalid Index:%d",aIndex);
		return 0;
	}
	...
	for(int n=0;n < MAX_KANTURU_BATTLE_USER;n++)
	{
		if(this->m_BattleUser[n].IsUseData() != 0 && this->m_BattleUser[n].GetIndex() == aIndex)
		{
			this->m_BattleUser[n].ResetData();
			this->m_BattleUserCount--;
			gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] Delete User - [%s][%s] Current Battle User:%d",gObj[aIndex].Account,gObj[aIndex].Name,this->m_BattleUserCount);
			return 1;
		}
	}
...
void CKanturuBattleUserMng::CheckUserState() // OK
{
	for(int n=0;n < MAX_KANTURU_BATTLE_USER;n++)
	{
		...
		if(gObjIsConnectedGP(this->m_BattleUser[n].GetIndex()) == 0)
		{
			...
			gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] Delete User - Disconnect [%s][%s]",gObj[this->m_BattleUser[n].GetIndex()].Account,gObj[this->m_BattleUser[n].GetIndex()].Name);
			continue;
		}
		...
		{
			this->DelUserData(this->m_BattleUser[n].GetIndex());
			gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] Delete User - Map Move [%s][%s]",gObj[this->m_BattleUser[n].GetIndex()].Account,gObj[this->m_BattleUser[n].GetIndex()].Name);
			continue;
		}
	}
}
...
void CKanturuBattleUserMng::MoveAllUser(int gate) // OK
{
	for(int n=0;n < MAX_KANTURU_BATTLE_USER;n++)
	{
		if(this->m_BattleUser[n].IsUseData() != 0)
		{
			...
			gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleUser ] [%s][%s] MoveGate(%d)",gObj[this->m_BattleUser[n].GetIndex()].Account,gObj[this->m_BattleUser[n].GetIndex()].Name,gate);
		}
	}
}
