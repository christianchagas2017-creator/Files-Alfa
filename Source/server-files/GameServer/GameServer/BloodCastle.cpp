	...
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				...
				gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(0),(lpLevel->MinutesLeft+1));
#else
...
void CBloodCastle::ProcState_STAND(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[Blood Castle] (%d) Not enough users",(lpLevel->Level+1));
...
{
	gLog.Output(LOG_EVENT,"[Blood Castle] (%d) SetState BLANK",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_EMPTY(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Blood Castle] (%d) SetState EMPTY",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_STAND(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Blood Castle] (%d) SetState STAND",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_START(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Blood Castle] (%d) SetState START",(lpLevel->Level+1));
}
...
void CBloodCastle::SetState_CLEAN(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Blood Castle] (%d) SetState CLEAN",(lpLevel->Level+1));
}
...
void CBloodCastle::CheckSync(BLOOD_CASTLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Blood Castle] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),(lpLevel->RemainTime/60));
}
...
void CBloodCastle::StartBC()
{
	...
	//LogAddEvent(LOG_BLUE, "[BloodCastle] Start At %2d:%2d:00", hour, minute);

	this->Init();

	this->m_BloodCastleStartTime.pop_back();
}
