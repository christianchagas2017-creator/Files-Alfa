	...
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				...
				gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(144),(lpLevel->MinutesLeft+1));
#else
...
void CDevilSquare::ProcState_STAND(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[Devil Square] (%d) Not enough users",(lpLevel->Level+1));
...
{
	gLog.Output(LOG_EVENT,"[Devil Square] (%d) SetState BLANK",(lpLevel->Level+1));
}
...
void CDevilSquare::SetState_EMPTY(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Devil Square] (%d) SetState EMPTY",(lpLevel->Level+1));
}
...
void CDevilSquare::SetState_STAND(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Devil Square] (%d) SetState STAND",(lpLevel->Level+1));
}
...
void CDevilSquare::SetState_START(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Devil Square] (%d) SetState START",(lpLevel->Level+1));
}
...
void CDevilSquare::SetState_CLEAN(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Devil Square] (%d) SetState CLEAN",(lpLevel->Level+1));
}
...
void CDevilSquare::CheckSync(DEVIL_SQUARE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Devil Square] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),(lpLevel->RemainTime/60));
}
