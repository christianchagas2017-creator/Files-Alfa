...
void CKanturuBattleOfNightmare::ProcState_START() // OK
{
	...
	if(OBJECT_RANGE(this->m_NightmareObjIndex) != 0 && gObj[this->m_NightmareObjIndex].Live == 0)
	{
		...
		gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] Success!! Nightmare Die");
	}
}
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] State(%d) -> NONE",this->m_BattleOfNightmareState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] State(%d) -> IDLE",this->m_BattleOfNightmareState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] State(%d) -> NOTIFY",this->m_BattleOfNightmareState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] State(%d) -> START",this->m_BattleOfNightmareState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] State(%d) -> END",this->m_BattleOfNightmareState);
	...
	if(this->GetSuccessValue() == 0)
	{
		...
		gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] Fail!! TimeOut");
	}
}

void CKanturuBattleOfNightmare::SetState_ENDCYCLE() // OK
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] State(%d) -> ENDCYCLE",this->m_BattleOfNightmareState);
...
void CKanturuBattleOfNightmare::CheckNightmareBattleUser() // OK
{
	if(this->GetBattleOfNightmareState() == KANTURU_NIGHTMARE_NOTIFY || this->GetBattleOfNightmareState() == KANTURU_NIGHTMARE_START)
	{
		if(gKanturuBattleUserMng.IsEmpty() != 0)
		{
			...
			gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleOfNightmare ] Fail!! All Battle User Die");
		}
	}
}
