...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	{
		if (gLuaGameServer.PlayerAttack(lpObj->Index, lpTarget->Index, 0) == 1)
		{
			return 0;
		}
	}
	...
	{
		if (gServerInfo.m_BlockTwisterSlash == 1)
		{
		...
		if(skill == SKILL_FALLING_SLASH || skill == SKILL_LUNGE || skill == SKILL_UPPERCUT || skill == SKILL_CYCLONE || skill == SKILL_SLASH || skill == SKILL_TWISTING_SLASH || skill == SKILL_RAGEFUL_BLOW || skill == SKILL_DEATH_STAB || skill == SKILL_CRESCENT_MOON_SLASH || skill == SKILL_STAR_FALL || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH || skill == SKILL_ICE_ARROW || skill == SKILL_PENETRATION || skill == SKILL_FIRE_SLASH || skill == SKILL_POWER_SLASH || skill == SKILL_SPIRAL_SLASH || skill == SKILL_FROZEN_STAB || skill == SKILL_SWORD_SLASH || skill == SKILL_CHARGE || skill == SKILL_BLOOD_STORM)
		{
			if(skill != SKILL_IMPALE || lpObj->Inventory[8].m_Index == GET_ITEM(13,2) || lpObj->Inventory[8].m_Index == GET_ITEM(13,3) || lpObj->Inventory[8].m_Index == GET_ITEM(13,37))
			{
				if(lpObj->Class == CLASS_DK)
				{
					if (skill == SKILL_TWISTING_SLASH)
					{
						damage = (damage * lpObj->DKDamageMultiplierRate) / gServerInfo.m_TwisterSlashConstA;
					}
					else
					{
	...
	if(damage > 0)
	{
		...
		if (gServerInfo.m_DisablePushSkills == 0) {
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	if (gImmortalPet.CheckIsItem(lpItem->m_Index)) {
		return;
	}
	...
	if(lpItem->m_Index == GET_ITEM(13,0))
	{
		lpItem->m_Durability -= (damage*(3.0f*DurabilityValue))/100;
	}
	else if(lpItem->m_Index == GET_ITEM(13,1) || lpItem->m_Index == GET_ITEM(13, 65))
	{
		lpItem->m_Durability -= (damage*(2.0f*DurabilityValue))/100;
	}
	else if(lpItem->m_Index == GET_ITEM(13,2) || lpItem->m_Index == GET_ITEM(13, 3)  || lpItem->m_Index == GET_ITEM(13, 64) || lpItem->m_Index == GET_ITEM(13, 67) || lpItem->m_Index == GET_ITEM(13, 80) || lpItem->m_Index == GET_ITEM(13, 106) || lpItem->m_Index == GET_ITEM(13, 123) || (gCustomPets.GetCustomPetType(lpItem->m_Index) >= 0 && gCustomPets.GetCustomPetType(lpItem->m_Index) <= 2))
	{
		lpItem->m_Durability -= (damage*(1.0f*DurabilityValue))/100;
	}
	else if(lpItem->m_Index == GET_ITEM(13,4) || gCustomPets.GetCustomPetType(lpItem->m_Index) == 4)
	{
	...
	else if(lpItem->m_Index == GET_ITEM(13,37) || gCustomPets.GetCustomPetType(lpItem->m_Index) == 3)
	{
	...
	if(lpItem->m_Durability < 1)
	{
		...
		if(lpObj->Map == MAP_ICARUS && lpObj->Inventory[7].IsItem() == 0 && (lpItem->m_Index == GET_ITEM(13,3) || lpItem->m_Index == GET_ITEM(13,37) || gCustomPets.GetCustomPetType(lpItem->m_Index) == 3 || gCustomPets.GetCustomPetType(lpItem->m_Index) == 2))
		{
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gServerInfo.m_StaffKillPlayer == 0)
	{
		if (lpObj->Authority > 1 && lpTarget->Authority <= 1)
		{
			return 0;
		}
	}

	if (gServerInfo.m_StaffKillStaff == 0)
	{
		if (lpObj->Authority > 1 && lpTarget->Authority > 1)
		{
			return 0;
	...
	if (gServerInfo.m_PlayerKillStaff == 0)
	{
		if (lpObj->Authority <= 1 && lpTarget->Authority > 1)
		{
			return 0;
...
int CAttack::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,WORD* effect,int TargetDefense) // OK
{
	...
	else
	{
		...
		if(DualHandWeapon != 0)
		{
			DamageMin = (((lpObj->PhysiDamageMinRight + lpObj->PhysiDamageMinLeft + SkillDamageMin) * gServerInfo.m_TwoWeaponsDamageMinConstA) / 100);
			DamageMax = (((lpObj->PhysiDamageMaxRight + lpObj->PhysiDamageMaxLeft + SkillDamageMax) * gServerInfo.m_TwoWeaponsDamageMaxConstA) / 100);
		}
