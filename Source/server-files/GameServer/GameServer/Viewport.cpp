...
void CViewport::GCViewportPlayerNewsSend(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];

	BYTE send[2192];

	PMSG_VIEWPORT_NEWS_SEND pMsg;
	...
	PMSG_VIEWPORT_PLAYER_NEWS info;
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if (lpTarget->Inventory[247].IsItem())
		{
			info.WingIndex = lpTarget->Inventory[247].m_Index - GET_ITEM(12, 0);
		}
		else
		{
			if (lpTarget->Inventory[7].IsItem())
			{
				if (gCustomWing.CheckCustomWingByItem(lpTarget->Inventory[7].m_Index) != 0)
				{
					info.WingIndex = lpTarget->Inventory[7].m_Index - GET_ITEM(12, 0);
				}
				else
				{
					info.WingIndex = 0;
		...
		if (lpTarget->Inventory[248].IsItem())
		{
			info.PetIndex = lpTarget->Inventory[248].m_Index - GET_ITEM(13, 0);
		}
		else
		{
			if (lpTarget->Inventory[8].IsItem())
			{
				if (gCustomPets.CheckCustomPet(lpTarget->Inventory[8].m_Index) != 0)
				{
		...
		if (lpTarget->Inventory[236].IsItem())
		{
			if (gElementSlots.CheckIsPet(236))
			{
				info.ElementSlot[0] = lpTarget->Inventory[236].m_Index;
			}
			else
			{
				info.ElementSlot[0] = 0;
			}
		}
		else {
			info.ElementSlot[0] = 0;
		...
		if (lpTarget->Inventory[237].IsItem())
		{
			if (gElementSlots.CheckIsPet(237))
			{
				info.ElementSlot[1] = lpTarget->Inventory[237].m_Index;
			}
			else
			{
				info.ElementSlot[1] = 0;
			}
		}
		else
		{
...
void CViewport::GCViewportSimplePlayerNewsSend(LPOBJ lpObj)
{
	if ((lpObj->Authority & 32) != 0 && gEffectManager.CheckEffect(lpObj, EFFECT_INVISIBILITY) != 0)
	{
		return;
	...
	if (lpObj->Inventory[247].IsItem() && lpObj->Inventory[7].IsItem())
	{
		info.WingIndex = lpObj->Inventory[247].m_Index - GET_ITEM(12, 0);
	}
	else
	{
		if (lpObj->Inventory[7].IsItem())
		{
			if (gCustomWing.CheckCustomWingByItem(lpObj->Inventory[7].m_Index) != 0)
			{
				info.WingIndex = lpObj->Inventory[7].m_Index - GET_ITEM(12, 0);
			}
			else
			{
				info.WingIndex = 0;
	...
	if (lpObj->Inventory[248].IsItem() && lpObj->Inventory[8].IsItem())
	{
		info.PetIndex = lpObj->Inventory[248].m_Index - GET_ITEM(13, 0);
	}
	else
	{
		if (lpObj->Inventory[8].IsItem())
		{
			if (gCustomPets.CheckCustomPet(lpObj->Inventory[8].m_Index) != 0)
			{
	...
	if (lpObj->Inventory[236].IsItem())
	{
		if (gElementSlots.CheckIsPet(236))
		{
			info.ElementSlot[0] = lpObj->Inventory[236].m_Index;
		}
		else
		{
			info.ElementSlot[0] = 0;
	...
	if (lpObj->Inventory[237].IsItem())
	{
		if (gElementSlots.CheckIsPet(237))
		{
			info.ElementSlot[1] = lpObj->Inventory[237].m_Index;
		}
		else
		{
			info.ElementSlot[1] = 0;
...
void CViewport::GCViewportPatentSend(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];

	BYTE send[2192];

	PMSG_VIEWPORT_PATENT_SEND pMsg;
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		info.patent = lpTarget->m_Patent;
		info.type = lpTarget->m_PatentType;
...
void CViewport::GCViewportSimplePatentSend(LPOBJ lpObj)
{
	if (gObjIsChangeSkin(lpObj->Index) != 0)
	{
		return;
	...
	info.patent = lpObj->m_Patent;
	info.type = lpObj->m_PatentType;
