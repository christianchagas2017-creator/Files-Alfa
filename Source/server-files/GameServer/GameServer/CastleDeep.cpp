...
void CCastleDeep::ProcState_START() // OK
{
	...
	if(this->m_RemainTime <= 0)
	{
		...
		gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(354));
#else
...
{
	gLog.Output(LOG_EVENT,"[Castle Deep] SetState BLANK");
}
...
void CCastleDeep::SetState_EMPTY() // OK
{
	...
	gLog.Output(LOG_EVENT,"[Castle Deep] SetState EMPTY");
}
...
void CCastleDeep::SetState_START() // OK
{
	...
	gLog.Output(LOG_EVENT,"[Castle Deep] SetState START");
}
...
void CCastleDeep::CheckSync() // OK
{
	...
	gLog.Output(LOG_EVENT,"[Castle Deep] Sync Start Time. [%d] min remain",(this->m_RemainTime/60));
}
...
void CCastleDeep::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
#ifdef GLOBAL_DASHBOARD
		gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(355),lpTarget->Name);
#else
	...
	{
#ifdef GLOBAL_DASHBOARD
		gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(356),lpObj->Name,lpTarget->Name);
#else
