...
void CWarehouse::CGWarehouseChangeRecv(PMSG_WAREHOUSE_CHANGE_RECV* lpMsg, int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	if (lpMsg->targetVault < 1 || lpMsg->targetVault > lpObj->WarehouseCount)
	{
	...
	lpObj->WarehouseNumber = lpMsg->targetVault;
...
void CWarehouse::CGWarehouseBuyCostRecv(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	gLuaGameServer.SendWarehouseCost(aIndex);
}


void CWarehouse::CGWarehouseBuyRecv(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
	...
	gLuaGameServer.PlayerBuyVaultOk(aIndex);
}
...
void CWarehouse::GCWarehouseMoneySend(int aIndex,BYTE result,int InventoryMoney,int WarehouseMoney) // OK
{
	...
	pMsg.CurrentWarehouse = lpObj->WarehouseNumber;

	pMsg.WarehouseCount = lpObj->WarehouseCount;
...
void CWarehouse::DGWarehouseItemRecv(SDHP_WAREHOUSE_ITEM_RECV* lpMsg) // OK
{
	...
	gDeathSystem.GDWarehouseGetInformation(lpObj->Index);
...
void CWarehouse::DGWarehouseInfoRecv(SDHP_WAREHOUSE_INFO_RECV* lpMsg) // OK
{
	if (gObjIsAccountValid(lpMsg->index, lpMsg->account) == 0)
	{
	...
	lpObj->WarehouseCount = lpMsg->WarehouseCount;
}
...
void CWarehouse::GDWarehouseInfoSend(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
