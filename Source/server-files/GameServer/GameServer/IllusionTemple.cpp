	...
	if(lpLevel->RemainTime > 0 && lpLevel->RemainTime <= (this->m_WarningTime*60))
	{
		...
		if(lpLevel->MinutesLeft != minutes)
		{
			...
			if(lpLevel->Level == 0)
			{
				...
				gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(176),(lpLevel->MinutesLeft+1));
#else
...
void CIllusionTemple::ProcState_STAND(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) Not enough users",(lpLevel->Level+1));
...
{
	gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) SetState BLANK",(lpLevel->Level+1));
}
...
void CIllusionTemple::SetState_EMPTY(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) SetState EMPTY",(lpLevel->Level+1));
}
...
void CIllusionTemple::SetState_STAND(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) SetState STAND",(lpLevel->Level+1));
}
...
void CIllusionTemple::SetState_START(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) SetState START",(lpLevel->Level+1));
}
...
void CIllusionTemple::SetState_CLEAN(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) SetState CLEAN",(lpLevel->Level+1));
}
...
void CIllusionTemple::CheckSync(ILLUSION_TEMPLE_LEVEL* lpLevel) // OK
{
	...
	gLog.Output(LOG_EVENT,"[Illusion Temple] (%d) Sync Start Time. [%d] min remain",(lpLevel->Level+1),(lpLevel->RemainTime/60));
}
