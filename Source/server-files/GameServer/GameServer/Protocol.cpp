...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	switch(head)
	{
		...
		case 0x4C:
			switch(lpMsg[3])
			{
				...
				case 0x03:
					...
					break;
				case 0x81:
					gCustomJewelStack.UnSplitJewel((CUSTOM_JEWEL_STACK_INFO_RECV*)lpMsg, aIndex);
					break;
			}
			break;
		case 0x4D:
		...
		case 0x66:
			...
			break;
		case 0x80:
			gWarehouse.CGWarehouseBuyCostRecv(aIndex);
			break;
		case 0x81:
		...
		case 0x83:
			...
			break;
		case 0x84:
			gWarehouse.CGWarehouseChangeRecv((PMSG_WAREHOUSE_CHANGE_RECV*)lpMsg, aIndex);
			break;
		case 0x85:
			gWarehouse.CGWarehouseBuyRecv(aIndex);
			break;
		case 0x86:
		...
		case 0xEF:
			switch(lpMsg[3])
			{
				...
				case 0x20:
					...
					gChaosBoxGenesis.CGChaosBoxGenesisSetInfoRecv((SDHP_CHAOS_BOX_GENESIS_SET_INFO_RECV*)lpMsg, aIndex);
				#endif
					break;
				case 0x22:
				#ifdef CHAOS_MACHINE_GENESIS
					gChaosBoxGenesis.CGChaosBoxGenesisChaosMix((SDHP_CHAOS_BOX_GENESIS_MIX_RECV*)lpMsg, aIndex);
				#endif
					break;
				case 0x23:
				#ifdef CHAOS_MACHINE_GENESIS
					gChaosBoxGenesis.CGChaosBoxGenesisCloseRecv(aIndex);
				#endif
					break;
			}
			break;
		...
		case 0xF3:
			switch(lpMsg[3])
			{
				...
				case 0x53:
					...
					gWarehouse.GDWarehouseItemSend(aIndex, gObj[aIndex].Account);
					break;
				case 0xFA:
					gClientInfo.ReceiveInfo((PMSG_CLIENT_INFO_RECV*)lpMsg, aIndex);
					break;
#ifdef TRADE_X
				case 0xC0:
					gTradeX.TradeXOpenWindowRecv((PMSG_TRADEX_OPEN_WINDOW_RECV*)lpMsg, aIndex);
					break;
				case 0xC1:
					gTradeX.TradeXHiddenWindowRecv((PMSG_TRADEX_HIDDEN_WINDOW_RECV*)lpMsg, aIndex);
					break;
#endif
			}
			break;
		...
		case 0xFC:
			...
			break;
		case 0xFA:
			gSocket.RecvPacketCustomSocket(aIndex, (PMSG_CUSTOM_SOCKET_RECV*)lpMsg, lpMsg, lpMsg[4], size);
			break;
	}
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->TradeDuel == 0) {
		if (gLuaGameServer.ChatProc(aIndex, lpMsg->message) == 1)
		{
			return;
		}
	}
...
void CGPositionRecv(PMSG_POSITION_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->x < (lpObj->X- distance) || lpMsg->x > (lpObj->X+ distance) || lpMsg->y < (lpObj->Y- distance) || lpMsg->y > (lpObj->Y+ distance))
	{
		return;
	}
...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->Type == OBJECT_USER && lpObj->Connected == OBJECT_ONLINE)
	{
		gLuaGameServer.PlayerMove(aIndex, lpObj->Map, lpMsg->x, lpMsg->y, lpObj->TX, lpObj->TY);
	}
	...
	if(lpObj->TX < (lpObj->X- distance) || lpObj->TX > (lpObj->X+ distance) || lpObj->TY < (lpObj->Y- distance) || lpObj->TY > (lpObj->Y+ distance))
	{
		lpObj->PathCur = 0;
		lpObj->PathCount = 0;
...
void CGCharacterDeleteRecv(PMSG_CHARACTER_DELETE_RECV* lpMsg,int aIndex) // OK
{
	...
	if (gLuaGameServer.DeleteCharacter(aIndex, name) != 0)
	{
		pMsg.result = 2;
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
...
#ifdef FAIRPLAY
void SendHunting(int aIndex, WORD level, int averageExp, int averageExpPerMinute, int estimatedTimeToLevelUp, int lastExpGained, int avarageZenPerMinute, int LastMoney)
{
	PMSG_CHARACTER_HUNTING_INFO_SEND pMsg;

	pMsg.header.set(0xFB, 0x16, sizeof(pMsg));

	pMsg.level = level;
	pMsg.avarageExp = averageExp;
	pMsg.expPerMinute = averageExpPerMinute;
	pMsg.etaLevelUp = estimatedTimeToLevelUp;
	pMsg.lastExp = lastExpGained;
	pMsg.zenPerMinue = avarageZenPerMinute;
	pMsg.lastZen = LastMoney;
...
void GCDamageSend(int aIndex,int bIndex,BYTE flag,int damage,int type,int ShieldDamage) // OK
{
	...
	if (gObj[aIndex].Monitored == 1 || gObj[bIndex].Monitored == 1) {
		int px = gObj[aIndex].X;
		int py = gObj[aIndex].Y;
		if (gObj[bIndex].Monitored == 1) {
			px = gObj[bIndex].X;
			py = gObj[bIndex].Y;
		}
		for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
		{
			...
			{
				if (gObj[n].Monitoring == 1)
				{
					int gx = gObj[n].X;
					int gy = gObj[n].Y;
					if (gx >= px - 6 && gx <= px + 6 && gy >= py - 6 && gy <= py + 6) {
						DataSend(n, (BYTE*)&pMsg, pMsg.header.size);
					}
				}
			}
...
void GCFireworksSendCustom(int map, int x, int y) //Ok
{
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
		if (gObjIsConnectedGP(n) != 0)
		{
			if (gViewport.CheckViewportObjectPosition(n, map, x, y, 16) != 0)
			{
