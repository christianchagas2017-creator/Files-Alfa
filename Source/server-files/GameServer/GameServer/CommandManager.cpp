...
void CCommandManager::Init() // OK
{
	...
	this->Add("/tradex", COMMAND_TRADEX);
	this->Add("/pick", COMMAND_PICK);
}
...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message) // OK
{
	...
	switch(code)
	{
		...
		case COMMAND_CUSTOM_ATTACK_OFFLINE:
			...
			break;
		case COMMAND_PICK:
			gCustomPick.ToggleUserPick(lpObj);
			break;
		//case COMMAND_TRADEX:
		//	this->CommandTradeX(lpObj, argument);
		//	break;
		default:
			return 0;
	}
...
void CCommandManager::CommandTradeX(LPOBJ lpObj, char* arg)
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Voc� est� ocupado no momento!");
		return;
	}
	...
	if (lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "O jogador alvo n�o est� online");
		return;
	}

	if (lpTarget->UsingTradeX != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Este jogador não pode negociar com você no momento!");
		return;
	}
...
void CCommandManager::CommandWare(LPOBJ lpObj,char* arg) // OK
{
	...
	if(number < 1 || number > lpObj->WarehouseCount)
	{
...
void CCommandManager::CommandGMPost(LPOBJ lpObj,char* arg) // OK
{
	...
	gNotice.GCNoticeSendToDash(0, "[%s] %s",lpObj->Name,arg);
#else
