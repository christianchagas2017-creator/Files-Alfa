...
void CKanturuMaya::KanturuMayaAct_IceStorm(int rate) // OK
{
	...
	gLog.Output(LOG_EVENT,"[ KANTURU ][ IceStorm ] Skill Using(%d) : Index(%d) %s",this->m_IceStormCount,this->m_MayaObjIndex,gObj[this->m_MayaObjIndex].Name);
}
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		if(gObjIsConnectedGP(n) != 0 && gObj[n].Map == MAP_KANTURU3)
		{
			...
			if((gObj[n].Inventory[10].m_Index == GET_ITEM(13,38) && gObj[n].Inventory[10].m_Durability == 0) || (gObj[n].Inventory[11].m_Index == GET_ITEM(13,38) && gObj[n].Inventory[11].m_Durability == 0))
			{
				...
				gLog.Output(LOG_EVENT,"[ KANTURU ][ BrokenShower ] [%s][%s] User Dying cause NOT wearing MoonStone Pandent",gObj[n].Account,gObj[n].Name);
			}
		}
	}

	gLog.Output(LOG_EVENT,"[ KANTURU ][ BrokenShower ] Skill Using : Index(%d) %s",this->m_MayaObjIndex,gObj[this->m_MayaObjIndex].Name);
}
