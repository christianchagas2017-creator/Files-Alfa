...
int CItemDrop::DropItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	else
	{
		...
		if (gCustomPick.IsPickItem(lpTarget, ItemIndex) && gItemManager.CheckItemInventorySpace(lpTarget, ItemIndex)) {
			GDCreateItemSend(lpTarget->Index, (BYTE)235, (BYTE)0, (BYTE)0, ItemIndex, ItemLevel, 0, ItemOption1, ItemOption2, ItemOption3, lpTarget->Index, ((ItemNewOption == 0) ? lpItemDropInfo->Grade : ItemNewOption), ItemSetOption, 0, 0, ItemSocketOption, 0xFF, ((lpItemDropInfo->Duration > 0) ? ((DWORD)time(0) + lpItemDropInfo->Duration) : 0));
		}
		else
		{
