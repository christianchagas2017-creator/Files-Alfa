...
struct PMSG_VIEWPORT_PATENT_SEND
{
...
struct PMSG_VIEWPORT_NEWS_SEND
{
	PSWMSG_HEAD header; // C2:[12:13:1F:20:45:65:68]
	BYTE count;
	BYTE Viewport;
...
struct PMSG_VIEWPORT_PLAYER_NEWS
{
	char name[11];
	WORD PetIndex;
	WORD SecondPetIndex;
...
public:
	...
	void GCViewportPlayerNewsSend(int aIndex);
	void GCViewportSimplePlayerNewsSend(LPOBJ lpObj);
	void GCViewportPatentSend(int aIndex);
	void GCViewportSimplePatentSend(LPOBJ lpObj);
};
