...
void CRaklionBattleUserMng::AddUserData(int aIndex) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[ RAKLION ][ BattleUser ] Add User Fail - Disconnect User [%s][%s]",gObj[aIndex].Account,gObj[aIndex].Name);
		return;
	}
	...
	gLog.Output(LOG_EVENT,"[ RAKLION ][ Battle User ] Add User [%s][%s] Current Battle User:%d",gObj[aIndex].Account,gObj[aIndex].Name,this->m_UserInfo.size());
}
...
void CRaklionBattleUserMng::DelUserData(int aIndex) // OK
{
	...
	{
		gLog.Output(LOG_EVENT,"[ RAKLION ][ BattleUser ] Delete User Fail - Invalid Index:%d",aIndex);
		return;
	}
	...
	for(int n=0;n < this->GetBattleUserCount();n++)
	{
		if(this->m_UserInfo[n] == aIndex)
		{
			...
			gLog.Output(LOG_EVENT,"[ RAKLION ][ BattleUser ] Delete User - [%s][%s] Current Battle User:%d",gObj[aIndex].Account,gObj[aIndex].Name,this->m_UserInfo.size());
			return;
		}
	}
}
