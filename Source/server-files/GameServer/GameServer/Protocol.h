...
struct PMSG_CHARACTER_LIST_NEWS_SEND
{
	PSWMSG_HEAD header;
	BYTE count;
};

struct PMSG_CHARACTER_LIST_NEWS
{
	char Name[10];
	WORD PetIndex;
	WORD WingIndex;
...
#ifdef FAIRPLAY
struct PMSG_CHARACTER_HUNTING_INFO_SEND
{
	PSBMSG_HEAD header;	// C1:0B
	WORD level;
	DWORD avarageExp;
	DWORD expPerMinute;
	DWORD etaLevelUp;
...
void GCFireworksSendCustom(int map, int x, int y);
#ifdef FAIRPLAY
