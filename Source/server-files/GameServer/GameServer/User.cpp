	...
	if(gCloseMsg != 0)
	{
		if((--gCloseMsgTime) <= 1)
		{
			if(gCloseMsgTime == 1)
			{
				...
				gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(480));
#else
		...
		else
		{
			...
			{
#ifdef GLOBAL_DASHBOARD
				gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(481),gCloseMsgTime);
#else
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	lpObj->RageFighterSkillTarget = 14000;
	...
	memset(lpObj->TradeXTarget, 0, sizeof(lpObj->TradeXTarget));
	lpObj->TradeXState = 0;
	lpObj->CoinsRate = 0;
...
#ifdef FAIRPLAY
	lpObj->lastExpGained = 0;
	lpObj->minuteCount = 0;
	lpObj->expReceiveCount = 0;
	lpObj->totalExpGained = 0;
	lpObj->LastMoneyObtained = 0;
	lpObj->MoneyObtainedTotal = 0;
	lpObj->startExpGainTick = GetTickCount();
	lpObj->lastExpGainTick = GetTickCount();
#endif
}
...
short gObjDel(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		//clear cache
		gPlayerCache[lpObj->Index].m_cacheInt.clear();
		gPlayerCache[lpObj->Index].m_cacheStr.clear();
	}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj) // OK
{
	if(lpObj->Type == OBJECT_USER)
	{
		...
		gViewport.GCViewportSimplePatentSend(lpObj);
	}
	else
	{
...
void gObjViewportListProtocol(int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		gViewport.GCViewportPatentSend(aIndex);
	}
...
void gObjInterfaceCheckTime(LPOBJ lpObj) // OK
{
	...
	if (lpObj->Interface.type == INTERFACE_TRADEX)
	{
		if (lpObj->Interface.state == 0)
		{
			if (OBJECT_RANGE(lpObj->TargetNumber) != 0)
			{
				if (gObj[lpObj->TargetNumber].Interface.type == INTERFACE_TRADEX)
				{
					gObj[lpObj->TargetNumber].Interface.use = 0;
					gObj[lpObj->TargetNumber].Interface.type = INTERFACE_NONE;
					gObj[lpObj->TargetNumber].Interface.state = 0;
					gObj[lpObj->TargetNumber].TargetNumber = -1;
					gObj[lpObj->TargetNumber].UsingTradeX = 0;
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if (gLuaGameServer.CharacterMove(aIndex, map, x, y)) {
		goto ERROR_JUMP;
	}
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
...
#ifdef FAIRPLAY
				if (GetTickCount() - lpObj->startExpGainTick >= 1000)
				{
					lpObj->minuteCount++;
					lpObj->startExpGainTick = GetTickCount();
				}

				if (GetTickCount() - lpObj->lastExpGainTick > 300000)
				{
					lpObj->minuteCount = 0;
					lpObj->totalExpGained = 0;
					lpObj->expReceiveCount = 0;
				...
				int averageExp = 0;
				int averageExpPerMinute = 0;
				int estimatedTimeToLevelUp = 0;
				int averageExpPerSecond = 0;
				int avarageZenPerMinute = 0;

				if (lpObj->expReceiveCount > 0)
				{
					averageExp = lpObj->totalExpGained / lpObj->expReceiveCount;
				}

				if (lpObj->minuteCount > 0)
				{
					averageExpPerSecond = floor(lpObj->totalExpGained / lpObj->minuteCount);
					avarageZenPerMinute = floor((lpObj->MoneyObtainedTotal / lpObj->minuteCount)*60);
					averageExpPerMinute = averageExpPerSecond * 60;
				}

				int remainingExp = lpObj->NextExperience - lpObj->Experience;

				if (averageExpPerSecond > 0)
				{
					estimatedTimeToLevelUp = remainingExp / averageExpPerSecond;
				}

				SendHunting(lpObj->Index, lpObj->Level, averageExp, averageExpPerMinute, estimatedTimeToLevelUp, lpObj->lastExpGained, avarageZenPerMinute, lpObj->LastMoneyObtained);
#endif
...
void gObjCustomLogPlusChaosMix(LPOBJ lpObj,int type,int index)
{
	...
	if(gServerInfo.m_AnnounceChaosMix != 0)
	{
		...
		if(gItemManager.GetInfo(index,&ItemInfo) != 0)
		{
			...
			{
#ifdef GLOBAL_DASHBOARD
				gNotice.GCNoticeSendToDash(0, buff);
#else
