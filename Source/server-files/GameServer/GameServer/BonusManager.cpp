...
void CBonusManager::ProcState_EMPTY(BONUS_INFO* lpInfo) // OK
{
	if(lpInfo->RemainTime <= 0)
	{
		...
		if(lpInfo->StartMessage != -1){ gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(lpInfo->StartMessage));}
#else
...
void CBonusManager::ProcState_START(BONUS_INFO* lpInfo) // OK
{
	...
	{
#ifdef GLOBAL_DASHBOARD
		if(lpInfo->FinalMessage != -1){ gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(lpInfo->FinalMessage));}
#else
