...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State(%d) -> NONE",this->m_TowerOfRefinementState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State(%d) -> REVITALIZATION",this->m_TowerOfRefinementState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State(%d) -> NOTIFY1",this->m_TowerOfRefinementState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State(%d) -> CLOSE",this->m_TowerOfRefinementState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State(%d) -> NOTIFY2",this->m_TowerOfRefinementState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State(%d) -> END",this->m_TowerOfRefinementState);
	...
	for(int n=OBJECT_START_USER;n < MAX_OBJECT;n++)
	{
		...
		{
			gObjMoveGate(n,136);
			gLog.Output(LOG_EVENT,"[ KANTURU ][ TowerOfRefinement ] State End:Kick Out [%s][%s]",gObj[n].Account,gObj[n].Name);
		}
	}
}
