...
void CTrade::ResetTrade(int aIndex) // OK
{
	...
	{
		gSkillManager.SkillChangeUse(aIndex);
	}

	gViewport.GCViewportPlayerNewsSend(lpObj->Index);
...
void CTrade::ExchangeTradeItemLog(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	for(int n=0;n < TRADE_SIZE;n++)
	{
		if(lpObj->Trade[n].IsItem() != 0)
		{
			gLog.Output(LOG_TRADE,"[ExchangeTradeItem][%s][%s] - (Account: %s, Name: %s, Item: %s, Level: %02d, Serial: %08X, Option1: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d, %03d, %03d, %03d, %03d)",lpObj->Account,lpObj->Name,lpTarget->Account,lpTarget->Name,lpObj->Trade[n].GetName(),lpObj->Trade[n].m_Level,lpObj->Trade[n].m_Serial,lpObj->Trade[n].m_Option1,lpObj->Trade[n].m_Option2,lpObj->Trade[n].m_Option3,lpObj->Trade[n].m_NewOption,lpObj->Trade[n].m_JewelOfHarmonyOption,lpObj->Trade[n].m_ItemOptionEx,lpObj->Trade[n].m_SocketOption[0],lpObj->Trade[n].m_SocketOption[1],lpObj->Trade[n].m_SocketOption[2],lpObj->Trade[n].m_SocketOption[3],lpObj->Trade[n].m_SocketOption[4]);
		}
	}
}
...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpMsg->isTradeX)
	{
		gTradeX.TradeXRequestSend(lpObj, lpTarget);
		return;
	}

	if (lpObj->UsingTradeX && lpTarget->UsingTradeX) {
		return;
	}
	else if (lpObj->UsingTradeX || lpTarget->UsingTradeX) {
		return;
	}

	if (lpObj->DieRegen != 0 || lpTarget->DieRegen != 0)
	{
		return;
	}
	...
	if (gLuaGameServer.PlayerSendTrade(aIndex, bIndex))
	{
		this->GCTradeResponseSend(aIndex, 0, lpTarget->Name, 0, 0);
		return;
	}
...
void CTrade::CGTradeResponseRecv(PMSG_TRADE_RESPONSE_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->UsingTradeX && lpTarget->UsingTradeX) {
		gTradeX.TradeXResponseRecv(lpMsg, lpObj, lpTarget);
		return;
	}
	else if (lpObj->UsingTradeX || lpTarget->UsingTradeX) {
		return;
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg,int aIndex) // OK
{
	...
	if (lpObj->UsingTradeX && lpTarget->UsingTradeX) {
		gTradeX.TradeXOkButtonRecv(lpMsg, lpObj, lpTarget);
		return;
	}
	...
	if (lpObj->TradeDuel == 1 && lpTarget->TradeDuel == 1)
	{
		...
		gDuel.StartDuelBet(lpObj, lpTarget);
		return;
	}

	if (gLuaGameServer.PlayerTradeOk(lpObj->Index, lpTarget->Index))
	{
		this->ResetTrade(lpObj->Index);
		this->GCTradeResultSend(lpObj->Index, 3);
		this->ResetTrade(lpTarget->Index);
		this->GCTradeResultSend(lpTarget->Index, 3);
		return;
	}
	...
	gDeathSystem.SendInformation(lpObj, 0);
	gDeathSystem.SendInformation(lpTarget, 0);
}
...
void CTrade::CGTradeCancelButtonRecv(int aIndex) // OK
{
	...
	if(gObjIsConnectedGP(bIndex) == 0)
	{
		if (lpObj->UsingTradeX) {
			...
			gTradeX.TradeXHiddenWindowRecv((PMSG_TRADEX_HIDDEN_WINDOW_RECV*)&pMsg, lpObj->Index);
		}
		return;
	}
...
void CTrade::GCTradeResponseSend(int aIndex,BYTE response,char* name,int level,int GuildNumber, BYTE TradeX) // OK
{
...
void CTrade::GCTradeOkButtonSend(int aIndex,BYTE flag, BYTE tradeX) // OK
{
...
void CTrade::GCTradeResultSend(int aIndex,BYTE result, BYTE tradeX) // OK
{
	PMSG_TRADE_RESULT_SEND pMsg;
