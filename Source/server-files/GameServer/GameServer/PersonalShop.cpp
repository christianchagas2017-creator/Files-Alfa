...
bool CPersonalShop::CheckPersonalShopOpen(int aIndex) // OK
{
	...
	for (int n = INVENTORY_EXT4_SIZE; n < INVENTORY_FULL_SIZE; n++)
	{
		if (gObj[aIndex].Inventory[n].IsItem() != 0)
		{
			...
			if (gObj[aIndex].Inventory[n].m_PShopValue1 <= 0 && gObj[aIndex].Inventory[n].m_PShopValue2 <= 0 && gObj[aIndex].Inventory[n].m_PShopValue3 <= 0 && gObj[aIndex].Inventory[n].m_PShopValue4 <= 0)
			{
				return 0;
			}
			else
...
void CPersonalShop::CGPShopSetItemPriceRecv(PMSG_PSHOP_SET_ITEM_PRICE_RECV* lpMsg, int aIndex) // OK
{
	...
	int price2 = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->price2[3], lpMsg->price2[2]), MAKE_NUMBERW(lpMsg->price2[1], lpMsg->price2[0]));
	int price3 = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->price3[3], lpMsg->price3[2]), MAKE_NUMBERW(lpMsg->price3[1], lpMsg->price3[0]));
	int price4 = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->price4[3], lpMsg->price4[2]), MAKE_NUMBERW(lpMsg->price4[1], lpMsg->price4[0]));

	if (price <= 0 && price2 <= 0 && price3 <= 0 && price4 <= 0)
	{
		this->GCPShopSetItemPriceSend(aIndex, 4, lpMsg->slot);
		return;
	}

	lpObj->Inventory[lpMsg->slot].m_PShopValue1 = price;
	lpObj->Inventory[lpMsg->slot].m_PShopValue2 = price2;
	lpObj->Inventory[lpMsg->slot].m_PShopValue3 = price3;
	lpObj->Inventory[lpMsg->slot].m_PShopValue4 = price4;
...
void CPersonalShop::CGPShopItemListRecv(PMSG_PSHOP_ITEM_LIST_RECV* lpMsg, int aIndex) // OK
{
	...
	gDeathSystem.SendPlayerShop(aIndex, bIndex);
}
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpTarget->Inventory[lpMsg->slot].m_PShopValue1 <= 0 && lpTarget->Inventory[lpMsg->slot].m_PShopValue2 <= 0 && lpTarget->Inventory[lpMsg->slot].m_PShopValue3 <= 0 && lpTarget->Inventory[lpMsg->slot].m_PShopValue4 <= 0)
	{
		this->GCPShopBuyItemSend(aIndex, bIndex, 0, 6);
		return;
	}

	int balanceCheck = gLuaGameServer.CheckPersonalShopValue(lpObj->Index, lpTarget->Index, lpMsg->slot);

	if (balanceCheck > 0)
	{
		balanceCheck += 10;
		this->GCPShopBuyItemSend(aIndex, bIndex, 0, balanceCheck);
		return;
	}
	...
	gLog.Output(LOG_TRADE, "[SellPesonalShopItem][%s][%s] - (Account: %s, Name: %s, Value1: %d, Value2: %d,Value3: %d,Value4: %d, Index: %04d, Level: %02d, Serial: %08X, Option1: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d, %03d, %03d, %03d, %03d)", lpTarget->Account, lpTarget->Name, lpObj->Account, lpObj->Name, lpTarget->Inventory[lpMsg->slot].m_PShopValue1, lpTarget->Inventory[lpMsg->slot].m_PShopValue2, lpTarget->Inventory[lpMsg->slot].m_PShopValue3, lpTarget->Inventory[lpMsg->slot].m_PShopValue4, lpTarget->Inventory[lpMsg->slot].m_Index, lpTarget->Inventory[lpMsg->slot].m_Level, lpTarget->Inventory[lpMsg->slot].m_Serial, lpTarget->Inventory[lpMsg->slot].m_Option1, lpTarget->Inventory[lpMsg->slot].m_Option2, lpTarget->Inventory[lpMsg->slot].m_Option3, lpTarget->Inventory[lpMsg->slot].m_NewOption, lpTarget->Inventory[lpMsg->slot].m_JewelOfHarmonyOption, lpTarget->Inventory[lpMsg->slot].m_ItemOptionEx, lpTarget->Inventory[lpMsg->slot].m_SocketOption[0], lpTarget->Inventory[lpMsg->slot].m_SocketOption[1], lpTarget->Inventory[lpMsg->slot].m_SocketOption[2], lpTarget->Inventory[lpMsg->slot].m_SocketOption[3], lpTarget->Inventory[lpMsg->slot].m_SocketOption[4]);

	int value[4] = { 0 };

	value[0] = lpTarget->Inventory[lpMsg->slot].m_PShopValue1;
	value[1] = lpTarget->Inventory[lpMsg->slot].m_PShopValue2;
	value[2] = lpTarget->Inventory[lpMsg->slot].m_PShopValue3;
	value[3] = lpTarget->Inventory[lpMsg->slot].m_PShopValue4;
	...
	gLuaGameServer.PersonalShopDecreaseValue(lpObj->Index, value[0], value[1], value[2], value[3]);

	gLuaGameServer.PersonalShopAddValue(lpTarget->Index, value[0], value[1], value[2], value[3]);
}
...
void CPersonalShop::GCPShopItemListSend(int aIndex, int bIndex, BYTE result, BYTE type) // OK
{
	...
	if (result == 1)
	{
		...
		for (int n = INVENTORY_EXT4_SIZE; n < INVENTORY_FULL_SIZE; n++)
		{
			...
			info.value1 = lpObj->Inventory[n].m_PShopValue1;
			info.value2 = lpObj->Inventory[n].m_PShopValue2;
			info.value3 = lpObj->Inventory[n].m_PShopValue3;
			info.value4 = lpObj->Inventory[n].m_PShopValue4;
