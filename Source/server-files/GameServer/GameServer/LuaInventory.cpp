...
public:
	...
	int getZenValue(int slot, int type) const {
		if (!OBJECT_USER_RANGE(this->aIndex)) {
			return 0;
		}
		...
		{
		case 1:
			return gObj[this->aIndex].Inventory[slot].m_PShopValue1;
		case 2:
			return gObj[this->aIndex].Inventory[slot].m_PShopValue2;
		case 3:
			return gObj[this->aIndex].Inventory[slot].m_PShopValue3;
		case 4:
			return gObj[this->aIndex].Inventory[slot].m_PShopValue4;
		default:
			return gObj[this->aIndex].Inventory[slot].m_PShopValue1;
		}
	};
...
void RegisterInventoryClass(lua_State * lua)
{
	...
	luaInventory.fun("getPersonalShopValue", &InventoryClass::getZenValue);
}
