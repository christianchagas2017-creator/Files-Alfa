...
void CNotice::MainProc() // OK
{
	...
	if((GetTickCount()-this->m_NoticeTime) >= ((DWORD)lpInfo->RepeatTime))
	{
		...
		this->GCNoticeSendToDash(lpInfo->Type,"%s",lpInfo->Message);
#else
...
void CNotice::GCNoticeSend2(int aIndex, BYTE type, BYTE count, BYTE opacity, WORD delay, DWORD color, BYTE speed, char* message)
{
...
void CNotice::GCNoticeSend3(int aIndex, BYTE type, BYTE count, BYTE opacity, WORD delay, DWORD color, BYTE speed, char* message, va_list arg)
{
	char buff[256] = { 0 };
...
void CNotice::GCNoticeSendToDash(int color, char* message, ...) // OK
{
	char buff[256] = { 0 };
	...
	CMySQL& mysql = CMySQL::getInstance();
	char query[1024];
	std::sprintf(query, "INSERT INTO messages (text, color) values ('%s', %d)", buff, color);
	mysql.addQuery(std::string(query), 1);
}
...
void CNotice::GCNoticeSendToAll2(BYTE type, BYTE count, BYTE opacity, WORD delay, DWORD color, BYTE speed, char* message)
{
	char buff[256] = { 0 };
