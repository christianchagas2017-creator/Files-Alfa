...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleStanby ] State(%d) -> NONE",this->m_BattleStanbyState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleStanby ] State(%d) -> START",this->m_BattleStanbyState);
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleStanby ] State(%d) -> NOTIFY",this->m_BattleStanbyState);

	this->SetBattleStanbyState(KANTURU_STANBY_NOTIFY);
#ifdef GLOBAL_DASHBOARD
	gNotice.GCNoticeSendToDash(0, gMessage.GetMessage(338));
#else
...
{
	gLog.Output(LOG_EVENT,"[ KANTURU ][ BattleStanby ] State(%d) -> END",this->m_BattleStanbyState);
