...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gLuaGameServer.MonsterDieGiveItem(lpTarget->Index, lpObj->Index))
	{
		return;
	}
	...
	if((DropIndex=gMonsterManager.GetMonsterItem(DropLevel,ExcItemDrop,gMapManager.GetMapSocketItemDrop(lpObj->Map))) >= GET_ITEM(0,0) && (GetLargeRand()%((lpObj->ItemRate==0)?1:lpObj->ItemRate)) < ItemDropRate)
	{
		...
		if (gCustomPick.IsPickItem(lpTarget, ItemIndex) && gItemManager.CheckItemInventorySpace(lpTarget, ItemIndex)) {
			GDCreateItemSend(lpTarget->Index, (BYTE)235, (BYTE)0, (BYTE)0, ItemIndex, ItemLevel, 0, ItemOption1, ItemOption2, ItemOption3, lpTarget->Index, ItemNewOption, ItemSetOption, 0, 0, ItemSocketOption, 0xFF, 0);
		}
		else
		{
...
#ifdef FAIRPLAY
		money = (money * gHappyHourZen.GetZenRate()) / 100;
#endif
...
void SetPositionMonsterRand(int aIndex, int Map)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return;
	...
	gMonsterSetBase.GetBoxPosition(Map, 5, 5, 254, 254, &x, &y);
...
void SetPositionMonster(int aIndex, int Map, int X, int Y)
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return;
