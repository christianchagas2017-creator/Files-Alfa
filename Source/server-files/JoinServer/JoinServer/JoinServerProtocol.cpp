...
void GJConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg, int index) // OK
{
	...
	std::string acc_lower = std::string(lpMsg->account);
	std::transform(acc_lower.begin(), acc_lower.end(), acc_lower.begin(), CheckAccountCaseSensitive);
	memcpy(lpMsg->account, acc_lower.c_str(), sizeof(lpMsg->account));
	memcpy(pMsg.account, acc_lower.c_str(), sizeof(pMsg.account));
	...
	if (MD5Encryption == 0)
	{
		...
		std::string db_acc_lower = std::string(account);
		std::transform(db_acc_lower.begin(), db_acc_lower.end(), db_acc_lower.begin(), CheckAccountCaseSensitive);

		if (strcmp(lpMsg->account, db_acc_lower.c_str()) != 0)
		{
			gQueryManager.Close();
			pMsg.result = 2;
			gSocketManager.DataSend(index, (BYTE*)&pMsg, pMsg.header.size);
