...
void CWarehouse::GDWarehouseInfoRecv(SDHP_WAREHOUSE_INFO_RECV* lpMsg, int index) // OK
{
	...
	pMsg.WarehouseCount = 1;

	if (gQueryManager.ExecQuery("SELECT VaultCounts FROM warehouse WHERE AccountID='%s'", lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
	}
	else {
		pMsg.WarehouseCount = gQueryManager.GetAsInteger("VaultCounts");
		gQueryManager.Close();
	}
...
void CWarehouse::GDWarehouseItemRecv(SDHP_WAREHOUSE_ITEM_RECV* lpMsg,int index) // OK
{
	...
	{
		int warehouseNumber = lpMsg->WarehouseNumber - 1;
		if(gQueryManager.ExecQuery("SELECT AccountID FROM ExtWarehouse WHERE AccountID='%s' AND Number=%d",lpMsg->account, warehouseNumber) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			gQueryManager.Close();
			gQueryManager.ExecQuery("INSERT INTO ExtWarehouse (AccountID,Money,Number) VALUES ('%s',0,%d)",lpMsg->account, warehouseNumber);
			gQueryManager.Close();
			gQueryManager.ExecQuery("UPDATE ExtWarehouse SET Items=CONVERT(varbinary(%d),REPLICATE(char(0xFF),%d)) WHERE AccountID='%s' AND Number=%d",sizeof(pMsg.WarehouseItem),sizeof(pMsg.WarehouseItem),lpMsg->account, warehouseNumber);
			gQueryManager.Close();
			this->DGWarehouseFreeSend(index,lpMsg->index,lpMsg->account);
			return;
		}
		else
		{
			...
			if(gQueryManager.ExecQuery("SELECT Items,Money FROM ExtWarehouse WHERE AccountID='%s' AND Number=%d",lpMsg->account, warehouseNumber) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
			{
				gQueryManager.Close();
...
void CWarehouse::GDWarehouseItemSaveRecv(SDHP_WAREHOUSE_ITEM_SAVE_RECV* lpMsg) // OK
{
	...
	else
	{
		int warehouseNumber = lpMsg->WarehouseNumber - 1;
		gQueryManager.BindParameterAsBinary(1,lpMsg->WarehouseItem[0],sizeof(lpMsg->WarehouseItem));
		gQueryManager.ExecQuery("UPDATE ExtWarehouse SET Items=?,Money=%d WHERE AccountID='%s' AND Number=%d",lpMsg->WarehouseMoney,lpMsg->account, warehouseNumber);
		gQueryManager.Close();
