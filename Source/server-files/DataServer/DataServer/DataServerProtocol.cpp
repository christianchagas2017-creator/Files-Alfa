...
void GDCharacterListRecv(SDHP_CHARACTER_LIST_RECV* lpMsg,int index) // OK
{
	...
	char CharacterName[CharacterCount][11];
	...
	for (int id = 0; id < CharacterCount; id++) {
		char buff[10];
		sprintf(buff, "GameID%d", id + 1);
		gQueryManager.GetAsString(buff, CharacterName[id], sizeof(CharacterName[id]));
	}
...
void GDCharacterCreateRecv(SDHP_CHARACTER_CREATE_RECV* lpMsg,int index) // OK
{
	...
	char CharacterName[CharacterCount][11] = {0};
