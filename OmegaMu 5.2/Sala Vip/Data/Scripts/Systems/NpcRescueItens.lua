...
function NpcRescueItem.Protocol(aIndex, Packet, PacketName)
	...
	if PacketName == string.format("3-%s-%d", playerInfo[1], playerInfo[2]) then
		...
		NpcRescueItem.SendItemList(player)
...
function NpcRescueItem.Withdraw(playerInfo, listID)
	...
	if withdrawResult == 1 then
		if InventoryCheckSpaceByItem(playerInfo[2], item.ItemIndex) ~= 0 then
			CreateItemInventory(playerInfo[2], item.ItemIndex, item.Level, item.Durabilidade, item.Skill, item.Luck, item.Opt, item.Excelente, item.Ancient, 0, item.Dias, 255, 255, 255, 255, 0, 0)
