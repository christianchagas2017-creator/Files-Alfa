...
battlePassSwitch = battlePassSwitch or false
...
function Reset.Make(player)
    ...
    -- Verificação de limite de resets
    if MAX_RESET ~= -1 then
        local Resets = player:getReset()
        
        if Resets >= MAX_RESET then
            SendMessage(string.format(RESET_MESSAGES[Language][12], MAX_RESET), aIndex, 0)
            return 0
        end
    end
    
    -- Executa o reset conforme o tipo configurado
    local result
    if RESET_TYPE == 0 then
        result = Reset.Acumulative(player)
    elseif RESET_TYPE == 1 then
        result = Reset.Tabulated(player)
    end
    
    -- Se o reset foi bem-sucedido, adiciona XP ao BattlePass
    if result == 1 and battlePassSwitch then
        ...
        if type(BattlePass) == "table" and type(BattlePass.AddExpPlayer) == "function" then
            ...
            -- Envia o packet de atualização (usando o mesmo método do BattlePass)
            local packetName = string.format("BP_Update_%s", player:getName())
            CreatePacket(packetName, battlePassPacket)  -- Usa a constante battlePassPacket definida no BattlePass
            ...
            SendMessage(string.format(RESET_MESSAGES[Language][15], xpToAdd, expPass), aIndex, 26)  -- [15] = "[BattlePass] XP Total: %d"
            
            -- Log do sistema
            --LogAddC(2, string.format("[Reset] %s ganhou 1 XP no BattlePass (Total: %d)", player:getName(), expPass))
        else
            --LogAddC(2, string.format("[Reset] Erro: BattlePass não disponível para %s", player:getName()))
        end
    end
...
function Reset.Acumulative(player)
	...
	for i = 0 , #COLUMN_RESET do
		DataBaseAsync.SetAddValue(TABLE_RESET, COLUMN_RESET[i], RESET_ACUMULATIVE_RESETS[vip], WHERE_RESET, player:getName())
	end
...
function Reset.Tabulated(player)
	...
	local money = Reset.Tabulated_GetResetMoney(Resets + 1, vip)
	...
	local leveluppoints = Reset.Tabulated_GetResetPoint(Resets+1, vip)
	...
	for i = 0 , #COLUMN_RESET do
		DataBaseAsync.SetAddValue(TABLE_RESET, COLUMN_RESET[i], RESET_TABLE_RESETS[vip], WHERE_RESET, player:getName())
	end
