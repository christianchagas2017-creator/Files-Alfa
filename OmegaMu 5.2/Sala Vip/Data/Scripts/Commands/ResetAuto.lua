...
function ResetAuto.Command(aIndex, Arguments)
    ...
    then
        SendMessage(string.format(AUTO_RESET_COMMAND_MESSAGES[Language][1]), aIndex, 35)
		return
    end
	...
	then
		...
		player:setAutoResetStats(0, Cmd:getNumber(Arguments, 1))
		player:setAutoResetStats(1, Cmd:getNumber(Arguments, 2))
		player:setAutoResetStats(2, Cmd:getNumber(Arguments, 3))
		player:setAutoResetStats(3, Cmd:getNumber(Arguments, 4))

        if player:getClass() == CLASS_DL
        then
		    player:setAutoResetStats(4, Cmd:getNumber(Arguments, 5))
        else
            player:setAutoResetStats(4, 0)
        end

		SendMessage(string.format(AUTO_RESET_COMMAND_MESSAGES[Language][4], AUTO_RESET_MRESETS), aIndex, 26)
    else
		...
		SendMessage(string.format(AUTO_RESET_COMMAND_MESSAGES[Language][5], AUTO_RESET_MRESETS), aIndex, 35)
	end
end
