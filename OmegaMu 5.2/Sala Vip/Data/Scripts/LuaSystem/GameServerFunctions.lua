...
-- Run before lua is reloaded
local CloseLua_Handles = {}
local MonsterReloadMap_Handles = {}

function CloseLua()
	for i = 1, #CloseLua_Handles do
		CloseLua_Handles[i].callback()
	end
end

function GameServerFunctions.CloseLua(callback)
	table.insert(CloseLua_Handles, { callback = callback })
end
...
function PlayerDropItem(aIndex, x, y, slot)
	...
	for i = 1, #PlayerDropItem_Handles do
		local ret = PlayerDropItem_Handles[i].callback(aIndex, x, y, slot)
...
-- when top kill item changes
local UpdateTopKill_Handles = {}
function UpdateTopKill(Serial, Kills)
	for i = 1, #UpdateTopKill_Handles do
		if UpdateTopKill_Handles[i].callback(Serial, Kills) == 1 then
			return 1
		end
	end
...
function GameServerFunctions.UpdateTopKill(callback)
	table.insert(UpdateTopKill_Handles, { callback = callback })
end
...
function PlayerSendTrade(aIndex, TargetIndex)
	...
	if Central.CheckPlayer(aIndex) == 1 or Central.CheckPlayer(TargetIndex) == 1 then
		return 1
	end		
...
function NpcTalk(Npc, Player)
	...
	if Central.CheckPlayer(Player) == 1  then
		return 1
	end	
...
function MonsterReload()
    for i = 1, #MonsterReload_Handles do
        if MonsterReload_Handles[i] and MonsterReload_Handles[i].callback then
...
function MonsterReloadMap(map)
    for i = 1, #MonsterReloadMap_Handles do
        local h = MonsterReloadMap_Handles[i]
...
function GameServerFunctions.MonsterReloadMap(callback)
    table.insert(MonsterReloadMap_Handles, { callback = callback })
end
